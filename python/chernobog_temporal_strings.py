...
def api(name, *arguments):
    encoded = [json.dumps(value) if isinstance(value, str) else str(int(value)) for value in arguments]
...
def current(snapshot, state):
    return bool(state.get("available") and state.get("fresh") and snapshot.get("ticket")
                and state.get("ticket") == snapshot["ticket"])


def select_rows(snapshot, observation, capture=None):
    witnesses = [row for row in snapshot.get("witnesses", []) if row["observation"] == str(observation)]
    fragments = [row for row in snapshot.get("fragments", [])
...
if QtWidgets is not None:
    class TemporalStringForm(ida_kernwin.PluginForm):
        ...
        def OnCreate(self, form):
            ...
            self.tables = {}
            for key, headers in (("observations", ["Observed string", "Read source", "Agreeing runs"]),
                                 ("witnesses", ["Capture / seed", "Allocation / generation", "Use interval", "Reads"]),
                ...
                table.setMaximumHeight({"observations": 100, "witnesses": 140, "fragments": 220}[key])
                table.currentItemChanged.connect(lambda item, previous, name=key: self.selected(name, item))
        ...
        def populate(self, key, rows):
            ...
            for row in rows:
                if key == "observations":
                    ...
                    columns = [row["capture"] + " / " + row["seed"], row["allocation"] + " / " + row["generation"],
                               row["first_sequence"] + "–" + row["last_sequence"], row["read_count"]]
                else:
                    columns = [row["sequence"] + " / " + row["data_sequence"], row["site"], row["address"], row["bytes"]]
        ...
        def selected(self, key, item):
            ...
            if key == "observations":
                ...
                _, fragments = select_rows(self.snapshot, row["observation"], row["capture"]) if row else ([], [])
        ...
        def update_detail(self):
            ...
            if self.observation:
                ...
                if self.witness:
                    lead += ("Capture " + self.witness["capture"] + ", seed " + self.witness["seed"]
                             + ": " + self.witness["read_count"] + " reads, allocation "
                             + self.witness["allocation"] + ", generation " + self.witness["generation"]
                             + "; omitted fragments: " + self.witness["fragments_omitted"] + "\n")
                lead += "\n"
            text = lead + json.dumps({"current_inputs": self.current,
                "scope": self.snapshot["scope"], "contract": self.snapshot["contract"],
                "consensus_available": self.snapshot["consensus_available"], "reason": self.snapshot["reason"],
                "observations_omitted": self.snapshot["observations_omitted"],
                "observation": self.observation, "witness": self.witness, "fragment": self.fragment,
                "runs": self.snapshot["runs"], "bindings": self.snapshot["bindings"],
                "input_arguments": self.snapshot["input_arguments"]}, indent=2)
        ...
        def poll(self):
            if self.closed:
                return
            try:
                self.current = current(self.snapshot, api("chernobog_vm_temporal_string_state", self.snapshot["ticket"]))
            except (RuntimeError, ValueError, KeyError):
                self.current = False
            prefix = "CURRENT INPUT SNAPSHOT" if self.current else "HISTORICAL / CHANGED / SUPERSEDED CAPTURE"
            message = (prefix + " | " + str(len(self.snapshot["observations"])) + " named-model string observations"
                       + " | omitted: " + str(self.snapshot["observations_omitted"]))
...
def show(ea, bindings, input_request=None):
    ...
    snapshot = api("chernobog_vm_temporal_strings", ea,
                   json.dumps(input_request or {"args": [], "objects": []}), json.dumps(bindings))
