...
const char *boundary_flow_name(BoundaryFlow flow)
{
    switch (flow)
    {
    case BoundaryFlow::linear:
        return "linear";
    case BoundaryFlow::direct:
        return "direct";
    case BoundaryFlow::conditional:
        return "conditional";
    case BoundaryFlow::call:
        return "call";
    case BoundaryFlow::indirect:
        return "indirect";
    case BoundaryFlow::ret:
        return "return";
    case BoundaryFlow::stop:
        return "stop";
    }
    return "unknown";
}

BoundaryAudit audit_boundaries(uint64_t entry, unsigned mode, const BoundaryReader &read,
                               BoundaryLimits limits)
{
    ...
    {
        frontier("invalid_request", entry);
        return result;
    }
    ...
    if (!limits.heads)
    {
        result.head_limit = true;
        frontier("head_limit", entry);
        return result;
    }
    ...
    while (!pending.empty())
    {
        ...
        const size_t allowance =
            std::min(limits.incoming_per_head, limits.incoming_total - result.incoming_examined);
        ...
        {
            node.incoming.resize(allowance);
            node.incoming_complete = false;
        }
        result.incoming_examined += node.incoming.size();
        if (!node.incoming_complete)
        {
            result.incoming_limit = true;
            frontier("incoming_limit", address);
        }
        if (node.address != address)
            node.rejection = "projection_address_mismatch";
        node.address = address;
        if (node.rejection.empty())
        {
            if (node.mode != mode)
                node.rejection = "mode_boundary";
            ...
            else if (node.shared_tail)
                node.rejection = "shared_function_tail";
            else if (node.owner != bad_address && node.owner != entry)
                node.rejection = "foreign_function";
            else if (address == entry && node.owner != entry)
                node.rejection = "entry_ownership_changed";
        }
        if (node.rejection.empty())
        {
        ...
        {
            frontier(current.rejection.c_str(), address);
            continue;
        }
        accepted.insert(address);
        spans.emplace(address, address + current.size);
        ++result.decoded;
        if (current.owner == bad_address)
            ++result.ownerless;
        switch (current.flow)
        {
        case BoundaryFlow::linear:
            follow(address, address + current.size, true);
            break;
        case BoundaryFlow::direct:
            follow(address, current.target, false);
            break;
        case BoundaryFlow::conditional:
            follow(address, current.target, false);
            follow(address, address + current.size, true);
        ...
        case BoundaryFlow::call:
            frontier("call_requires_summary", address, current.target);
            break;
        case BoundaryFlow::indirect:
            frontier("unresolved_indirect", address);
            break;
        // A native RET may be a VMP dispatch; never claim it proves region exit.
        case BoundaryFlow::ret:
            frontier("unresolved_return", address);
            break;
        case BoundaryFlow::stop:
            frontier("unsupported_control", address);
            break;
        }
    }
    for (const auto &node : result.nodes)
    {
        if (!accepted.count(node.address))
            continue;
        if (node.address == entry)
        {
            result.entry_incoming = node.incoming.size();
            continue;
        }
        for (uint64_t source : node.incoming)
        {
            if (!accepted.count(source))
                frontier("external_code_entry", source, node.address);
