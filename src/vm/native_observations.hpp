...
struct NativeObservationView : ObservationView
{
  ...
  size_t unsupported_path_stops=0,recognizer_rejections=0,path_length_stops=0,capture_end_stops=0;
...
NativeObservationView project_native_observations(const NativeRegion &,
    const std::map<uint64_t,Instruction> &,const hybrid::EmuEvents &,
    const hybrid::EmuOutcome &,unsigned mode,uint64_t capture,uint64_t function,
