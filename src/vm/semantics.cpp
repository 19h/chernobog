...
z3::expr resize(const z3::expr &value, unsigned bits, bool sign = false)
{
    ...
    return old == bits  ? value
           : old > bits ? value.extract(bits - 1, 0)
           : sign       ? z3::sext(value, bits - old)
                        : z3::zext(value, bits - old);
}
std::vector<std::string> roles(const Candidate &c)
{
    ...
    for (auto &name : result)
        if (name.empty())
            name = "preserved_" + std::to_string(other++);
    return result;
}
class Evaluator
{
    ...
    z3::expr address(const Operand &o)
    {
        ...
        if (o.base >= 0)
            value = value + out.registers.at(size_t(o.base));
        if (o.index >= 0)
            value = value + out.registers.at(size_t(o.index)) * bv(ctx, o.scale, mode);
        return value;
    }
    z3::expr read_memory(const z3::expr &at, unsigned bits)
    {
        ...
        out.accesses.push_back({false, bits, at, value, site});
        return value;
    }
    void write_memory(const z3::expr &at, const z3::expr &value)
    {
        ...
        for (unsigned offset = 0; offset < bits / 8; ++offset)
            out.memory = z3::store(out.memory, at + bv(ctx, offset, mode),
                                   value.extract(offset * 8 + 7, offset * 8));
    }
    z3::expr read(const Operand &o, unsigned bits)
    {
        if (o.kind == Kind::reg)
            return resize(
                out.registers.at(size_t(o.reg)).extract(o.bit_offset + o.bits - 1, o.bit_offset),
                bits);
        if (o.kind == Kind::immediate)
            return bv(ctx, o.value, bits);
        if (o.kind == Kind::memory)
            return resize(read_memory(address(o), o.bits), bits);
    ...
    void write(const Operand &o, const z3::expr &input)
    {
        ...
        if (o.bit_offset)
        {
            destination = z3::concat(destination.extract(mode - 1, 16),
                                     z3::concat(value, destination.extract(7, 0)));
            return;
        }
        if (o.bits == mode || (mode == 64 && o.bits == 32))
            destination = resize(value, mode);
        else
            destination = z3::concat(destination.extract(mode - 1, o.bits), value);
    }
                          ...
                          bool subtract, bool logical, bool preserve_carry)
    {
        ...
        auto bit = [&](const z3::expr &v, unsigned index)
        { return v.extract(index, index) == bv(ctx, 1, 1); };
        if (!preserve_carry)
        {
            out.flags[0] = logical    ? ctx.bool_val(false)
                           : subtract ? z3::ult(a, b)
                                      : z3::ult(result, a);
            out.defined[0] = true;
        }
        ...
        for (unsigned i = 1; i < 8; ++i)
            parity = parity != bit(result, i);
        out.flags[1] = !parity;
        out.defined[1] = true;
        out.flags[2] = bit(a ^ b ^ result, 4);
        out.defined[2] = !logical;
        out.flags[3] = result == bv(ctx, 0, bits);
        out.defined[3] = true;
        out.flags[4] = bit(result, bits - 1);
        out.defined[4] = true;
        out.flags[5] = logical ? ctx.bool_val(false)
                               : bit((subtract ? (a ^ b) : ~(a ^ b)) & (a ^ result), bits - 1);
        out.defined[5] = true;
    }
  ...
  public:
    ...
    void step(const Instruction &i)
    {
        ...
        if (i.op == Op::compare || i.op == Op::test)
        {
            ...
            arithmetic_flags(a, b, logical ? (a & b) : (a - b), !logical, logical, false);
            return;
        }
        if (i.op == Op::scan_forward)
        {
            ...
            auto index =
                ctx.bv_const(("undefined_bsf_" + std::to_string(site)).c_str(), i.dst.bits);
            for (unsigned bit = i.src.bits; bit-- > 0;)
                index = z3::ite(input.extract(bit, bit) == bv(ctx, 1, 1), bv(ctx, bit, i.dst.bits),
                                index);
            write(i.dst, index);
            out.defined.fill(false);
            out.flags[3] = input == bv(ctx, 0, i.src.bits);
            out.defined[3] = true;
            return;
        }
        if (i.op == Op::load)
        {
            write(i.dst, read(i.src, i.src.bits));
            return;
        }
        if (i.op == Op::sign_extend)
        {
            write(i.dst, resize(read(i.src, i.src.bits), i.dst.bits, true));
            return;
        }
        if (i.op == Op::push)
        {
            ...
            write_memory(out.registers[4], value);
            return;
        }
        if (i.op == Op::pop)
        {
            const auto value = read_memory(out.registers[4], mode);
            write(i.dst, value);
            out.registers[4] = out.registers[4] + bv(ctx, mode / 8, mode);
            return;
        }
        ...
        else if (i.op == Op::byte_swap)
        {
            ...
            for (unsigned offset = 1; offset < bits / 8; ++offset)
                result = z3::concat(result, a.extract(offset * 8 + 7, offset * 8));
        }
...
Summary::Summary(z3::context &ctx, Candidate c)
      ...
      next_pc(ctx.bv_val(0, candidate.address_bits)), domain(ctx.bool_val(true))
{
    ...
    for (unsigned i = 0; i < 6; ++i)
        flags.push_back(ctx.bool_const(("input_flag_" + std::to_string(i)).c_str()));
}
...
std::unique_ptr<Summary> summarize(z3::context &ctx, const Candidate &candidate)
{
    try
    {
        ...
        if (c->address_bits == 32 && c->read_bits == 8 && c->key >= 0 &&
            (c->value > 3 || c->key > 3))
            return {};
...
Comparison compare(const Summary &a, const Summary &b, unsigned timeout, unsigned resource)
{
    ...
    try
    {
        auto &ctx = a.memory.ctx();
        auto mismatch = a.next_pc != b.next_pc || a.memory != b.memory;
        for (size_t i = 0; i < a.roles.size(); ++i)
        {
            const auto it = std::find(b.roles.begin(), b.roles.end(), a.roles[i]);
            if (it == b.roles.end())
                return {Equivalence::incompatible, "register role map differs"};
        ...
        for (size_t i = 0; i < a.accesses.size(); ++i)
        {
            ...
            if (x.write != y.write || x.bits != y.bits)
                return {Equivalence::incompatible, "memory access contract differs"};
    ...
    {
        return {Equivalence::unknown, "solver exception"};
    }
}
} // namespace chernobog::vm
