...
{
    if (number < 0 || (bits != 8 && bits != 16 && bits != 32 && bits != 64))
        return -1;
    qstring name;
    if (get_reg_name(&name, number, bits / 8) <= 0)
        return -1;
    if (bits == 8)
    {
        if (name == "ah")
...
{
    Operand r;
    r.bits = unsigned(get_dtype_size(o.dtype) * 8);
    if (o.type == o_void)
        return {};
    if (o.type == o_reg)
    {
        r.kind = Kind::reg;
        r.reg = canonical(o.reg, r.bits);
        qstring name;
        if (r.bits == 8 && get_reg_name(&name, o.reg, 1) > 0 &&
            (name == "ah" || name == "ch" || name == "dh" || name == "bh"))
            r.bit_offset = 8;
    }
    else if (o.type == o_imm)
    {
        r.kind = Kind::immediate;
        r.value = o.value;
    }
    else if (o.type == o_phrase || o.type == o_displ || o.type == o_mem)
    {
        r.kind = Kind::memory;
        r.address_bits = ad64(i) ? 64 : ad32(i) ? 32 : 16;
        if (r.address_bits != mode)
            return {};
        ...
        if ((base != R_none && r.base == -1) || (index != R_none && r.index == -1))
            return {};
        const int shift = x86_scale(o);
        if (shift < 0 || shift > 3)
            return {};
        r.scale = 1u << unsigned(shift);
        r.value = o.type == o_phrase ? 0 : uint64_t(o.addr);
    }
    return r;
}
Instruction decode(const insn_t &i, unsigned mode)
{
    ...
    r.address = i.ea;
    r.size = i.size;
    if (!natad(i) || i.segpref != 0 || (i.auxpref & (aux_lock | aux_rep | aux_repne)))
        return r;
    ...
    case NN_clc:
        ...
        break;
    case NN_cmc:
        r.op = Op::carry_toggle;
        break;
    case NN_bsf:
        r.op = Op::scan_forward;
        break;
    default:
        break;
    }
...
std::string bytes(const Candidate &c)
{
    ...
    for (const auto &i : c.support)
        for (uint64_t ea = i.address; ea < i.address + i.size; ++ea)
        {
            const auto byte = get_byte(ea_t(ea));
...
BoundaryNode boundary_node(uint64_t address, size_t incoming_budget)
{
    ...
    {
        if (node.incoming.size() == incoming_budget)
        {
            node.incoming_complete = false;
            break;
        }
    ...
    if (!seg)
    {
        node.rejection = "unmapped_target";
        return node;
    }
    node.mode = seg->bitness == 2 ? 64 : seg->bitness == 1 ? 32 : 16;
    if (!(seg->perm & SEGPERM_EXEC))
    {
        node.rejection = "nonexecutable_target";
        return node;
    }
    if (!is_head(get_flags(ea)) || !is_code(get_flags(ea)))
    {
        node.rejection = "missing_code_head";
        return node;
    }
    insn_t insn;
    if (decode_insn(&insn, ea) <= 0)
    {
        node.rejection = "decode_failure";
        return node;
    }
    node.size = insn.size;
    ...
    for (unsigned offset = 0; offset < node.size; ++offset)
    {
        ...
        if (!is_loaded(ea + offset))
        {
            node.rejection = "unloaded_instruction_byte";
            return node;
        }
        if (identity != node.owner || get_item_head(ea + offset) != ea)
        {
            node.rejection = "instruction_ownership_boundary";
            return node;
        }
        const unsigned byte = get_byte(ea + offset);
        node.bytes += digits[byte >> 4];
        node.bytes += digits[byte & 15];
    }
    ...
    {
        node.rejection = "unsupported_prefix";
        return node;
    }
    const bool near = insn.Op1.type == o_near;
    if (near)
        node.target = uint64_t(to_ea(insn.cs, insn.Op1.addr));
    if (is_call_insn(insn))
        node.flow = BoundaryFlow::call;
    else if (is_ret_insn(insn))
        node.flow = BoundaryFlow::ret;
...
void boundary_json(std::ostream &out, const BoundaryAudit &audit, bool available)
{
    ...
    for (const auto &node : audit.nodes)
    {
        ...
        nodes.push_back({{"site", hex(node.address)},
                         {"owner", node.owner == bad_address ? "none" : hex(node.owner)},
                         {"size", std::to_string(node.size)},
                         {"address_bits", std::to_string(node.mode)},
                         {"bytes", node.bytes},
                         {"flow", boundary_flow_name(node.flow)},
                         {"target", node.target == bad_address ? "unknown" : hex(node.target)},
                         {"rejection", node.rejection},
                         {"shared_tail", node.shared_tail ? "true" : "false"},
                         {"incoming", incoming},
                         {"incoming_complete", node.incoming_complete ? "true" : "false"}});
    }
    for (const auto &edge : audit.edges)
        edges.push_back({{"source", hex(edge.from)},
                         {"target", hex(edge.to)},
                         {"kind", edge.fallthrough ? "fallthrough" : "decoded direct branch"}});
    for (const auto &frontier : audit.frontiers)
        frontiers.push_back(
            {{"reason", frontier.reason},
             {"site", hex(frontier.site)},
             {"target", frontier.target == bad_address ? "unknown" : hex(frontier.target)}});
    out << ",\"boundary_audit\":{\"schema\":1,\"available\":" << (available ? "true" : "false")
        << ",\"execution_admitted\":false,\"decoded_heads\":" << audit.decoded
        << ",\"ownerless_heads\":" << audit.ownerless
        << ",\"incoming_examined\":" << audit.incoming_examined
    ...
    inspection_json_rows(out, "nodes", nodes);
...
bool decode_native_semantic_heads(const NativeRegion &region, unsigned mode,
                                  std::map<uint64_t, Instruction> &out)
{
    out.clear();
    if (!region.available() || (mode != 32 && mode != 64) || region.heads().size() > 4096)
        return false;
    for (const auto &head : region.heads())
    {
        insn_t instruction;
        uint8_t data[15] = {};
        if (decode_insn(&instruction, ea_t(head.address)) <= 0 ||
            instruction.size != head.bytes.size() ||
            (mode == 64 ? !mode64(instruction) : !mode32(instruction)) ||
            head.bytes.size() > sizeof(data) ||
...
std::string inspect_regions(uint64_t function, bool include_summaries,
                            const hybrid::TargetEvidence *source, uint64_t revision, bool fresh,
                            bool validate_transitions)
{
    using Row = std::map<std::string, std::string>;
    std::vector<Row> rows;
    std::vector<Candidate> candidates;
    size_t visited = 0, omitted = 0;
    bool truncated = false;
    auto *owner = get_func(ea_t(function));
    auto *segment = getseg(ea_t(function));
    const unsigned mode = segment && segment->bitness == 2   ? 64
                          : segment && segment->bitness == 1 ? 32
                                                             : 0;
    ...
    {
        if (!recorded.emplace(c.start, c.dispatch).second)
            return;
        if (rows.size() == 64)
        {
            ++omitted;
        ...
        rows.push_back(
            {{"vm_candidate", hex(c.start) + ":" + hex(c.dispatch)},
             ...
             {"dispatch_kind",
              c.dispatch_kind == Dispatch::indexed_table ? "indexed table" : "relative register"},
    ...
    if (available)
    {
        ...
        for (bool ok = iter.first(); ok; ok = iter.next_code())
        {
            ...
            const ea_t ea = iter.current();
            insn_t insn;
            const auto *seg = getseg(ea);
            const auto *actual = get_func(ea);
            if (!actual || actual->start_ea != function || !seg ||
                seg->bitness != segment->bitness || !(seg->perm & SEGPERM_EXEC) ||
                !is_code(get_flags(ea)) || decode_insn(&insn, ea) <= 0)
            {
                window.clear();
                continue;
            }
            ...
            for (unsigned j = 0; j < insn.size; ++j)
            {
                ...
                if (!is_loaded(ea + j) || !byte_owner || byte_owner->start_ea != function)
                    loaded = false;
            }
            if (!loaded)
            {
            ...
            for (bool x = xref.first_to(ea, XREF_ALL); x; x = xref.next_to())
            {
                if (incoming == 64)
                {
                    i.alternate_entry = true;
                    truncated = true;
                    break;
                }
                ++incoming;
                if (xref.iscode && xref.from != previous)
                {
                    i.alternate_entry = true;
                    break;
                }
        ...
        std::deque<uint64_t> pending{function};
        std::set<uint64_t> scheduled{function};
        while (!pending.empty())
        {
            const uint64_t ea = pending.front();
            pending.pop_front();
            ++reachable_heads;
            const auto *actual = get_func(ea), *root = get_func(function);
            const auto *seg = getseg(ea);
            if (actual && (!root || actual->start_ea != root->start_ea))
            {
                ++foreign_stops;
                continue;
            }
            if (!seg || seg->bitness != segment->bitness || !(seg->perm & SEGPERM_EXEC) ||
                !is_code(get_flags(ea)) || !is_head(get_flags(ea)))
                continue;
            if (!decoded_path_heads.count(ea))
            {
                if (decoded_path_heads.size() == 1024)
                {
                    reachability_truncated = true;
                    break;
                }
                insn_t instruction;
            ...
            {
                if (reachability_edges++ == 4096)
                {
                    reachability_truncated = true;
                    break;
                }
                const int type = int(xref.type) & XREF_MASK;
                if (!xref.iscode || (type != fl_F && type != fl_JN))
                    continue;
                if (scheduled.count(xref.to))
                    continue;
                if (scheduled.size() == 1024)
                {
                    reachability_truncated = true;
                    break;
                }
                scheduled.insert(xref.to);
                pending.push_back(xref.to);
            }
        ...
        for (const auto &head : decoded_path_heads)
        {
            ...
            while (path.size() < 128)
            {
                ...
                if (found == decoded_path_heads.end() || !seen.insert(at).second)
                    break;
                auto current = found->second;
                if (current.op == Op::unsupported)
                    break;
                if (!path.empty())
                {
                    bool linked = false;
                    current.alternate_entry = false;
                    xrefblk_t xref;
                    size_t incoming = 0;
                    for (bool x = xref.first_to(at, XREF_ALL); x; x = xref.next_to())
                    {
                        if (incoming++ == 64)
                        {
                            current.alternate_entry = true;
                            path_truncated = true;
                            break;
                        }
                        if (xref.iscode)
                        {
                            if (xref.from == path.back().address)
                                linked = true;
                            else
                                current.alternate_entry = true;
                        }
                    }
                    if (!linked || current.alternate_entry)
                        break;
                }
                path.push_back(current);
                if (current.op == Op::jump || current.op == Op::near_return)
                {
                    if (const auto candidate = recognize(path, mode))
                        record(*candidate);
                    break;
                }
    ...
    out << "{\"schema\":1,\"available\":" << (available ? "true" : "false")
        ...
        << ",\"instructions\":" << visited << ",\"truncated\":" << (truncated ? "true" : "false")
        << ",\"omitted\":" << omitted;
    out << ",\"path_steps\":" << path_steps
        << ",\"path_scan_truncated\":" << (path_truncated ? "true" : "false");
    out << ",\"reachable_heads\":" << reachable_heads
        << ",\"foreign_function_stops\":" << foreign_stops
        << ",\"reachability_truncated\":" << (reachability_truncated ? "true" : "false");
    inspection_json_rows(out, "records", rows);
    boundary_json(out,
                  available ? audit_boundaries(function, mode, boundary_node) : BoundaryAudit{},
                  available);
    ...
    {
        const auto view =
            project_observations(candidates, *source, function, revision, fresh && available,
