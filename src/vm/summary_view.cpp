...
{
    if (++visits > 512 || depth > 64 || !e.is_app())
        return false;
    for (unsigned i = 0; i < e.num_args(); ++i)
        if (!printable(e.arg(i), visits, depth + 1))
            return false;
    return true;
}
...
{
    SummaryRow row{
        {"summary_id", std::to_string(reference)},
        {"vm_candidate", id(s.candidate)},
        {"site", hex(s.candidate.start)},
        {"address_bits", std::to_string(s.candidate.address_bits)},
        {"contract",
         "normal completion; unchanged instruction stream; valid flat little-endian memory accesses; no concurrency, device, segment-base or exception effects"},
        {"frame",
         "all architectural GPRs compared under a bijective role map; all unmodified roles preserved"},
        {"memory",
         "ordered data reads/writes and final byte-addressed array; arbitrary data aliasing retained"},
        {"transition", "one local dispatch; target execution and VM-region ownership not admitted"},
        {"flags",
         "final CF/PF/AF/ZF/SF/OF; architecturally undefined outputs excluded only when both contracts agree"}};
    ...
    auto expression = [&](const std::string &name, const z3::expr &value)
    {
        ...
        if (!printable(value, visits) || payload + name.size() * 2 + 2080 > 31744)
        {
            ++omitted;
            return;
        }
    ...
    for (unsigned i = 0; i < 6; ++i)
        if (s.defined[i])
            expression(std::string("flag_") + names[i], s.flags[i]);
        else
            row[std::string("flag_") + names[i]] = "undefined";
    ...
    row["expressions_omitted"] = std::to_string(omitted);
    return row;
}
}

SummaryView project_summaries(const std::vector<Candidate> &candidates, int64_t database,
                              uint64_t function)
{
    SummaryView view;
    z3::context ctx;
    ...
    for (size_t index = 0; index < candidates.size(); ++index)
    {
        if (index == 16)
        {
            view.omitted = candidates.size() - index;
            break;
        }
        ...
        if (!summary)
        {
            binding["status"] = "unsupported; no summary";
            view.bindings.push_back(std::move(binding));
            continue;
        }
        size_t match = references.size();
        std::string reason = "new modeled-effect reference; no reuse";
        for (size_t i = 0; i < references.size() && view.queries < 32; ++i)
        {
            ...
            if (r.address_bits != c.address_bits || (r.key >= 0) != (c.key >= 0) ||
                (r.dispatch_base >= 0) != (c.dispatch_base >= 0))
                continue;
            ...
            if (comparison.result == Equivalence::equivalent)
            {
                match = i;
        ...
        {
            if (view.queries == 32)
                reason = "new reference; comparison budget exhausted; no reuse";
            ...
            binding["status"] = "reused after UNSAT";
