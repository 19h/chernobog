...
bool JumpOptimizationRule::operands_equal(const mop_t &left, const mop_t &right)
{
    return left.size > 0 && left.size == right.size && left.equal_mops(right, EQ_IGNSIZE);
}
...
bool JnzRule1::matches(mblock_t *blk, minsn_t *jcc)
{
    ...
    if (is_const(and_ins->r, &const_val) && const_val == 1)
    {
        ...
        if (bnot_ins && bnot_ins->opcode == m_bnot && operands_equal(bnot_ins->l, jcc->r))
            return true;
    }
