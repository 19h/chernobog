...
{
  public:
    void configure_automatic(bool enabled) { automatic_ = enabled; }

    bool automatic() const { return automatic_; }

    void request(uint64_t function_ea) { explicitly_requested_.insert(function_ea); }

    bool allows(uint64_t function_ea) const
    {
        return automatic_ || explicitly_requested_.find(function_ea) != explicitly_requested_.end();
    }

    void clear_requests() { explicitly_requested_.clear(); }
