...
std::set<ea_t> collect_referenced_encrypted_objects(ea_t func_ea)
{
    ...
    for (bool ok = iterator.first(); ok; ok = iterator.next_code())
    {
        ...
        for (ea_t to = get_first_dref_from(from); to != BADADDR; to = get_next_dref_from(from, to))
        {
            const ea_t object = get_item_head(to);
            qstring name;
            if (get_name(&name, object) > 0 && is_hikari_encrypted_name(name))
            {
...
bool static_memory_address(const mop_t &operand, ea_t *address)
{
    ...
    ea_t base = BADADDR;
    uint64_t offset = 0;
    if (!static_memory_address(instruction->l, &base) || instruction->r.t != mop_n ||
        !instruction->r.nnn)
    {
        return false;
    }
...
bool loaded_global_byte(const mop_t &operand, const minsn_t *before, ea_t *address)
{
    ...
    {
        for (const minsn_t *definition = before->prev; definition; definition = definition->prev)
        {
...
bool loaded_global_value(const mop_t &operand, const minsn_t *before, int width, ea_t *address)
{
    ...
    if (operand.t == mop_r && before)
    {
        for (const minsn_t *definition = before->prev; definition; definition = definition->prev)
        {
            ...
            if (definition->opcode == m_ldx)
                return static_memory_address(definition->r, address);
            if (definition->opcode == m_mov)
                return loaded_global_value(definition->l, definition, width, address);
            return false;
        }
    }
    return false;
...
{
    if (!address || operand.t != mop_v || operand.size < 1 || operand.size > 8)
    {
        return false;
    }
    *address = operand.g;
...
const minsn_t *nearest_register_definition(const mop_t &operand, const minsn_t *before, int width)
{
...
bool evaluate_static_instruction(const minsn_t *instruction, int width, ea_t *source,
                                 uint64_t *value, static_write_kind_t *kind, int depth);

bool evaluate_static_operand(const mop_t &operand, const minsn_t *before, int width, ea_t *source,
                             uint64_t *value, static_write_kind_t *kind, int depth)
{
    ...
    if (operand.t == mop_d && operand.d)
        return evaluate_static_instruction(operand.d, width, source, value, kind, depth + 1);
    const minsn_t *definition = nearest_register_definition(operand, before, width);
    return definition != nullptr &&
           evaluate_static_instruction(definition, width, source, value, kind, depth + 1);
}
                                 ...
                                 uint64_t *value, static_write_kind_t *kind, int depth)
{
    ...
    if (instruction->opcode == m_xor)
    {
        ...
        ea_t address = BADADDR;
        if (!global || !number || !loaded_global_value(*global, instruction, width, &address))
        {
            return false;
        }
        const std::optional<uint64_t> memory_value =
        ...
        if (!memory_value)
            return false;
        *source = address;
        *value = chernobog::bitvector::truncate(*memory_value ^ number->nnn->value, width);
    ...
    {
        ea_t address = BADADDR;
        if (!loaded_global_value(instruction->l, instruction, width, &address))
        {
            return false;
        }
        const std::optional<uint64_t> memory_value =
    ...
    if (instruction->opcode == m_mov)
        return evaluate_static_operand(instruction->l, instruction, width, source, value, kind,
                                       depth + 1);
    if (instruction->opcode == m_ldx)
    {
        ea_t address = BADADDR;
        if (instruction->d.size != width || !static_memory_address(instruction->r, &address))
        {
            return false;
        }
        const std::optional<uint64_t> memory_value =
...
void append_target_bytes(uint64_t value, int width, bool big_endian, std::vector<uint8_t> *output)
{
    if (!output)
        return;
    for (int index = 0; index < width; ++index)
    {
        ...
        output->push_back(static_cast<uint8_t>(value >> static_cast<unsigned>(shift_index * 8)));
    }
}

bool parse_static_write(mblock_t *block, minsn_t *instruction, static_write_t *result)
{
    if (!block || !instruction || !result)
        return false;
    const bool extended_store = instruction->opcode == m_stx;
    const int width = extended_store ? instruction->l.size : instruction->d.size;
    ea_t destination = BADADDR;
    ...
    const bool evaluated =
        extended_store
            ? evaluate_static_operand(instruction->l, instruction, width, &source, &value, &kind, 0)
            : evaluate_static_instruction(instruction, width, &source, &value, &kind, 0);
    if (!evaluated)
        return false;

    segment_t *source_segment = getseg(source);
    segment_t *destination_segment = getseg(destination);
    if (!source_segment || !destination_segment || is_code(get_flags(source)) ||
        is_code(get_flags(destination)) || (destination_segment->perm & SEGPERM_WRITE) == 0)
    {
        return false;
    }
...
bool ranges_overlap(ea_t first, size_t first_size, ea_t second, size_t second_size)
{
...
bool zero_bytes_at(ea_t address, size_t count)
{
    ...
    return chernobog::ida_memory::read_exact(bytes.data(), count, address) &&
           std::all_of(bytes.begin(), bytes.end(), [](uint8_t byte) { return byte == 0; });
}
                         ...
                         std::vector<recovered_initializer_t> *result)
{
    ...
    if (transformed < MIN_STATIC_WRITE_COUNT || transformed * 2 < writes.size())
    {
        return;
    }

    const size_t byte_count = writes.size() * static_cast<size_t>(width);
    if (ranges_overlap(writes.front().source, byte_count, writes.front().destination, byte_count))
    {
        return;
    }
    ...
    const ea_t after_destination = writes.front().destination + static_cast<ea_t>(byte_count);
    const bool implicit_terminator = zero_bytes_at(after_destination, static_cast<size_t>(width));
    const bool allow_unterminated =
        implicit_terminator || transformed >= MIN_UNTERMINATED_TEXT_CHARACTERS;
    ...
    if (!text || text->characters < MIN_STATIC_TEXT_CHARACTERS)
        return;
    if (!text->explicitly_terminated && implicit_terminator && text->characters < 2)
    {
        return;
    }
        ...
        for (minsn_t *instruction = block->head; instruction; instruction = instruction->next)
        {
            ...
            if (!parse_static_write(block, instruction, &write))
            {
                const bool deferred_static_definition =
                    ...
                    (instruction->opcode == m_xor || instruction->opcode == m_bnot ||
                     instruction->opcode == m_mov || is_mcode_set(instruction->opcode));
...
{
    return address >= object && static_cast<uint64_t>(address - object) < object_size;
}
...
{
    return mba != nullptr && (!find_static_initializers(mba).empty() ||
...
int string_decrypt_handler_t::run(mbl_array_t *mba, deobf_ctx_t *ctx)
{
    ...
    const std::vector<recovered_initializer_t> initializers = find_static_initializers(mba);
    ...
    for (const recovered_initializer_t &initializer : initializers)
    {
        ...
        qstring comment;
        comment.sprnt("Recovered static XOR string: \"%s\"", escaped.c_str());
        set_cmt(destination, comment.c_str(), true);
        set_cmt(initializer.writes.front().instruction->ea, comment.c_str(), false);

        std::vector<uint8_t> current(initializer.output.size());
        const bool readable =
            chernobog::ida_memory::read_exact(current.data(), current.size(), destination);
        if (!readable || current != initializer.output)
            patch_bytes(destination, initializer.output.data(), initializer.output.size());
        std::vector<uint8_t> materialized(initializer.output.size());
        if (chernobog::ida_memory::read_exact(materialized.data(), materialized.size(),
                                              destination) &&
            materialized == initializer.output)
        {
            ...
            (void)chernobog::hybrid::hybrid_authorize_deobfuscation_patch(
                uint64_t(ctx->func_ea), uint64_t(destination), materialized.size());
        }
        ...
        xor_string_debug("func=%llX src=%llX dst=%llX bytes=%zu chars=%zu term=%s text=\"%s\"\n",
                         static_cast<unsigned long long>(mba->entry_ea),
                         static_cast<unsigned long long>(source),
                         static_cast<unsigned long long>(destination), initializer.output.size(),
                         initializer.text.characters,
                         initializer.text.explicitly_terminated ? "explicit"
                         : initializer.implicit_terminator      ? "implicit"
                                                                : "length",
                         escaped.c_str());
...
string_decrypt_handler_t::find_encrypted_strings(ea_t func_ea)
{
    ...
    for (ea_t ea : collect_referenced_encrypted_objects(func_ea))
    {
        ...
        const size_t segment_remaining = static_cast<size_t>(segment->end_ea - ea);
        ...
        if (!chernobog::ida_memory::read_exact(string.encrypted_data.data(), object_size, ea))
            continue;
        ...
        if (get_name(&encrypted_name, ea) > 0)
        {
            ...
            string.decrypt_space_addr = get_name_ea(BADADDR, decrypt_name.c_str());
        }
...
bool string_decrypt_handler_t::extract_xor_keys(mbl_array_t *mba, encrypted_string_t *str)
{
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        for (minsn_t *ins = blk->head; ins; ins = ins->next)
        {
            ...
            uint64_t key_val = 0;
            bool found =
                ins->r.t == mop_n && ins->r.nnn && loaded_global_byte(ins->l, ins, &ref_addr);
            if (found)
                key_val = ins->r.nnn->value;
            else
            {
                found =
                    ins->l.t == mop_n && ins->l.nnn && loaded_global_byte(ins->r, ins, &ref_addr);
                if (found)
                    key_val = ins->l.nnn->value;
            }

            if (found &&
                address_in_object(ref_addr, str->encrypted_addr, str->encrypted_data.size()))
            {
                if (key_val > UINT8_MAX)
                    return false;
                const size_t offset = static_cast<size_t>(ref_addr - str->encrypted_addr);
...
{
    return chernobog::string_recovery::recover_hikari_xor_ascii(str.encrypted_data, str.xor_keys);
}
