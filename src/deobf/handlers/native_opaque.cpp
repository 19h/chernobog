...
struct block_evaluator_t
{
    ...
    block_evaluator_t() : frame_register(str2reg("X29"))
    {
        if (frame_register >= 0)
    ...
    static bool plain_memory_operand(const op_t &operand)
    {
        return (operand.type == o_mem || operand.type == o_displ || operand.type == o_phrase) &&
               operand.specflag1 == 0 && operand.specflag2 == 0 && operand.specflag3 == 0 &&
               operand.specflag4 == 0;
    }
    ...
    std::optional<uint64_t> register_value(const op_t &operand) const
    {
        ...
        return found == registers.end() ? std::nullopt
    ...
    std::optional<uint64_t> value(const op_t &operand) const
    {
        ...
        else if (operand.type == o_idpspec0)
        {
            ...
            if (operand.specflag1 != 0 || operand.specflag2 != 0 || operand.specflag3 != 0 ||
                operand.specflag4 != 0)
            {
                return std::nullopt;
            }
    ...
    void write_register(const op_t &destination, const std::optional<uint64_t> &known)
    {
    ...
    {
        for (auto iterator = stack_values.begin(); iterator != stack_values.end();)
        {
            iterator = stack_slots_overlap(iterator->first, written) ? stack_values.erase(iterator)
                                                                     : std::next(iterator);
        }
    }
    ...
    bool load(const insn_t &instruction)
    {
        ...
        if (bytes != 0 && destination_bytes != 0 && stack_address)
        {
            ...
            write_register(instruction.Op1, found == stack_values.end()
                                                ? std::nullopt
        ...
        const auto loaded = global_const_handler_t::read_admitted_scalar(*address, bytes);
    ...
    {
        return (instruction.auxpref & 1U) != 0 || instruction.itype == ARM_cmp ||
               instruction.itype == ARM_cmn || instruction.itype == ARM_tst ||
    ...
    bool binary(const insn_t &instruction)
    {
        ...
        if (sets_flags(instruction))
        {
            ...
            {
                flags = chernobog::arm64_predicate::add_flags(*left, *right, bytes);
            }
            else if (instruction.itype == ARM_sub)
            {
                flags = chernobog::arm64_predicate::sub_flags(*left, *right, bytes);
            }
            else
            {
    ...
    void process(const insn_t &instruction)
    {
        ...
        if (instruction.itype == ARM_adrp || instruction.itype == ARM_adrl ||
            instruction.itype == ARM_adr)
        {
            const std::optional<uint64_t> address =
                instruction.Op2.type == o_imm ? std::optional<uint64_t>(instruction.Op2.value)
                                              : std::nullopt;
            write_register(instruction.Op1, address);
            return;
        }

        if (instruction.itype == ARM_ldr || instruction.itype == ARM_ldur ||
        ...
        if (instruction.itype == ARM_mov || instruction.itype == ARM_movl)
        {
        ...
        if (instruction.itype == ARM_mvn || instruction.itype == ARM_neg)
        {
            ...
            if (source && bytes != 0)
            {
                result = instruction.itype == ARM_mvn ? truncate(~*source, bytes)
            ...
            instruction.Op1.type == o_reg && instruction.Op2.type == o_reg)
        {
            ...
            if (frame != frame_offsets.end() && delta && operand_bytes(instruction.Op1) == 8 &&
                !sets_flags(instruction))
            {
                const int64_t signed_delta = static_cast<int64_t>(*delta);
                write_frame_register(instruction.Op1, instruction.itype == ARM_add
    ...
    {
        if (instruction.itype == ARM_b && instruction.Op1.type == o_near &&
            instruction.segpref <= 0xD)
        {
            return flags ? chernobog::arm64_predicate::evaluate(
                               static_cast<uint8_t>(instruction.segpref), *flags)
                         : std::nullopt;
        }
        if ((instruction.itype == ARM_cbz || instruction.itype == ARM_cbnz) &&
            instruction.Op1.type == o_reg && instruction.Op2.type == o_near)
        {
...
{
    if (instruction.itype == ARM_b && instruction.Op1.type == o_near && instruction.segpref <= 0xD)
    {
        if (target)
...
std::optional<predicate_proof_t> analyze_block(ea_t function_ea, const qbasic_block_t &block)
{
    ...
    insn_t branch;
    ea_t target = BADADDR;
    if (decode_insn(&branch, tail_ea) != 4 || !conditional_branch(branch, &target) ||
...
bool apply_proof(const predicate_proof_t &proof)
{
    if (proof.branch_ea == BADADDR || proof.taken_target == BADADDR ||
        static_cast<uint32_t>(get_dword(proof.branch_ea)) != proof.original_word)
    {
        return false;
    }
    ...
    if (proof.taken)
        replacement = chernobog::arm64_branch::encode_b(proof.branch_ea, proof.taken_target);
    else
    ...
    xrefblk_t xref;
    for (bool ok = xref.first_from(proof.branch_ea, XREF_FAR); ok; ok = xref.next_from())
    {
    ...
    qstring comment;
    comment.sprnt("DEOBF: reversible native constant predicate; original=0x%08X; "
    ...
    deobf::log("[native_opaque] Patched ARM64 conditional at %a to %s "
               "(%zu admitted scalars)\n",
...
{
    qstring value;
    return qgetenv("CHERNOBOG_NATIVE_OPAQUE", &value) && !value.empty() && value[0] == '1' ? 1 : 0;
}
...
native_opaque_stats_t native_opaque_handler_t::run()
{
    ...
    for (size_t function_index = 0; function_index < function_count; ++function_index)
    {
        ...
        qflow_chart_t flowchart("", function, BADADDR, BADADDR, FC_NOEXT);
    ...
    deobf::log("[native_opaque] functions=%d blocks=%d conditional=%d "
               "proved=%d patched=%d\n",
