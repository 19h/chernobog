...
stack_string_handler_t::find_stack_strings(mblock_t *blk)
{
    ...
    std::sort(stores.begin(), stores.end(),
              [](const byte_store_t &a, const byte_store_t &b) { return a.offset < b.offset; });
...
//--------------------------------------------------------------------------
bool stack_string_handler_t::analyze_byte_sequence(const std::vector<byte_store_t> &stores,
                                                   stack_string_t *out)
{
...
bool stack_string_handler_t::is_stack_byte_store(minsn_t *ins, byte_store_t *out)
{
    ...
    else if (ins->l.t == mop_d && ins->l.d)
    {
        ...
        value = *resolved;
        transformed =
            (ins->l.d->opcode == m_bnot || ins->l.d->opcode == m_lnot || ins->l.d->opcode == m_xor);
    }
    else
    {
        return false;
...
//--------------------------------------------------------------------------
std::optional<uint8_t> stack_string_handler_t::resolve_byte_value(minsn_t *ins, int depth)
{
    if (!ins || depth > 16)
        return std::nullopt;
    ...
    if (ins->opcode == m_bnot)
    {
        ...
        if (ins->l.t == mop_d && ins->l.d)
        {
            ...
            return value ? std::optional<uint8_t>(static_cast<uint8_t>(~*value)) : std::nullopt;
        }
        return std::nullopt;
    }
    ...
    if (ins->opcode == m_lnot)
    {
        auto value = ins->l.t == mop_n && ins->l.nnn
                         ...
                         : (ins->l.t == mop_d && ins->l.d ? resolve_byte_value(ins->l.d, depth + 1)
                                                          : std::nullopt);
        return value ? std::optional<uint8_t>(*value == 0 ? 1 : 0) : std::nullopt;
    }
    ...
    if (ins->opcode == m_xor)
    {
        auto operand = [depth](const mop_t &op) -> std::optional<uint8_t>
        {
            ...
            if (op.t == mop_d && op.d)
                return stack_string_handler_t::resolve_byte_value(op.d, depth + 1);
            return std::nullopt;
        };
