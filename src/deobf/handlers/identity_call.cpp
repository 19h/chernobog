...
std::map<ssize_t, std::map<ea_t, ea_t>> identity_call_handler_t::s_trampoline_cache;
...
identity_call_handler_t::deferred_cache_t &identity_call_handler_t::deferred_cache()
{
...
bool identity_call_handler_t::detect(mbl_array_t *mba)
{
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        {
            if (preceding_identity != BADADDR && (ins->opcode == m_ijmp || ins->opcode == m_icall))
            {
                deobf::log_verbose(
            ...
            preceding_identity =
                target != BADADDR && is_identity_function(target) ? target : BADADDR;
        }
    }
    return false;
}
...
int identity_call_handler_t::run(mbl_array_t *mba, deobf_ctx_t *ctx)
{
    ...
    for (const auto &ic : identity_calls)
    {
        ...
        deobf::log("[identity_call] Analyzed: call@%a -> %s (%a)\n", ic.call_ea,
                   target_name.c_str(), ic.final_target);
    }
...
identity_call_handler_t::find_identity_calls(mbl_array_t *mba, deobf_ctx_t *ctx)
{
    ...
    for (const auto &call_pair : calls)
    {
        ...
        for (const auto &branch_pair : indirect_branches)
        {
            ...
            const bool is_match = branch_blk == call_blk && cinfo.container_ins->next == branch_ins;
            ...
            if (global_ptr != BADADDR)
            {
                ...
                if (resolved != BADADDR)
                {
                    ...
                    {
                        deobf::log(
                            "[identity_call] Initial ptr %a resolved to self-ref %a, trying LEA search\n",
            ...
            if (final_target == BADADDR && cinfo.call_ea != BADADDR)
            {
                ...
                if (table_base != BADADDR)
                {
                    ...
                    if (target0 != BADADDR)
                    {
                        ...
                        ea_t final1 =
                            (target1 != BADADDR) ? resolve_trampoline_chain(target1) : BADADDR;

                        deobf::log("[identity_call] Resolved: [0]=%a->%a, [1]=%a->%a\n", target0,
                                   final0, target1, final1);
            ...
            {
                deobf::log_verbose(
                    "[identity_call] find: pattern found but couldn't resolve target\n");
                continue;
            }
            ...
            func_t *curr_func = get_func(mba->entry_ea);
            if (curr_func && final_target >= curr_func->start_ea &&
                final_target < curr_func->end_ea)
            {
                deobf::log(
                    "[identity_call] Skipping internal reference: target %a is within function\n",
                    final_target);
                continue;
            }
            ...
            deobf::log(
                "[identity_call] find: pattern matched: call@%a -> ptr=%a -> %a -> final %a\n",
...
identity_call_handler_t::resolve_indexed_table(ea_t table_base, ea_t func_ea)
{
    ...
    if (func_ea != BADADDR)
    {
        ...
        bool t0_is_self =
            (final0 == func_ea) || (func && final0 >= func->start_ea && final0 < func->end_ea);
        bool t1_is_self =
            (final1 == func_ea) || (func && final1 >= func->start_ea && final1 < func->end_ea);
