...
struct switch_fold_visitor_t : public ctree_visitor_t
{
    ...
    explicit switch_fold_visitor_t(cfunc_t *f) : ctree_visitor_t(CV_PARENTS), func(f) {}
    ...
    int idaapi visit_insn(cinsn_t *ins) override
    {
        ...
        deobf::log_verbose("[ctree_switch_fold] inspected switch at %a cases=%zu expr-op=%d\n",
        ...
        ++proved;
        deobf::log_verbose("[ctree_switch_fold] proved selector at %a value=0x%llx\n", ins->ea,
        ...
        {
            ++rejected_no_case;
            deobf::log("[ctree_switch_fold] proved but rejected at %a: "
        ...
        if (static_cast<size_t>(matching_idx + 1) < sw->cases.size() && !had_trailing_break)
        {
            delete replacement;
        ...
        changes++;
        deobf::log("[ctree_switch_fold] rewritten at %a: value=0x%llx case=%d\n", ins->ea,
                   (unsigned long long)const_val, matching_idx);
        return 0;
    }
};
...
int ctree_switch_fold_handler_t::run(cfunc_t *cfunc)
{
    ...
    {
        deobf::log("[ctree_switch_fold] summary function=%a inspected=%d proved=%d "
