...
bool block_merge_handler_t::detect_split_blocks(mbl_array_t *mba)
{
    ...
    const double ratio = static_cast<double>(small_blocks) / static_cast<double>(mba->qty);
    if (ratio <= 0.30)
        return false;
    ...
    for (int start = 0; start < mba->qty; ++start)
    {
        ...
        int current = start;
        int chain_length = 0;
        while (current >= 0 && current < mba->qty && candidates[static_cast<size_t>(current)] &&
               std::find(seen, seen + chain_length, current) == seen + chain_length)
        {
