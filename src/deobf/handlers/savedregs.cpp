...
bool extract_stack_source(const mop_t &operand, sval_t *offset)
{
    ...
    if (operand.t == mop_a && operand.a && operand.a->t == mop_S && operand.a->s &&
        operand.a->s->off < 0)
    {
...
bool savedregs_handler_t::is_savedregs_ref(const mop_t &operand, sval_t *offset)
{
...
bool savedregs_handler_t::detect(mbl_array_t *mba)
{
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        for (minsn_t *instruction = block->head; instruction; instruction = instruction->next)
        {
            ...
            if (instruction->opcode != m_call && instruction->opcode != m_icall)
                continue;
            const mop_t &target = instruction->opcode == m_icall ? instruction->r : instruction->l;
...
bool savedregs_handler_t::is_objc_msgsend(ea_t address)
{
    ...
    if (symbol[0] == '_')
        ++symbol;
    return strncmp(symbol, "objc_msgSend", 12) == 0 || strncmp(symbol, "objc_msgLookup", 14) == 0;
}

void savedregs_handler_t::extract_call_args(mblock_t *block, minsn_t *call, resolved_call_t *out)
{
    ...
    for (size_t i = 0; i < info->args.size(); ++i)
    {
        ...
        {
            auto address =
                stack_tracker_t::trace_address(block, call, offset, arch::get_ptr_size());
...
bool savedregs_handler_t::resolve_indirect_call(mblock_t *block, minsn_t *call,
                                                resolved_call_t *out)
{
    if (!block || !call || !out || (call->opcode != m_icall && call->opcode != m_call))
    {
        return false;
    }
    ...
    auto address = stack_tracker_t::trace_address(block, call, offset, arch::get_ptr_size());
    if (!address)
        return false;
...
void savedregs_handler_t::annotate_call(const resolved_call_t &resolved)
{
    ...
    {
        comment.sprnt("DEOBF: Indirect call to %s (0x%llX)", resolved.target_name.c_str(),
    ...
    for (size_t i = 0; i < resolved.args.size(); ++i)
    {
        if (resolved.args[i].empty() ||
            (resolved.is_objc && i == 1 && resolved.args[i] == resolved.selector))
            continue;
        comment.cat_sprnt("\n  arg[%zu]: \"%s\"", i, resolved.args[i].c_str());
    }
