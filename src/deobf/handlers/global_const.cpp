...
void global_const_debug(const char *format, ...)
{
    ...
    deobf::debug_vlog("/tmp/chernobog_global_const_debug.log", format, arguments);
    va_end(arguments);
}
...
bool dead_global_store_mode_enabled()
{
    ...
    if (cached == -1)
    {
        qstring value;
        cached =
            qgetenv("CHERNOBOG_DEAD_GLOBAL_STORES", &value) && !value.empty() && value[0] == '1'
                ? 1
                : 0;
    }
    return cached == 1;
...
int global_const_handler_t::remove_write_only_stores(mbl_array_t *mba)
{
    ...
    for (int block_index = 0; block_index < mba->qty; ++block_index)
    {
        ...
        for (minsn_t *instruction = block->head; instruction; instruction = instruction->next)
        {
            ...
            else if (instruction->opcode == m_stx && instruction->d.t == mop_v)
            {
                address = instruction->d.g;
    ...
    if (changes > 0)
    {
        deobf::log("[global_const] Removed %d closed-world write-only global stores\n", changes);
    }
    return changes;
}
...
//--------------------------------------------------------------------------
int global_const_handler_t::simplify_value_operand(mblock_t *blk, mop_t *operand, int depth)
{
    if (!operand || depth > 32)
        return 0;
    if (operand->t == mop_d && operand->d)
        return simplify_value_instruction(blk, operand->d, depth + 1);
    if (operand->t != mop_v || operand->g == BADADDR || operand->size <= 0 || operand->size > 8)
    {
        return 0;
    }
    ...
    if (is_code(get_flags(address)) ||
        (!is_const_data(address) && !is_write_free_writable_data(address, size)))
    {
        return 0;
    }
...
int global_const_handler_t::simplify_value_instruction(mblock_t *blk, minsn_t *ins, int depth)
{
    if (!ins || depth > 32)
        return 0;
...
bool global_const_handler_t::is_global_const_load(minsn_t *ins, global_const_t *out)
{
    ...
    if (!is_const_data(gv_addr) && !is_write_free_writable_data(gv_addr, size))
        return false;
...
bool global_const_handler_t::is_write_free_writable_data(ea_t addr, int size)
{
    ...
    if (mode == 0 || addr == BADADDR || (size != 1 && size != 2 && size != 4 && size != 8) ||
    ...
    if (segment && (segment->perm & SEGPERM_WRITE) != 0 && !is_code(get_flags(addr)) &&
        is_loaded(addr))
    {
        ...
        if (item_start != BADADDR && item_end > item_start && is_data(get_flags(item_start)) &&
            item_end - item_start <= MAX_WRITABLE_OBJECT_SIZE && addr >= item_start &&
        ...
        if (mode >= 2 && size == 1 && object_start == addr && object_end == addr + 1)
        {
            xrefblk_t exact_xref;
            if (!exact_xref.first_to(addr, XREF_DATA))
            {
                for (asize_t distance = 1; distance <= MAX_WRITABLE_ANCHOR_DISTANCE &&
                     ...
                     ++distance)
                {
                    ...
                    if (!is_loaded(candidate) || is_code(flags) || exists_fixup(candidate))
                    {
                        break;
                    }
        ...
        const bool object_model =
            object_start != addr || object_end != addr + static_cast<ea_t>(size);
        ...
        const writable_object_key_t object_key(get_dbctx_id(), object_start, object_end);
        if (object_model)
        {
            const auto object_cached = s_writable_object_classification.find(object_key);
        ...
        if (!has_fixup)
        {
            ...
            for (ea_t target = object_start;
                 target < object_end && !unclassified_reference && !saw_write; ++target)
            {
                xrefblk_t xref;
                for (bool ok = xref.first_to(target, XREF_DATA); ok; ok = xref.next_to())
                {
                    bool reads = false;
                    bool writes = false;
                    if (is_code(get_flags(xref.from)))
                    {
                        ...
                        if ((xref.type == dr_R && !reads) || (xref.type == dr_W && !writes))
                        {
                            unclassified_reference = true;
                            break;
                        }
                    ...
                    fixup_data_t fixup;
                    if (mode >= 2 || (get_fixup(&fixup, xref.from) && !fixup.is_unused()))
                    {
                        address_taken = true;
                        continue;
                    }
            ...
            admitted =
                (saw_read || caller_proves_object_read) && !saw_write && !unclassified_reference;
            if (admitted && address_taken)
            {
                deobf::log_verbose("[global_const] Tier-2 admitted address-taken writable "
    ...
    if (admitted)
    {
        deobf::log_verbose("[global_const] Admitted write-free writable scalar at %a (%d bytes)\n",
                           addr, size);
    }
    return admitted;
}
...
bool global_const_handler_t::is_direct_write_only_data(ea_t addr, int size)
{
    if (!dead_global_store_mode_enabled() || addr == BADADDR ||
        (size != 1 && size != 2 && size != 4 && size != 8) || addr % static_cast<ea_t>(size) != 0)
    {
        return false;
    }
    ...
    global_const_debug("sink candidate=%llX size=%d seg=%p perm=%d code=%d user=%d loaded=%d\n",
                       ...
                       is_loaded(addr) ? 1 : 0);
    if (segment && (segment->perm & SEGPERM_WRITE) != 0 && (segment->perm & SEGPERM_EXEC) == 0 &&
        !is_code(flags) && !has_user_name(flags) && is_loaded(addr))
    {
        ...
        if (!has_fixup)
        {
            ...
            bool rejected = false;
            xrefblk_t xref;
            for (bool ok = xref.first_to(addr, XREF_DATA); ok; ok = xref.next_to())
            {
                global_const_debug("  xref from=%llX type=%d from_code=%d\n",
...
void global_const_handler_t::clear_cache()
{
    ...
    for (auto it = s_writable_classification.begin(); it != s_writable_classification.end();)
    {
    ...
    for (auto it = s_write_only_classification.begin(); it != s_write_only_classification.end();)
    {
        if (std::get<0>(it->first) == database)
...
//--------------------------------------------------------------------------
std::optional<uint64_t> global_const_handler_t::read_admitted_scalar(ea_t addr, int size)
{
    ...
    if (has_fixup || (!is_const_data(addr) && !is_write_free_writable_data(addr, size)))
    {
        return std::nullopt;
    }
    const std::optional<uint64_t> value = read_global_value(addr, size);
    return value && !looks_like_pointer(*value, size) ? value : std::nullopt;
}
