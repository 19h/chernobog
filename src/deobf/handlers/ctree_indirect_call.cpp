            ...
            if (callee->op == cot_sub)
            {
                ...
                if (right && right->op == cot_num)
                {
                    ...
                    if (left && left->op == cot_idx)
                    {
                        ...
                        {
                            ctree_icall_debug(
                                "[ctree_icall]   PATTERN FOUND: table=0x%llx, idx=%lld, off=%lld\n",
                                (unsigned long long)table_addr, (long long)index,
                                (long long)offset);
...
int ctree_indirect_call_handler_t::run(cfunc_t *cfunc, deobf_ctx_t *ctx)
{
    ...
    for (cexpr_t *call_expr : finder.found_patterns)
    {
        ...
        qstring comment;
        comment.sprnt("DEOBF: Resolved indirect call -> %s (0x%llX)", target_name.c_str(),
                      (unsigned long long)target);
        deobf::set_cmt_if_changed(call_expr->ea, comment.c_str(), false);
                                                        ...
                                                        uint64_t offset)
{
    ...
    auto entry = chernobog::ida_memory::read_integer(entry_addr, static_cast<int>(entry_size));
    if (!entry)
    {
    ...
    ctree_icall_debug("[ctree_icall] table[%lld] = 0x%llx, - %lld = 0x%llx\n", (long long)index,
                      (unsigned long long)entry_val, (long long)offset, (unsigned long long)target);
