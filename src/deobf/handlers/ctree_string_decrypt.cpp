...
static std::vector<uint8_t> aes_decrypt(const std::vector<uint8_t> &ciphertext,
                                        ...
                                        const std::vector<uint8_t> &iv, bool pkcs7_padding,
                                        bool use_ecb)
{
    ...
    if (key_size != kCCKeySizeAES128 && key_size != kCCKeySizeAES192 &&
    ...
    CCCryptorStatus status =
        CCCrypt(kCCDecrypt, kCCAlgorithmAES, options, key.data(), key_size,
                use_ecb ? nullptr : (iv.empty() ? nullptr : iv.data()), ciphertext.data(),
                ciphertext.size(), plaintext.data(), out_size, &decrypted_size);
...
static bool try_aes_decrypt_at_address(ea_t data_addr, size_t data_len,
                                       const std::vector<uint8_t> &key,
                                       const std::vector<uint8_t> &iv, bool pkcs7_padding,
                                       bool use_ecb, qstring *out_plaintext)
{
    ...
    if (!chernobog::ida_memory::read_exact(ciphertext.data(), data_len, data_addr))
    {
...
struct string_call_visitor_t : public ctree_visitor_t
{
    ...
    int idaapi visit_expr(cexpr_t *e) override
    {
        ...
        if (name == "strcpy" || name == "strcpy_chk" || name == "strncpy" || name == "strlcpy")
        {
            process_strcpy(e, name);
        }
        else if (name == "memcpy" || name == "memcpy_chk" || name == "memmove" ||
                 name == "qmemcpy" || name == "bcopy")
        {
  ...
  private:
        ...
        if (func_name == "strncpy" || func_name == "strlcpy")
        {
            ...
            else
            {
                if (capacity == 0)
                    return;
                const size_t copied =
                    std::min<size_t>(str_val.length(), static_cast<size_t>(capacity - 1));
    ...
    void process_crypto_call(cexpr_t *call, const qstring &func_name)
    {
        ...
        constexpr size_t MAX_STATIC_CRYPTO_INPUT = size_t{16} * size_t{1024} * size_t{1024};
        ...
        if (!op_arg || !alg_arg || !options_arg || !key_len_arg || !input_len_arg ||
            op_arg->op != cot_num || alg_arg->op != cot_num || options_arg->op != cot_num ||
        ...
        if (op_arg->numval() != 1 || alg_arg->numval() != 0 || (options & ~allowed_options) != 0)
            return;
        ...
        if (key_addr != BADADDR)
        {
            ...
            if (!chernobog::ida_memory::read_exact(crypto.key.data(), crypto.key.size(), key_addr))
                return;
        }
        else
        {
            qstring key_string;
            if (!extract_string_constant(key_arg, &key_string) || key_string.length() != key_len)
                return;
            crypto.key.assign(reinterpret_cast<const uint8_t *>(key_string.c_str()),
        ...
        if (!use_ecb)
        {
            ...
            if (iv_addr != BADADDR)
            {
                ...
                if (!chernobog::ida_memory::read_exact(crypto.iv.data(), crypto.iv.size(), iv_addr))
                    return;
            }
            else if (!is_null_pointer(iv_arg))
            {
                ...
                if (!extract_string_constant(iv_arg, &iv_string) ||
                    ...
                    return;
                crypto.iv.assign(reinterpret_cast<const uint8_t *>(iv_string.c_str()),
        ...
        ctree_str_debug("[crypto] Attempting AES-%d decryption: input=0x%llx len=%zu\n",
                        crypto.algorithm_bits, static_cast<unsigned long long>(crypto.input_addr),
                        crypto.input_len);
        if (try_aes_decrypt_at_address(crypto.input_addr, crypto.input_len, crypto.key, crypto.iv,
...
static bool read_value_at_address(ea_t addr, size_t size, uint64_t *out)
{
    ...
    const auto value = global_const_handler_t::read_admitted_scalar(addr, static_cast<int>(size));
    if (!value)
        return false;
    *out = *value;
    return true;
...
static bool resolve_const_address(cexpr_t *e, ea_t *out_addr)
{
    ...
    if (e->op == cot_add)
    {
        ...
        if (base_addr == BADADDR || displacement > uint64_t(BADADDR - base_addr - 1))
            return false;
...
static bool eval_expr_u64(cexpr_t *e, const std::map<int, uint64_t> &locals, uint64_t *out,
                          int size_hint, int depth = 0)
{
    if (!e || !out || depth > 16)
        return false;
    ...
    const int expression_size =
        raw_expression_size <= 8 ? static_cast<int>(raw_expression_size) : 0;
    const int result_size =
        chernobog::bitvector::valid_byte_width(size_hint)
            ? size_hint
            : (chernobog::bitvector::valid_byte_width(expression_size) ? expression_size : 8);
    ...
    case cot_obj:
    {
        ...
        const int tsize = raw_type_size <= 8 ? static_cast<int>(raw_type_size) : 0;
    ...
    case cot_idx:
    {
        ...
        if (!resolve_const_address(e->x, &base))
            return false;
        const size_t raw_type_size = e->type.get_size();
        const int element_size = raw_type_size <= 8 ? static_cast<int>(raw_type_size) : 0;
        const int read_size = size_hint ? size_hint : element_size;
        if (!chernobog::bitvector::valid_byte_width(read_size) || element_size <= 0)
            return false;
        const uint64_t index = e->y->numval();
        if (index > uint64_t(BADADDR) / static_cast<uint64_t>(element_size))
            return false;
        const uint64_t displacement = index * static_cast<uint64_t>(element_size);
        if (base == BADADDR || displacement > uint64_t(BADADDR - base - 1))
            return false;
        return read_value_at_address(base + static_cast<ea_t>(displacement),
    ...
    case cot_ushr:
    {
        ...
        if ((e->op == cot_shl || e->op == cot_sshr || e->op == cot_ushr) && r >= bits)
            return false;
        switch (e->op)
        {
        case cot_add:
            *out = chernobog::bitvector::truncate(l + r, result_size);
            break;
        case cot_sub:
            *out = chernobog::bitvector::truncate(l - r, result_size);
            break;
        ...
        case cot_shl:
            *out = chernobog::bitvector::shift_left(l, r, result_size);
            break;
        case cot_sshr:
            *out = chernobog::bitvector::shift_right_arithmetic(l, r, result_size);
            break;
        case cot_ushr:
            *out = chernobog::bitvector::shift_right_logical(l, r, result_size);
            break;
        default:
            return false;
        }
...
struct char_assign_visitor_t : public ctree_visitor_t
{
    ...
    using byte_assignments_t = std::map<int, std::pair<uint8_t, ea_t>>;
    ...
    int idaapi visit_expr(cexpr_t *e) override
    {
        ...
        if (lhs->op == cot_var)
        {
            ...
            int size_hint = raw_size_hint <= 8 ? static_cast<int>(raw_size_hint) : 0;
    ...
    std::vector<ctree_string_decrypt_handler_t::char_string_t> get_reconstructed_strings()
    {
        ...
        for (auto &kv : var_assignments)
        {
            ...
            std::vector<ctree_string_decrypt_handler_t::char_string_t> candidates;
        ...
        for (auto &kv : global_assignments)
        {
            ...
            for (auto &context_entry : kv.second)
            {
                ...
                {
                    candidate.var_name.sprnt("global_%llX", (unsigned long long)addr);
                }
                if (try_reconstruct(offsets, &candidate))
                    candidates.push_back(std::move(candidate));
            }
  ...
  private:
    ...
    context_t linear_context() const
    {
        ...
        for (const citem_t *parent : parents)
        {
            ...
            if (parent->is_expr())
            {
                if (parent->op == cot_tern || parent->op == cot_land || parent->op == cot_lor)
                    return nullptr;
                continue;
            }
            ...
            else if (parent->op == cit_for || parent->op == cit_while || parent->op == cit_do ||
    ...
    bool extract_byte_value(cexpr_t *e, const std::map<int, uint64_t> &values, uint8_t *out)
    {
        if (!e || !out)
            return false;
    ...
    bool extract_qword_value(cexpr_t *e, const std::map<int, uint64_t> &values, uint64_t *out)
    {
        if (!e || !out)
            return false;
    ...
    bool try_handle_vector_assign(cexpr_t *lhs, cexpr_t *rhs, ea_t ea, context_t context)
    {
        if (!lhs || !rhs)
            return false;
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            record_assignment(target, target.base_index + i, b, ea, context);
        }
    ...
    static bool unique_reconstruction(
        const std::vector<ctree_string_decrypt_handler_t::char_string_t> &candidates)
    {
        if (candidates.empty())
            return false;
        return std::all_of(std::next(candidates.begin()), candidates.end(),
                           [&](const auto &candidate)
                           { return candidate.reconstructed == candidates.front().reconstructed; });
    }
...
// Main entry point
//--------------------------------------------------------------------------
static int apply_runtime_string_arguments(cfunc_t *cfunc,
                                          const std::map<ea_t, qstring> &runtime_plaintexts);
static int preserve_runtime_cfstring_addresses(cfunc_t *cfunc,
                                               const std::map<ea_t, qstring> &runtime_plaintexts);
static bool protected_runtime_cfstring_address(const cfunc_t *cfunc, const cexpr_t *assignment,
                                               ea_t *address);

static void record_plaintext_fact(deobf_ctx_t *ctx, ea_t address, const char *plaintext,
                                  const char *source)
{
    if (ctx == nullptr || address == BADADDR || plaintext == nullptr || plaintext[0] == '\0' ||
                                        ...
                                        ctree_maturity_t event_maturity)
{
    ...
    for (const chernobog::hybrid::RuntimeStringCandidate &candidate : runtime_strings)
    {
        if (candidate.address > uint64_t(BADADDR))
            continue;
        const ea_t address = ea_t(candidate.address);
        record_plaintext_fact(ctx, address, candidate.value.c_str(), "rax cross-run consensus");
    ...
    for (const auto &reveal : call_visitor.reveals)
    {
        ...
        {
            record_plaintext_fact(ctx, reveal.dest_addr, reveal.plaintext.c_str(), "strcpy/memcpy");
        }
    ...
    for (const auto &crypto : call_visitor.crypto_calls)
    {
        ...
        if (!crypto.decrypted.empty())
        {
            if (crypto.input_addr != BADADDR)
            {
                record_plaintext_fact(ctx, crypto.input_addr, crypto.decrypted.c_str(),
                                      "crypto input");
            }
            if (crypto.output_addr != BADADDR)
            {
                record_plaintext_fact(ctx, crypto.output_addr, crypto.decrypted.c_str(),
                                      "crypto output");
            }
    ...
    for (const auto &str : char_strings)
    {
        ...
        {
            record_plaintext_fact(ctx, str.var_addr, str.reconstructed.c_str(), "bytewise");
        }
    ...
    ctree_str_debug(
        "[ctree_string] Found %zu strcpy/memcpy reveals, %zu crypto calls, %zu char strings\n",
    ...
    {
        runtime_object_replacements = replace_encrypted_strings(cfunc, runtime_plaintexts, false);
    ...
    {
        int replaced = replace_encrypted_strings(cfunc, addr_to_plaintext, true);
        if (replaced > 0)
        {
    ...
    if (!runtime_plaintexts.empty())
    {
        runtime_argument_replacements = apply_runtime_string_arguments(cfunc, runtime_plaintexts);
        replacements += runtime_argument_replacements;
        deobf::log(
            "[chernobog][rax] Runtime plaintext: %zu consensus strings, %d CFString/object uses and %d C-string argument uses applied\n",
            runtime_plaintexts.size(), runtime_object_replacements, runtime_argument_replacements);
    }
    ...
    const int addresses = (event_maturity == CMAT_FINAL || cfunc->maturity == CMAT_FINAL)
                              ? preserve_runtime_cfstring_addresses(cfunc, runtime_plaintexts)
                              : 0;
...
//--------------------------------------------------------------------------
static bool replace_ctree_string_literal(cexpr_t *target, const qstring &plaintext)
{
    if (target == nullptr || plaintext.empty())
        return false;
    if (target->op == cot_str && target->string != nullptr && plaintext == target->string)
        return false;
...
struct encrypted_string_replacer_t : public ctree_visitor_t
{
    ...
    encrypted_string_replacer_t(cfunc_t *cf, const std::map<ea_t, qstring> &plaintexts,
                                bool persist)
        : ctree_visitor_t(CV_PARENTS), cfunc(cf), known_plaintexts(plaintexts),
          persist_annotations(persist)
    {
    }
    ...
    static bool is_encrypted_string(ea_t addr, size_t max_len = 256)
    {
        ...
        for (size_t i = 0; i < max_len; ++i)
        {
            if (addr > BADADDR - 1 - static_cast<ea_t>(i))
                return false;
            const auto byte = chernobog::ida_memory::read_integer(addr + static_cast<ea_t>(i), 1);
            if (!byte)
                return false;
    ...
    static bool read_materialized_string(ea_t addr, uint64_t length, qstring *out)
    {
        if (out == nullptr || addr == BADADDR || length == 0 || length >= 4096 ||
            length > static_cast<uint64_t>(SIZE_MAX) || addr > BADADDR - static_cast<ea_t>(length))
        {
            return false;
        }
    ...
    bool replacement_requires_lvalue(cexpr_t *current, cexpr_t *target)
    {
    ...
    int idaapi visit_expr(cexpr_t *e) override
    {
        ...
        if (e->op == cot_asg && protected_runtime_cfstring_address(cfunc, e, nullptr))
        {
            prune_now();
            return 0;
        }
        ...
        else if (str_addr != BADADDR)
        {
            ...
            bool is_cfstring_struct =
                seg_name.find("cfstring") != qstring::npos ||
            ...
            if (!is_cfstring_struct && seg_name == "__data")
            {
                ...
                const auto maybe_flags =
                    chernobog::ida_memory::read_integer(str_addr + flags_offset, pointer_bytes);
                const ea_t maybe_ptr = arch::read_ptr(str_addr + pointer_offset);
                const auto maybe_len =
                    chernobog::ida_memory::read_integer(str_addr + length_offset, pointer_bytes);

                // Heuristic: flags should be non-zero but reasonable, ptr should be valid, len should be small
                if (maybe_flags && *maybe_flags != 0 && *maybe_flags < 0x10000 && maybe_ptr != 0 &&
                    maybe_ptr != BADADDR && is_loaded(maybe_ptr) && maybe_len && *maybe_len > 0 &&
                    *maybe_len < 4096)
                {
                    ...
                    if (p != known_plaintexts.end())
                    {
                        ...
                        ctree_str_debug(
                            "[replace] Detected CFString structure by layout at 0x%llx (ptr=0x%llx)\n",
            ...
            ctree_str_debug(
                "[replace] Checking as CFSTR structure: ptr = 0x%llx, len = %llu, is_loaded=%d\n",
            ...
            if (ptr != 0 && ptr != BADADDR && is_loaded(ptr))
            {
                ...
                else
                {
                    ...
                    ctree_str_debug(
                        "[replace] Length field invalid (len=%llu), computing actual length\n",
                    ...
                    for (size_t i = 0; i < 256; ++i)
                    {
                        if (ptr > BADADDR - 1 - static_cast<ea_t>(i))
                            break;
                        const auto byte =
                            chernobog::ida_memory::read_integer(ptr + static_cast<ea_t>(i), 1);
                        if (!byte)
                            break;
                    ...
                    if (actual_len > 0)
                    {
                        actual_str_addr = ptr;
                        ctree_str_debug(
                            "[replace] Using CFSTR string ptr 0x%llx (computed len=%zu)\n",
            ...
            if (p != known_plaintexts.end())
            {
                decrypted = p->second;
                found = true;
                ctree_str_debug("[replace] Direct address match%s! Using plaintext: \"%s\"\n",
                                is_encrypted_string(actual_str_addr) ? " (encrypted bytes)"
                                                                     : " (materialized bytes)",
                                decrypted.c_str());
            }
            else if (read_materialized_string(actual_str_addr, len_field, &decrypted))
            {
                found = true;
                ctree_str_debug("[replace] Exact materialized CFSTR bytes: \"%s\"\n",
                                decrypted.c_str());
            }
            else
            {
        ...
        if (replacement_requires_lvalue(e, target_expr))
        {
            ...
            ctree_str_debug("[replace] Skipping lvalue-required %s child of %s at 0x%llx\n",
        ...
        if (persist_annotations && target_ea != BADADDR)
        {
            ...
            deobf::set_cmt_if_changed(target_ea, comment.c_str(), false);
        }
...
static bool known_runtime_string_argument(const std::string &name, const carglist_t &arguments,
                                          size_t index)
{
    if ((name == "objc_msgSend" || name == "objc_msgSendSuper") && index >= 2)
        return true;
    // Resolved Objective-C calls retain the selector as argument one.
    if (arguments.size() >= 2 && arguments[1].op == cot_str && index >= 2)
        return true;

    if (name == "strstr" || name == "strcasestr" || name == "strcmp" || name == "strcasecmp" ||
        name == "strncmp" || name == "strncasecmp" || name == "popen" || name == "fopen")
        return index <= 1;
    if (name == "strlen" || name == "strdup" || name == "strchr" || name == "strrchr" ||
        name == "strpbrk" || name == "strspn" || name == "strcspn" || name == "system" ||
        name == "puts" || name == "atoi" || name == "atol" || name == "atoll" || name == "strtol" ||
        name == "strtoul" || name == "strtoll" || name == "strtoull" || name == "printf" ||
        name == "scanf")
        return index == 0;
...
struct runtime_string_argument_replacer_t : public ctree_visitor_t
{
    ...
    explicit runtime_string_argument_replacer_t(const std::map<ea_t, qstring> &values)
    ...
    int idaapi visit_expr(cexpr_t *expression) override
    {
        ...
        for (size_t index = 0; index < expression->a->size(); ++index)
        {
            ...
            {
                ++replacements;
                ctree_str_debug("[rax] Applied runtime literal at call 0x%llx arg %zu: \"%s\"\n",
...
{
    if (address == BADADDR || size == 0 || size - 1 > uint64_t(BADADDR - 1 - address))
    {
        return false;
    }
...
static bool only_preceding_flow_enters(ea_t target, ea_t preceding_head)
{
    ...
    {
        if (reference.iscode && (reference.type != fl_F || reference.from != preceding_head))
        {
            return false;
        }
    }
...
static bool native_cfstring_address_origin(const cfunc_t *cfunc, ea_t store_ea,
                                           ea_t expected_address)
{
    if (cfunc == nullptr || !arch::is_arm64() || inf_is_be() || store_ea == BADADDR ||
        store_ea < 8 || (store_ea & 3U) != 0 || expected_address == BADADDR)
    {
        return false;
    }
    ...
    if (function == nullptr || function->start_ea != cfunc->entry_ea ||
        !is_same_fchunk(adrp_ea, store_ea) || !is_same_fchunk(add_ea, store_ea) ||
        cfunc->entry_ea == add_ea || cfunc->entry_ea == store_ea ||
        !fully_loaded_range(adrp_ea, 12) || get_item_head(adrp_ea) != adrp_ea ||
        get_item_head(store_ea) != store_ea || !is_code(get_full_flags(adrp_ea)) ||
    ...
    if ((add_head != add_ea && add_head != adrp_ea) || get_item_end(add_head) != store_ea ||
    ...
    if (!adrp || !add || !store)
        return false;
    const auto address = chernobog::aarch64_address_origin::decode_adrp_add_str_address(
...
{
    if (assignment == nullptr || assignment->op != cot_asg || assignment->x == nullptr ||
        assignment->y == nullptr || (assignment->x->op != cot_ptr && assignment->x->op != cot_idx))
    {
        return false;
    }
    const tinfo_t &left = assignment->x->type;
    const tinfo_t &right = assignment->y->type;
    return left.is_integral() && !left.is_enum() && !left.is_bool() && right.is_integral() &&
           !right.is_enum() && !right.is_bool() && left.get_size() == 8 && right.get_size() == 8 &&
           (left.is_signed() || left.is_unsigned()) && left.get_sign() == right.get_sign() &&
           left == right;
}
                                               ...
                                               ea_t *address)
{
    ...
    if (cast->op != cot_cast || (cast->exflags & EXFL_UCAST) == 0 || cast->ea != assignment->ea ||
        cast->x == nullptr || cast->x->op != cot_ref || cast->x->x == nullptr ||
        cast->x->x->op != cot_obj || cast->x->ea != assignment->ea ||
        cast->x->x->ea != assignment->ea || !cast->x->type.is_ptr() ||
        cast->x->type.get_size() != 8 || cast->x->type.get_pointed_object() != cast->x->x->type ||
        cast->x->x->refwidth != -1)
    {
        return false;
    }
    const ea_t object_address = cast->x->x->obj_ea;
    if (!native_cfstring_address_origin(cfunc, assignment->ea, object_address))
    {
        return false;
    }
    if (address != nullptr)
...
static bool strict_runtime_cfstring_relation(ea_t object_address,
                                             const std::map<ea_t, qstring> &runtime_plaintexts,
                                             runtime_cfstring_relation_t *relation)
{
    ...
    if (header_segment == nullptr || getseg(object_address + 31) != header_segment ||
        (header_segment->perm & SEGPERM_READ) == 0 || header_segment->type == SEG_XTRN)
    {
        return false;
    }
    uint64_t slots[4] = {};
    ...
    {
        const auto value = chernobog::ida_memory::read_integer(object_address + ea_t(index * 8), 8);
        if (!value)
            return false;
        slots[index] = *value;
    }
    ...
    if (slots[0] == 0 || slots[0] >= uint64_t(BADADDR) || slots[1] != 0x7C8 || slots[2] == 0 ||
        slots[2] >= uint64_t(BADADDR) || slots[3] == 0 || slots[3] > 4095)
    {
        return false;
    }
    ...
    if (fact == runtime_plaintexts.end() || fact->second.length() != slots[3] ||
    ...
    {
        const unsigned char byte = static_cast<unsigned char>(fact->second[index]);
                                               ...
                                               const std::map<ea_t, qstring> &runtime_plaintexts)
{
    ...
    struct preserver_t : public ctree_visitor_t
    {
        ...
        int idaapi visit_expr(cexpr_t *assignment) override
        {
            ...
            if (number->op != cot_num || number->n == nullptr || number->ea != assignment->ea ||
                number->is_undef_val() || number->is_type_partial() || number->n->nf.is_fixed() ||
                !number->n->nf.is_numop() || number->n->nf.org_nbytes != 8 ||
                !number->n->nf.type_name.empty() || number->n->nf.serial != 0 ||
                number->numval() != number->n->_value || number->numval() >= uint64_t(BADADDR))
            {
                return 0;
            }
            const ea_t object_address = ea_t(number->numval());
            runtime_cfstring_relation_t relation;
            if (!native_cfstring_address_origin(function, assignment->ea, object_address) ||
                !strict_runtime_cfstring_relation(object_address, facts, &relation))
            {
                return 0;
            }

            tinfo_t object_type;
            if (!get_tinfo(&object_type, object_address) || object_type.empty() ||
                object_type.is_func() || object_type.is_void())
            {
...
static qstring bounded_runtime_cfstring_text(ea_t object_address,
                                             const runtime_cfstring_relation_t &relation)
{
    qstring text;
    text.sprnt("IDB CFString 0x%llX -> bytes 0x%llX; rax-final: \"",
               (unsigned long long)object_address, (unsigned long long)relation.payload);
    constexpr size_t MAX_DISPLAY_BYTES = 128;
    const size_t shown = std::min(size_t(relation.plaintext.length()), MAX_DISPLAY_BYTES);
    ...
    {
        text.cat_sprnt(" [first %zu of %zu bytes]", shown, size_t(relation.plaintext.length()));
    }
    return text;
}
...
int ctree_string_decrypt_handler_t::annotate_runtime_use_strings(cfunc_t *cfunc)
{
    ...
    const auto candidates =
        chernobog::hybrid::hybrid_current_use_strings_for_decompilation(uint64_t(cfunc->entry_ea));
    if (candidates.empty())
        return 0;
        ...
        int idaapi visit_expr(cexpr_t *call) override
        {
            ...
            for (const auto *candidate : found->second)
            {
                ...
                if (native)
                {
                    ...
                    if (call->op != cot_ptr && call->op != cot_idx)
                        continue;
                }
                ...
                int x = -1, y = -1;
                if ((native ||
                     !function->find_item_coords(&(*call->a)[size_t(use.argument)], &x, &y)) &&
                    !function->find_item_coords(call, &x, &y))
                    continue;
                if (y < 0 || size_t(y) >= function->sv.size())
                    continue;
                if (lines.count(y) == 0 && lines.size() >= 64)
                    continue;
                if (lines[y].size() >= 2)
                {
                    omitted.insert(y);
                ...
                if (native)
                    text.sprnt("rax-use(%s,reads=%zu,use=%llu,runs=%zu): \"",
                               ...
                               candidate->read_fragments.empty()
                                   ? size_t(1)
                ...
                while (shown < candidate->value.size() && shown > 0 &&
                       (uint8_t(candidate->value[shown]) & 0xC0) == 0x80)
                    --shown;
                for (size_t index = 0; index < shown; ++index)
                {
    ...
    for (const auto &line : annotator.lines)
    {
        ...
        for (const auto &text : line.second)
        {
            ...
            if (annotator.omitted.count(line.first))
                rendered.append(" [additional uses omitted]");
...
int ctree_string_decrypt_handler_t::annotate_runtime_cfstring_addresses(cfunc_t *cfunc)
{
    if (cfunc == nullptr || cfunc->maturity != CMAT_FINAL || cfunc->sv.empty() ||
        !arch::is_arm64() || inf_is_be())
    {
        return 0;
    }
    ...
    const auto candidates = chernobog::hybrid::hybrid_current_runtime_strings_for_decompilation(
        uint64_t(cfunc->entry_ea));
    std::map<ea_t, qstring> runtime_plaintexts;
    std::set<ea_t> ambiguous;
    for (const auto &candidate : candidates)
    {
        if (candidate.address >= uint64_t(BADADDR) || candidate.value.empty() ||
            candidate.value.find('\0') != std::string::npos)
        {
            continue;
        }
        const ea_t address = ea_t(candidate.address);
        const auto inserted = runtime_plaintexts.emplace(address, candidate.value.c_str());
        if (!inserted.second && inserted.first->second != candidate.value.c_str())
        {
    ...
    struct annotator_t : public ctree_visitor_t
    {
        ...
        int idaapi visit_expr(cexpr_t *assignment) override
        {
            ...
            runtime_cfstring_relation_t relation;
            if (!protected_runtime_cfstring_address(function, assignment, &object_address) ||
                !strict_runtime_cfstring_relation(object_address, facts, &relation))
            {
                return 0;
            }
            ...
            if (y >= 0 && size_t(y) < function->sv.size() && lines[y].size() < 2)
            {
                lines[y].insert(bounded_runtime_cfstring_text(object_address, relation));
            }
            return 0;
        }
...
int ctree_string_decrypt_handler_t::replace_encrypted_strings(
    cfunc_t *cfunc, const std::map<ea_t, qstring> &known_plaintexts, bool persist_annotations)
{
    encrypted_string_replacer_t replacer(cfunc, known_plaintexts, persist_annotations);
    replacer.apply_to(&cfunc->body, nullptr);
    bool comments_changed = false;
    for (const auto &entry : replacer.recovered_comments)
    {
        static constexpr char DECRYPTED_PREFIX[] = "DEOBF: Decrypted CFSTR = ";
        treeloc_t location{entry.first, ITP_SEMI};
        const char *existing = cfunc->get_user_cmt(location, RETRIEVE_ALWAYS);
        if (existing != nullptr && existing[0] != '\0' &&
            qstrncmp(existing, DECRYPTED_PREFIX, sizeof(DECRYPTED_PREFIX) - 1) != 0)
        {
            continue; // Preserve unrelated analyst comments verbatim.
        }
        qstring comment;
        comment.sprnt("DEOBF: Decrypted CFSTR = \"%s\"", entry.second.c_str());
...
void ctree_string_decrypt_handler_t::annotate_reveal(const string_reveal_t &reveal)
{
    ...
    {
        comment.sprnt("DEOBF: %s reveals \"%s\" -> %s", type, reveal.plaintext.c_str(),
                      reveal.dest_name.c_str());
    }
    else
    {
    ...
    if (reveal.dest_addr != BADADDR)
    {
        ...
        deobf::set_cmt_if_changed(reveal.dest_addr, dest_comment.c_str(), true);
    }
}
...
void ctree_string_decrypt_handler_t::annotate_char_string(const char_string_t &str)
{
    ...
    deobf::set_cmt_if_changed(str.insn_addrs[0], comment.c_str(), false);
    ...
    if (str.var_addr != BADADDR)
    {
        ...
        deobf::set_cmt_if_changed(str.var_addr, var_comment.c_str(), true);
    }
}
...
void ctree_string_decrypt_handler_t::annotate_crypto_call(const crypto_call_t &crypto)
{
    ...
    if (!crypto.decrypted.empty())
    {
        ...
        comment.sprnt("DEOBF: %s AES-%d -> \"%s\" (key=%s)", crypto.function.c_str(),
                      crypto.algorithm_bits, decrypted_display.c_str(), key_hex.c_str());
    }
    else
    {
        comment.sprnt("DEOBF: %s AES-%d key=%s", crypto.function.c_str(), crypto.algorithm_bits,
                      key_hex.c_str());
    }
    ...
    if (!crypto.decrypted.empty() && crypto.output_addr != BADADDR)
    {
        ...
        deobf::set_cmt_if_changed(crypto.output_addr, output_comment.c_str(), true);
    }
    ...
    if (!crypto.decrypted.empty() && crypto.input_addr != BADADDR)
    {
        ...
        deobf::set_cmt_if_changed(crypto.input_addr, input_comment.c_str(), true);
    }
}
