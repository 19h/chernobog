    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        for (minsn_t *ins = blk->head; ins; ins = ins->next)
        {
            ...
            if (is_indirect)
            {
                ...
                {
                    icall_debug(
                        "[indirect_call] Analyzed call in block %d: table=0x%llx, index=%d, offset=%lld\n",
                                                    ...
                                                    indirect_call_t *out)
{
    ...
    const std::optional<uint64_t> target = call_insn->opcode == m_icall && target_op->t == mop_v
    ...
    if (!target)
    {
        ...
        {
            deobf::log_verbose("[indirect_call][rax] Observed candidate %a at %a "
                               "(%zu observations, %zu runs); retained indirect because "
                               "concrete coverage is non-exhaustive\n",
                               ea_t(candidate.target), call_insn->ea, candidate.observations,
                               candidate.runs.size());
        }
        return false;
    }
    ...
    if (!is_code(target_flags) && !get_func_safe(target_ea) && !has_any_name(target_flags))
        return false;
                                                   ...
                                                   indirect_call_t &ic, deobf_ctx_t *ctx)
{
    ...
    icall_debug(
        "[indirect_call] Attempting to replace call in block %d with direct call to 0x%llx\n",
        ic.block_idx, (unsigned long long)ic.resolved_target);
    icall_debug("[indirect_call]   Original opcode: %d, l.t=%d, r.t=%d, d.t=%d\n",
                ic.call_insn->opcode, ic.call_insn->l.t, ic.call_insn->r.t, ic.call_insn->d.t);
    ...
    {
        icall_debug(
            "[indirect_call]   Target 0x%llx is outside database EA range, skipping replacement\n",
    ...
    {
        icall_debug(
            "[indirect_call]   Target is NOT a function start - skipping replacement to avoid crash\n");
        ...
        if (!is_func_start)
        {
            qstring comment;
            comment.sprnt(
                "DEOBF: Resolved indirect call -> 0x%llX (not a function start, not replaced)",
                (unsigned long long)ic.resolved_target);
    ...
    icall_debug(
        "[indirect_call]   After: opcode=%d, l.t=%d, l.g=0x%llx, l.size=%d, r.t=%d, d.t=%d\n",
        call->opcode, call->l.t, (unsigned long long)call->l.g, call->l.size, call->r.t, call->d.t);
