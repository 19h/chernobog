...
bool stack_slots_overlap(const stack_slot_t &left, const stack_slot_t &right)
{
    ...
    {
        return static_cast<uint64_t>(right.offset) - static_cast<uint64_t>(left.offset) <
               static_cast<uint64_t>(left.size);
    }
    return static_cast<uint64_t>(left.offset) - static_cast<uint64_t>(right.offset) <
...
struct local_value_t
{
    ...
    {
        return std::tie(type, identity, size) < std::tie(other.type, other.identity, other.size);
    }
};
...
bool local_values_overlap(const local_value_t &left, const local_value_t &right)
{
    if (left.type != right.type || left.size <= 0 || right.size <= 0)
        return false;
    if (left.identity <= right.identity)
    {
        return static_cast<uint64_t>(right.identity) - static_cast<uint64_t>(left.identity) <
               static_cast<uint64_t>(left.size);
    }
    return static_cast<uint64_t>(left.identity) - static_cast<uint64_t>(right.identity) <
           static_cast<uint64_t>(right.size);
}
...
struct stack_address_collector_t final : public mop_visitor_t
{
    ...
    int idaapi visit_mop(mop_t *operand, const tinfo_t *, bool) override
    {
        ...
        if (std::find_if(this->addressed.begin(), this->addressed.end(),
                         [&](const auto &existing)
                         {
                             return existing.offset == slot.offset && existing.size == slot.size;
                         }) == this->addressed.end())
        {
...
{
    explicit local_constant_replacer_t(const std::map<local_value_t, uint64_t> &constants)
        : constants(constants)
    {
    }
    ...
    {
        if (operand == nullptr || is_target || (operand->t != mop_S && operand->t != mop_r))
            return 0;
        local_value_t value;
...
bool stack_value_is_addressed(const local_value_t &value,
                              const std::vector<stack_slot_t> &known_addressed)
{
    ...
    return std::any_of(known_addressed.begin(), known_addressed.end(),
                       [&](const auto &addressed) { return stack_slots_overlap(slot, addressed); });
}
...
// predecessor state.
std::optional<bool>
evaluate_local_constant_branch(mblock_t *block, minsn_t *branch,
                               const std::vector<stack_slot_t> *known_addressed = nullptr)
{
    ...
    for (minsn_t *instruction = block->head; instruction != nullptr && instruction != branch;
         instruction = instruction->next)
    {
        if (instruction->contains_call(true))
        {
            ...
            {
                iterator =
                    iterator->first.type == mop_r ? constants.erase(iterator) : std::next(iterator);
            }
        }
        ...
        for (auto iterator = constants.begin(); iterator != constants.end();)
        {
            iterator = local_values_overlap(iterator->first, written) ? constants.erase(iterator)
                                                                      : std::next(iterator);
        }
        ...
        if (constant)
        {
            constants[written] = chernobog::bitvector::truncate(*constant, written.size);
        }
    }
    ...
    bool result = false;
    return opaque_eval_t::evaluate_condition(&resolved, &result) ? std::optional<bool>(result)
                                                                 : std::nullopt;
}

std::optional<int> find_not_taken_successor(const mblock_t *blk, int taken_target)
{
...
int jump_optimizer_handler_t::run_local_constant_branches(mbl_array_t *mba, deobf_ctx_t *ctx)
{
    if (mba == nullptr)
        return 0;
    ...
    for (int index = 0; index < mba->qty; ++index)
    {
        ...
        if (branch == nullptr || !is_mcode_jcond(branch->opcode))
            continue;
        const auto result = evaluate_local_constant_branch(block, branch, &addresses.addressed);
        if (result)
    ...
    if (changes > 0)
    {
        deobf::log_verbose("[JumpOpt] Simplified %d same-block constant branches\n", changes);
    }
    return changes;
}
...
void jump_optimizer_handler_t::replace_successors(mblock_t *blk, int new_target)
{
    ...
    if (new_target >= 0 && new_target < blk->mba->qty)
    {
        ...
        if (new_dst && std::find(new_dst->predset.begin(), new_dst->predset.end(), blk->serial) ==
                           new_dst->predset.end())
        {
