...
bool is_constant_tree(const mop_t &op)
{
    if (op.t == mop_n || op.t == mop_z)
        return true;
    return op.t == mop_d && op.d != nullptr && is_constant_tree(op.d->l) &&
           is_constant_tree(op.d->r);
}

bool is_add_one_of(const mop_t &candidate, const mop_t &value)
{
    if (candidate.t != mop_d || candidate.d == nullptr || candidate.d->opcode != m_add)
        return false;
    ...
    return (equal_width_value(add->l) && add->r.t == mop_n && add->r.nnn &&
            add->r.nnn->value == 1) ||
           (equal_width_value(add->r) && add->l.t == mop_n && add->l.nnn && add->l.nnn->value == 1);
}
...
bool is_even_consecutive_product(const mop_t &candidate)
{
    ...
    if ((mod->opcode != m_smod && mod->opcode != m_umod) || mod->r.t != mop_n || !mod->r.nnn ||
        mod->r.nnn->value != 2 || mod->l.t != mop_d || mod->l.d == nullptr ||
...
void replace_successors(mblock_t *blk, int new_target)
{
    ...
    if (new_target >= 0 && new_target < blk->mba->qty)
    {
        ...
        if (new_dst != nullptr && std::find(new_dst->predset.begin(), new_dst->predset.end(),
                                            blk->serial) == new_dst->predset.end())
        {
...
// Find all opaque predicates
//--------------------------------------------------------------------------
std::vector<bogus_cf_handler_t::opaque_info_t>
bogus_cf_handler_t::find_opaque_predicates(mbl_array_t *mba, deobf_ctx_t *ctx)
{
...
bool bogus_cf_handler_t::check_const_comparison(minsn_t *insn, bool *result)
{
    if (!insn || !result || !is_mcode_jcond(insn->opcode))
        return false;
    if (!is_constant_tree(insn->l) || (insn->opcode != m_jcnd && !is_constant_tree(insn->r)))
        return false;
...
{
    if (!insn || !result || (insn->opcode != m_jz && insn->opcode != m_jnz) || insn->r.t != mop_n ||
        !insn->r.nnn)
        return false;
    ...
    if (insn->l.t != mop_d || insn->l.d == nullptr || insn->l.d->opcode != m_setz ||
        insn->l.d->r.t != mop_n || !insn->l.d->r.nnn || insn->l.d->r.nnn->value != 0 ||
