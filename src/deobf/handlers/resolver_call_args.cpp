...
struct direct_call_collector_t final : public minsn_visitor_t
{
    ...
    {
        if (curins == nullptr || curins->opcode != m_call || curins->l.t != mop_v ||
            curins->d.t != mop_f || curins->d.f == nullptr)
        {
            return 0;
        }
        ...
        if (operand_target == BADADDR ||
            (metadata_target != BADADDR && metadata_target != operand_target))
        {
            return 0;
        }
...
bool indexed_memory_operand(const insn_t &instruction, const op_t &operand, indexed_memory_t *out)
{
    if (out == nullptr || (operand.type != o_phrase && operand.type != o_displ))
    {
        return false;
    }
    ...
    out->displacement = operand.type == o_displ ? static_cast<sval_t>(operand.addr) : 0;
    return out->base != R_none && out->base >= 0;
}

bool full_register_destination(const insn_t &instruction, int register_id)
{
    if (instruction.Op1.type != o_reg || instruction.Op1.reg != register_id)
    {
        return false;
    }
...
bool zero_idiom(const insn_t &instruction, int register_id)
{
    return (instruction.itype == NN_xor || instruction.itype == NN_sub) &&
           instruction.Op1.type == o_reg && instruction.Op2.type == o_reg &&
           instruction.Op1.reg == register_id && instruction.Op2.reg == register_id &&
           full_register_destination(instruction, register_id);
}

bool x86_unconditional_jump(const insn_t &instruction)
{
    return instruction.itype == NN_jmp || instruction.itype == NN_jmpfi ||
           instruction.itype == NN_jmpni || instruction.itype == NN_jmpshort;
}
...
bool operand_is_read(const insn_t &instruction, int operand_index)
{
    ...
    if (operand_index == 0 && instruction.itype == NN_imul && instruction.Op3.type != o_void)
    {
        return false;
    }
    return has_cf_use(instruction.get_canon_feature(PH), operand_index);
}

bool x86_setcc(const insn_t &instruction)
{
    return instruction.itype >= NN_seta && instruction.itype <= NN_setz;
}

bool x86_implicit_system_transfer(const insn_t &instruction)
{
    return instruction.itype == NN_syscall || instruction.itype == NN_sysenter ||
           instruction.itype == NN_int || instruction.itype == NN_into;
}
...
bool full_x86_gpr_write(const reg_access_t &access)
{
    ...
    return access.range.empty() || (access.range.bitoff() == 0 && access.range.bitsize() >= 32);
}
...
uint64_t register_access_mask(const reg_access_t &access)
{
    ...
    if (offset >= 64)
        return 0;
    const unsigned width = std::min<unsigned>(access.range.bitsize(), 64 - offset);
    if (width == 0)
        return 0;
    const uint64_t low_mask = width == 64 ? ~uint64_t{0} : (uint64_t{1} << width) - 1;
...
bool processor_access_reads_incoming_value(const insn_t &instruction, const reg_access_t &access)
{
    ...
    if (access.opnum < UA_MAXOP && instruction.ops[access.opnum].type != o_void)
    {
...
std::optional<int64_t> read_static_integer(ea_t address, size_t width, bool sign_extend)
{
    ...
    {
    case 1:
        return sign_extend ? int64_t(int8_t(get_byte(address))) : int64_t(get_byte(address));
    case 2:
        return sign_extend ? int64_t(int16_t(get_word(address))) : int64_t(get_word(address));
    case 4:
        return sign_extend ? int64_t(int32_t(get_dword(address))) : int64_t(get_dword(address));
    case 8:
...
void update_entry_facts(const insn_t &instruction, std::map<int, ea_t> &bases,
                        std::map<int, std::set<int64_t>> &constants)
{
    ...
    const bool sign_extend = instruction.itype == NN_movsx || instruction.itype == NN_movsxd;
    const auto value = read_static_integer(*address, width, sign_extend);
    if (value)
        constants[destination] = {*value};
}

std::optional<decoded_block_t> decode_entry_block(ea_t entry, const func_t *function)
{
    decoded_block_t result;
    result.start = entry;
    ea_t address = entry;
    while (result.instructions.size() < maximum_entry_instructions && address < function->end_ea &&
           function_contains(function->start_ea, address))
    {
        ...
        {
            const bool indirect = instruction.Op1.type != o_near && instruction.Op1.type != o_far;
...
std::optional<split_dispatch_t> find_split_dispatch(const decoded_block_t &block,
                                                    const std::map<int, ea_t> &initial_bases)
{
    ...
    for (size_t sub_pos = block.instructions.size() - 1; sub_pos-- > 0;)
    {
        const insn_t &sub = block.instructions[sub_pos];
        indexed_memory_t subtract_memory;
        if (sub.itype != NN_sub || sub.Op1.type != o_reg || sub.Op1.reg != target_register ||
            !indexed_memory_operand(sub, sub.Op2, &subtract_memory) || subtract_memory.scale != 8)
        {
            continue;
        }
        for (size_t load_pos = sub_pos; load_pos-- > 0;)
        {
            ...
            if (load.itype != NN_mov || load.Op1.type != o_reg || load.Op1.reg != target_register ||
                !indexed_memory_operand(load, load.Op2, &encoded_memory) ||
                encoded_memory.scale != 8 || encoded_memory.index != subtract_memory.index)
            {
                continue;
            }
            ...
            {
                if (full_register_destination(block.instructions[index], target_register))
                {
                    clobbered = true;
                    break;
                }
            ...
            if (clobbered || encoded == load_bases.end() || subtract == sub_bases.end() ||
                encoded_memory.displacement != 0 || subtract_memory.displacement != 0)
            {
                continue;
            }
            return split_dispatch_t{encoded->second, subtract->second, encoded_memory.index,
                                    load_pos};
        }
    }
    return std::nullopt;
...
std::vector<ea_t> enumerate_split_targets(const split_dispatch_t &dispatch,
                                          const segment_t *code_segment)
{
    ...
    for (size_t index = 0; index < maximum_dispatch_targets; ++index)
    {
        ...
        if (!mapped_range(encoded_address, 8) || !mapped_range(subtract_address, 8))
        {
            break;
        }
        const ea_t target = ea_t(get_qword(encoded_address) - get_qword(subtract_address));
        if (!executable_target(target, code_segment))
            break;
        targets.push_back(target);
    }
    if (targets.size() < minimum_dispatch_targets || targets.size() == maximum_dispatch_targets)
    {
        targets.clear();
    }
    return targets;
...
std::optional<std::set<int64_t>> memory_values(const insn_t &instruction, const op_t &source,
                                               ...
                                               const std::map<int, std::set<int64_t>> &constants)
{
    ...
    else
    {
        ...
        if (base == bases.end() || indexes == constants.end() || indexes->second.empty() ||
            indexes->second.size() > 2)
            return std::nullopt;
        for (int64_t index : indexes->second)
        {
            const int64_t signed_address =
                int64_t(base->second) + memory.displacement + index * memory.scale;
            if (signed_address < 0)
                return std::nullopt;
...
std::map<int, std::set<int64_t>> constants_before(const decoded_block_t &block, size_t stop,
    ...
    for (size_t index = 0; index < stop; ++index)
    {
        ...
        if (!full_register_destination(instruction, destination))
        {
            ...
            if (x86_setcc(instruction) && has_cf_chg(features, 0) &&
                constants.count(destination) != 0 && constants[destination] == std::set<int64_t>{0})
            {
        ...
        if ((instruction.itype == NN_mov || instruction.itype == NN_movsx ||
             instruction.itype == NN_movzx || instruction.itype == NN_movsxd) &&
            (instruction.Op2.type == o_mem || instruction.Op2.type == o_phrase ||
             instruction.Op2.type == o_displ))
        {
            ...
            loaded_values =
                memory_values(instruction, instruction.Op2, proof.static_bases, constants);
        }
        constants.erase(destination);
        if (instruction.itype == NN_mov && instruction.Op2.type == o_imm)
        {
...
std::optional<std::vector<ea_t>> split_jump_targets(const decoded_block_t &block,
                                                    const dispatcher_proof_t &proof)
{
    const std::map<int, ea_t> no_bases;
    const auto dispatch =
        find_split_dispatch(block, block.start == proof.entry ? no_bases : proof.static_bases);
    if (!dispatch || dispatch->encoded_table != proof.split.encoded_table ||
        dispatch->subtract_table != proof.split.subtract_table)
        return std::nullopt;
    const auto constants = constants_before(block, dispatch->load_instruction, proof);
    ...
    for (int64_t index : found->second)
    {
        if (index < 0 || size_t(index) >= proof.targets.size())
            return std::nullopt;
        const ea_t target = ea_t(get_qword(proof.split.encoded_table + size_t(index) * 8) -
                                 get_qword(proof.split.subtract_table + size_t(index) * 8));
        if (std::find(proof.targets.begin(), proof.targets.end(), target) == proof.targets.end())
            return std::nullopt;
...
std::optional<std::vector<ea_t>> direct_table_targets(const decoded_block_t &block,
                                                      const dispatcher_proof_t &proof,
                                                      const segment_t *code_segment)
{
    if (block.instructions.empty())
    ...
    if (jump.Op1.type != o_phrase && jump.Op1.type != o_displ)
        return std::nullopt;
    if (!indexed_memory_operand(jump, jump.Op1, &memory) || memory.scale != 8 ||
    ...
    bool adjacent = direct_end <= proof.split.encoded_table &&
                    proof.split.encoded_table - direct_end <= maximum_direct_table_padding;
    for (ea_t padding = direct_end; adjacent && padding < proof.split.encoded_table; padding += 8)
    {
        adjacent = mapped_range(padding, 8) && get_qword(padding) == 0;
    }
    if (direct_targets.size() < minimum_direct_dispatch_targets || !adjacent)
        return std::nullopt;
...
bool nop_padding_to(ea_t address, ea_t limit)
{
    ...
    {
        insn_t instruction;
        if (decode_insn(&instruction, address) <= 0 || instruction.itype != NN_nop ||
...
bool decode_dispatch_blocks(dispatcher_proof_t &proof, const segment_t *code_segment)
{
    ...
    for (size_t block_index = 0; block_index < starts.size(); ++block_index)
    {
        ...
        const ea_t hard_limit = block_index + 1 < starts.size() ? starts[block_index + 1]
                                                                : starts[block_index] + 0x1000;
        ea_t address = block.start;
        bool terminated = false;
        while (address < hard_limit &&
               block.instructions.size() < maximum_dispatch_block_instructions)
        {
            insn_t instruction;
            if (decode_insn(&instruction, address) <= 0)
                return false;
            ...
            if (!is_call_insn(instruction) &&
                (instruction.get_canon_feature(PH) & (CF_JUMP | CF_STOP)) != 0)
            {
                deobf::log_verbose("[resolver_args] %a: unsupported internal flow at %a\n",
        ...
        {
            deobf::log_verbose("[resolver_args] %a: target boundary rejected at %a\n", block.start,
                               block.end);
            return false;
        }
        ...
        if (x86_unconditional_jump(terminator))
        {
            ...
            if (!closed_dispatch)
            {
                closed_dispatch = direct_table_targets(block, proof, code_segment).has_value();
            }
        }
        if (!closed_dispatch)
        {
            deobf::log_verbose("[resolver_args] %a: terminal dispatch rejected at %a\n",
                               block.start, terminator.ea);
            return false;
        }
        proof.blocks.push_back(std::move(block));
    }
    return !proof.blocks.empty() && is_ret_insn(proof.blocks.back().instructions.back());
}

std::optional<dispatcher_proof_t> prove_split_dispatcher(ea_t entry, const func_t *function)
{
    ...
    for (const insn_t &instruction : entry_block->instructions)
        update_entry_facts(instruction, proof.static_bases, proof.entry_constants);
...
{
    static constexpr std::array<const char *, 6> sysv_names = {"rdi", "rsi", "rdx",
                                                               "rcx", "r8",  "r9"};
    static constexpr std::array<const char *, 4> win_names = {"rcx", "rdx", "r8", "r9"};
...
{
    if (!is_call_insn(instruction) && !x86_unconditional_jump(instruction))
    {
        return BADADDR;
    }
...
std::optional<argument_usage_t> infer_native_argument_usage(ea_t entry,
                                                            argument_inference_t &inference);

bool call_observes_argument(const insn_t &instruction, int register_id, size_t argument_index,
                            argument_inference_t &inference)
{
    ...
    if (is_userti(target))
    {
        ...
        if (!get_tinfo(&user_type, target) || !user_type.get_func_details(&user_details) ||
        ...
        for (const funcarg_t &argument : user_details)
        {
            ...
            if (location.is_reg1() && location.reg1() == register_id)
                return true;
            if (location.is_reg2() &&
                (location.reg1() == register_id || location.reg2() == register_id))
            {
                return true;
            }
            if (location.is_scattered())
            {
                for (const argpart_t &part : location.scattered())
                {
                    if ((part.is_reg1() && part.reg1() == register_id) ||
                        (part.is_reg2() &&
                         (part.reg1() == register_id || part.reg2() == register_id)))
                    {
                        return true;
                    }
...
bool block_observes_incoming_register(const decoded_block_t &block, int register_id,
                                      size_t argument_index, uint64_t incoming_bits,
                                      uint64_t *live_after, argument_inference_t &inference)
{
    uint64_t live_bits = incoming_bits;
    for (const insn_t &instruction : block.instructions)
    {
        const ea_t transfer_target = direct_transfer_target(instruction);
        if (live_bits != 0 && (is_call_insn(instruction) || transfer_target != BADADDR) &&
            call_observes_argument(instruction, register_id, argument_index, inference))
        {
            return true;
        }
        ...
        if (PH.get_reg_accesses(&accesses, instruction, 0) > 0)
        {
            ...
            for (const reg_access_t &access : accesses)
            {
                if (access.regnum != register_id)
                    continue;
                if (processor_access_reads_incoming_value(instruction, access))
                    read_bits |= register_access_mask(access);
                if ((access.access_type & WRITE_ACCESS) != 0)
                    written_bits |= register_write_mask(access);
            }
            const auto transfer =
                chernobog::analysis::transfer_bit_liveness(live_bits, read_bits, written_bits);
        ...
        for (int operand_index = 0; operand_index < UA_MAXOP; ++operand_index)
        {
            ...
            {
                if (live_bits != 0 && operand_is_read(instruction, operand_index))
                    return true;
                if (has_cf_chg(features, operand_index) && operand_index == 0 &&
                    full_register_destination(instruction, register_id))
                {
                    live_bits = 0;
                }
            }
            else if (live_bits != 0 && (operand.type == o_phrase || operand.type == o_displ))
            {
...
std::optional<decoded_block_t> decode_linear_function(ea_t entry, const func_t *function)
{
...
std::optional<std::vector<ea_t>> dispatcher_successors(const decoded_block_t &block,
                                                       const dispatcher_proof_t &proof)
{
    if (block.instructions.empty())
        return std::nullopt;
...
std::optional<argument_usage_t>
infer_argument_usage_from_blocks(const std::vector<decoded_block_t> &blocks,
                                 const dispatcher_proof_t *proof, argument_inference_t &inference)
{
    ...
    for (size_t argument_index = 0; argument_index < arguments.size(); ++argument_index)
    {
        if (proof == nullptr)
        {
            if (block_observes_incoming_register(blocks.front(), arguments[argument_index],
        ...
        while (!worklist.empty())
        {
            ...
            if (block_observes_incoming_register(blocks[block_index], arguments[argument_index],
                                                 argument_index, live_after, &live_after,
                                                 inference))
            {
                result.used[argument_index] = 1;
                break;
            ...
            const auto successors = dispatcher_successors(blocks[block_index], *proof);
            if (!successors)
                return std::nullopt;
            for (ea_t successor : *successors)
            {
                ...
                if (found == block_by_start.end())
                    return std::nullopt;
                const uint64_t merged = incoming_bits[found->second] | live_after;
                                                            ...
                                                            argument_inference_t &inference)
{
    ...
    if (PH.id != PLFM_386 || !inf_is_64bit() || inference.active.count(entry) != 0)
    {
        return std::nullopt;
    }

    const func_t *function = get_func(entry);
    if (function == nullptr || (function->flags & FUNC_LIB) != 0 || is_userti(entry))
    {
    ...
    {
        result = infer_argument_usage_from_blocks(proof->blocks, &*proof, inference);
    }
    else if (auto linear = decode_linear_function(entry, function))
    {
        result = infer_argument_usage_from_blocks({*linear}, nullptr, inference);
    }
    ...
    if (result)
    {
        ...
        if (guess_tinfo(&guessed, entry) != GUESS_FUNC_FAILED && guessed.get_func_details(&details))
        {
            ...
            else
            {
                ...
                for (size_t index = 0; index < typed_register_arguments; ++index)
                {
                    result->used[index] = 1;
                }
            }
...
std::optional<argument_usage_t> unresolved_argument_usage(ea_t entry)
{
    ...
    const func_t *function = get_func(entry);
    if (function == nullptr || function->start_ea != entry || (function->flags & FUNC_LIB) != 0)
    {
        return std::nullopt;
    }
    ...
    argument_inference_t inference;
    inference.active.insert(entry);
    const auto usage = infer_argument_usage_from_blocks(proof->blocks, &*proof, inference);
    inference.active.erase(entry);
    ...
    func_type_data_t details;
    if (guess_tinfo(&guessed, entry) == GUESS_FUNC_FAILED || !guessed.get_func_details(&details) ||
        details.is_vararg_cc())
    {
        deobf::log_verbose("[resolver_args] %a: guessed prototype rejected: args=%zu "
                           "vararg=%d\n",
                           entry, details.size(), details.is_vararg_cc() ? 1 : 0);
        return std::nullopt;
    }
    deobf::log_verbose(
        "[resolver_args] Proven closed split dispatcher at %a: "
        "%zu targets, register-use mask=%d%d%d%d%d%d\n",
        entry, proof->targets.size(), usage->used.size() > 0 ? usage->used[0] : 0,
        usage->used.size() > 1 ? usage->used[1] : 0, usage->used.size() > 2 ? usage->used[2] : 0,
        usage->used.size() > 3 ? usage->used[3] : 0, usage->used.size() > 4 ? usage->used[4] : 0,
        usage->used.size() > 5 ? usage->used[5] : 0);
    return usage;
}

int argument_usage_index(const argument_usage_t &usage, int register_id)
{
    const auto found = std::find(usage.registers.begin(), usage.registers.end(), register_id);
    return found == usage.registers.end() ? -1 : static_cast<int>(found - usage.registers.begin());
}

bool neutralizable_call_argument(const mcallarg_t &argument, const argument_usage_t &usage)
{
    if (!argument.argloc.is_reg1() || !chernobog::bitvector::valid_byte_width(argument.size) ||
...
size_t neutralizable_register_arguments(const mcallinfo_t &info, const argument_usage_t &usage)
{
    if ((info.flags & FCI_FINAL) != 0 || info.args.empty() || info.solid_args < 0 ||
...
int resolver_call_args_handler_t::run(mbl_array_t *mba, deobf_ctx_t *ctx)
{
    ...
    for (auto &cohort : collector.calls)
    {
        ...
        if (sites.size() < minimum_recurrent_calls)
            continue;
        deobf::log_verbose("[resolver_args] Checking %zu recurrent calls to %a\n", sites.size(),
                           cohort.first);
        const std::optional<argument_usage_t> usage = unresolved_argument_usage(cohort.first);
        if (!usage)
            continue;
        ...
        {
            if (site.instruction != nullptr && site.instruction->d.t == mop_f &&
                site.instruction->d.f != nullptr &&
                neutralizable_register_arguments(*site.instruction->d.f, *usage) != 0)
            {
                ++neutralizable_sites;
            }
        }
        ...
        for (const call_site_t &site : sites)
        {
            ...
            for (mcallarg_t &argument : info.args)
            {
                if (neutralizable_call_argument(argument, *usage))
                {
                    ...
                    argument.make_number(0, argument.size, argument.ea);
                    ++removed_here;
                }
            }
            if (removed_here == 0)
        ...
        if (cohort_removed != 0)
        {
            deobf::log("[resolver_args] Neutralized %zu proven-unused register "
