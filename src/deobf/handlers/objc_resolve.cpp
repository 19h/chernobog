...
int pointer_size() { return arch::is_64bit() ? 8 : 4; }
...
bool objc_resolve_handler_t::is_objc_msgsend(const char *name)
{
    ...
    return symbol == "objc_msgSend" || symbol == "objc_msgSendSuper" ||
           symbol == "objc_msgSendSuper2" || symbol == "objc_msgSend_stret" ||
           symbol == "objc_msgSend_fpret" || symbol == "objc_msgSend_fp2ret" ||
           symbol == "objc_msgSendSuper_stret" || symbol == "objc_msgSendSuper2_stret" ||
           symbol == "objc_msgSend_fixup" || symbol == "objc_msgLookup" ||
           symbol == "objc_msgLookupSuper" ||
           symbol.compare(0, sizeof("objc_msgSend$") - 1, "objc_msgSend$") == 0;
}

//--------------------------------------------------------------------------
// Classify objc_msgSend variant
//--------------------------------------------------------------------------
objc_resolve_handler_t::msgsend_variant_t objc_resolve_handler_t::classify_msgsend(ea_t addr)
{
    qstring name;
...
objc_resolve_handler_t::msgsend_variant_t objc_resolve_handler_t::classify_msgsend(const char *name)
{
    if (!name)
        return MSGSEND_UNKNOWN;
    ...
    if (symbol == "objc_msgSend" || symbol == "objc_msgSend_fixup" || symbol == "objc_msgLookup" ||
        symbol.compare(0, sizeof("objc_msgSend$") - 1, "objc_msgSend$") == 0)
...
bool objc_resolve_handler_t::detect(mbl_array_t *mba)
{
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        for (minsn_t *ins = blk->head; ins; ins = ins->next)
        {
            ...
            {
                auto address =
                    stack_tracker_t::trace_address(blk, ins, target.s->off, pointer_size());
...
void objc_resolve_handler_t::find_msgsend_calls(
    mbl_array_t *mba, std::vector<std::pair<mblock_t *, minsn_t *>> &calls)
{
    calls.clear();
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        for (minsn_t *ins = blk->head; ins; ins = ins->next)
        {
            ...
            {
                auto addr = stack_tracker_t::trace_address(blk, ins, target.s->off, pointer_size());
...
//--------------------------------------------------------------------------
bool objc_resolve_handler_t::get_selector_string(mbl_array_t *mba, const mop_t &sel_op,
...
//--------------------------------------------------------------------------
bool objc_resolve_handler_t::trace_selector(mbl_array_t *mba, mblock_t *blk, minsn_t *call_insn,
                                            qstring *out_selector)
{
    ...
    {
        auto address =
            stack_tracker_t::trace_address(blk, call_insn, sel_arg.s->off, pointer_size());
...
//--------------------------------------------------------------------------
bool objc_resolve_handler_t::trace_receiver_class(mbl_array_t *mba, mblock_t *blk,
                                                  minsn_t *call_insn, qstring *out_class)
{
...
//--------------------------------------------------------------------------
bool objc_resolve_handler_t::is_class_object(mbl_array_t *mba, const mop_t &receiver,
                                             qstring *out_class)
{
    if (!out_class)
        return false;
    ...
    if (receiver.t == mop_v)
    {
        ...
        if (get_name(&name, receiver.g) > 0)
        {
            ...
            if (name.length() > prefix_len && strncmp(name.c_str(), prefix, prefix_len) == 0)
            {
...
//--------------------------------------------------------------------------
bool objc_resolve_handler_t::resolve_msgsend_call(mbl_array_t *mba, mblock_t *blk,
                                                  minsn_t *call_insn, objc_call_info_t *out)
{
    ...
    // pair.
    const mop_t &target = call_insn->opcode == m_icall ? call_insn->r : call_insn->l;
    if (target.t == mop_v)
    {
    ...
    {
        auto addr = stack_tracker_t::trace_address(blk, call_insn, target.s->off, pointer_size());
        if (addr.has_value())
        {
    ...
    out->is_super_call = variant == MSGSEND_SUPER || variant == MSGSEND_SUPER2 ||
                         variant == MSGSEND_SUPER_STRET || variant == MSGSEND_SUPER2_STRET;
    out->is_stret = variant == MSGSEND_STRET || variant == MSGSEND_SUPER_STRET ||
...
qstring objc_resolve_handler_t::format_method_signature(const objc_call_info_t &info)
{
    ...
    {
        result.sprnt("%c[%s %s]", method_type, info.receiver_class.c_str(), info.selector.c_str());
    }
    else
    {
