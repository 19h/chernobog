...
bool rax_vetoes_universal_branch_claim(const minsn_t *condition, bool expected_taken)
{
    ...
    const chernobog::hybrid::HybridBranchCheck check =
        chernobog::hybrid::hybrid_check_current_branch_claim(
            uint64_t(function->start_ea), uint64_t(condition->ea), expected_taken);
    if (check.veto())
    {
        deobf::log("[z3][rax] Concrete counterexample vetoed universal predicate "
                   "claim at %a (expected=%s matching=%zu opposing=%zu "
                   "context-complete-opposing=%zu generation=%llu)\n",
                   condition->ea, expected_taken ? "taken" : "fallthrough", check.claim.matching,
                   check.claim.opposing, check.claim.opposing_context_complete,
                   static_cast<unsigned long long>(check.generation));
        return true;
    }
    if (check.snapshot_current &&
        check.claim.verdict == chernobog::hybrid::BranchClaimVerdict::CORROBORATED)
    {
        deobf::log_verbose("[z3][rax] Concrete runs corroborate (but do not prove) the "
...
// Clear cache
//--------------------------------------------------------------------------
void opaque_eval_t::clear_cache() { s_global_cache.erase(get_dbctx_id()); }
...
bool opaque_eval_t::evaluate_condition(minsn_t *cond, bool *out_result)
{
    ...
    if (cond->opcode == m_jnz || cond->opcode == m_jz || cond->opcode == m_jg ||
        cond->opcode == m_jge || cond->opcode == m_jl || cond->opcode == m_jle ||
        cond->opcode == m_ja || cond->opcode == m_jae || cond->opcode == m_jb ||
        cond->opcode == m_jbe)
    {
    ...
    if (cond->opcode == m_setnz || cond->opcode == m_setz || cond->opcode == m_setg ||
        cond->opcode == m_setge || cond->opcode == m_setl || cond->opcode == m_setle ||
        cond->opcode == m_seta || cond->opcode == m_setae || cond->opcode == m_setb ||
        cond->opcode == m_setbe)
    {
...
std::optional<uint64_t> opaque_eval_t::eval_insn(minsn_t *ins, eval_state_t &state)
{
    ...
    case m_setl:
        if (l_val && r_val)
        {
            auto b = eval_setl(sign_extend(*l_val, ins->l.size), sign_extend(*r_val, ins->r.size));
            if (b)
                result = *b ? 1 : 0;
        }
        break;
    ...
    case m_setle:
        if (l_val && r_val)
        {
            auto b = eval_setle(sign_extend(*l_val, ins->l.size), sign_extend(*r_val, ins->r.size));
            if (b)
                result = *b ? 1 : 0;
        }
        break;
    ...
    case m_setg:
        if (l_val && r_val)
        {
            auto b = eval_setg(sign_extend(*l_val, ins->l.size), sign_extend(*r_val, ins->r.size));
            if (b)
                result = *b ? 1 : 0;
        }
        break;
    ...
    case m_setge:
        if (l_val && r_val)
        {
            auto b = eval_setge(sign_extend(*l_val, ins->l.size), sign_extend(*r_val, ins->r.size));
            if (b)
                result = *b ? 1 : 0;
        }
        break;
...
{
    return chernobog::bitvector::shift_right_arithmetic(static_cast<uint64_t>(a), b, size);
}

//--------------------------------------------------------------------------
// Comparison operations
//--------------------------------------------------------------------------
std::optional<bool> opaque_eval_t::eval_setz(uint64_t a) { return a == 0; }

std::optional<bool> opaque_eval_t::eval_setnz(uint64_t a) { return a != 0; }

std::optional<bool> opaque_eval_t::eval_setl(int64_t a, int64_t b) { return a < b; }

std::optional<bool> opaque_eval_t::eval_setle(int64_t a, int64_t b) { return a <= b; }

std::optional<bool> opaque_eval_t::eval_setg(int64_t a, int64_t b) { return a > b; }

std::optional<bool> opaque_eval_t::eval_setge(int64_t a, int64_t b) { return a >= b; }

std::optional<bool> opaque_eval_t::eval_setb(uint64_t a, uint64_t b) { return a < b; }

std::optional<bool> opaque_eval_t::eval_setbe(uint64_t a, uint64_t b) { return a <= b; }

std::optional<bool> opaque_eval_t::eval_seta(uint64_t a, uint64_t b) { return a > b; }

std::optional<bool> opaque_eval_t::eval_setae(uint64_t a, uint64_t b) { return a >= b; }
...
opaque_eval_t::opaque_result_t opaque_eval_t::check_opaque_predicate(minsn_t *cond)
{
    ...
    try
    {
        ...
        if (cond->ea != BADADDR)
        {
            ...
            if (function != nullptr)
            {
                const auto concrete_inputs = chernobog::hybrid::hybrid_collect_current_z3_inputs(
                ...
                {
                    deobf::log_verbose("[z3][rax] Collected %zu predicate states / %zu "
