                                   ...
                                   MatchBindings &bindings)
{
    ...
    if (pattern->is_leaf())
    {
        ...
        // New binding
        return bindings.add(pat_leaf->name.c_str(), candidate->mop, candidate->dest_size,
                            candidate->ea);
    }
    ...
    const auto match_operands = [&](const AstBase *candidate_left, const AstBase *candidate_right)
    {
        if (pat_node->left &&
    ...
    if (commutative && match_operands(cand_node->right.get(), cand_node->left.get()))
    {
        return true;
    }
