...
bool is_x86() { return PH.id == PLFM_386; }

bool is_x86_64() { return PH.id == PLFM_386 && inf_is_64bit(); }

bool is_arm() { return PH.id == PLFM_ARM; }

bool is_arm64() { return PH.id == PLFM_ARM && inf_is_64bit(); }
...
bool is_call_insn(uint16_t itype) { return is_direct_call(itype) || is_indirect_call(itype); }
...
bool is_identity_mov(const insn_t &insn)
{
    if (is_x86_64())
    {
        ...
        if (insn.itype == NN_mov && insn.Op1.type == o_reg && insn.Op1.reg == get_return_reg() &&
    ...
    else if (is_arm64())
    {
        ...
        if (insn.itype == ARM_mov && insn.Op1.type == o_reg && insn.Op1.reg == 0 &&
...
bool is_indirect_jump_via_return_reg(const insn_t &insn)
{
    if (is_x86_64())
    {
        ...
        if ((insn.itype == NN_jmpni || insn.itype == NN_jmp) && insn.Op1.type == o_reg &&
    ...
    else if (is_arm64())
    {
        ...
        if (insn.itype == ARM_br && insn.Op1.type == o_reg && insn.Op1.reg == get_return_reg())
        {
            return true;
        }
    }
...
bool is_arg_load_from_mem(const insn_t &insn, ea_t *out_mem_addr)
{
    ...
    if (is_x86_64())
    {
        ...
        if (insn.itype == NN_mov && insn.Op1.type == o_reg && insn.Op1.reg == arg_reg &&
            insn.Op2.type == o_mem)
        {
            if (out_mem_addr)
    ...
    else if (is_arm64())
    {
        ...
        if ((insn.itype == ARM_ldr || insn.itype == ARM_ldur) && insn.Op1.type == o_reg &&
            insn.Op1.reg == arg_reg)
        {
...
int get_ptr_size() { return is_64bit() ? 8 : 4; }
