...
class symbolic_var_t
{
  public:
    ...
    symbolic_var_t(var_kind_t kind, uint64_t id, int size) : m_kind(kind), m_id(id), m_size(size) {}
    ...
    struct hash_t
    {
        ...
        {
            return std::hash<int>()(v.m_kind) ^ (std::hash<uint64_t>()(v.m_id) << 1) ^
...
class mcode_translator_t
{
  public:
    ...
    void invalidate_values_if(const std::function<bool(const symbolic_var_t &)> &predicate);
  ...
  private:
    ...
    std::unordered_map<symbolic_var_t, std::shared_ptr<z3::expr>, symbolic_var_t::hash_t>
        m_var_cache;
...
class symbolic_executor_t
{
  public:
    ...
    void set_value(const mop_t &op, const z3::expr &value, bool preserve_across_calls = false);
...
class state_machine_solver_t
{
  public:
    ...
    struct block_transition_t
    {
        ...
        block_transition_t()
            : from_block(-1), to_block(-1), to_block_true(-1), to_block_false(-1), next_state(0),
              is_conditional(false), solved(false)
        {
        }
    };
...
class predicate_simplifier_t
{
  public:
    ...
    std::optional<bool> check_comparison_constant(mcode_t cmp_op, const mop_t &left,
                                                  const mop_t &right);
