...
constexpr std::size_t percentage_threshold(std::size_t denominator, unsigned percent)
{
    return (denominator / 100U) * percent + ((denominator % 100U) * percent + 99U) / 100U;
}

constexpr bool ratio_at_least(std::size_t numerator, std::size_t denominator, unsigned percent)
{
    return denominator != 0 && numerator >= percentage_threshold(denominator, percent);
}

inline unsigned bounded_percent(std::size_t numerator, std::size_t denominator)
{
    if (denominator == 0)
        return 0;
...
inline switch_dispatch_assessment_t
assess_switch_dispatch(const switch_dispatch_features_t &features)
{
    ...
    result.target_diversity_percent =
        bounded_percent(features.unique_target_count, features.case_count);
    result.recurrence_percent =
        bounded_percent(features.returning_target_count, features.unique_target_count);
    result.direct_return_percent =
        bounded_percent(features.direct_return_target_count, features.returning_target_count);
    result.frontier_percent =
        bounded_percent(features.return_frontier_count, features.returning_target_count);

    const bool enough_cases = features.case_count >= 8 && features.unique_target_count >= 6;
    const bool diverse_targets =
        ratio_at_least(features.unique_target_count, features.case_count, 50);
    const bool recurrent_targets =
        ratio_at_least(features.returning_target_count, features.unique_target_count, 60);
    ...
    const bool distributed_loop =
        features.return_frontier_count >= 3 &&
        ratio_at_least(features.return_frontier_count, features.returning_target_count, 10);
    ...
    const bool state_evidence =
        features.selector_transform_count >= 3 || features.state_assignment_count >= 5;
    ...
    if (diverse_targets)
    {
        ...
        result.score +=
            ratio_at_least(features.unique_target_count, features.case_count, 80) ? 10 : 5;
    }
    if (recurrent_targets)
    {
        result.signals |= SDS_RECURRENT_TARGETS;
        result.score += 20;
        if (ratio_at_least(features.returning_target_count, features.unique_target_count, 80))
            result.score += 10;
        if (ratio_at_least(features.returning_target_count, features.unique_target_count, 95))
            result.score += 10;
    }
    if (distributed_loop)
    {
        ...
        result.score += 15;
        if (ratio_at_least(features.return_frontier_count, features.returning_target_count, 25))
            result.score += 5;
    }
    if (ratio_at_least(features.direct_return_target_count, features.returning_target_count, 25))
    {
    ...
    result.accepted = features.has_indirect_jump && features.cfg_complete && enough_cases &&
                      diverse_targets && recurrent_targets && distributed_loop && state_evidence &&
                      features.dispatcher_predecessor_count >= 3 && result.score >= 65;
    return result;
}
