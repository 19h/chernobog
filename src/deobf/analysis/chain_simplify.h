...
struct alignas(16) chain_operand_t
{
    ...
    chain_operand_t(const mop_t &m, bool neg = false) : mop(m), is_negated(neg) {}
};
...
struct chain_result_t
{
    ...
    chain_result_t()
        : simplified(false), const_result(0), has_const(false), has_identity(false),
          is_single_operand(false), is_zero(false), is_all_ones(false)
    {
    }
};
...
class ChainSimplifier
{
  ...
  private:
    ...
    static bool remove_identity_pairs(std::vector<chain_operand_t> &operands, mcode_t op, int size);
    ...
    static minsn_t *build_simplified(mblock_t *blk, minsn_t *orig, const chain_result_t &result,
                                     mcode_t op);
