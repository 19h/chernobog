...
bool ranges_overlap(sval_t left_offset, int left_size, sval_t right_offset, int right_size)
{
    ...
    {
        return static_cast<uint64_t>(right_offset) - static_cast<uint64_t>(left_offset) <
               static_cast<uint64_t>(left_size);
    }
    return static_cast<uint64_t>(left_offset) - static_cast<uint64_t>(right_offset) <
...
std::optional<mop_t> stack_tracker_t::trace_source(mblock_t *block, const minsn_t *before,
                                                   sval_t offset, int size)
{
    ...
    while (block)
    {
        ...
        for (const minsn_t *instruction = cursor; instruction; instruction = instruction->prev)
        {
            ...
            if (is_mcode_call(instruction->opcode) || instruction->opcode == m_stx)
            {
                return std::nullopt;
            }
        }
...
std::optional<ea_t> stack_tracker_t::source_address(const mop_t &source)
{
    ...
    else if (source.t == mop_v)
    {
        ...
        const size_t string_length = get_max_strlit_length(source.g, STRTYPE_C);
        if (is_code(flags) || get_func(source.g) || (string_length > 0 && string_length < 1024))
        {
            address = source.g;
        }
        else
...
std::optional<ea_t> stack_tracker_t::trace_address(mblock_t *block, const minsn_t *before,
                                                   sval_t offset, int size)
{
