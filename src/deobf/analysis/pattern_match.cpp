...
static bool find_likely_state_var(const std::vector<state_cmp_t> &cmps, mop_t *out_var)
{
    ...
    {
        auto group = std::find_if(groups.begin(), groups.end(), [&](const variable_count_t &entry)
    ...
    const auto best = std::max_element(groups.begin(), groups.end(),
                                       [](const variable_count_t &a, const variable_count_t &b)
...
static void map_switch_targets(mbl_array_t *mba, jmptbl_info_t *info)
{
    ...
    {
        const auto after = std::upper_bound(intervals.begin(), intervals.end(), target,
...
static jmptbl_info_t *find_switch_candidate(std::vector<jmptbl_info_t> *tables, int block_idx,
                                            ea_t jump_ea)
{
    if (!tables)
        return nullptr;
...
static void record_ida_switch(mbl_array_t *mba, std::vector<jmptbl_info_t> *tables, ea_t jump_ea,
                              const switch_info_t &switch_info)
{
    ...
    jmptbl_info_t *candidate = find_switch_candidate(tables, block_idx, jump_ea);
    if (!candidate)
    {
        tables->emplace_back();
...
static bool detect_jump_table_pattern(mbl_array_t *mba, std::vector<jmptbl_info_t> *out_tables)
{
    ...
    for (int i = 0; i < mba->qty; ++i)
    {
        ...
        jmptbl_info_t *candidate = find_switch_candidate(&tables, i, blk->tail->ea);
        switch_info_t si;
        ...
        if (!candidate && blk->succset.size() >= 8)
        {
            ...
            candidate->num_cases = static_cast<std::size_t>(blk->succset.size());
        }
    }
...
static dispatcher_region_t find_dispatcher_region(mbl_array_t *mba, int switch_block_idx,
                                                  std::size_t max_chain_blocks = 8)
{
    ...
    while (region.blocks.size() < max_chain_blocks)
    {
        ...
        if (predecessor < 0 || predecessor >= mba->qty || !visited.insert(predecessor).second)
            break;
...
{
    std::size_t count = 0;
    for (const minsn_t *insn = block ? block->head : nullptr; insn != nullptr; insn = insn->next)
        ++count;
    return count;
}
...
static std::size_t nested_transform_count(const minsn_t *instruction, std::size_t depth = 0)
{
...
{
    std::size_t count = 0;
    for (const minsn_t *instruction = block ? block->head : nullptr; instruction;
         instruction = instruction->next)
        count += nested_transform_count(instruction);
    return count;
}

static int count_state_index_assignments(mbl_array_t *mba, int max_index,
...
static switch_dispatch_evidence_t analyze_switch_dispatch(mbl_array_t *mba,
                                                          const jmptbl_info_t &jump_table,
                                                          std::size_t max_return_distance = 32)
{
    ...
    const mblock_t *switch_block = mba->get_mblock(evidence.region.switch_block);
    const mblock_t *dispatcher_block = mba->get_mblock(evidence.region.dispatcher_block);
    if (!switch_block || !dispatcher_block)
        return evidence;

    std::set<int> region_blocks(evidence.region.blocks.begin(), evidence.region.blocks.end());
    ...
    std::vector<int> return_distance(static_cast<std::size_t>(mba->qty), -1);
    ...
    while (!worklist.empty())
    {
        ...
        worklist.pop_front();
        const int distance = return_distance[static_cast<std::size_t>(block_idx)];
        if (distance < 0 || static_cast<std::size_t>(distance) >= max_return_distance)
            continue;
        const mblock_t *block = mba->get_mblock(block_idx);
        if (!block)
            continue;
        for (int predecessor : block->predset)
        {
            if (predecessor < 0 || predecessor >= mba->qty)
                continue;
            int &predecessor_distance = return_distance[static_cast<std::size_t>(predecessor)];
    ...
    features.case_count =
        jump_table.num_cases != 0 ? jump_table.num_cases : jump_table.target_blocks.size();
    ...
    for (int target : jump_table.target_blocks)
    {
        ...
        features.max_return_distance =
            std::max(features.max_return_distance, static_cast<std::size_t>(distance));
    }
    ...
    std::vector<int> target_distance(static_cast<std::size_t>(mba->qty), -1);
    worklist.clear();
    for (int target : jump_table.target_blocks)
    {
        if (target < 0 || target >= mba->qty || region_blocks.count(target) != 0)
            continue;
    ...
    while (!worklist.empty())
    {
        const int block_idx = worklist.front();
        worklist.pop_front();
        const int distance = target_distance[static_cast<std::size_t>(block_idx)];
        if (distance < 0 || static_cast<std::size_t>(distance) >= max_return_distance)
            continue;
        const mblock_t *block = mba->get_mblock(block_idx);
        if (!block)
        ...
        {
            if (successor < 0 || successor >= mba->qty || region_blocks.count(successor) != 0)
                continue;
            int &successor_distance = target_distance[static_cast<std::size_t>(successor)];
    ...
    features.dispatcher_predecessor_count = static_cast<std::size_t>(dispatcher_block->npred());
    ...
    features.selector_transform_count = selector_transform_count(dispatcher_block);
    if (features.selector_transform_count < 3)
    {
        const int capped_cases = static_cast<int>(std::min<std::size_t>(
            features.case_count, static_cast<std::size_t>(std::numeric_limits<int>::max() - 10)));
        features.state_assignment_count = static_cast<std::size_t>(count_state_index_assignments(
            mba, capped_cases + 10, &return_distance, &target_distance));
    }
    features.has_indirect_jump =
        jump_table.ida_detected || (switch_block->tail && switch_block->tail->opcode == m_ijmp);

    const std::size_t expected_targets =
        !jump_table.target_eas.empty() ? jump_table.target_eas.size() : features.case_count;
    features.cfg_complete =
        features.unique_target_count >= 6 &&
        (expected_targets == 0 || switch_dispatch_detail::ratio_at_least(
                                         ...
                                         const std::vector<int> *target_distance)
{
    ...
    for (int i = 0; i < mba->qty; i++)
    {
        ...
        if (return_distance &&
            (block_index >= return_distance->size() || (*return_distance)[block_index] < 0))
            continue;
        if (target_distance &&
            (block_index >= target_distance->size() || (*target_distance)[block_index] < 0))
            continue;
        mblock_t *blk = mba->get_mblock(i);
        if (!blk)
            continue;
...
bool detect_flatten_pattern(mbl_array_t *mba, flatten_info_t *out)
{
    ...
    if (detect_jump_table_pattern(mba, &jump_tables))
    {
        ...
        for (const auto &jt : jump_tables)
        {
            ...
            const switch_dispatch_evidence_t &evidence = evidence_by_table.back();
            ...
            deobf::log_verbose(
                ...
                evidence.region.switch_block, evidence.region.dispatcher_block, features.case_count,
                features.unique_target_count, features.returning_target_count,
                assessment.recurrence_percent, features.direct_return_target_count,
                assessment.direct_return_percent, features.return_frontier_count,
                assessment.frontier_percent, features.dispatcher_predecessor_count,
                features.dispatcher_chain_blocks, features.selector_instruction_count,
                features.selector_transform_count, features.state_assignment_count,
                features.max_return_distance, features.cfg_complete ? 1 : 0, assessment.score,
                assessment.accepted ? 1 : 0);
            if (assessment.accepted &&
                (!best_table || assessment.score > best_evidence.assessment.score ||
                 (assessment.score == best_evidence.assessment.score &&
                  features.returning_target_count > best_evidence.features.returning_target_count)))
            {
                best_table = &jt;
        ...
        if (best_table)
        {
            ...
            deobf::log("[pattern][cff-switch] Detected recurrent switch dispatcher: "
                       "switch=%d dispatcher=%d cases=%zu returning=%zu "
                       "frontier=%zu score=%u\n",
                       best_evidence.region.switch_block, best_evidence.region.dispatcher_block,
                       features.case_count, features.returning_target_count,
                       features.return_frontier_count, best_evidence.assessment.score);
            if (out)
            {
                ...
                out->returning_target_count = features.returning_target_count;
                out->direct_return_target_count = features.direct_return_target_count;
                out->return_frontier_count = features.return_frontier_count;
                out->confidence_score = best_evidence.assessment.score;
                out->dispatcher_blocks.insert(best_evidence.region.blocks.begin(),
        ...
        for (std::size_t table_index = 0; table_index < jump_tables.size(); ++table_index)
        {
            const jmptbl_info_t &jt = jump_tables[table_index];
            const switch_dispatch_evidence_t &evidence = evidence_by_table[table_index];
            if (evidence.features.cfg_complete || jt.num_cases < 20 ||
                evidence.region.dispatcher_block < 0)
                continue;
            const mblock_t *dispatcher = mba->get_mblock(evidence.region.dispatcher_block);
            if (!dispatcher || dispatcher->npred() < 3)
                continue;
            const int capped_cases = static_cast<int>(std::min<std::size_t>(
                jt.num_cases, static_cast<std::size_t>(std::numeric_limits<int>::max() - 10)));
            const int index_count =
                count_state_index_assignments(mba, capped_cases + 10, nullptr, nullptr);
            if (index_count < 5)
                continue;
            deobf::log("[pattern][cff-switch] Detected indexed dispatcher through "
            ...
            if (out)
            {
                ...
                const std::size_t state_count = std::min<std::size_t>(
                    jt.num_cases, static_cast<std::size_t>(std::numeric_limits<int>::max()));
