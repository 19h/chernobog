...
namespace
{
bool register_range_contains(const symbolic_var_t &outer, const symbolic_var_t &inner);
}
...
unsigned get_global_timeout() { return g_timeout_ms; }
...
void z3_context_t::reset() { m_solver.reset(); }
...
//--------------------------------------------------------------------------
mcode_translator_t::mcode_translator_t(z3_context_t &ctx) : m_ctx(ctx), m_fresh_counter(0) {}
...
{
    const int bits = var.size() > 0 && var.size() <= 128 ? var.size() * 8 : 32;
    ...
    if (var.kind() == symbolic_var_t::VAR_REGISTER && var.size() > 0)
    {
        for (const auto &entry : m_known_values)
        {
            ...
            if (!register_range_contains(stored, var) || stored.size() > 8)
                continue;
            const uint64_t byte_offset = var.id() - stored.id();
            if (byte_offset >= 8)
                continue;
            const uint64_t mask =
                var.size() >= 8 ? UINT64_MAX : ((uint64_t(1) << (var.size() * 8)) - 1);
            return make_const((entry.second >> (byte_offset * 8)) & mask, bits);
        }
        for (const auto &entry : m_var_cache)
        {
            ...
            if (low > static_cast<uint64_t>(INT_MAX) ||
                low + static_cast<uint64_t>(bits) > static_cast<uint64_t>(value_bits))
                continue;
            if (value_bits == bits && low == 0)
                return value;
            return extract(value, static_cast<int>(low) + bits - 1, static_cast<int>(low));
        }
    }
    ...
    name += "_" + std::to_string(var.size()) + "_v" + std::to_string(m_fresh_counter++);
...
void mcode_translator_t::set_symbolic_value(const symbolic_var_t &var, const z3::expr &value)
{
    m_known_values.erase(var);
    const int bits = var.size() > 0 ? var.size() * 8 : value.get_sort().bv_size();
    m_var_cache[var] = std::make_shared<z3::expr>(resize(value, bits));
}

void mcode_translator_t::set_symbolic_value(const mop_t &op, const z3::expr &value)
{
...
void mcode_translator_t::invalidate_memory_values()
{
    const auto is_memory = [](const symbolic_var_t &var)
    {
        return var.kind() == symbolic_var_t::VAR_STACK ||
               var.kind() == symbolic_var_t::VAR_GLOBAL || var.kind() == symbolic_var_t::VAR_MEMORY;
    };
...
bool unsigned_ranges_overlap(uint64_t left, int left_size, uint64_t right, int right_size)
{
...
bool signed_ranges_overlap(uint64_t left_id, int left_size, uint64_t right_id, int right_size)
{
...
bool unsigned_range_contains(uint64_t outer, int outer_size, uint64_t inner, int inner_size)
{
    ...
    if (outer > inner || outer > UINT64_MAX - outer_width || inner > UINT64_MAX - inner_width)
        return false;
    return inner + inner_width <= outer + outer_width;
}

bool signed_range_contains(uint64_t outer_id, int outer_size, uint64_t inner_id, int inner_size)
{
    const int64_t outer = static_cast<int64_t>(outer_id);
    const int64_t inner = static_cast<int64_t>(inner_id);
    if (outer_size <= 0 || inner_size <= 0)
        return outer == inner && outer_size == inner_size;
    if (outer > inner || outer > INT64_MAX - outer_size || inner > INT64_MAX - inner_size)
        return false;
...
bool register_range_contains(const symbolic_var_t &outer, const symbolic_var_t &inner)
{
    if (outer.kind() != symbolic_var_t::VAR_REGISTER ||
        inner.kind() != symbolic_var_t::VAR_REGISTER || outer.size() <= 0 || inner.size() <= 0 ||
        outer.id() > inner.id())
        return false;
    const uint64_t outer_size = static_cast<uint64_t>(outer.size());
    const uint64_t inner_size = static_cast<uint64_t>(inner.size());
    if (outer.id() > UINT64_MAX - outer_size || inner.id() > UINT64_MAX - inner_size)
        return false;
    return inner.id() + inner_size <= outer.id() + outer_size;
}

bool variable_range_contains(const symbolic_var_t &outer, const symbolic_var_t &inner)
{
    ...
    case symbolic_var_t::VAR_STACK:
        return signed_range_contains(outer.id(), outer.size(), inner.id(), inner.size());
    case symbolic_var_t::VAR_GLOBAL:
        return unsigned_range_contains(outer.id(), outer.size(), inner.id(), inner.size());
    case symbolic_var_t::VAR_LOCAL:
        return outer.id() == inner.id() &&
               (outer.size() <= 0 || inner.size() <= 0 ? outer.size() == inner.size()
...
bool variables_may_alias(const symbolic_var_t &left, const symbolic_var_t &right)
{
    const auto is_memory = [](symbolic_var_t::var_kind_t kind)
    {
        return kind == symbolic_var_t::VAR_STACK || kind == symbolic_var_t::VAR_GLOBAL ||
               kind == symbolic_var_t::VAR_MEMORY;
    };

    if (is_memory(left.kind()) && is_memory(right.kind()))
    {
        if (left.kind() == symbolic_var_t::VAR_MEMORY || right.kind() == symbolic_var_t::VAR_MEMORY)
            return true;
        if (left.kind() != right.kind())
            return false;
        if (left.kind() == symbolic_var_t::VAR_STACK)
            return signed_ranges_overlap(left.id(), left.size(), right.id(), right.size());
        if (left.kind() == symbolic_var_t::VAR_GLOBAL)
            return unsigned_ranges_overlap(left.id(), left.size(), right.id(), right.size());
...
std::optional<symbolic_var_t> symbolic_var_from_mop(const mop_t &op)
{
    ...
    case mop_S:
        if (op.s)
            return symbolic_var_t(symbolic_var_t::VAR_STACK, static_cast<uint64_t>(op.s->off),
                                  op.size);
        break;
    case mop_v:
        return symbolic_var_t(symbolic_var_t::VAR_GLOBAL, op.g, op.size);
    case mop_l:
        if (op.l)
            return symbolic_var_t(symbolic_var_t::VAR_LOCAL, static_cast<uint64_t>(op.l->idx),
                                  op.size);
        break;
    default:
        break;
...
void mcode_translator_t::invalidate_aliases(const symbolic_var_t &var)
{
    ...
    for (auto p = m_var_cache.begin(); p != m_var_cache.end();)
    {
        p = variables_may_alias(p->first, var) ? m_var_cache.erase(p) : std::next(p);
    }
    for (auto p = m_known_values.begin(); p != m_known_values.end();)
    {
        p = variables_may_alias(p->first, var) ? m_known_values.erase(p) : std::next(p);
    }
}
...
z3::expr mcode_translator_t::translate_insn(const minsn_t *ins)
{
    ...
    if (ins->is_fpinsn())
    {
        return m_ctx.ctx().bv_const(("fp_" + std::to_string(m_fresh_counter++)).c_str(),
                                    result_bits);
    }
    ...
    if (ins->opcode == m_call && ins->d.t == mop_f && ins->d.f && ins->d.f->args.size() >= 2)
    {
        const funcrole_t role = ins->d.f->role;
        const char *helper = ins->l.t == mop_h ? ins->l.helper : nullptr;
        const bool is_rol = role == ROLE_ROL || (helper && std::strstr(helper, "ROL") != nullptr);
        const bool is_ror = role == ROLE_ROR || (helper && std::strstr(helper, "ROR") != nullptr);
        if (is_rol || is_ror)
        {
            ...
            {
                z3::expr shift = resize(translate_operand(ins->d.f->args[1]), width);
                const z3::expr mask = make_const(static_cast<uint64_t>(width - 1), width);
                shift = shift & mask;
                const z3::expr inverse =
                    (make_const(static_cast<uint64_t>(width), width) - shift) & mask;
                z3::expr rotated = is_rol ? (z3::shl(value, shift) | z3::lshr(value, inverse))
    ...
    if (l.get_sort().bv_size() != r.get_sort().bv_size())
    {
        ...
        const bool signed_operands = ins->opcode == m_setl || ins->opcode == m_setle ||
    ...
    case m_ldx:
        ...
        result = m_ctx.ctx().bv_const(("load_" + std::to_string(m_fresh_counter++)).c_str(),
                                      result_bits);
        break;
    ...
    case m_smod:
    {
        ...
        const int operand_bytes = static_cast<int>(r.get_sort().bv_size() / 8U);
        const int divisor_bytes =
            chernobog::bitvector::valid_byte_width(ins->r.size) ? ins->r.size : operand_bytes;
        const std::optional<uint64_t> divisor_value = opaque_eval_t::evaluate_operand(ins->r);
        const uint64_t divisor =
            divisor_value ? chernobog::bitvector::truncate(*divisor_value, divisor_bytes) : 0;
        ...
        {
            const std::optional<uint64_t> dividend_value = opaque_eval_t::evaluate_operand(ins->l);
            const int dividend_bytes =
                chernobog::bitvector::valid_byte_width(ins->l.size) ? ins->l.size : operand_bytes;
            safe = dividend_value.has_value() &&
                   chernobog::bitvector::sign_extend(*dividend_value, dividend_bytes) !=
                       chernobog::bitvector::signed_min(operand_bytes);
        }

        if (!safe)
        {
            result = m_ctx.ctx().bv_const(("div_" + std::to_string(m_fresh_counter++)).c_str(),
                                          result_bits);
        }
    ...
    case m_lnot:
        result =
            z3::ite(l == 0, m_ctx.ctx().bv_val(1, result_bits), m_ctx.ctx().bv_val(0, result_bits));
        break;
    ...
    case m_seto:
    case m_setp:
        result = m_ctx.ctx().bv_const(("flag_" + std::to_string(m_fresh_counter++)).c_str(),
                                      result_bits);
        break;
    ...
    case m_setz:
        result =
            z3::ite(l == r, m_ctx.ctx().bv_val(1, result_bits), m_ctx.ctx().bv_val(0, result_bits));
        break;
    ...
    default:
        ...
        result = m_ctx.ctx().bv_const(("insn_" + std::to_string(m_fresh_counter++)).c_str(),
                                      result_bits);
        break;
    }
...
z3::expr mcode_translator_t::translate_jcc_condition(const minsn_t *jcc)
{
    ...
    if (jcc->is_fpinsn())
    {
        return m_ctx.ctx().bool_const(("fpjcc_" + std::to_string(m_fresh_counter++)).c_str());
    }
    ...
    default:
        ...
        return m_ctx.ctx().bool_const(("jcc_" + std::to_string(m_fresh_counter++)).c_str());
    }
}
...
//--------------------------------------------------------------------------
symbolic_executor_t::symbolic_executor_t(z3_context_t &ctx) : m_ctx(ctx), m_translator(ctx) {}
...
bool is_modeled_pure_rotate_call(const minsn_t *instruction)
{
    ...
    return helper &&
           (std::strstr(helper, "ROL") != nullptr || std::strstr(helper, "ROR") != nullptr);
}
...
struct nested_call_collector_t : public minsn_visitor_t
{
    ...
    explicit nested_call_collector_t(const minsn_t *instruction) : root(instruction) {}

    int idaapi visit_minsn() override
    {
        if (curins != nullptr && curins != root && is_mcode_call(curins->opcode) &&
...
{
    const mcallinfo_t *call_info = ins && ins->d.t == mop_f ? ins->d.f : nullptr;
    const auto preserved = [&](const symbolic_var_t &candidate)
    {
        return std::any_of(m_call_preserved.begin(), m_call_preserved.end(),
    ...
    const auto invalidated = [&](const symbolic_var_t &candidate)
    {
        ...
        {
            return !call_info || call_info->spoiled.reg.has_any(static_cast<mreg_t>(candidate.id()),
                                                                candidate.size());
        }
...
void symbolic_executor_t::handle_assignment(const minsn_t *ins)
{
    ...
    const std::optional<symbolic_var_t> destination = symbolic_var_from_mop(ins->d);
    if (destination)
    {
        const symbolic_var_t &dst_var = *destination;
        for (auto p = m_state.begin(); p != m_state.end();)
        {
            p = variables_may_alias(p->first, dst_var) ? m_state.erase(p) : std::next(p);
        }
...
void symbolic_executor_t::handle_store(const minsn_t *ins)
{
    ...
    if (stored_value.has_value())
    {
        symbolic_var_t dst_var = symbolic_var_t(
            ...
            ins->d.t == mop_S && ins->d.s ? ins->d.s->off : ins->d.g, ins->d.size);
...
z3::expr symbolic_executor_t::evaluate_operand(const mop_t &op, int default_size)
{
...
symbolic_executor_t::feasibility_t symbolic_executor_t::check_feasibility()
{
...
{
    if (&condition.ctx() != &m_ctx.ctx() || static_cast<Z3_ast>(condition) == nullptr ||
        !condition.is_bool())
...
symbolic_executor_t::query_feasibility(const z3::expr *extra_condition)
{
    try
    {
        ...
        const z3::check_result result = chernobog::solver_evidence::check(
            m_ctx.solver(), extra_condition ? "path plus temporary condition" : "path feasibility");
        if (result == z3::sat)
        {
        ...
        deobf::log_verbose("[z3] path-feasibility query returned unknown: %s\n",
                           m_ctx.solver().reason_unknown().c_str());
    }
    catch (...)
    {
        deobf::log_verbose("[z3] path-feasibility query raised an exception\n");
    }
    return feasibility_t::unknown;
}

void symbolic_executor_t::set_value(const mop_t &op, const z3::expr &value,
                                    bool preserve_across_calls)
{
    const std::optional<symbolic_var_t> destination = symbolic_var_from_mop(op);
    if (!destination)
        return;
    for (auto p = m_state.begin(); p != m_state.end();)
    {
        p = variables_may_alias(p->first, *destination) ? m_state.erase(p) : std::next(p);
    }
    ...
    if (preserve_across_calls &&
        std::none_of(m_call_preserved.begin(), m_call_preserved.end(),
                     [&](const symbolic_var_t &existing) { return existing == *destination; }))
    {
...
void symbolic_executor_t::preserve_across_calls(const mop_t &op)
{
    ...
    if (!value)
        return;
    if (std::none_of(m_call_preserved.begin(), m_call_preserved.end(),
                     [&](const symbolic_var_t &existing) { return existing == *value; }))
    {
...
void symbolic_executor_t::invalidate_memory_values()
{
    const auto is_memory = [](const symbolic_var_t &var)
    {
        return var.kind() == symbolic_var_t::VAR_STACK ||
               ...
               var.kind() == symbolic_var_t::VAR_LOCAL || var.kind() == symbolic_var_t::VAR_MEMORY;
    };
    for (auto p = m_state.begin(); p != m_state.end();)
    {
    ...
    if (var.kind() == symbolic_var_t::VAR_REGISTER && var.size() > 0)
    {
        for (const auto &entry : m_state)
        {
            ...
            if (value_bits == static_cast<unsigned>(bits) && low == 0)
                return value;
            return value.extract(static_cast<unsigned>(low) + bits - 1, static_cast<unsigned>(low));
        }
    }
    return std::nullopt;
}
...
std::optional<uint64_t> symbolic_executor_t::solve_for_value(const z3::expr &expr)
{
    try
    {
        if (&expr.ctx() != &m_ctx.ctx() || static_cast<Z3_ast>(expr) == nullptr || !expr.is_bv() ||
        ...
        uint64_t concrete = 0;
        if (!value.is_numeral() || !Z3_get_numeral_uint64(expr.ctx(), value, &concrete))
            return std::nullopt;

        const z3::expr different = expr != expr.ctx().bv_val(concrete, expr.get_sort().bv_size());
        const feasibility_t alternative = query_feasibility(&different);
        return alternative == feasibility_t::infeasible ? std::optional<uint64_t>(concrete)
                                                        : std::nullopt;
    }
    catch (...)
    {
        deobf::log_verbose("[z3] unique-value query raised an exception\n");
        return std::nullopt;
    }
}
...
//--------------------------------------------------------------------------
state_machine_solver_t::state_machine_solver_t(z3_context_t &ctx) : m_ctx(ctx), m_translator(ctx) {}
...
//--------------------------------------------------------------------------
static bool extract_state_comparison(mbl_array_t *mba, int block_idx, mop_t *out_var,
                                     uint64_t *out_state, int *out_target)
{
    ...
    for (const minsn_t *ins = blk->head; ins; ins = ins->next)
    {
        ...
        if (is_mcode_set(ins->opcode) && block_idx >= 2 && block_idx <= 5)
        {
            deobf::log("[z3] Block %d: setXX opcode=%d l.t=%d l.r=%d r.t=%d r.r=%d\n", block_idx,
                       ins->opcode, ins->l.t, ins->l.t == mop_r ? ins->l.r : -1, ins->r.t,
                       ins->r.t == mop_r ? ins->r.r : -1);
        }
        ...
        if (is_mcode_set(ins->opcode))
        {
            ...
            {
                deobf::log(
                    "[z3] Block %d: checking setXX - l.t=%d l.r=%d (have=%d) r.t=%d r.r=%d (have=%d)\n",
                    block_idx, ins->l.t, ins->l.t == mop_r ? (int)ins->l.r : -1,
                    ins->l.t == mop_r ? (int)reg_to_const.count(ins->l.r) : 0, ins->r.t,
                    ins->r.t == mop_r ? (int)ins->r.r : -1,
                ...
                for (const minsn_t *jcc = ins->next; jcc; jcc = jcc->next)
                {
                    if (deobf::is_jcc(jcc->opcode))
                    {
                        ...
                        else if (jcc->d.t == mop_v)
                        {
                            ...
                            {
                                deobf::log(
                                    "[z3] Block %d: found setXX(%d) state cmp 0x%llx -> block %d (addr 0x%llx)\n",
                            ...
                            {
                                deobf::log(
                                    "[z3] Block %d: found setXX(%d) state cmp 0x%llx -> unresolved addr 0x%llx\n",
                                    block_idx, ins->opcode, (unsigned long long)state_val,
                        ...
                        {
                            deobf::log(
                                "[z3] Block %d: found setXX(%d) state cmp 0x%llx -> block %d\n",
                                block_idx, ins->opcode, (unsigned long long)state_val,
                                target_block);
                            if (out_var)
                                *out_var = var;
        ...
        if (deobf::is_jcc(ins->opcode))
        {
            if (ins->l.t == mop_d && ins->l.d)
            {
                ...
                if (is_mcode_set(cmp->opcode))
                {
                    ...
                    {
                        deobf::log(
                            "[z3] Block %d: found nested setXX state cmp 0x%llx -> block %d\n",
...
state_machine_solver_t::analyze_dispatcher(mbl_array_t *mba, int block_idx)
{
    ...
    if (state_to_target.size() >= 2 && found_state_var)
    {
        ...
        {
            result.state_var = symbolic_var_t(symbolic_var_t::VAR_STACK, potential_state_var.s->off,
                                              potential_state_var.size);
        }
        else if (potential_state_var.t == mop_r)
        {
            result.state_var = symbolic_var_t(symbolic_var_t::VAR_REGISTER, potential_state_var.r,
                                              potential_state_var.size);
        }
        else if (potential_state_var.t == mop_v)
        {
            result.state_var = symbolic_var_t(symbolic_var_t::VAR_GLOBAL, potential_state_var.g,
                                              potential_state_var.size);
        }

        deobf::log("[z3] Found dispatcher at block %d with %zu state mappings\n", block_idx,
                   state_to_target.size());
    ...
    int first_target = -1;
    bool has_comparison =
        extract_state_comparison(mba, block_idx, &first_var, &first_state, &first_target);
    ...
    if (has_comparison)
    {
        ...
        for (int scan_blk = 0; scan_blk < mba->qty; scan_blk++)
        {
            ...
            if (extract_state_comparison(mba, scan_blk, &var, &state_val, &target) && target >= 0)
            {
                ...
                if (state_to_target.find(state_val) == state_to_target.end())
                {
                    ...
                    deobf::log(
                        "[z3] Scan: block %d added state 0x%llx -> block %d (map size=%zu)\n",
        ...
        if (state_to_target.size() >= 2 && found_state_var)
        {
            ...
            {
                result.state_var =
                    symbolic_var_t(symbolic_var_t::VAR_STACK, potential_state_var.s->off,
                                   potential_state_var.size);
            }
            else if (potential_state_var.t == mop_r)
            {
                result.state_var = symbolic_var_t(symbolic_var_t::VAR_REGISTER,
                                                  potential_state_var.r, potential_state_var.size);
            }
            else if (potential_state_var.t == mop_v)
            {
                result.state_var = symbolic_var_t(symbolic_var_t::VAR_GLOBAL, potential_state_var.g,
            ...
            deobf::log(
                "[z3] Found CASCADING dispatcher starting at block %d with %zu state mappings\n",
                                                ...
                                                const symbolic_var_t &state_var)
{
    ...
    for (const minsn_t *ins = blk->tail; ins; ins = ins->prev)
    {
        ...
        if (matches)
        {
            if ((ins->opcode == m_mov || ins->opcode == m_stx) && ins->l.t == mop_n && ins->l.nnn &&
        ...
        if (ins->opcode == m_stx && (state_var.kind() == symbolic_var_t::VAR_STACK ||
                                                 ...
                                                 const symbolic_var_t &state_var)
{
    ...
    if (written_state.has_value())
    {
        ...
        deobf::log_verbose("[z3] Block %d writes state 0x%llx\n", block_idx,
                           (unsigned long long)*written_state);
    }
    ...
    if (blk->tail && deobf::is_jcc(blk->tail->opcode))
    {
        ...
        result.condition =
            std::make_shared<z3::expr>(m_translator.translate_jcc_condition(blk->tail));
...
opaque_predicate_solver_t::analyze_condition(const minsn_t *cond)
{
    ...
    try
    {
        ...
        m_ctx.solver().add(!condition);
        const auto negated =
            chernobog::solver_evidence::check(m_ctx.solver(), "predicate negation feasibility");
        if (negated == z3::unsat)
        {
        ...
        m_ctx.solver().add(condition);
        const auto positive =
            chernobog::solver_evidence::check(m_ctx.solver(), "predicate feasibility");
...
//--------------------------------------------------------------------------
predicate_simplifier_t::predicate_simplifier_t(z3_context_t &ctx) : m_ctx(ctx), m_translator(ctx) {}
...
std::optional<bool> predicate_simplifier_t::check_comparison_constant(mcode_t cmp_op,
                                                                      const mop_t &left,
                                                                      const mop_t &right)
{
...
int predicate_simplifier_t::simplify_jcc(const minsn_t *jcc)
{
    ...
    try
    {
        ...
        if (chernobog::solver_evidence::check(m_ctx.solver(), "Jcc negation feasibility") ==
            z3::unsat)
        {
