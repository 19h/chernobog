...
template <typename T> SIMD_FORCE_INLINE T load_unaligned(const void *ptr)
{
...
template <size_t N> SIMD_FORCE_INLINE bool is_aligned(const void *ptr)
{
...
template <size_t N> SIMD_FORCE_INLINE void *align_up(void *ptr)
{
    static_assert((N & (N - 1)) == 0, "N must be power of 2");
    return reinterpret_cast<void *>((reinterpret_cast<uintptr_t>(ptr) + (N - 1)) & ~(N - 1));
}

// Align pointer down to N bytes
template <size_t N> SIMD_FORCE_INLINE void *align_down(void *ptr)
{
    static_assert((N & (N - 1)) == 0, "N must be power of 2");
    return reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(ptr) & ~(N - 1));
}
...
SIMD_FORCE_INLINE bool mem_eq(const void *a, const void *b, size_t n)
{
    ...
    case 1:
        return *pa == *pb;
    case 2:
        return load_unaligned<uint16_t>(pa) == load_unaligned<uint16_t>(pb);
    case 4:
        return load_unaligned<uint32_t>(pa) == load_unaligned<uint32_t>(pb);
    case 8:
        return load_unaligned<uint64_t>(pa) == load_unaligned<uint64_t>(pb);
    default:
        break;
    }
...
SIMD_FORCE_INLINE uint64_t hash_bytes(const void *data, size_t len)
{
    ...
    if (len >= 32)
    {
        ...
        while (len >= 32)
        {
            ...
            acc1 = vaddq_u64(
                acc1, vmull_u32(vget_low_u32(d1_32), vdup_n_u32(XXH_PRIME64_2 & 0xFFFFFFFF)));
            acc2 = vaddq_u64(
                acc2, vmull_u32(vget_low_u32(d2_32), vdup_n_u32(XXH_PRIME64_2 & 0xFFFFFFFF)));
...
SIMD_FORCE_INLINE uint64_t next_pow2(uint64_t x)
{
    if (x == 0)
        return 1;
    if (x > (uint64_t{1} << 63))
        return 0; // no representable power of two
...
SIMD_FORCE_INLINE bool is_pow2(uint64_t x) { return x != 0 && (x & (x - 1)) == 0; }
...
struct alignas(16) PatternSignature
{
    ...
    SIMD_FORCE_INLINE bool operator==(const PatternSignature &other) const
    {
        ...
        return _mm_movemask_epi8(cmp) == 0xFFFF && depth == other.depth &&
               node_count == other.node_count && leaf_count == other.leaf_count;
#elif defined(CHERNOBOG_NEON)
        ...
        return vgetq_lane_u64(cmp64, 0) == ~0ULL && vgetq_lane_u64(cmp64, 1) == ~0ULL &&
               depth == other.depth && node_count == other.node_count &&
               leaf_count == other.leaf_count;
#else
        return opcode_bits == other.opcode_bits && structure_bits == other.structure_bits &&
               depth == other.depth && node_count == other.node_count &&
               leaf_count == other.leaf_count;
#endif
    }
    ...
    SIMD_FORCE_INLINE bool compatible_with(const PatternSignature &pattern) const
    {
        ...
        if (structure_bits != pattern.structure_bits)
            return false;
...
SIMD_FORCE_INLINE void memcpy_32(void *dst, const void *src)
{
    ...
    _mm_store_si128(reinterpret_cast<__m128i *>(dst),
                    _mm_load_si128(reinterpret_cast<const __m128i *>(src)));
    _mm_store_si128(
        reinterpret_cast<__m128i *>(static_cast<char *>(dst) + 16),
...
template <typename T, size_t InlineCapacity> class SmallVector
{
  ...
  public:
    ...
    SmallVector(const SmallVector &other)
        : size_(0), capacity_(InlineCapacity), data_(inline_storage())
    {
        reserve(other.size_);
...
class StringInterner
{
  public:
    ...
    StringId intern(const char *str, size_t len)
    {
        ...
        {
            if (entries_[i].hash == h && entries_[i].len == len &&
    ...
    StringId intern(const std::string &str) { return intern(str.c_str(), str.size()); }
...
template <typename T, size_t BlockSize = 64> class ObjectPool
{
  public:
    ObjectPool() = default;
...
{
  public:
    explicit ScopedArena(Arena &arena) : arena_(arena), saved_bytes_(arena.bytes_allocated()) {}
    ...
    void *allocate(size_t size, size_t align = 8) { return arena_.allocate(size, align); }
