...
enum Flag : uint8_t { CF = 1, PF = 2, AF = 4, ZF = 8, SF = 16, OF = 32, ALL = 63 };
...
struct Flags
{
    ...
    {
        if ( (known & mask) != mask ) return std::nullopt;
        return (value & mask) != 0;
    }
};
...
{
    overflow, not_overflow, below, above_equal, equal, not_equal,
    below_equal, above, sign, not_sign, parity, not_parity,
...
inline bool evaluate_complete(Condition condition, uint8_t flags)
{
    ...
    switch ( condition )
    {
        case Condition::overflow: return o;
        case Condition::not_overflow: return !o;
        case Condition::below: return c;
        case Condition::above_equal: return !c;
        case Condition::equal: return z;
        case Condition::not_equal: return !z;
        case Condition::below_equal: return c || z;
        case Condition::above: return !c && !z;
        case Condition::sign: return s;
        case Condition::not_sign: return !s;
        case Condition::parity: return p;
        case Condition::not_parity: return !p;
        case Condition::less: return s != o;
        case Condition::greater_equal: return s == o;
        case Condition::less_equal: return z || s != o;
        case Condition::greater: return !z && s == o;
    }
    return false;
}
...
{
    if ( unsigned(condition) >= 16 ) return std::nullopt;
    std::optional<bool> result;
    for ( unsigned bits = 0; bits <= ALL; ++bits )
    {
        if ( (bits & flags.known) != (flags.value & flags.known) ) continue;
        const bool outcome = evaluate_complete(condition, uint8_t(bits));
        if ( result && *result != outcome ) return std::nullopt;
        result = outcome;
    }
    return result;
}
...
{
    return bits == 64 ? UINT64_MAX : bits == 0 || bits > 64 ? 0
                                 : (uint64_t{1} << bits) - 1;
}
...
struct Word
{
    ...
    {
        if ( !valid_width(bits) || offset > 64 - bits ) return std::nullopt;
        const uint64_t m = mask(bits);
        if ( ((known >> offset) & m) != m ) return std::nullopt;
        return (value >> offset) & m;
    }

    void write(unsigned bits, unsigned offset, std::optional<uint64_t> v,
               bool zero_extend_32)
    {
...
{
    unknown, add, adc, sub, sbb, compare, increment, decrement, negate,
    bit_and, bit_or, bit_xor, test, bit_not, shift_left, shift_right,
    arithmetic_right, clear_carry, set_carry, complement_carry,
};
...
inline void result_flags(Flags &flags, uint64_t result, unsigned width)
{
    ...
    unsigned parity = 0;
    for ( unsigned i = 0; i < 8; ++i ) parity ^= unsigned((result >> i) & 1);
...
inline std::optional<uint64_t> transfer(
    Operation op, unsigned width, std::optional<uint64_t> left,
    std::optional<uint64_t> right, bool same_operand, Flags &flags)
{
    ...
    {
        if ( flags.known & CF ) flags.value ^= CF;
        return std::nullopt;
    }
    ...
    if ( right && op != Operation::shift_left && op != Operation::shift_right
         && op != Operation::arithmetic_right ) *right &= m;
         ...
         || op == Operation::arithmetic_right )
    {
        ...
        if ( !right ) { flags.forget(); return std::nullopt; }
    ...
    if ( op == Operation::bit_and || op == Operation::bit_or
         || op == Operation::bit_xor || op == Operation::test )
    {
        flags.forget();
        flags.set(CF | OF, false);
        std::optional<uint64_t> result;
        if ( same_operand && op == Operation::bit_xor ) result = 0;
        else if ( left && right )
        {
            if ( op == Operation::bit_or ) result = *left | *right;
            else if ( op == Operation::bit_xor ) result = *left ^ *right;
            ...
            result = m;
        if ( result ) result_flags(flags, *result, width);
        return result;
    }
    ...
    flags.forget();
    if ( op == Operation::increment || op == Operation::decrement ) right = 1;
    if ( op == Operation::negate ) { right = left; left = 0; }
    if ( same_operand && (op == Operation::sub || op == Operation::compare) )
        left = right = 0;
    if ( preserve_carry && old_carry ) flags.set(CF, *old_carry);
    if ( !left || !right || (with_carry && !old_carry) ) return std::nullopt;
    ...
    const bool subtract = op == Operation::sub || op == Operation::sbb
        || op == Operation::compare || op == Operation::decrement || op == Operation::negate;
    uint64_t result;
    bool cf, of;
    if ( subtract )
    {
    ...
    if ( !preserve_carry ) flags.set(CF, cf);
    flags.set(OF, of);
