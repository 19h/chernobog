...
inline std::optional<uint32_t> encode_b_cond(uint64_t source, uint64_t target, uint8_t condition)
{
    ...
    return 0x54000000U | ((immediate & 0x0007FFFFU) << 5) | static_cast<uint32_t>(condition);
}
