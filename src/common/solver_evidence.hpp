...
{
    if (++visits > 512 || depth > 64 || !expression.is_app())
        return false;
    if (!expression.is_bool() && (!expression.is_bv() || expression.get_sort().bv_size() > 64))
        return false;
    const auto symbol = expression.decl().name();
    if (symbol.kind() == Z3_STRING_SYMBOL &&
        !printable_name(prefix(Z3_get_symbol_string(expression.ctx(), symbol), 129)))
        return false;
    for (unsigned i = 0; i < expression.num_args(); ++i)
        if (!printable(expression.arg(i), visits, depth + 1))
            return false;
    return true;
}
...
inline void describe(z3::solver &solver, const z3::check_result &result, Row &row)
{
    ...
    if (bounded)
    {
        ...
        row["formula_status"] = "AST/sort/name budget rejected";
    row["model_interpretation"] =
        "symbolic assignment satisfying the recorded query; not a native input replay";
    ...
    for (unsigned i = 0; i < model.num_consts() && i < model_limit; ++i)
    {
        ...
        visits = 0;
        if (!printable(value, visits) ||
            (!value.is_numeral() && !value.is_true() && !value.is_false()))
        {
            complete = false;
            continue;
        }
        ...
        const std::string name = symbol.kind() == Z3_STRING_SYMBOL
                                     ? prefix(Z3_get_symbol_string(solver.ctx(), symbol), 129)
                                     : symbol.str();
        ...
        row[key + "_sort"] =
            value.is_bool() ? "Bool" : "BV" + std::to_string(value.get_sort().bv_size());
    ...
    try
    {
        if (collector && origin && collector->accepts(*origin))
        {
            Row row{
                ...
                {"applicability",
                 "recorded formula only; current IR and native-input applicability not revalidated"}};
            try
            {
                describe(solver, result, row);
            }
            catch (...)
            {
                row["capture_error"] = "formula/model serialization failed";
            }
            collector->publish(*origin, std::move(row));
        }
    }
    catch (...)
    { /* Inspection must not affect the analysis result. */
    }
    return result;
}
} // namespace chernobog::solver_evidence
