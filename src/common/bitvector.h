...
constexpr bool valid_byte_width(int bytes) { return bytes >= 1 && bytes <= 8; }
...
constexpr uint64_t truncate(uint64_t value, int bytes) { return value & mask(bytes); }

constexpr uint64_t logical_not(uint64_t value) { return value == 0 ? 1 : 0; }
...
{
    const unsigned bits = bit_width(bytes);
    return bits == 64 ? std::numeric_limits<int64_t>::min() : -(int64_t{1} << (bits - 1));
}
