...
enum class Operation : uint8_t { CONSTANT, INDEX, INPUT, CAST, ADD, SUB, XOR, ROL32, ROR32 };
struct Node
{
...
struct Program { std::vector<Node> nodes; size_t result = 0; };
struct Contract
{
    uint32_t key = 0;
    size_t units = 0;
...
inline uint64_t mask(uint8_t bits)
...
inline SymbolicValue resize(SymbolicValue value, uint8_t bits)
{
    if ( bits > value.bits && value.is_signed )
    {
        ...
        for ( uint8_t bit = value.bits; bit < bits; ++bit )
        {
            ...
            if ( sign ) value.constant |= uint64_t(1) << bit;
        }
    }
    for ( uint8_t bit = bits; bit < 64; ++bit ) value.coefficients[bit] = 0;
    value.constant &= mask(bits);
    value.bits = bits;
    return value;
}

inline std::optional<SymbolicValue> evaluate(const Program &program,
                                            uint64_t index, uint8_t input_bits)
{
    ...
    for ( size_t position = 0; position < program.nodes.size(); ++position )
    {
        ...
        if ( !supported_width(node.bits) ) return std::nullopt;
        SymbolicValue value;
        ...
        switch ( node.operation )
        {
            ...
            case Operation::INDEX:
                if ( node.step != 0 && index > (mask(node.bits) - (node.value & mask(node.bits))) / node.step )
                    return std::nullopt;
                value.constant = (node.value + index * node.step) & mask(node.bits);
                if ( node.is_signed && value.constant > (mask(node.bits) >> 1) ) return std::nullopt;
                break;
            case Operation::INPUT:
                if ( node.bits != input_bits ) return std::nullopt;
                for ( uint8_t bit = 0; bit < input_bits; ++bit )
                    value.coefficients[bit] = uint16_t(uint32_t(1) << bit);
                break;
            case Operation::CAST:
                if ( node.left >= position ) return std::nullopt;
            ...
            {
                if ( node.left >= position || node.right >= position ) return std::nullopt;
                ...
                {
                    if ( !left.is_constant() || !right.is_constant() ) return std::nullopt;
                    if ( node.operation == Operation::ADD || node.operation == Operation::SUB )
                        value.constant = (node.operation == Operation::ADD
                            ? left.constant + right.constant : left.constant - right.constant) & mask(node.bits);
                    else
                    {
                        if ( node.bits != 32 ) return std::nullopt;
                        const uint64_t count = node.operation == Operation::ROL32
                            ? right.constant : (32 - (right.constant & 31));
...
{
    if ( (contract.unit_bytes != 1 && contract.unit_bytes != 2)
      || contract.units == 0 || contract.units > maximum_bytes / contract.unit_bytes )
        return std::nullopt;
    ...
    for ( size_t index = 0; index < contract.units; ++index )
    {
        ...
        if ( output.constant != expected ) return std::nullopt;
        for ( uint8_t bit = 0; bit < bits; ++bit )
            if ( output.coefficients[bit] != uint16_t(uint32_t(1) << bit) ) return std::nullopt;
    }
...
inline std::optional<std::vector<uint8_t>> decode_bytes(
    const Program &program, const Contract &contract, const std::vector<uint8_t> &ciphertext)
{
    if ( !prove(program, contract)
      || ciphertext.size() != contract.units * contract.unit_bytes ) return std::nullopt;
...
inline std::optional<string_recovery::recovered_text_t> decode_text(
    const std::vector<uint8_t> &plaintext, string_recovery::text_encoding_t encoding)
{
    ...
    {
        valid = plaintext.size() > 1 && plaintext.back() == 0
    ...
    if ( !valid || text.characters < 4 ) return std::nullopt;
