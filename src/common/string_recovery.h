...
inline bool decode_utf8_payload(const std::vector<uint8_t> &bytes, size_t payload_bytes,
...
inline std::optional<recovered_text_t>
recover_runtime_utf8_prefix(const std::vector<uint8_t> &bytes, size_t minimum_characters = 4,
                            size_t maximum_payload_bytes = 4096)
{
    ...
    {
        if (length == maximum_payload_bytes || bytes[length] < 0x20 || bytes[length] == 0x7F)
            return std::nullopt;
        ++length;
    }
    if (length == bytes.size())
        return std::nullopt;
    recovered_text_t result;
    if (!decode_utf8_payload(bytes, length, &result) || result.characters < minimum_characters)
        return std::nullopt;
    result.explicitly_terminated = true;
    return result;
}

inline bool decode_wide_payload(const std::vector<uint8_t> &bytes, size_t payload_bytes,
                                size_t unit_width, bool big_endian, recovered_text_t *result)
{
    if (!result || (unit_width != 2 && unit_width != 4) || payload_bytes == 0 ||
        payload_bytes > bytes.size() || payload_bytes % unit_width != 0)
    {
        return false;
    }
    ...
    {
        uint32_t codepoint = decode_unit(bytes.data() + offset, unit_width, big_endian);
        if (unit_width == 2 && codepoint >= 0xD800 && codepoint <= 0xDBFF)
        {
            if (payload_bytes - offset < 4)
                return false;
            const uint32_t low = decode_unit(bytes.data() + offset + 2, 2, big_endian);
...
inline std::optional<recovered_text_t> recover_static_text(const std::vector<uint8_t> &bytes,
                                                           size_t write_width,
                                                           ...
                                                           bool allow_unterminated)
{
    ...
    if (decode_unit(bytes.data() + bytes.size() - write_width, write_width, target_big_endian) == 0)
    {
    ...
    if (!wide_terminated && !allow_unterminated)
        return std::nullopt;
    if (!decode_wide_payload(bytes, wide_payload, write_width, target_big_endian, &result))
    {
        return std::nullopt;
    }
...
inline std::string recover_hikari_xor_ascii(const std::vector<uint8_t> &encrypted_data,
                                            const std::vector<uint8_t> &xor_keys)
{
    ...
    for (size_t i = 0; i < length; ++i)
    {
        ...
        if (decrypted == 0)
            return result;
        if ((decrypted < 32 || decrypted >= 127) && decrypted != '\n' && decrypted != '\t' &&
            decrypted != '\r')
        {
            return {};
        }
