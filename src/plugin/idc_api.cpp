...
// idc_value_t helpers
//--------------------------------------------------------------------------
void make_object(idc_value_t *r) { idcv_object(r); }
...
void set_size(idc_value_t *obj, const char *key, size_t value) { set_num(obj, key, sval_t(value)); }

void set_bool(idc_value_t *obj, const char *key, bool value) { set_num(obj, key, value ? 1 : 0); }
...
// Argument helpers
//--------------------------------------------------------------------------
ea_t arg_ea(const idc_value_t &value) { return ea_t(value.num); }
...
const option_alias_t option_aliases[] = {
    ...
    {"verbose", "CHERNOBOG_VERBOSE", false, "Verbose logging"},
    {"debug", "CHERNOBOG_DEBUG", false, "File-based debug logging (macOS/Linux)"},
    {"reset", "CHERNOBOG_RESET", false, "Clear the decompiler cache on activation"},
    ...
    {"ida_analysis", "CHERNOBOG_IDA_ANALYSIS", false, "Native IDA analysis enrichment"},
    ...
    {"mba_affine", "CHERNOBOG_MBA_AFFINE", false, "Exact-Z3 affine MBA reconstruction"},
    {"vm", "CHERNOBOG_VM", false, "VM-family detection and rewriting"},
    {"vm_z3", "CHERNOBOG_VM_Z3", false, "Exact-Z3 VM residual simplification"},
    {"vm_carrier_pool", "CHERNOBOG_VM_CARRIER_POOL", false, "VM carrier constants"},
    {"rax", "CHERNOBOG_RAX_ENABLED", false, "rax exploratory emulation"},
    {"rax_runs", "CHERNOBOG_RAX_EXPLORE_RUNS", false, "Concrete runs per exploration (1..32)"},
    {"rax_max_insns", "CHERNOBOG_RAX_MAX_INSNS", false, "Instruction budget per concrete run"},
    {"rax_timeout_ms", "CHERNOBOG_RAX_TIMEOUT_MS", false, "Wall-clock budget per concrete run"},
    {"rax_log_level", "CHERNOBOG_RAX_LOG_LEVEL", false, "0 quiet, 1 summary, 2 trace"},
    {"rax_poll_ms", "CHERNOBOG_RAX_POLL_MS", false, "Completion polling interval"},
    {"rax_static", "CHERNOBOG_RAX_STATIC", false, "Static decode observations"},
    {"rax_smir", "CHERNOBOG_RAX_SMIR", false, "SMIR effect observations"},
...
bool is_chernobog_variable(const char *name)
{
    ...
    return name != nullptr && strncmp(name, prefix, sizeof(prefix) - 1) == 0 &&
...
bool ends_with_address_suffix(const char *name)
{
    ...
    return length >= sizeof(suffix) - 1 && strieq(name + length - (sizeof(suffix) - 1), suffix);
}
...
{
    qstring value;
    return qgetenv(variable, &value) && !value.empty() && value != "0" &&
           !strieq(value.c_str(), "false") && !strieq(value.c_str(), "no") &&
           !strieq(value.c_str(), "off");
}
...
std::unique_ptr<mba_t> generate_microcode(ea_t function_ea, mba_maturity_t maturity, qstring *error)
{
    hexrays_failure_t failure;
    std::unique_ptr<mba_t> mba(
        gen_microcode(decomp_ranges_t(function_ea), &failure, nullptr, DECOMP_NO_CACHE, maturity));
...
bool decompile_text(ea_t function_ea, bool no_cache, qstring *out)
{
    ...
    if (function == nullptr)
        return false;
    hexrays_failure_t failure;
    cfuncptr_t cfunc = decompile(function, &failure, no_cache ? DECOMP_NO_CACHE : 0);
    if (cfunc == nullptr)
    {
...
{
    qstring text;
    text.sprnt("chernobog %s%s (source %s, IDA SDK %s, rax %.12s)", build_provenance::revision,
               streq(build_provenance::dirty, "1") ? "-dirty" : "",
               build_provenance::source_fingerprint, build_provenance::ida_sdk,
...
error_t idaapi idc_status(idc_value_t *, idc_value_t *r)
{
    ...
    set_bool(r, "disabled", host != nullptr && host->transformations_disabled());
...
error_t idaapi idc_detect(idc_value_t *argv, idc_value_t *r)
{
    ...
    qstring error;
    const std::unique_ptr<mba_t> mba = generate_microcode(function_ea, MMAT_LOCOPT, &error);
    if (mba == nullptr)
    {
        fill_obfuscation_object(r, function_ea, OBF_NONE);
        set_bool(r, "ok", false);
...
//--------------------------------------------------------------------------
void fill_flatten_object(idc_value_t *r, ea_t function_ea, bool ok, bool detected,
                         const pattern_match::flatten_info_t &info, const char *error)
{
    make_object(r);
    set_u64(r, "ea", uint64_t(function_ea));
    set_bool(r, "ok", ok);
    ...
    set_size(r, "direct_return_target_count", info.direct_return_target_count);
...
error_t idaapi idc_detect_flatten(idc_value_t *argv, idc_value_t *r)
{
    ...
    if (mba == nullptr)
    {
        fill_flatten_object(r, function_ea, false, false, empty, error.c_str());
        return eOk;
    }
...
void deobfuscate_range(ea_t start, ea_t end, idc_value_t *r)
{
    ...
    msg("[chernobog][idc] Deobfuscated %zu of %zu functions in %a..%a%s\n", processed, considered,
        start, end, cancelled != 0 ? " (cancelled)" : "");
}
...
error_t idaapi idc_enabled_for(idc_value_t *argv, idc_value_t *r)
{
    const ea_t function_ea = resolve_function(argv[0]);
    r->set_long(
        function_ea != BADADDR && chernobog_function_deobfuscation_enabled(function_ea) ? 1 : 0);
    return eOk;
}
...
error_t idaapi idc_pending(idc_value_t *argv, idc_value_t *r)
{
    const ea_t function_ea = resolve_function(argv[0]);
    r->set_long(
        function_ea != BADADDR && chernobog_function_requires_deobfuscation(function_ea) ? 1 : 0);
    return eOk;
}
...
error_t idaapi idc_clear_function(idc_value_t *argv, idc_value_t *r)
{
    ...
    chernobog_clear_function_tracking(function_ea);
    hybrid::Session *session = current_host() != nullptr ? current_host()->rax_session() : nullptr;
    if (session != nullptr)
...
error_t idaapi idc_hikari_cfg(idc_value_t *, idc_value_t *r)
{
    ...
    set_num(r, "terminal_indirect_branches", sval_t(stats.terminal_indirect_branches));
...
error_t idaapi idc_early_stats(idc_value_t *, idc_value_t *r)
{
    ...
    const ida_analysis::EarlyHexRaysStats &stats = early != nullptr ? early->stats() : fallback;
...
const char *ensure_result_name(hybrid::EnsureExploredResult result)
{
    switch (result)
    {
    case hybrid::EnsureExploredResult::ALREADY_FRESH:
        return "already_fresh";
    case hybrid::EnsureExploredResult::EXPLORED:
        return "explored";
    case hybrid::EnsureExploredResult::DISABLED:
        return "disabled";
    case hybrid::EnsureExploredResult::UNAVAILABLE:
        return "unavailable";
    case hybrid::EnsureExploredResult::CANCELLED:
        return "cancelled";
    case hybrid::EnsureExploredResult::FAILED:
        return "failed";
    }
    return "unknown";
}
...
const char *arch_name(hybrid::HybridArch value)
{
    switch (value)
    {
    case hybrid::HybridArch::UNSUPPORTED:
        return "unsupported";
    case hybrid::HybridArch::X86_16:
        return "x86_16";
    case hybrid::HybridArch::X86_32:
        return "x86_32";
    case hybrid::HybridArch::X86_64:
        return "x86_64";
    case hybrid::HybridArch::ARM64:
        return "arm64";
    case hybrid::HybridArch::ARM32:
        return "arm32";
    case hybrid::HybridArch::RISCV64:
        return "riscv64";
    case hybrid::HybridArch::CORTEX_M:
        return "cortex_m";
    case hybrid::HybridArch::HEXAGON:
        return "hexagon";
    }
    return "unknown";
}
...
const char *edge_kind_name(hybrid::ExecEdge::Kind kind)
{
    switch (kind)
    {
    case hybrid::ExecEdge::Kind::Unknown:
        return "unknown";
    case hybrid::ExecEdge::Kind::Call:
        return "call";
    case hybrid::ExecEdge::Kind::Jump:
        return "jump";
    case hybrid::ExecEdge::Kind::Return:
        return "return";
    }
    return "unknown";
}
...
{
    const ea_t function_ea = resolve_function(argv[0]);
    hybrid::Session *session = ready_host() != nullptr ? current_session() : nullptr;
    if (session == nullptr || function_ea == BADADDR)
    {
        r->set_long(sval_t(hybrid::EnsureExploredResult::UNAVAILABLE));
        return eOk;
    }
    const hybrid::EnsureExploredResult result = session->ensure_explored(uint64_t(function_ea));
...
{
    r->set_string(ensure_result_name(hybrid::EnsureExploredResult(uint8_t(argv[0].num))));
    return eOk;
}
...
error_t idaapi idc_evidence_view(idc_value_t *argv, idc_value_t *r)
{
    ...
    const auto json = hybrid::evidence_view_json(
        *source, view, revision != 0 && hybrid::hybrid_evidence_snapshot_is_fresh(source),
        int64_t(get_dbctx_id()), revision);
...
{
    const auto json =
        vm::trace_native_region(uint64_t(resolve_function(argv[0])), uint64_t(argv[1].num));
    r->set_string(json.c_str());
    return eOk;
}
...
error_t idaapi idc_vm_trace_candidate_shadow(idc_value_t *argv, idc_value_t *r)
{
    const auto path = arg_string(argv[2]);
    const auto json = vm::trace_native_candidate_shadow(
...
error_t idaapi idc_vm_trace_candidate_shadow_replay(idc_value_t *argv, idc_value_t *r)
{
    const auto request = arg_string(argv[2]);
    const auto json =
        vm::trace_native_candidate_shadow_replay(uint64_t(arg_ea(argv[0])), uint64_t(argv[1].num),
                                                 std::string(request.c_str(), request.length()));
    r->set_string(json.c_str());
    return eOk;
}
...
{
    const auto request = arg_string(argv[2]);
    const auto json = vm::trace_native_candidate_shadow_replay_memory(
...
error_t idaapi idc_vm_trace_input(idc_value_t *argv, idc_value_t *r)
{
    const auto request = arg_string(argv[2]);
    const auto json =
        vm::trace_native_region_input(uint64_t(resolve_function(argv[0])), uint64_t(argv[1].num),
                                      std::string(request.c_str(), request.length()));
    r->set_string(json.c_str());
    return eOk;
}
...
error_t idaapi idc_vm_trace_walk(idc_value_t *argv, idc_value_t *r)
{
    const auto request = arg_string(argv[2]);
    const auto json =
        vm::trace_native_region_walk(uint64_t(resolve_function(argv[0])), uint64_t(argv[1].num),
                                     std::string(request.c_str(), request.length()));
    r->set_string(json.c_str());
    return eOk;
}
...
error_t idaapi idc_vm_trace_check(idc_value_t *argv, idc_value_t *r)
{
    const auto request = arg_string(argv[2]);
    const auto json =
        vm::trace_native_region_check(uint64_t(resolve_function(argv[0])), uint64_t(argv[1].num),
                                      std::string(request.c_str(), request.length()));
    r->set_string(json.c_str());
    return eOk;
}
...
error_t idaapi idc_vm_trace_temporal(idc_value_t *argv, idc_value_t *r)
{
    ...
    const auto json =
        vm::trace_native_region_temporal(uint64_t(resolve_function(argv[0])), uint64_t(argv[1].num),
                                         std::string(request.c_str(), request.length()),
...
error_t idc_vm_observations(idc_value_t *argv, idc_value_t *r, bool validate)
{
    ...
    {
        r->set_string(
            "{\"schema\":1,\"available\":false,\"states_available\":false,\"states\":[],\"states_reason\":\"no capture for selected function\"}");
        return eOk;
    }
    const auto revision = hybrid::hybrid_evidence_snapshot_revision(source);
    const auto json = vm::inspect_regions(
        function, false, source.get(), revision,
...
error_t idaapi idc_native_evidence(idc_value_t *argv, idc_value_t *r)
{
    ...
    const auto view = engine && function != BADADDR ? engine->inspect(uint64_t(function))
                                                    : ida_analysis::NativeInspection{};
...
error_t idaapi idc_evidence_state(idc_value_t *argv, idc_value_t *r)
{
    ...
    const bool available =
        source && function_ea != BADADDR && source->scope.function_start == uint64_t(function_ea);
    const auto revision = available ? hybrid::hybrid_evidence_snapshot_revision(source) : 0;
    std::ostringstream out;
    out << "{\"database\":\"" << int64_t(get_dbctx_id())
        << "\",\"available\":" << (available ? "true" : "false") << ",\"fresh\":"
        << (available && revision != 0 && hybrid::hybrid_evidence_snapshot_is_fresh(source)
                ? "true"
                : "false")
...
error_t idaapi idc_rax_summary(idc_value_t *argv, idc_value_t *r)
{
    ...
    set_bool(r, "fresh",
             matches && hybrid::hybrid_current_evidence_is_fresh(uint64_t(function_ea)));
    ...
    set_size(r, "explicit_arguments", data.function_profile.explicit_arguments);
    set_bool(r, "explicit_arguments_known", data.function_profile.explicit_arguments_known);
    ...
    set_u64(r, "context_identity_bytes", data.summary.context_identity_bytes);
...
error_t idaapi idc_rax_string(idc_value_t *argv, idc_value_t *r)
{
    ...
    const hybrid::RuntimeStringCandidate &candidate = valid ? candidates[size_t(index)] : fallback;
...
error_t idaapi idc_rax_use_string_count(idc_value_t *argv, idc_value_t *r)
{
    ...
    r->set_long(function_ea == BADADDR
                    ? 0
                    : sval_t(hybrid::hybrid_current_use_strings(uint64_t(function_ea)).size()));
    return eOk;
}
...
error_t idaapi idc_rax_use_string(idc_value_t *argv, idc_value_t *r)
{
    ...
    set_size(r, "read_count",
             candidate.read_fragments.empty() ? 1 : candidate.read_fragments.front().size());
...
//--------------------------------------------------------------------------
std::vector<hybrid::HybridObservedTargetCandidate> indirect_targets(ea_t function_ea,
                                                                    ea_t instruction)
{
    if (function_ea == BADADDR || instruction == BADADDR)
        return {};
    return hybrid::hybrid_current_indirect_target_candidates(uint64_t(function_ea),
                                                             uint64_t(instruction));
}

error_t idaapi idc_rax_target_count(idc_value_t *argv, idc_value_t *r)
{
    r->set_long(sval_t(indirect_targets(resolve_function(argv[0]), arg_ea(argv[1])).size()));
    return eOk;
}
...
error_t idaapi idc_rax_branch(idc_value_t *argv, idc_value_t *r)
{
    ...
    const hybrid::HybridBranchCheck check =
        addressable ? hybrid::hybrid_check_current_branch_claim(
    ...
    set_str(r, "verdict_name", hybrid::hybrid_branch_verdict_name(check.claim.verdict));
    set_size(r, "matching", check.claim.matching);
    set_size(r, "opposing", check.claim.opposing);
    set_size(r, "opposing_context_complete", check.claim.opposing_context_complete);
...
error_t idaapi idc_rule_name(idc_value_t *argv, idc_value_t *r)
{
    ...
    r->set_string(index >= 0 && size_t(index) < names.size() ? names[size_t(index)].c_str() : "");
    return eOk;
}
...
const idc_entry_t idc_entries[] = {
    ...
    {"chernobog_help", idc_help, args_none, "chernobog_help()", "This table as text"},
    {"chernobog_version", idc_version, args_none, "chernobog_version()",
     "Build revision, source fingerprint, SDK, and pinned rax revision"},
    {"chernobog_status", idc_status, args_none, "chernobog_status()",
     "Lifecycle, mode, rax, rule, and architecture state"},
    {"chernobog_activate", idc_activate, args_none, "chernobog_activate()",
     "Install the Hex-Rays lifecycle now; 1 when ready"},

    // Options
    {"chernobog_get_option", idc_get_option, args_str, "chernobog_get_option(name)",
     "Current value of an option alias or CHERNOBOG_ variable"},
    {"chernobog_set_option", idc_set_option, args_str_value, "chernobog_set_option(name, value)",
     "Set an option for this process; 1 on success"},
    {"chernobog_list_options", idc_list_options, args_none, "chernobog_list_options()",
     "Aliases, variables, current values, and descriptions"},

    // Analysis
    {"chernobog_detect", idc_detect, args_ea, "chernobog_detect(ea)",
     "Obfuscation candidates from uncached LOCOPT microcode"},
    {"chernobog_obf_names", idc_obf_names, args_long, "chernobog_obf_names(mask)",
     "Comma-separated names for an obfuscation mask"},
    {"chernobog_analyze", idc_analyze, args_ea, "chernobog_analyze(ea)",
     "Print the Ctrl+Shift+A candidate report"},
    {"chernobog_detect_flatten", idc_detect_flatten, args_ea, "chernobog_detect_flatten(ea)",
     "Control-flow flattening detector evidence"},

    // Transformation
    {"chernobog_deobfuscate", idc_deobfuscate, args_ea, "chernobog_deobfuscate(ea)",
     "Run the Ctrl+Shift+D action on one function"},
    {"chernobog_deobfuscate_text", idc_deobfuscate_text, args_ea, "chernobog_deobfuscate_text(ea)",
     "Admit, re-decompile uncached, and return the pseudocode"},
    {"chernobog_decompile", idc_decompile, args_ea, "chernobog_decompile(ea)",
    ...
    {"chernobog_deobfuscate_all", idc_deobfuscate_all, args_none, "chernobog_deobfuscate_all()",
     "Deobfuscate every function in the database"},

    // Admission and tracking
    {"chernobog_request", idc_request, args_ea, "chernobog_request(ea)",
     "Admit a function for the next decompilation"},
    {"chernobog_enabled_for", idc_enabled_for, args_ea, "chernobog_enabled_for(ea)",
     "1 when auto mode or an explicit request admits this function"},
    {"chernobog_pending", idc_pending, args_ea, "chernobog_pending(ea)",
     "1 when the LOCOPT pipeline has not completed for this function"},
    {"chernobog_clear_function", idc_clear_function, args_ea, "chernobog_clear_function(ea)",
     "Drop per-function tracking and rax evidence"},
    {"chernobog_clear_all", idc_clear_all, args_none, "chernobog_clear_all()",
     "Drop every cache and tracking set for this database"},
    {"chernobog_clear_cache", idc_clear_cache, args_none, "chernobog_clear_cache()",
     "Clear the Hex-Rays decompiler cache"},

    // Native, pre-Hex-Rays passes
    {"chernobog_hikari_cfg", idc_hikari_cfg, args_none, "chernobog_hikari_cfg()",
     "Run Hikari ARM64 dispatch recovery and report its statistics"},
    {"chernobog_native_opaque", idc_native_opaque, args_none, "chernobog_native_opaque()",
    ...
    // rax exploratory emulation
    {"chernobog_rax_explore", idc_rax_explore, args_ea, "chernobog_rax_explore(ea)",
     "Bounded synchronous exploration of one function"},
    {"chernobog_rax_result_name", idc_rax_result_name, args_long, "chernobog_rax_result_name(code)",
     "Name for a chernobog_rax_explore() result code"},
    {"chernobog_rax_fresh", idc_rax_fresh, args_ea, "chernobog_rax_fresh(ea)",
     "1 when exact evidence for this function is still current"},
    {"chernobog_rax_show", idc_rax_show, args_none, "chernobog_rax_show()",
     "Print the last evidence report"},
    {"chernobog_evidence_view", idc_evidence_view, args_ea, "chernobog_evidence_view(ea)",
     "Bounded JSON inspection snapshot, retaining explicitly stale observations"},
    {"chernobog_evidence_state", idc_evidence_state, args_ea, "chernobog_evidence_state(ea)",
    ...
    {"chernobog_vm_summaries", idc_vm_summaries, args_ea, "chernobog_vm_summaries(ea)",
    ...
    {"chernobog_vm_trace_candidate_shadow_replay_memory",
     ...
     "chernobog_vm_trace_candidate_shadow_replay_memory(data_head_ea, seed, entry_json)",
     "Read-only bounded x86-64 replay with caller-supplied executable shadow, writable data and stack windows; no function evidence or VM identity claim"},
    ...
    {"chernobog_vm_transitions", idc_vm_transitions, args_ea, "chernobog_vm_transitions(ea)",
     "Explicit bounded local-model checks against complete captured transitions"},
    {"chernobog_solver_state", idc_solver_state, args_ea, "chernobog_solver_state(ea)",
     "Query identity and source-navigation state without formula/model copying"},
    {"chernobog_rax_cancel", idc_rax_cancel, args_none, "chernobog_rax_cancel()",
     "Stop queued runs at the next instruction boundary"},
    {"chernobog_rax_clear", idc_rax_clear, args_none, "chernobog_rax_clear()",
     "Discard the session and its published evidence"},
    {"chernobog_rax_summary", idc_rax_summary, args_ea, "chernobog_rax_summary(ea)",
     "Scope, provenance, and every evidence counter"},
    {"chernobog_rax_string_count", idc_rax_string_count, args_ea, "chernobog_rax_string_count(ea)",
     "Number of consensus runtime string witnesses"},
    {"chernobog_rax_string", idc_rax_string, args_ea_long, "chernobog_rax_string(ea, index)",
    ...
    // MBA rule registry
    {"chernobog_rule_stats", idc_rule_stats, args_none, "chernobog_rule_stats()",
     "Registry size, verification outcome, and match counters"},
    {"chernobog_rule_count", idc_rule_count, args_none, "chernobog_rule_count()",
...
size_t function_count() { return registered_functions; }
