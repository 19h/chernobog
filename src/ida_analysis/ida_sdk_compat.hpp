...
inline int effective_address_size_compat() { return int(inf_get_app_bitness() / 8); }
...
inline bool reg_value_address_compat(const reg_value_info_t &value, ea_t *address)
{
    if (address == nullptr)
        return false;
...
inline bool find_register_value_info_compat(reg_value_info_t *value, ea_t address,
                                            const char *register_name, int processor_register,
                                            int maximum_depth)
{
#if IDA_SDK_VERSION >= 940
    if (register_name != nullptr && register_name[0] != '\0')
    {
        return find_regname_value_info(value, address, register_name, maximum_depth);
    }
#else
    if (processor_register < 0 && register_name != nullptr && register_name[0] != '\0')
    {
        processor_register = str2reg(register_name);
    }
#endif
    return processor_register >= 0 &&
           find_reg_value_info(value, address, processor_register, maximum_depth);
}
