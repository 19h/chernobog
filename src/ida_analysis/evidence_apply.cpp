...
bool nonexecutable_data(ea_t address)
{
    ...
    return segment != nullptr && (segment->perm == 0 || (segment->perm & SEGPERM_EXEC) == 0);
}
...
{
    xrefblk_t xref;
    for (bool ok = xref.first_from(from, XREF_DATA); ok; ok = xref.next_from())
    {
        if (xref.to == to)
            return true;
    }
...
bool add_code_reference(ea_t from, ea_t to, bool call, const EvidenceApplyConfig &config,
...
bool range_is_undefined_data(ea_t address, size_t size, bool allow_dummy_names = false)
{
    ...
    if (size == 0 || !nonexecutable_data(address) || segment == nullptr ||
        address >= segment->end_ea || size > size_t(segment->end_ea - address))
        return false;
    for (size_t offset = 0; offset < size; ++offset)
    {
        const ea_t current = address + ea_t(offset);
        const flags64_t flags = get_flags(current);
        if (getseg(current) != segment || !is_mapped(current) || !is_unknown(flags) ||
            has_protected_metadata(flags, allow_dummy_names))
        {
            return false;
        }
    }
...
bool create_scalar(ea_t address, uint32_t size)
{
    ...
    {
    case 1:
        return create_byte(address, 1);
    case 2:
        return create_word(address, 2);
    case 4:
        return create_dword(address, 4);
    case 8:
        return create_qword(address, 8);
    case 16:
        return create_oword(address, 16);
    default:
        return false;
    }
}
...
{
    const segment_t *segment = getseg(address);
    if (segment == nullptr || !nonexecutable_data(address) || address >= segment->end_ea)
    {
        return 0;
    }
    ...
    for (; length < maximum; ++length)
    {
        ...
        const flags64_t flags = get_flags(current);
        if (getseg(current) != segment || !is_mapped(current) || !is_loaded(current) ||
...
bool idb_matches_string(ea_t address, const std::string &value)
{
    ...
    return is_loaded(address + ea_t(value.size())) && get_byte(address + ea_t(value.size())) == 0;
}

void apply_static_targets(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                          EvidenceApplyStats &stats)
{
    for (const StaticInstructionEvidence &item : evidence.static_analysis.instructions)
    {
        ...
        if (target.kind != DecoderTargetKind::Call && target.kind != DecoderTargetKind::Jump)
        {
            continue;
        }
        add_code_reference(ea_t(item.address), ea_t(target.address), call, config, stats, false);
    }
}
...
struct EdgeKey
{
    ...
    {
        return std::tie(from, to, call) < std::tie(other.from, other.to, other.call);
    }
};

std::map<EdgeKey, std::set<RunKey>> corroborated_edges(const TargetEvidence &evidence)
{
    std::map<EdgeKey, std::set<RunKey>> result;
    for (const auto &observation : evidence.indirect_targets)
    {
        if (observation.kind != ExecEdge::Kind::Call && observation.kind != ExecEdge::Kind::Jump)
        {
            continue;
        }
        EdgeKey key;
        ...
        result[key].insert({observation.provenance.run_id, observation.provenance.seed});
    }
    return result;
}

void recover_function(ea_t target, const EvidenceApplyConfig &config, EvidenceApplyStats &stats)
{
...
void apply_dynamic_targets(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                           EvidenceApplyStats &stats)
{
    for (const auto &entry : corroborated_edges(evidence))
    {
        ...
        const bool added =
            add_code_reference(ea_t(edge.from), ea_t(edge.to), edge.call, config, stats, true);
        if (edge.call)
            recover_function(ea_t(edge.to), config, stats);
        if (config.comments && (added || cref_exists(ea_t(edge.from), ea_t(edge.to))))
        {
            qstring text;
            text.sprnt("corroborated %s target %s (%zu runs)", edge.call ? "call" : "jump",
...
struct DataKey
{
    ...
    bool operator<(const DataKey &other) const
    {
        return std::tie(from, address, value, size, kind) <
               std::tie(other.from, other.address, other.value, other.size, other.kind);
    }
};
...
struct DrefKey
{
    ...
    {
        return std::tie(from, address, kind) < std::tie(other.from, other.address, other.kind);
    }
};

void apply_data_evidence(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                         EvidenceApplyStats &stats)
{
    ...
    for (const DataAcc &access : evidence.events.data)
    {
        ...
        observations[{access.from, access.addr, access.value, access.size, access.kind}].insert(
            run);
        reference_observations[{access.from, access.addr, access.kind}].insert(run);
    }
    ...
    for (const auto &entry : observations)
    {
        ...
        if (!source_is_instruction(from) || !is_mapped(address) || is_code(get_flags(address)))
        {
            continue;
        }
        if (classified_data.count(address) != 0 || !range_is_undefined_data(address, key.size))
        {
            continue;
        }

        bool classified = false;
        if (config.pointer_offsets && key.kind == RAX_MEM_READ && key.size == pointer_size &&
            is_loaded(address) && is_mapped(ea_t(key.value)))
        {
            const ea_t stored =
                pointer_size == 8 ? ea_t(get_qword(address)) : ea_t(get_dword(address));
            if (stored == ea_t(key.value) && stored != address && is_mapped(stored) &&
                create_pointer_offset(address, pointer_size))
            {
                ...
                {
                    qstring text;
                    text.sprnt("pointer slot %s -> %s (%zu runs)", address_label(address).c_str(),
...
void apply_consensus_runtime_strings(const TargetEvidence &evidence,
                                     const EvidenceApplyConfig &config, EvidenceApplyStats &stats)
{
    if (!config.strings)
        return;
    int utf8_encoding = -1;
...
void apply_stack_purge(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                       EvidenceApplyStats &stats)
{
    ...
    if (function == nullptr || function->start_ea != ea_t(evidence.scope.function_start) ||
...
void apply_no_return(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                     EvidenceApplyStats &stats)
{
    ...
    for (const auto &run : evidence.runs)
    {
        if (!run.ran)
            continue;
        ++completed;
        if (run.outcome.returned || !run.outcome.conclusive() || run.outcome.permission_violation ||
            run.outcome.escaped_image || run.outcome.environment_model_failure)
        {
            return;
        }
...
void apply_argument_registers(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                              EvidenceApplyStats &stats)
{
    ...
    { return std::find(arguments.begin(), arguments.end(), reg) != arguments.end(); };
    ...
    ea_t address = entry;
    for (size_t step = 0; step < 64 && address != BADADDR && get_func(address) == function; ++step)
    {
        ...
        for (int index = 0; index < UA_MAXOP; ++index)
        {
            ...
            if (operand.type == o_reg)
            {
                ...
                if (has_cf_use(features, index) && written.count(reg) == 0 && is_argument(reg) &&
        ...
        if ((features & CF_STOP) != 0 && !is_ret_insn(instruction))
        {
            ...
            size_t successors = 0;
            xrefblk_t xref;
            for (bool ok = xref.first_from(address, XREF_CODE | XREF_NOFLOW); ok;
                 ok = xref.next_from())
            {
...
void apply_opaque_observations(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                               EvidenceApplyStats &stats)
{
    ...
    for (const auto &branch : evidence.branches)
    {
        ...
        if (branch.disposition == BranchDisposition::TAKEN)
            ...
            site.fallthrough.insert({branch.provenance.run_id, branch.provenance.seed});
        else
            site.incomplete = true;
    }
    for (const auto &entry : sites)
    {
        ...
        if (site.incomplete || site.taken.empty() == site.fallthrough.empty())
            continue;
        const size_t runs = site.taken.empty() ? site.fallthrough.size() : site.taken.size();
...
void apply_switches(const TargetEvidence &evidence, const EvidenceApplyConfig &config,
                    EvidenceApplyStats &stats)
{
    ...
    for (const auto &entry : corroborated_edges(evidence))
    {
        if (entry.first.call || entry.second.size() < config.minimum_dynamic_runs)
        {
            continue;
        }
    ...
    for (auto &entry : groups)
    {
        ...
        if (targets.size() < 2)
            continue;
        insn_t instruction;
        if (decode_insn(&instruction, jump) <= 0 || !is_indirect_jump_insn(instruction))
        {
            continue;
        }
...
EvidenceApplyStats apply_evidence_to_ida(const TargetEvidence &evidence)
{
    ...
    if (stats.total() != 0)
    {
        msg("[chernobog][rax-analysis] applied to %a: static-crefs=%zu "
            ...
            ea_t(evidence.scope.function_start), stats.static_crefs, stats.dynamic_crefs,
            stats.drefs, stats.code_items, stats.pointer_offsets, stats.typed_globals,
            stats.strings, stats.functions, stats.purges,
            stats.noret_annotations + stats.argument_annotations + stats.opaque_annotations,
            stats.switches);
    }
    return stats;
}
