...
bool is_x86_database() { return PH.id == PLFM_386; }
...
bool executable_code(ea_t address)
{
    ...
    return address != BADADDR &&
           ...
           && (segment->perm == 0 || (segment->perm & SEGPERM_EXEC) != 0) && is_mapped(address) &&
...
{
    insn_t instruction;
    if (decode_insn(&instruction, address) <= 0 || instruction.itype != NN_call ||
        instruction.Op1.type != o_near)
    {
        return false;
    }
    xrefblk_t xref;
    for (bool ok = xref.first_from(address, XREF_FLOW); ok; ok = xref.next_from())
    {
...
std::optional<classifier::get_pc_candidate_t> resolve_gadget_return_summary(ea_t return_ea,
                                                                            int scan_depth)
{
    ...
    for (int index = 0; index < scan_depth; ++index)
    {
        ...
        size_t references = 0;
        xrefblk_t xref;
        for (bool ok = xref.first_to(previous, XREF_FLOW); ok; ok = xref.next_to())
        {
            if (++references > 64)
                return std::nullopt;
            if ((int(xref.type) & XREF_MASK) != fl_JN)
                continue;
            insn_t call;
            if (decode_insn(&call, xref.from) <= 0 || call.itype != NN_call ||
                call.Op1.type != o_near || call.Op1.addr != previous ||
                !func_contains(function, call.ea))
            {
                continue;
            }
            const auto candidate = classify_ida_get_pc_call(call, size_t(scan_depth), true);
...
template <class Flowchart> std::map<ea_t, int> index_blocks_by_start(const Flowchart &flowchart)
{
...
int block_containing(const Flowchart &flowchart, const std::map<ea_t, int> &blocks_by_start,
                     ea_t address)
{
...
void propagate_reachability(const Flowchart &flowchart, bitset_t &reachable)
{
    ...
    for (size_t cursor = 0; cursor < worklist.size(); ++cursor)
    {
        ...
        {
            if (successor >= 0 && successor < int(flowchart.blocks.size()) &&
...
template <class Flowchart>
int repair_flowchart(Flowchart &flowchart, bitset_t *reachable, const EarlyHexRaysConfig &config,
                     bool &bounded)
{
    bounded = false;
    if (!is_x86_database() || flowchart.blocks.size() > config.maximum_blocks)
    {
    ...
    size_t scanned = 0;
    int changes = 0;
    const std::map<ea_t, int> blocks_by_start = index_blocks_by_start(flowchart);
    for (size_t index = 0; index < flowchart.blocks.size(); ++index)
    {
        qbasic_block_t &block = flowchart.blocks[index];
        for (ea_t address = block.start_ea; address != BADADDR && address < block.end_ea;)
        {
            ...
            insn_t call;
            if (is_marked_get_pc_call(address) && decode_insn(&call, address) > 0)
            {
                const auto gadget =
                    classify_ida_get_pc_call(call, size_t(config.gadget_scan_depth), true);
                    ...
                    gadget->return_instruction != classifier::k_bad_address)
                {
                    ...
                    const int return_block = block_containing(flowchart, blocks_by_start,
                                                              ea_t(gadget->return_instruction));
...
bool mba_within_bounds(const mba_t &mba, const EarlyHexRaysConfig &config)
{
    ...
    for (int index = 0; index < mba.qty; ++index)
    {
        ...
        for (const minsn_t *instruction = block->head; instruction != nullptr;
...
{
    for (auto iterator = predecessors.begin(); iterator != predecessors.end();)
    {
        if (*iterator == serial)
...
int convert_resolved_returns(mba_t &mba, const EarlyHexRaysConfig &config)
{
    ...
    for (int index = 0; index < mba.qty; ++index)
    {
        ...
        const ea_t target = resolve_gadget_return(block->tail->ea, config.gadget_scan_depth);
        if (target == BADADDR)
            continue;
        ...
        if (target < target_range->start || target >= target_range->end || target_block == index)
        {
            continue;
        }
...
bool evaluate_forward_operand(const ForwardState &state, const minsn_t *current,
                              const mop_t &operand, ConstantValue &value, int depth = 0);

ForwardEffect analyze_forward_effect(const mblock_t &block, const minsn_t &instruction,
                                     const ForwardState &state);

void apply_forward_effect(ForwardState &state, const ForwardEffect &effect);
...
ea_t instruction_write_target(ea_t instruction_ea)
{
    ea_t result = BADADDR;
    xrefblk_t xref;
    for (bool ok = xref.first_from(instruction_ea, XREF_DATA); ok; ok = xref.next_from())
    {
...
bool character_byte(uint8 value) { return value == 0 || (value >= 0x20 && value <= 0x7E); }
...
{
    ea_t last = BADADDR;
    if (!add_signed_delta(base, size - 1, last) || !is_mapped(base) || !is_mapped(last))
    {
        return false;
    }
...
void collect_character_store(qvector<CharacterCandidate> &candidates, const minsn_t &instruction,
                             const ForwardState &state)
{
    const int size = instruction.l.size;
    if (instruction.opcode != m_stx || (size != 1 && size != 2 && size != 4 && size != 8))
    {
        return;
    }

    ConstantValue source;
    if (!evaluate_forward_operand(state, &instruction, instruction.l, source) ||
...
int mark_character_runs(mba_t &mba, qvector<CharacterCandidate> &candidates)
{
    if (candidates.empty())
        return 0;
    ...
    for (size_t index = 0; index < candidates.size();)
    {
        size_t end = index + 1;
        bool conflict = false;
        while (end < candidates.size() && candidates[end].target == candidates[index].target)
        {
        ...
        if (!conflict)
        {
            ...
            for (size_t candidate = index; candidate < end; ++candidate)
            {
                ...
                {
                    keys.emplace(candidates[candidate].operand_ea,
    ...
    {
        size_t end = begin;
        while (end + 1 < unique.size() && unique[end + 1].target == unique[end].target + 1)
        {
            ++end;
        }
        ...
        if (!interior_nul && (length >= 4 || (nul_terminated && length >= 2)))
        {
            ...
            for (size_t index = begin; index <= end; ++index)
            {
                ...
                for (const auto &key_pair : found_keys->second)
                {
                    ...
                    if (user_numforms_find(formats, key) != user_numforms_end(formats))
                    {
...
int force_character_strings(mba_t &mba, const EarlyHexRaysConfig &config)
{
    ...
    for (int index = 0; index < mba.qty; ++index)
    {
             ...
             instruction = instruction->next)
        {
            ...
            const ForwardEffect effect = analyze_forward_effect(*block, *instruction, state);
...
bool read_constant_memory(ea_t address, int size, uint64 &value)
{
    if (size != 1 && size != 2 && size != 4 && size != 8)
        return false;
    ea_t last = BADADDR;
    if (!add_signed_delta(address, size - 1, last) || !is_mapped(address) || !is_mapped(last))
    {
        return false;
    }
    const segment_t *first_segment = getseg(address);
    if (first_segment == nullptr || getseg(last) != first_segment ||
        first_segment->type == SEG_XTRN || (first_segment->perm & SEGPERM_READ) == 0 ||
    ...
    for (int offset = 0; offset < size; ++offset)
    {
        ...
        if (!is_loaded(current))
            return false;
        xrefblk_t xref;
        for (bool ok = xref.first_to(current, XREF_DATA); ok; ok = xref.next_to())
        {
            if ((int(xref.type) & XREF_MASK) == dr_W)
                return false;
        }
...
bool valid_scalar_size(int size) { return size == 1 || size == 2 || size == 4 || size == 8; }
...
bool extract_slot_address(const mop_t &memory, SlotAddress &address)
{
    ...
    if (inner.opcode == m_add && inner.l.is_constant(&immediate, false) && inner.r.t == mop_r)
    {
...
bool slot_ranges_overlap(const SlotKey &left, const SlotKey &right)
{
    ...
    if (left.offset > maximum - (left.size - 1) || right.offset > maximum - (right.size - 1))
    {
        return true;
    }
...
bool slot_base_is_defined(const rlist_t &definitions, mreg_t base)
{
    ...
    return register_list_overlaps(definitions, RegisterKey{base, pointer_size});
}

void erase_register_range(std::map<RegisterKey, ConstantValue> &registers, const RegisterKey &range)
{
    ...
    while (iterator != registers.end() && int64(iterator->first.start) < end)
    {
...
void erase_overlapping_slots(std::map<SlotKey, ConstantValue> &slots, const SlotKey &range)
{
    auto iterator = slots.lower_bound(SlotKey{range.base, range.offset, 0});
    ...
    const sval_t maximum = std::numeric_limits<sval_t>::max();
    const sval_t last =
        range.offset > maximum - (range.size - 1) ? maximum : range.offset + range.size - 1;
    while (iterator != slots.end() && iterator->first.base == range.base &&
...
bool evaluate_unary(mcode_t opcode, uint64 operand, int source_bits, uint64 &value)
{
    switch (opcode)
    {
    case m_ldc:
...
bool evaluate_forward_instruction(const ForwardState &state, const minsn_t *instruction,
                                  ConstantValue &value, int depth)
{
    ...
    {
        ConstantValue lhs;
        if (!evaluate_forward_operand(state, instruction, instruction->l, lhs, depth + 1))
        {
            return false;
        }
        uint64 result = 0;
        if (evaluate_unary(instruction->opcode, lhs.value, 8 * instruction->l.size, result))
        {
        ...
        ConstantValue rhs;
        if (!evaluate_forward_operand(state, instruction, instruction->r, rhs, depth + 1) ||
            !evaluate_binary(instruction->opcode, lhs.value, rhs.value, result))
        {
            return false;
        }
        if (is_mcode_shift(instruction->opcode))
        {
            if (instruction->l.size <= 0 || rhs.value > uint64(8 * instruction->l.size - 1))
            {
                return false;
            }
        }
    ...
    if (extract_slot_address(instruction->r, slot_address) && stable_frame_base(slot_address.base))
    {
    ...
    if (!evaluate_forward_operand(state, instruction, instruction->r, address_value, depth + 1))
    {
        return false;
    }
...
bool evaluate_forward_operand(const ForwardState &state, const minsn_t *current,
                              const mop_t &operand, ConstantValue &value, int depth)
{
    ...
    if (operand.t == mop_a && operand.a != nullptr && operand.a->t == mop_v &&
        operand.a->g != BADADDR && !operand.is_lowaddr() && !operand.a->is_lowaddr() &&
        !operand.is_undef_val() && !operand.a->is_undef_val() && valid_scalar_size(operand.size))
    {
    ...
    {
        const auto found = state.registers.find(RegisterKey{operand.r, operand.size});
    ...
    {
        return evaluate_forward_instruction(state, operand.d, value, depth + 1);
    }
    return false;
}
                                     ...
                                     const ForwardState &state)
{
    ...
    effect.definitions =
        block.build_def_list(instruction, MAY_ACCESS | INCLUDE_SPOILED_REGS | INCLUDE_DEAD_RETREGS);
    effect.writes_memory = !effect.definitions.mem.empty();

    if (instruction.modifies_d() && instruction.d.t == mop_r && instruction.d.size > 0)
    {
    ...
    if (instruction.opcode == m_stx && valid_scalar_size(instruction.l.size))
    {
        SlotAddress address;
        if (extract_slot_address(instruction.d, address) && stable_frame_base(address.base))
        {
            ...
            if (evaluate_forward_operand(state, &instruction, instruction.l, stored))
            {
...
void apply_forward_effect(ForwardState &state, const ForwardEffect &effect)
{
    ...
    for (auto iterator = state.registers.begin(); iterator != state.registers.end();)
    {
    ...
    if (state.slot_base && slot_base_is_defined(effect.definitions.reg, *state.slot_base))
    {
        state.slots.clear();
        state.slot_base.reset();
    }
...
bool flatten_forward_operand(const ForwardState &state, minsn_t *current, mop_t &operand)
{
        ...
        evaluate_forward_operand(state, current, operand, value, 0))
    {
        ...
        operand.make_number(truncate_to_size(value.value, operand.size), operand.size, current->ea);
        return true;
    }
    if (operand.t != mop_d || operand.d == nullptr)
        return false;
...
bool fold_instruction(minsn_t *instruction, const ForwardEffect &effect)
{
    const int size = instruction->d.size;
    if (!valid_scalar_size(size) || !effect.result || !effect.result_register ||
        instruction->d.t != mop_r || instruction->is_fpinsn() || instruction->opcode == m_mov ||
        instruction->opcode == m_ldc)
    {
        return false;
    }
    ...
    instruction->l.make_number(truncate_to_size(effect.result->value, size), size, instruction->ea);
    instruction->r.erase();
    return true;
}
...
int fold_constants(mba_t &mba, const EarlyHexRaysConfig &config)
{
    ...
    for (int index = 0; index < mba.qty; ++index)
    {
             ...
             instruction = instruction->next)
        {
            ...
            {
                changed = flatten_forward_operand(state, instruction, instruction->l);
                if (instruction->r.t != mop_z)
                {
                    changed =
                        flatten_forward_operand(state, instruction, instruction->r) || changed;
                }
...
struct EarlyHexRaysAnalysis::Impl final : microcode_filter_t
{
    ...
    std::optional<bool> known_condition(const codegen_t &codegen) const
    {
        if (!config.condition_codegen || codegen.mba == nullptr || codegen.mb == nullptr ||
            codegen.mba->is_snippet() || !executable_code(codegen.insn.ea))
            return std::nullopt;
        ...
        {
            if (get_dtype_size(destination.dtype) != 1 || !natad(codegen.insn) ||
                codegen.insn.segpref != 0 ||
        ...
        else
        {
            ...
            const bool memory =
                source.type == o_phrase || source.type == o_displ || source.type == o_mem;
            if (destination.type != o_reg || (source.type != o_reg && !memory) ||
                (memory && (!natad(codegen.insn) || codegen.insn.segpref != 0)) ||
                (width != 2 && width != 4 && width != 8) || (width == 8 && !mode64(codegen.insn)) ||
                get_dtype_size(source.dtype) != width)
                return std::nullopt;
        }
        ...
        {
            insn_t supporting;
            if (!executable_code(ea_t(address)) || decode_insn(&supporting, ea_t(address)) <= 0 ||
    ...
    merror_t apply_condition(codegen_t &codegen, bool value)
    {
        ...
        if (condition->use == X86ConditionUse::set_byte)
        {
            ...
            if (!codegen.store_operand(0, result))
                return MERR_INSN;
        ...
        else
        {
            ...
            if (memory)
            {
                ...
                const char *helper = width == 2   ? "__chernobog_read_u16"
                                     : width == 4 ? "__chernobog_read_u32"
                                                  : "__chernobog_read_u64";
                ...
                if (!value)
                    input = codegen.load_operand(0);
            }
            else
                input = codegen.load_operand(value ? 1 : 0);
            if (input == mr_none)
                return MERR_INSN;
            const mreg_t temporary = codegen.mba->alloc_kreg(size_t(width));
            if (temporary == mr_none)
                return MERR_INSN;
            mop_t source;
            ...
            if (memory)
                ++statistics.codegen_cmov_memory;
        }
    ...
    {
        return get_dbctx_id() == owner_database && config.enabled && is_x86_database() &&
               (known_condition(codegen).has_value() ||
                (config.call_pop_codegen && is_return_instruction(codegen.insn) &&
                 resolve_gadget_return(codegen.insn.ea, config.gadget_scan_depth) != BADADDR));
    }
    ...
    merror_t apply(codegen_t &codegen) override
    {
        ...
        const auto summary =
            resolve_gadget_return_summary(codegen.insn.ea, config.gadget_scan_depth);
    ...
    bool install_filter()
    {
        if (installed || !config.enabled || (!config.call_pop_codegen && !config.condition_codegen))
            return true;
    ...
    template <class Flowchart> int flowchart(const Flowchart *input, bitset_t *reachable)
    {
        if (get_dbctx_id() != owner_database || !config.enabled || !config.call_pop_flowchart ||
            input == nullptr)
            return 0;
        bool bounded = false;
        Flowchart *mutable_flowchart = const_cast<Flowchart *>(input);
        const int changes = repair_flowchart(*mutable_flowchart, reachable, config, bounded);
    ...
    {
        if (get_dbctx_id() != owner_database || !config.enabled || !config.generated_gotos ||
            mba == nullptr)
            return 0;
        if (!mba_within_bounds(*mba, config))
        {
...
EarlyHexRaysAnalysis::EarlyHexRaysAnalysis() : impl_(new Impl) {}

EarlyHexRaysAnalysis::~EarlyHexRaysAnalysis() { uninstall(get_hexdsp() != nullptr); }

bool EarlyHexRaysAnalysis::install() { return impl_ != nullptr && impl_->install_filter(); }
...
bool EarlyHexRaysAnalysis::enabled() const { return impl_ != nullptr && impl_->config.enabled; }
...
int EarlyHexRaysAnalysis::on_flowchart(const qflow_chart_t *flowchart, bitset_t *reachable)
{
    return impl_ != nullptr ? impl_->flowchart(flowchart, reachable) : 0;
}

#if IDA_SDK_VERSION >= 940
int EarlyHexRaysAnalysis::on_flowchart(const qflow_chart_ea_t *flowchart, bitset_t *reachable)
{
    return impl_ != nullptr ? impl_->flowchart(flowchart, reachable) : 0;
}
#endif
