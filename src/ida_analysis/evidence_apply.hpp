...
struct EvidenceApplyStats
{
    ...
    size_t total() const
    {
        return static_crefs + dynamic_crefs + drefs + code_items + pointer_offsets + typed_globals +
               strings + functions + purges + noret_annotations + argument_annotations +
               opaque_annotations + switches + comments;
    }
};

EvidenceApplyStats apply_evidence_to_ida(const chernobog::hybrid::TargetEvidence &evidence);
