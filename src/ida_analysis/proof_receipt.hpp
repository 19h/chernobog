...
inline std::optional<std::vector<uint8_t>> encode(const Receipt &receipt)
{
  ...
  {
    if ( edge.type > 31 ) return std::nullopt;
...
{
  if ( bytes == nullptr || size < 23 || size > maximum_size
    || bytes[0] != 'N' || bytes[1] != 'P' || bytes[2] != 'R' || bytes[3] != 1 )
    return std::nullopt;
  size_t cursor = 4;
  ...
  if ( count > maximum_edges || size < 23 + count * 10 ) return std::nullopt;
  for ( size_t i = 0; i < count; ++i )
  {
    ...
    if ( edge.type > 31 || user > 1 ) return std::nullopt;
    edge.user = user != 0;
    result.edges.push_back(edge);
  }
  const size_t length = size_t(get(2));
  if ( length > maximum_comment || size - cursor != length ) return std::nullopt;
  result.comment.assign(reinterpret_cast<const char *>(bytes + cursor), length);
  if ( !valid_comment(result.comment) ) return std::nullopt;
  return result;
}
