...
EvidenceProvenance provenance_for(const ProgramImage &image, const FuncRange &function,
...
bool conditional_projection(const StaticInstructionEvidence &item, uint64_t *target,
                            uint64_t *fallthrough)
{
    const DecoderInstruction *projection = nullptr;
    if (!item.ida_macro_component && item.ida.valid && item.ida.flow == RAX_FLOW_COND_BRANCH &&
        item.ida.has_target && item.ida.has_fallthrough)
    {
        projection = &item.ida;
    }
    else if (item.rax.status == DecoderDecodeStatus::Valid &&
             item.rax.instruction.flow == RAX_FLOW_COND_BRANCH && item.rax.instruction.has_target &&
...
bool static_indirect_at(const StaticAnalysisResult &analysis, uint64_t address)
{
    ...
    return (!item->ida_macro_component && item->ida.indirect) ||
           (item->rax.status == DecoderDecodeStatus::Valid && item->rax.instruction.indirect);
}
...
void append_dependency_range(std::vector<DependencyRange> &ranges, const ProgramImage &image,
...
Identity capture_identity(const ProgramImage &image, uint64_t start, uint64_t end)
{
    Identity identity;
    ...
    for (; segment != image.segs.end() && segment->start < end; ++segment)
    {
        ...
        if (source_offset >= segment->bytes.size())
            continue;
        const uint64_t available = std::min<uint64_t>(std::min(end, segment->end) - cursor,
                                                      segment->bytes.size() - source_offset);
        ...
        while (cursor < run_end)
        {
            ...
            const size_t count =
                static_cast<size_t>(std::min<uint64_t>(8 - (source & 7), run_end - cursor));
                    ...
                    for (size_t i = 0; i < count; ++i)
                        if ((bits & (1u << i)) != 0)
                        {
                            ...
                            identity.bytes[offset] =
                                segment->bytes[static_cast<size_t>(source) + i];
            ...
            const LoadedByteView view =
                segment->loaded_view(cursor, static_cast<size_t>(run_end - cursor));
            ...
            if (offset < stop)
                identity.loaded_mask[offset / 8] |=
                    static_cast<uint8_t>((1u << (stop - offset)) - 1);
            cursor += view.size;
        }
    }
    return identity;
...
IdentityComparison hybrid_compare_identity_bytes(const std::vector<uint8_t> &expected_bytes,
                                                 ...
                                                 const std::vector<uint8_t> &actual_mask)
{
    ...
    const size_t required_mask_size = expected_bytes.size() / 8 + (expected_bytes.size() % 8 != 0);
    if (expected_mask.size() < required_mask_size || actual_mask.size() < required_mask_size)
    {
        ...
        if (count == 64)
        {
            ...
            if ((expected_bits == 0 && actual_bits == 0) ||
                (expected_bits == actual_bits &&
                 std::memcmp(expected_bytes.data() + begin, actual_bytes.data() + begin, count) ==
                     0))
            {
                begin += count;
                continue;
...
bool TargetEvidence::matches(uint64_t function_start, uint64_t function_hash) const
{
    return scope.function_start == function_start && scope.function_hash == function_hash;
}

std::vector<RuntimeStringCandidate> hybrid_consensus_runtime_strings(const TargetEvidence &evidence,
                                                                     ...
                                                                     size_t maximum_length)
{
    ...
    for (const auto &address_values : values)
    {
        ...
        if (per_run.size() != eligible.size() || ambiguous.count(address) != 0)
        {
            continue;
        }
...
std::vector<RuntimeUseStringCandidate> hybrid_consensus_use_strings(const TargetEvidence &evidence,
                                                                    size_t minimum_length,
                                                                    size_t maximum_length)
{
    ...
    for (const auto &use : evidence.events.uses)
    {
        ...
        if (use.scope == DataScope::HEAP)
        {
            ...
            if (found == objects.end() || conflicting_objects.count(object))
                valid = false;
            else
            {
        ...
        else if (use.scope != DataScope::IMAGE && use.scope != DataScope::STACK)
            valid = false;
        const auto decoded =
            string_recovery::recover_runtime_utf8_prefix(use.bytes, minimum_length, maximum_length);
        if (!valid || !decoded)
        {
    ...
    {
        if (ambiguous.count(entry.first) || entry.second.size() != eligible.size())
            continue;
        ...
        candidate.value = string_recovery::recover_runtime_utf8_prefix(
                              candidate.use.bytes, minimum_length, maximum_length)
                              ->utf8;
        candidate.eligible_runs = eligible.size();
        bool same = true;
        for (const auto &run : entry.second)
        {
            ...
            if (!decoded || decoded->utf8 != candidate.value)
            {
                same = false;
                break;
            }
    ...
    result.insert(result.end(), std::make_move_iterator(native.begin()),
                  std::make_move_iterator(native.end()));
    return result;
}

BranchClaimCheck TargetEvidence::check_branch_claim(uint64_t instruction, bool expected_taken) const
{
    ...
    for (const BranchObservation &observation : branches)
    {
        ...
        const bool observed_taken = observation.disposition == BranchDisposition::TAKEN;
        ...
        else
        {
            ...
            if (observation.consumed_context_complete)
            {
                ...
                result.opposing_context_complete_runs.insert(observation.provenance.run_id);
            }
        }
    }
                                            ...
                                            const EmulationJobResult &emulation)
{
    ...
    result.scope = provenance_for(image, function, focus_address, emulation.ticket, 0, 0);
    ...
    for (const FuncChunk &chunk : function.chunks)
    {
        if (chunk.end <= chunk.start)
            continue;
        result.function_identity.push_back(
            capture_identity<FunctionChunkIdentity>(image, chunk.start, chunk.end));
    }
    ...
    result.summary.decoder_disagreements = static_analysis.stats.mismatched_instructions;
    result.summary.decoder_disagreement_flags = static_analysis.stats.size_disagreements +
    ...
    for (const EmulationRunResult &run : emulation.runs)
    {
        ...
        observation.provenance =
            provenance_for(image, function, focus_address, emulation.ticket, run.run_id, run.seed);
    ...
    for (const ExecPoint &point : result.events.execution)
        append_dependency_range(dependencies, image, point.pc, point.size == 0 ? 1 : point.size);
    ...
    for (const DependencyRange &range : dependencies)
    {
        ...
        const SegImage *prior_segment = merged_dependencies.empty()
                                            ? nullptr
                                            : image.segment_at(merged_dependencies.back().start);
        if (!merged_dependencies.empty() && segment == prior_segment &&
            range.start <= merged_dependencies.back().end)
        {
            merged_dependencies.back().end = std::max(merged_dependencies.back().end, range.end);
        }
        else
        {
    ...
    for (const DependencyRange &range : merged_dependencies)
    {
        ContextRangeIdentity identity =
            capture_identity<ContextRangeIdentity>(image, range.start, range.end);
    ...
    result.summary.executed_addresses_without_static_record = executed_without_static.size();
    ...
    for (const EmulationRunResult &run : emulation.runs)
        complete_context_by_run[{run.run_id, run.seed}] = run.outcome.consumed_context_complete;
    for (const ExecPoint &point : result.events.execution)
    ...
    for (auto &entry : by_run)
    {
        ...
        std::sort(
            points.begin(), points.end(), [](const ExecPoint *left, const ExecPoint *right)
            { return std::tie(left->sequence, left->pc) < std::tie(right->sequence, right->pc); });
        for (size_t index = 0; index + 1 < points.size(); ++index)
        {
            const ExecPoint &current = *points[index];
            const ExecPoint &next = *points[index + 1];
            const StaticInstructionEvidence *instruction = static_analysis.find(current.pc);
            uint64_t target = 0;
            ...
            branch.provenance = provenance_for(image, function, focus_address, emulation.ticket,
    ...
    std::sort(result.branches.begin(), result.branches.end(),
              ...
              {
                  return std::tie(left.instruction, left.provenance.run_id, left.provenance.seed,
                                  left.sequence, left.observed_successor) <
                         std::tie(right.instruction, right.provenance.run_id, right.provenance.seed,
                                  right.sequence, right.observed_successor);
              });
    ...
    {
        if (edge.kind == ExecEdge::Kind::Unknown || !function.contains(edge.from) ||
            !static_indirect_at(static_analysis, edge.from))
            continue;
        result.indirect_targets.push_back(
            IndirectTargetObservation{edge.from, edge.to, edge.kind,
                                      provenance_for(image, function, focus_address,
                                                     emulation.ticket, edge.run_id, edge.seed)});
    }
    std::sort(result.indirect_targets.begin(), result.indirect_targets.end(),
              [](const IndirectTargetObservation &left, const IndirectTargetObservation &right)
              {
                  return std::tie(left.instruction, left.target, left.provenance.run_id,
                                  left.provenance.seed) < std::tie(right.instruction, right.target,
...
const char *hybrid_branch_verdict_name(BranchClaimVerdict verdict)
{
    switch (verdict)
    {
    case BranchClaimVerdict::NO_OBSERVATION:
        return "no-observation";
    case BranchClaimVerdict::CORROBORATED:
        return "corroborated";
    case BranchClaimVerdict::COUNTEREXAMPLE:
        return "counterexample";
    case BranchClaimVerdict::MIXED:
        return "mixed";
    default:
        return "unknown";
    }
