...
inline EvidenceView project_evidence_view(const TargetEvidence &source)
{
    ...
    size_t serial = 0;
    const auto event = [&](const char *kind, uint32_t run, uint64_t seed, uint64_t sequence,
                           uint64_t site, EvidenceViewRow row)
    {
        row["kind"] = kind;
        row["run"] = view_hex(run);
        row["seed"] = view_hex(seed);
        row["sequence"] = view_hex(sequence);
        row["site"] = view_hex(site);
    ...
    for (const auto &item : source.static_analysis.instructions)
    {
        ...
        {
            if (encoded++ >= EvidenceView::edge_limit / 2)
            {
                ++view.omitted["encoded_edges"];
                return;
            }
            view.edges.push_back(
                {{"kind", kind},
                 {"truth", "encoding"},
                 {"site", view_hex(item.address)},
                 {"target", view_hex(target)},
                 {"assumption", "IDA decoded instruction; reachability is not established"}});
        };
        if (item.ida.has_target && !item.ida.indirect)
            edge(item.ida.target, "encoded-target");
        if (item.ida.has_fallthrough)
            edge(item.ida.fallthrough, "encoded-fallthrough");
    }
    for (const auto &item : source.events.edges)
    {
        EvidenceViewRow row{
            {"target", view_hex(item.to)},
            {"transfer_kind", view_hex(uint8_t(item.kind))},
            {"truth", "witness"},
            {"assumption", "observed transfer for this run; not a unique-target proof"}};
        event("transfer", item.run_id, item.seed, item.sequence, item.from, row);
        row.insert({{"kind", "observed-transfer"},
                    {"site", view_hex(item.from)},
                    {"run", view_hex(item.run_id)},
                    {"seed", view_hex(item.seed)},
                    {"sequence", view_hex(item.sequence)}});
        if (observed++ < EvidenceView::edge_limit / 2)
            view.edges.push_back(std::move(row));
        else
            ++view.omitted["observed_edges"];
    }
    for (const auto &item : source.events.allocations)
    {
        EvidenceViewRow row{
            {"allocation", view_hex(item.id)},        {"generation", view_hex(item.generation)},
            {"address", view_hex(item.address)},      {"size", view_hex(item.size)},
            {"context", view_hex(item.context)},      {"callee", view_hex(item.callee)},
            {"allocation_site", view_hex(item.site)}, {"occurrence", view_hex(item.occurrence)},
            {"allocated", view_hex(item.allocated)},  {"released", view_hex(item.released)},
        ...
        if (!item.live)
            event("release", item.run_id, item.seed, item.released, 0, row);
        row.insert({{"site", view_hex(item.site)},
                    {"run", view_hex(item.run_id)},
                    {"seed", view_hex(item.seed)}});
    ...
    for (const auto &item : source.events.uses)
        event("use", item.run_id, item.seed, item.sequence, item.site,
              {{"allocation", view_hex(item.allocation_id)},
               {"generation", view_hex(item.generation)},
               {"address", view_hex(item.address)},
               {"context", view_hex(item.context)},
               {"callee", view_hex(item.callee)},
               {"occurrence", view_hex(item.occurrence)},
               {"argument", std::to_string(item.argument)},
               {"scope", view_hex(uint8_t(item.scope))},
               {"offset", std::to_string(item.offset)},
               {"status", view_hex(uint8_t(item.status))},
               {"producer", use_producer_name(item.producer)},
               {"model_kind", view_hex(item.model_kind)},
               {"observed_size", view_hex(item.observed_size)},
               {"captured_size", view_hex(item.bytes.size())},
               {"bytes_hex", view_bytes(item.bytes)},
    ...
    for (const auto &candidate : hybrid_consensus_native_read_strings(source))
        for (size_t run = 0; run < candidate.read_fragments.size(); ++run)
        {
            ...
            EvidenceViewRow row{
                ...
                {"bytes_hex", view_bytes(use.bytes)},
                {"captured_size", std::to_string(use.bytes.size())},
            ...
            row.insert({{"kind", "read-stream"},
                        {"run", view_hex(first.run_id)},
                        {"seed", view_hex(first.seed)},
                        {"sequence", view_hex(last.sequence)},
                        {"site", view_hex(first.site)}});
            bounded(view.read_streams, EvidenceView::read_stream_limit, "read_streams",
                    std::move(row));
        }
    for (const auto &item : source.events.data)
        event("memory", item.run_id, item.seed, item.sequence, item.from,
              {{"address", view_hex(item.addr)},
               {"size", view_hex(item.size)},
               {"access_kind", std::to_string(item.kind)},
               {"scope", view_hex(uint8_t(item.scope))},
               {"hook_low64", view_hex(item.value)}});
    for (const auto &item : source.events.execution)
        event("execute", item.run_id, item.seed, item.sequence, item.pc,
              {{"size", view_hex(item.size)}});
    for (const auto &item : source.events.states)
    {
        EvidenceViewRow row{{"source", view_hex(item.source)},
                            {"state_kind", view_hex(uint8_t(item.kind))}};
        for (size_t i = 0; i < std::min<size_t>(32, item.regs.size()); ++i)
            row["reg_" + std::to_string(item.regs[i].reg)] =
                view_hex(item.regs[i].value) + "/" + std::to_string(item.regs[i].width) + "B";
        row["registers_omitted"] =
            std::to_string(item.regs.size() > 32 ? item.regs.size() - 32 : 0);
    ...
    for (const auto &item : source.runs)
    {
        ...
        bounded(view.runs, EvidenceView::run_limit, "runs",
                {{"run", view_hex(item.provenance.run_id)},
                 {"seed", view_hex(item.provenance.seed)},
                 ...
                 {"ran", item.ran ? "true" : "false"},
                 {"returned", outcome.returned ? "true" : "false"},
                 {"kind", hybrid_emu_outcome_name(outcome)},
                 {"stop_valid", outcome.stop_valid ? "true" : "false"},
                 {"stop_reason", std::to_string(outcome.stop_reason)},
                 {"stop_reason_name", hybrid_rax_stop_reason_name(outcome.stop_reason)},
                 {"stop_status", std::to_string(outcome.stop_status)},
                 {"site", view_hex(outcome.stop_pc)},
                 ...
                 {"memory_observation_available",
                  outcome.memory_observation_available ? "true" : "false"},
    ...
    for (const auto &item : source.branches)
    {
        ...
        event("branch", item.provenance.run_id, item.provenance.seed, item.sequence,
              item.instruction,
              {{"target", view_hex(item.observed_successor)},
               {"encoded_target", view_hex(item.encoded_target)},
               {"fallthrough", view_hex(item.fallthrough)},
               {"disposition", view_hex(uint8_t(item.disposition))},
               {"context_complete", item.consumed_context_complete ? "true" : "false"}});
    }
    for (uint64_t site : sites)
        for (bool taken : {false, true})
        {
            if (view.claims.size() >= EvidenceView::claim_limit)
            {
                ++view.omitted["claims"];
                continue;
            }
            ...
            bounded(
                view.claims, EvidenceView::claim_limit, "claims",
                {{"site", view_hex(site)},
                 {"expected_taken", taken ? "true" : "false"},
                 {"verdict", hybrid_branch_verdict_name(claim.verdict)},
                 {"matching", std::to_string(claim.matching)},
                 {"opposing", std::to_string(claim.opposing)},
    ...
    for (auto &item : retained)
        view.events.push_back(std::move(item.second));
    const auto key = [](const EvidenceViewRow &row)
    {
        return std::make_tuple(std::stoull(row.at("run"), nullptr, 16),
                               std::stoull(row.at("seed"), nullptr, 16),
                               std::stoull(row.at("sequence"), nullptr, 16), row.at("kind"));
    };
                                      ...
                                      bool fresh, int64_t database_id, uint64_t revision = 0)
{
    ...
    out << "{\"schema\":1,\"available\":true,\"fresh\":" << (fresh ? "true" : "false")
        ...
        << ",\"ticket\":" << quote(view_hex(source.scope.ticket)) << ",\"static_truncated\":"
        << (source.summary.static_analysis_truncated ? "true" : "false");
    const auto rows = [&](const char *name, const std::vector<EvidenceViewRow> &items)
    { inspection_json_rows(out, name, items); };
    rows("edges", view.edges);
    rows("events", view.events);
    rows("lifetimes", view.lifetimes);
    rows("runs", view.runs);
