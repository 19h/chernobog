                              ...
                              const std::vector<int> &argument_registers)
{
    ...
    if (index >= argument_registers.size())
        return false;
    return std::any_of(input->register_overrides.begin(), input->register_overrides.end(),
...
{
    uint64_t end = 0;
    return size != 0 && hi >= lo && address >= lo && checked_add(address, size, &end) && end <= hi;
}

bool image_contains_range(const ProgramImage *image, uint64_t address, uint64_t size)
{
...
{
    if (c->region != nullptr)
        return c->region->at(ea) != nullptr;
    return c->func != nullptr ? c->func->contains(ea) : (ea >= c->flo && ea < c->fhi);
}

bool image_access_allowed(const HookCtx *c, uint64_t address, uint32_t size, HybridSegPerm required)
{
...
bool summary_access_allowed(HookCtx *c, rax_engine *engine, uint64_t address, uint64_t size,
                            HybridSegPerm required)
{
    if (size > std::numeric_limits<uint32_t>::max() ||
        (c->strict_permissions && !image_access_allowed(c, address, uint32_t(size), required)))
    {
...
void restore_snapshot_bytes(HookCtx *c, rax_engine *engine, uint64_t address, uint32_t size)
{
    if (c->api == nullptr || c->image == nullptr || size == 0 || size > (1u << 20))
        return;
...
bool read_scalar(HookCtx *c, rax_engine *engine, uint64_t address, size_t size, uint64_t *out)
{
...
bool summary_arg(HookCtx *c, rax_engine *engine, size_t index, uint64_t *out)
{
    ...
    if (stack_index > std::numeric_limits<uint64_t>::max() / ptr)
        return false;
    uint64_t address = 0;
    return checked_add(sp, return_slot, &address) && checked_add(address, home, &address) &&
           checked_add(address, uint64_t(stack_index) * ptr, &address) &&
           read_scalar(c, engine, address, ptr, out);
}

DataScope access_scope(const HookCtx *c, uint64_t address, uint32_t size, bool *recordable)
{
...
uint64_t next_scope_boundary(const HookCtx *c, uint64_t address)
{
    ...
    if (c->image != nullptr)
    {
        const auto next = std::upper_bound(c->image->segs.begin(), c->image->segs.end(), address,
                                           [](uint64_t value, const SegImage &segment)
...
void record_summary_access(HookCtx *c, int kind, uint64_t address, uint64_t value, uint32_t size)
{
    if (c->out == nullptr)
        return;
    ...
    if (kind == RAX_MEM_READ && scope == DataScope::IMAGE)
    {
        if (c->out->consumed_image_reads.size() >= c->dependency_cap)
            ...
            c->out->consumed_image_reads.push_back(
                ConsumedImageRange{address, size, c->run_id, c->seed});
    }
...
void capture_summary_use(HookCtx *c, const EmuCallSummary &summary, int argument, uint64_t address,
                         const uint8_t *bytes, size_t size)
{
    if (!c->temporal.enabled || !hook_in_function(c, c->summary_source))
        return;
    bool recordable = false;
    const DataScope scope = size <= std::numeric_limits<uint32_t>::max()
                                ? access_scope(c, address, uint32_t(size), &recordable)
                                : DataScope::OTHER;
    c->temporal.capture(c->summary_source, summary.address, argument, UseProducer::MODELED_ARGUMENT,
                        scope, address, bytes, size, size, c->sequence++, uint8_t(summary.kind));
}

bool read_c_string(HookCtx *c, rax_engine *engine, uint64_t address, std::vector<uint8_t> &out,
                   size_t limit = 65536)
{
    out.clear();
    ...
    for (size_t i = 0; i < limit; ++i)
    {
        ...
        if (!checked_add(address, uint64_t(i), &current))
            return false;
        uint8_t ch = 0;
        if (!summary_access_allowed(c, engine, current, 1, HybridSegPerm::READ))
            return false;
...
bool apply_summary(HookCtx *c, rax_engine *engine, const EmuCallSummary &summary)
{
    ...
    case EmuSummaryKind::STRCMP:
    {
        ...
        if (!args(2) || !read_c_string(c, engine, a0, bytes) || !read_c_string(c, engine, a1, rhs))
            return false;
        const size_t n = std::min(bytes.size(), rhs.size());
        int cmp = std::memcmp(bytes.data(), rhs.data(), n);
        if (cmp == 0)
            cmp = bytes.size() < rhs.size() ? -1 : bytes.size() > rhs.size() ? 1 : 0;
        ...
        record_summary_access(c, RAX_MEM_READ, a0,
                              scalar_from_memory(c, bytes.data(), bytes.size()),
                              uint32_t(bytes.size()));
        record_summary_access(c, RAX_MEM_READ, a1, scalar_from_memory(c, rhs.data(), rhs.size()),
                              uint32_t(rhs.size()));
        break;
    }
    ...
    case EmuSummaryKind::MEMMOVE:
    {
        ...
        bytes.resize(n);
        if (n == 0)
            capture_summary_use(c, summary, 1, a1, nullptr, 0);
        if (n != 0)
        {
    ...
    case EmuSummaryKind::MEMSET:
    {
        ...
        if (n != 0)
        {
            if (!summary_access_allowed(c, engine, a0, n, HybridSegPerm::WRITE) ||
                ...
                return false;
            record_summary_access(c, RAX_MEM_WRITE, a0, scalar_from_memory(c, bytes.data(), n),
                                  uint32_t(n));
        }
        result = a0;
        break;
    ...
    case EmuSummaryKind::STRCPY:
        if (!args(2) || !read_c_string(c, engine, a1, bytes) ||
            !summary_access_allowed(c, engine, a0, bytes.size(), HybridSegPerm::WRITE))
            return false;
    ...
    case EmuSummaryKind::STRNCPY:
    {
        ...
        const size_t n = size_t(a2);
        bytes.assign(n, 0);
        if (n != 0 && !summary_access_allowed(c, engine, a0, n, HybridSegPerm::WRITE))
            return false;
        size_t read_count = 0;
    ...
    case EmuSummaryKind::CALLOCATE:
    {
        ...
        if (n > kMaxModelBytes)
            n = 0;
        result = c->temporal.allocate(n, c->summary_source, summary.address, c->sequence++);
        if (c->temporal.allocation_exhausted)
            return false;
        if (result != 0)
        {
            if (summary.kind == EmuSummaryKind::CALLOCATE)
            {
                ...
                if (!summary_access_allowed(c, engine, result, bytes.size(),
                                            HybridSegPerm::WRITE) ||
    ...
    case EmuSummaryKind::DEALLOCATE:
        if (!args(1))
            return false;
        if (!c->temporal.release(a0, c->sequence++))
            return false;
        result = 0;
        break;
    ...
    case EmuSummaryKind::ALLOCATE_OBJECT:
    {
        ...
        result =
            c->temporal.allocate(object_size, c->summary_source, summary.address, c->sequence++);
        if (c->temporal.allocation_exhausted)
            return false;
        if (result != 0)
        {
...
bool decode_at(const HookCtx *c, uint64_t pc, uint32_t mode, uint32_t *size, ExecEdge::Kind *kind,
               int32_t *flow = nullptr)
{
    if (size != nullptr)
        *size = 0;
    if (kind != nullptr)
    ...
    if (c->region)
    {
        ...
        if (!head)
            return false;
        if (size)
            *size = uint32_t(head->bytes.size());
        if (flow)
            *flow = head->flow;
        if (kind)
        {
            if (head->flow == RAX_FLOW_CALL || head->flow == RAX_FLOW_INDIRECT_CALL)
                *kind = ExecEdge::Kind::Call;
            else if (head->flow == RAX_FLOW_BRANCH || head->flow == RAX_FLOW_COND_BRANCH ||
                     head->flow == RAX_FLOW_INDIRECT_JUMP)
                *kind = ExecEdge::Kind::Jump;
            else if (head->flow == RAX_FLOW_RETURN)
                *kind = ExecEdge::Kind::Return;
        }
        return true;
    }
    ...
    for (const SegImage &segment : c->image->segs)
    {
        ...
        if (loaded == 0)
            return false;
        rax_decoded decoded{};
        if (c->api->decode(c->rax_arch, mode, pc, segment.bytes.data() + offset, loaded,
                           &decoded) != RAX_OK ||
        ...
        if (kind != nullptr)
        {
            ...
            {
            case RAX_FLOW_CALL:
            case RAX_FLOW_INDIRECT_CALL:
                *kind = ExecEdge::Kind::Call;
                break;
            case RAX_FLOW_BRANCH:
            case RAX_FLOW_COND_BRANCH:
            case RAX_FLOW_INDIRECT_JUMP:
                *kind = ExecEdge::Kind::Jump;
                break;
            case RAX_FLOW_RETURN:
                *kind = ExecEdge::Kind::Return;
                break;
            default:
                break;
...
uint32_t decode_mode_at_hook(const HookCtx *c, rax_engine *engine)
{
    ...
    mode |= (cpsr & (UINT64_C(1) << 5)) != 0 ? RAX_MODE_THUMB : RAX_MODE_ARM;
    return mode;
}
...
void code_tr(rax_engine *engine, uint64_t addr, uint32_t size, void *user)
{
    ...
    const SegImage *current_segment = c->image != nullptr ? c->image->segment_at(addr) : nullptr;
    ...
    if (region_resume)
    {
        c->region_resume_pending = false;
        c->region_boundary = false;
    ...
    if (c->has_prev && !region_resume)
    {
        ...
        const bool fallthrough_valid =
            instruction_size != 0 && checked_add(c->prev_pc, instruction_size, &fallthrough);
        summary_transfer = (!fallthrough_valid || addr != fallthrough) &&
                           hook_in_function(c, c->prev_pc) && addr >= c->lo && addr < c->hi;
        ...
        if (summary_transfer && c->out->edges.size() < c->edge_cap)
        {
            c->out->edges.push_back(
                ExecEdge{c->prev_pc, addr, c->run_id, c->seed, transfer_kind, event_sequence});
    ...
    if (c->native_temporal && c->modeled_thunk && c->has_prev && c->prev_pc == c->modeled_thunk &&
        transfer_kind == ExecEdge::Kind::Jump)
    {
        ...
        if (thunk_call)
            c->summary_source = c->modeled_call_source;
    }
    c->modeled_thunk = 0;
    const EmuCallSummary *summary =
        summary_transfer && (transfer_kind == ExecEdge::Kind::Call || thunk_call)
            ? find_summary(c, addr)
            : nullptr;
    ...
    if (c->region != nullptr)
    {
        ...
        {
            StatePoint point;
            point.kind = StatePoint::Kind::RegionEntry;
            point.source = addr;
            point.pc = addr;
        ...
        {
            if (c->out->states.size() >= c->state_cap)
                c->native_sample_incomplete = true;
            else
            {
                StatePoint point;
                point.kind = StatePoint::Kind::NativeInstructionEntry;
                point.source = c->has_prev ? c->prev_pc : addr;
                point.pc = addr;
                point.sequence = event_sequence;
                point.run_id = c->run_id;
                point.seed = c->seed;
            ...
            (head->flow == RAX_FLOW_BRANCH || head->flow == RAX_FLOW_INDIRECT_JUMP))
        {
            ...
            uint64_t sp = 0, return_address = 0;
            if (caller &&
                (caller->flow == RAX_FLOW_CALL || caller->flow == RAX_FLOW_INDIRECT_CALL) &&
                c->api->reg_read_u64(engine, c->sp_reg, &sp) == RAX_OK &&
                read_scalar(c, engine, sp, c->is64 ? 8 : 4, &return_address) &&
                return_address == caller->address + caller->bytes.size())
            {
                c->modeled_thunk = addr;
                c->modeled_call_source = caller->address;
                c->modeled_call_sp = sp;
                c->modeled_call_return = return_address;
            }
        }
    }
    ...
    if (c->rax_arch == RAX_ARCH_X86 && current_decode_mode == RAX_MODE_32 && effective_size > 0 &&
        effective_size <= 15)
    {
        uint8_t code[15] = {};
        if (c->api->mem_read(engine, addr, code, effective_size) != RAX_OK)
        {
            c->environment_model_failure = true;
            c->external_target = addr;
            c->external_name = "legacy x86 instruction-byte read";
            c->api->emu_stop(engine);
            return;
        }
        size_t opcode = 0;
        while (opcode + 1 < effective_size)
        {
            ...
            if (byte != 0x66 && byte != 0x67 && byte != 0xf0 && byte != 0xf2 && byte != 0xf3 &&
                byte != 0x26 && byte != 0x2e && byte != 0x36 && byte != 0x3e && byte != 0x64 &&
                byte != 0x65)
                break;
            ++opcode;
        }
        if (opcode + 1 == effective_size && code[opcode] >= 0x40 && code[opcode] <= 0x4f)
        {
                ...
                c->api->reg_write_u64(engine, RAX_X86_REG_EFLAGS, flags) != RAX_OK)
            {
                c->environment_model_failure = true;
                c->external_target = addr;
                c->external_name = "legacy x86 carry materialization";
                c->api->emu_stop(engine);
                return;
            }
        }
...
void mem_tr(rax_engine *engine, int kind, uint64_t addr, uint32_t size, uint64_t value, void *user)
{
    HookCtx *c = static_cast<HookCtx *>(user);
    if (kind == RAX_MEM_FETCH)
        return; // instruction fetch is control flow, not a data reference
    const HybridSegPerm required =
        kind == RAX_MEM_WRITE ? HybridSegPerm::WRITE : HybridSegPerm::READ;
    if (c->strict_permissions && !image_access_allowed(c, addr, size, required))
    {
    ...
    if (recordable && hook_in_function(c, c->last_pc) && c->out->data.size() >= c->data_cap)
        c->data_truncated = true;
    if (kind == RAX_MEM_READ && hook_in_function(c, c->last_pc) && c->temporal.enabled)
    {
        ...
        for (size_t i = 0; i < available; ++i)
            observed[i] = uint8_t(value >> (8 * i));
        c->temporal.capture(c->last_pc, 0, -1, UseProducer::EXECUTED_READ, scope, addr, observed,
                            available, size, c->sequence++);
    }
    if (kind == RAX_MEM_READ && recordable && scope == DataScope::IMAGE)
    {
        if (c->out->consumed_image_reads.size() >= c->dependency_cap)
            ...
            c->out->consumed_image_reads.push_back(
                ConsumedImageRange{addr, size, c->run_id, c->seed});
    }
    if (recordable && hook_in_function(c, c->last_pc) && c->out->data.size() < c->data_cap)
    {
        DataAcc a;
...
bool arch_params(HybridArch a, bool big_endian, HybridEntryMode entry_mode, int &rax_arch,
                 uint32_t &mode, int &sp_reg, int &fp_reg, int &lr_reg, int &pc_reg, int &ret_reg,
                 bool &is64)
{
    ...
    case HybridArch::X86_32:
        rax_arch = RAX_ARCH_X86;
        mode = RAX_MODE_32;
        sp_reg = RAX_X86_REG_ESP;
        fp_reg = RAX_X86_REG_EBP;
        pc_reg = RAX_X86_REG_EIP;
        ret_reg = RAX_X86_REG_EAX;
        is64 = false;
        return true;
    case HybridArch::X86_64:
        rax_arch = RAX_ARCH_X86;
        mode = RAX_MODE_64;
        sp_reg = RAX_X86_REG_RSP;
        fp_reg = RAX_X86_REG_RBP;
        pc_reg = RAX_X86_REG_RIP;
        ret_reg = RAX_X86_REG_RAX;
        is64 = true;
        return true;
    case HybridArch::ARM64:
        rax_arch = RAX_ARCH_ARM64;
        mode = big_endian ? RAX_MODE_BIG_ENDIAN : RAX_MODE_LITTLE_ENDIAN;
        sp_reg = RAX_ARM64_REG_SP;
        lr_reg = RAX_ARM64_X(30);
        pc_reg = RAX_ARM64_REG_PC;
        ret_reg = RAX_ARM64_X(0);
        is64 = true;
        return true;
    case HybridArch::ARM32:
        if (entry_mode != HybridEntryMode::ARM && entry_mode != HybridEntryMode::THUMB)
            return false;
        rax_arch = RAX_ARCH_ARM;
        mode = (entry_mode == HybridEntryMode::THUMB ? RAX_MODE_THUMB : RAX_MODE_ARM) |
               (big_endian ? RAX_MODE_BIG_ENDIAN : RAX_MODE_LITTLE_ENDIAN);
        sp_reg = RAX_ARM_REG_SP;
        lr_reg = RAX_REG_LR;
        pc_reg = RAX_ARM_REG_PC;
        ret_reg = RAX_ARM_R(0);
        is64 = false;
        return true;
    case HybridArch::RISCV64:
        ...
        mode = big_endian ? RAX_MODE_BIG_ENDIAN : RAX_MODE_LITTLE_ENDIAN;
        sp_reg = RAX_RISCV_X(2);
        fp_reg = RAX_RISCV_X(8);
        lr_reg = RAX_RISCV_X(1);
        pc_reg = RAX_RISCV_REG_PC;
        ret_reg = RAX_RISCV_X(10);
        is64 = true;
        return true;
    case HybridArch::CORTEX_M:
        ...
        sp_reg = RAX_REG_SP;
        lr_reg = RAX_CM_REG_LR;
        pc_reg = RAX_CM_REG_PC;
        ret_reg = RAX_CM_R(0);
        is64 = false;
        return true;
...
const char *hybrid_rax_status_name(int status)
{
    ...
    case RAX_ERR_HOOK:
        return "hook-error";
    case RAX_ERR_UNSUPPORTED:
        return "unsupported-operation";
    case RAX_ERR_INTERNAL:
...
void EmuEvents::merge_from(const EmuEvents &other)
{
    ...
    consumed_image_reads.insert(consumed_image_reads.end(), other.consumed_image_reads.begin(),
...
void EmuEvents::normalize()
{
    ...
    std::sort(uses.begin(), uses.end(),
              [&](const auto &a, const auto &b) { return use_key(a) < use_key(b); });
    uses.erase(std::unique(uses.begin(), uses.end(),
                           [&](const auto &a, const auto &b) { return use_key(a) == use_key(b); }),
               uses.end());
    std::sort(edges.begin(), edges.end(),
              [](const ExecEdge &a, const ExecEdge &b)
              {
    ...
    edges.erase(std::unique(edges.begin(), edges.end(),
                            [](const ExecEdge &a, const ExecEdge &b)
                            {
                                return a.run_id == b.run_id && a.seed == b.seed &&
                                       a.sequence == b.sequence && a.from == b.from &&
                                       a.to == b.to && a.kind == b.kind;
                            }),
                edges.end());

    std::sort(execution.begin(), execution.end(),
    ...
    consumed_image_reads.erase(
        std::unique(consumed_image_reads.begin(), consumed_image_reads.end(),
                    [](const ConsumedImageRange &a, const ConsumedImageRange &b)
                    {
                        return a.run_id == b.run_id && a.seed == b.seed && a.addr == b.addr &&
                               a.size == b.size;
                    }),
    ...
    std::sort(data.begin(), data.end(),
              [](const DataAcc &a, const DataAcc &b)
              {
                  return std::tie(a.run_id, a.seed, a.sequence, a.from, a.addr, a.kind, a.size,
                                  a.value, a.scope) < std::tie(b.run_id, b.seed, b.sequence, b.from,
                                                               b.addr, b.kind, b.size, b.value,
                                                               b.scope);
              });
    data.erase(std::unique(data.begin(), data.end(),
                           [](const DataAcc &a, const DataAcc &b)
                           {
                               return a.run_id == b.run_id && a.seed == b.seed &&
                                      a.sequence == b.sequence && a.from == b.from &&
                                      a.addr == b.addr && a.kind == b.kind && a.size == b.size &&
                                      a.value == b.value && a.scope == b.scope;
                           }),
               data.end());
    ...
    {
        std::sort(
            state.regs.begin(), state.regs.end(), [](const RegisterValue &a, const RegisterValue &b)
            { return std::tie(a.reg, a.width, a.value) < std::tie(b.reg, b.width, b.value); });
        state.regs.erase(
            std::unique(state.regs.begin(), state.regs.end(),
                        [](const RegisterValue &a, const RegisterValue &b)
                        { return a.reg == b.reg && a.width == b.width && a.value == b.value; }),
            state.regs.end());
    }

    auto regs_less = [](const std::vector<RegisterValue> &a, const std::vector<RegisterValue> &b)
    {
    ...
    std::sort(states.begin(), states.end(),
              ...
              {
                  const auto ak = std::tie(a.run_id, a.source, a.pc, a.seed, a.sequence, a.kind);
                  const auto bk = std::tie(b.run_id, b.source, b.pc, b.seed, b.sequence, b.kind);
                  if (ak != bk)
                      return ak < bk;
    ...
    states.erase(std::unique(states.begin(), states.end(),
                             [](const StatePoint &a, const StatePoint &b)
                             {
                                 if (a.run_id != b.run_id || a.source != b.source || a.pc != b.pc ||
                                     a.seed != b.seed || a.sequence != b.sequence ||
                                     a.kind != b.kind || a.regs.size() != b.regs.size())
                                     return false;
    ...
    final_writes.erase(std::unique(final_writes.begin(), final_writes.end(),
                                   [](const MemoryBytes &a, const MemoryBytes &b)
                                   {
                                       return a.run_id == b.run_id && a.addr == b.addr &&
                                              a.seed == b.seed && a.scope == b.scope &&
                                              a.bytes == b.bytes;
                                   }),
                       final_writes.end());
}
      ...
      summaries_(summaries)
{
    ...
    uint32_t mode = 0;
    bool is64 = false;
    const HybridEntryMode entry_mode =
        img_.entries.size() == 1 ? img_.entries.front().entry_mode : HybridEntryMode::UNKNOWN;
    if (!arch_params(img_.arch, img_.big_endian, entry_mode, rax_arch, mode, sp, fp, lr, pc, ret,
                     is64))
        return;
    sp_reg_ = sp;
    fp_reg_ = fp;
    ...
    {
        if (reg >= 0 &&
            std::find(capture_regs_.begin(), capture_regs_.end(), reg) == capture_regs_.end())
    ...
    case HybridArch::X86_64:
        for (int index = 0; index < 16; ++index)
            add_capture(RAX_X86_GPR64(index));
        add_capture(RAX_X86_REG_RFLAGS);
        break;
    case HybridArch::X86_32:
        for (int index = 0; index < 8; ++index)
            add_capture(RAX_X86_GPR32(index));
        add_capture(RAX_X86_REG_EFLAGS);
        break;
    case HybridArch::ARM64:
        for (int index = 0; index <= 30; ++index)
            add_capture(RAX_ARM64_X(index));
        add_capture(RAX_ARM64_REG_PSTATE);
        break;
    case HybridArch::ARM32:
        for (int index = 0; index <= 12; ++index)
            add_capture(RAX_ARM_R(index));
        add_capture(RAX_ARM_REG_CPSR);
        break;
    case HybridArch::CORTEX_M:
        for (int index = 0; index <= 12; ++index)
            add_capture(RAX_CM_R(index));
        add_capture(RAX_CM_REG_XPSR);
        break;
    case HybridArch::RISCV64:
        for (int index = 0; index < 32; ++index)
            add_capture(RAX_RISCV_X(index));
        break;
    case HybridArch::HEXAGON:
        for (int index = 0; index < 32; ++index)
            add_capture(RAX_HEX_R(index));
        break;
    default:
        break;
    }
    ...
    static const uint64_t cand64[] = {0x00007ffd00000000ull, 0x0000600000000000ull,
...
bool EmuDriver::map_image()
{
    ...
    for (const SegImage &s : img_.segs)
    {
        ...
        {
            if ((s.perm & 4u) != 0)
                prot |= RAX_PROT_READ; // SEGPERM_READ
            if ((s.perm & 2u) != 0)
                prot |= RAX_PROT_WRITE; // SEGPERM_WRITE
            if ((s.perm & 1u) != 0)
                prot |= RAX_PROT_EXEC; // SEGPERM_EXEC
        }
...
bool EmuDriver::seed_arg_regs(uint64_t seed, const FuncRange *function)
{
    ...
    const HybridFunctionProfile *profile = function != nullptr ? &function->profile : nullptr;
    const size_t implicit = profile != nullptr ? profile->implicit_arguments() : 0;
    const size_t available = implicit < arg_regs_.size() ? arg_regs_.size() - implicit : 0;
    const size_t explicit_count = profile != nullptr && profile->explicit_arguments_known
                                      ? std::min(profile->explicit_arguments, available)
                                      : available;
    const std::vector<HybridSeedValue> corpus =
        seed == 0
            ? std::vector<HybridSeedValue>{}
            : hybrid_seed_argument_corpus(seed, explicit_count, img_.lo, stack_base_, stack_size_);
    for (size_t index = 0; index < corpus.size(); ++index)
    {
        if (api_->reg_write_u64(engine_, arg_regs_[implicit + index], corpus[index].value) !=
            RAX_OK)
            return false;
    }
    ...
    if (profile != nullptr && implicit == 2 && arg_regs_.size() >= 2)
    {
        ...
        if (!checked_add(stack_base_, 0x10000, &self) || !checked_add(self, 0x200, &selector))
            return false;
        ...
        if (api_->mem_write(engine_, selector, selector_text.data(), selector_text.size()) !=
                RAX_OK ||
...
void EmuDriver::capture_final_writes(EmuEvents &out, const HybridConfig &cfg, uint32_t run_id,
...
bool EmuDriver::emulate_from(uint64_t entry, uint64_t func_end, const HybridConfig &cfg,
                             EmuEvents &out, EmuOutcome *outcome, bool record_pcs, uint64_t seed,
...
bool EmuDriver::emulate_region_walk(vm::NativeRegion &region, const HybridConfig &requested,
                                    ...
                                    const EmuInput *input, bool sample_native_instructions)
{
    if (!decoder || !maximum_extensions || maximum_extensions > 64)
    {
        outcome = EmuOutcome{};
        outcome.native_region = true;
        outcome.native_walk = true;
        outcome.native_walk_stop = "invalid_walk_request";
        return false;
    }
    return emulate_region_impl(region, requested, out, outcome, input, &region, &decoder,
                               maximum_extensions, sample_native_instructions);
}
                                        ...
                                        const vm::NativeDecoder &decoder, const EmuInput *input)
{
    if (!decoder || (input && !input->native_objects.empty()))
    {
        outcome = EmuOutcome{};
        outcome.native_region = true;
        outcome.native_temporal_requested = true;
                                    ...
                                    bool native_temporal)
{
    outcome = EmuOutcome{};
    outcome.native_region = true;
    outcome.region_identity = region.identity();
    if (!requested.max_insns || !requested.timeout_ms || !strict_perms_ || !api_ ||
        !api_->mem_read || !region.matches(img_) || region.image_hash() != baseline_image_hash_ ||
        !out.edges.empty() || !out.execution.empty() || !out.data.empty() || !out.states.empty() ||
        !out.final_writes.empty() || !out.consumed_image_reads.empty() ||
        !out.allocations.empty() || !out.uses.empty() || !out.exec_pcs.empty())
        return false;
    HybridConfig cfg = requested;
    cfg.max_insns = std::min(cfg.max_insns, uint64_t(65536));
    if (sample_native_instructions)
        cfg.max_insns = std::min(cfg.max_insns, uint64_t(4096));
    cfg.timeout_ms = std::min(cfg.timeout_ms, uint64_t(1000));
    cfg.want_runtime_strings = native_temporal;
    cfg.want_import_summaries = native_temporal;
                              ...
                              bool sample_native_instructions, bool native_temporal)
{
    ...
    if (input && !input->native_objects.empty())
    {
        ...
        {
            if (object.bytes.empty() || object.bytes.size() > 4096 ||
                object.offset >= object.bytes.size() || object.argument >= input->args.size() ||
                input->args[object.argument] != 0 || !arguments.insert(object.argument).second)
                return false;
        ...
        for (const auto &object : input->native_objects)
        {
            ...
            state.argument = object.argument;
            state.offset = object.offset;
            state.address = cursor;
            state.initial = object.bytes;
    ...
    if (outcome)
        for (const auto &object : outcome->native_objects)
            if (api_->mem_write(engine_, object.address, object.initial.data(),
                                object.initial.size()) != RAX_OK)
                return false;
    ...
    if (lr_reg_ >= 0)
    {
        ...
        const bool thumb_return = img_.arch == HybridArch::CORTEX_M ||
                                  (img_.arch == HybridArch::ARM32 && function != nullptr &&
    ...
    ctx.temporal.byte_budget =
        size_t(std::min<uint64_t>(cfg.max_runtime_bytes, TemporalMemory::total_byte_limit));
    ctx.sp_reg = sp_reg_;
    ctx.lr_reg = lr_reg_;
    ctx.pc_reg = pc_reg_;
    ...
    ctx.record_memory = cfg.want_drefs || cfg.want_runtime_strings || cfg.want_smc_evidence;
    ...
    ctx.dependency_cap = per_run_cap(out.consumed_image_reads.size(), cfg.max_insns);
    ctx.state_cap =
        per_run_cap(out.states.size(), ctx.sample_native_instructions
                                           ? std::min<uint64_t>(3 * cfg.max_insns + 1, 12289)
    ...
    {
        mem_ok = api_->hook_add_mem(engine_, RAX_HOOK_MEM_READ | RAX_HOOK_MEM_WRITE, 1, 0, mem_tr,
                                    &ctx, &mem_id) == RAX_OK;
    }
    ...
    if (code_ok)
    {
        ...
        for (unsigned resumptions = 0; resumptions <= 256; ++resumptions)
        {
            ...
            const int status = api_->emu_start(engine_, begin, sentinel_, timeout_total - elapsed,
                                               cfg.max_insns - used);
            rax_exit slice_exit;
            ...
            if (api_->emu_last_exit(engine_, &slice_exit) == RAX_OK)
            {
                if (exit_value_is_attempted_steps(slice_exit.reason))
                {
                    const uint64_t remaining =
                        std::numeric_limits<uint64_t>::max() - attempted_steps;
                ...
                !ctx.region_code_changed)
            {
                ...
                {
                    outcome->native_walk_stop = "extension_limit";
                    break;
                }
                if (!mem_ok || !inv_ok || ctx.data_truncated || ctx.execution_truncated ||
                    ctx.dependency_truncated)
                {
                    outcome->native_walk_stop = "incomplete_prefix";
                    break;
                }
                const uint64_t source = ctx.region_boundary_source,
                               target = ctx.region_boundary_target;
                const auto *source_head = expanding->at(source);
                if (!source_head || (source_head->flow != RAX_FLOW_INDIRECT_JUMP &&
                                     source_head->flow != RAX_FLOW_INDIRECT_CALL &&
                                     source_head->flow != RAX_FLOW_RETURN))
                {
                    outcome->native_walk_stop = "non_transfer_boundary";
                    break;
                }
                uint64_t pc = 0;
                if (pc_reg_ < 0 || api_->reg_read_u64(engine_, pc_reg_, &pc) != RAX_OK ||
                    pc != target || out.edges.empty() || out.edges.back().from != source ||
                    out.edges.back().to != target || out.edges.back().sequence + 1 != ctx.sequence)
                {
                    outcome->native_walk_stop = "unobserved_transfer";
                    break;
                }
                const auto extension = vm::extend_native_region(*expanding, img_, api_, source,
                                                                target, 4096, *decoder);
                outcome->native_admissions.push_back(
                    {source, target, ctx.sequence - 1, expanding->identity(),
                     extension.region.identity(), extension.added_heads, extension.admitted,
                     extension.reason});
                if (!extension.admitted)
                {
                    outcome->native_walk_stop = extension.reason;
                    break;
                }
    ...
    if (code_ok && outcome != nullptr)
    {
            ...
            if (ex.reason == RAX_STOP_ERROR && api_->engine_errmsg != nullptr)
            {
                ...
                if (api_->engine_errmsg(engine_, detail, sizeof(detail)) > 0)
                {
                    ...
                    std::replace(outcome->engine_error.begin(), outcome->engine_error.end(), '\n',
                                 ' ');
                    std::replace(outcome->engine_error.begin(), outcome->engine_error.end(), '\r',
                                 ' ');
                }
            }
        }
        ...
        outcome->data_trace_complete =
            ctx.record_memory && mem_ok && code_ok && outcome->stop_valid &&
            stop_preserves_observed_context(outcome->stop_reason) && !ctx.data_truncated &&
            !ctx.data_filtered && !ctx.execution_truncated && !ctx.permission_violation &&
            !ctx.cancellation_requested && !ctx.escaped_image && !ctx.unmodeled_external &&
            !ctx.environment_model_failure && !ctx.region_code_changed &&
            ctx.summarized_calls == 0 && !synthetic_entry_context;
        outcome->consumed_context_complete =
            !region && ctx.record_memory && mem_ok && outcome->stop_valid &&
            stop_preserves_observed_context(outcome->stop_reason) && !ctx.execution_truncated &&
            !ctx.dependency_truncated && !ctx.permission_violation && !ctx.cancellation_requested &&
            !ctx.escaped_image && !ctx.function_boundary && !ctx.unmodeled_external &&
            !ctx.environment_model_failure && ctx.summarized_calls == 0 && !synthetic_entry_context;
        outcome->summarized_calls = ctx.summarized_calls;
        outcome->temporal_observation_available = ctx.temporal.enabled && mem_ok;
        outcome->temporal_capture_truncated =
            ctx.temporal.truncated || ctx.temporal.allocation_exhausted;
        outcome->temporal_capture_complete =
            outcome->temporal_observation_available && outcome->conclusive() &&
        ...
        if (outcome->returned && sp_reg_ >= 0)
        {
            ...
            {
                if (sp_final >= sp_entry &&
                    sp_final - sp_entry <= uint64_t(std::numeric_limits<int64_t>::max()))
                {
                    outcome->sp_delta = int64_t(sp_final - sp_entry);
                    outcome->sp_valid = true;
                }
                else if (sp_entry > sp_final &&
                         sp_entry - sp_final <= uint64_t(std::numeric_limits<int64_t>::max()))
                {
    ...
    if (code_ok)
        api_->hook_del(engine_, code_id);
    if (mem_ok)
        api_->hook_del(engine_, mem_id);
    if (inv_ok)
        api_->hook_del(engine_, inv_id);
    ...
    if (code_ok && region && outcome)
    {
        ...
        for (auto &object : outcome->native_objects)
        {
            ...
            object.readable = api_->mem_read(engine_, object.address, object.final.data(),
                                             object.final.size()) == RAX_OK;
            if (!object.readable)
                object.final.clear();
        }
    }
