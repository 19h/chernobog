...
FunctionLog &ensure(DatabaseLog &database, uint64_t function)
{
  ...
  if (found != database.functions.end()) return found->second;
  if (database.functions.size() == function_limit)
  {
    const auto oldest = database.order.front(); database.order.pop_front();
  ...
  auto &log = database.functions[function]; log.generation = identity(); return log;
}
...
{
  if (site == UINT64_MAX) return {};
  const auto *owner = get_func(ea_t(site));
  if (!owner || owner->start_ea != function || !is_code(get_flags(ea_t(site)))) return {};
  ...
  {
    if (!is_loaded(ea_t(site + i))) return {};
...
public:
  ...
  void publish(const solver_evidence::Origin &origin, solver_evidence::Row row) override
  {
    ...
    row["query_id"] = hex(identity()); row["generation"] = hex(log.generation);
    row["function"] = hex(origin.function); row["site"] = hex(origin.site);
    row["phase"] = origin.phase; row["maturity"] = std::to_string(origin.maturity);
    if(origin.capture_revision && origin.transition_check)
    {
      row["capture_revision"]=hex(origin.capture_revision);
      row["run"]=hex(origin.run);row["seed"]=hex(origin.seed);row["sequence"]=hex(origin.sequence);
      row["transition_check"]=hex(origin.transition_check);
    }
    row["site_bytes"] = site_bytes(origin.site, origin.function);
    row["source_contract"] = "matching instruction bytes/owner permit navigation only; whole query applicability remains historical";
    size_t bytes = 0;
    for (const auto &field : row) bytes += field.first.size() + field.second.size();
    if (bytes > byte_limit - database.bytes) { ++log.omitted; return; }
    log.bytes += bytes; database.bytes += bytes;
...
std::string solver_inspection_json(uint64_t function, bool state_only)
{
  ...
  if (found != databases.end())
  {
    ...
    if (entry != found->second.functions.end()) log = &entry->second;
  }
  ...
  out << "{\"schema\":1,\"available\":" << (log ? "true" : "false")
      ...
      << ",\"omitted\":" << (log ? log->omitted : 0)
      << ",\"evicted_functions\":" << (found != databases.end() ? found->second.evicted_functions : 0);
  std::vector<solver_evidence::Row> rows;
  if (log) for (const auto &record : log->records)
  {
    const auto bytes = site_bytes(std::stoull(record.at("site"), nullptr, 16), function);
    auto row = state_only ? solver_evidence::Row{{"query_id", record.at("query_id")}} : record;
    row["source_bytes_current"] = record.at("query_id") != "0x0" && !bytes.empty()
        && bytes == record.at("site_bytes") ? "true" : "false";
    rows.push_back(std::move(row));
  }
  inspection_json_rows(out, "records", rows); out << '}'; return out.str();
}
} // namespace chernobog::hybrid
