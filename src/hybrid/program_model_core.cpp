...
bool SegImage::contains(uint64_t ea) const { return ea >= start && ea < end; }
...
bool SegImage::has_perm(HybridSegPerm required) const
{
...
LoadedByteView SegImage::loaded_view(uint64_t ea, size_t maximum_bytes) const
{
    ...
    const size_t limit = static_cast<size_t>(
        std::min<uint64_t>(end - ea, std::min(maximum_bytes, bytes.size() - begin)));
    size_t length = 0;
    while (length < limit)
    {
...
{
    // hybrid_snapshot() keeps segments sorted. The binary search also behaves
...
const FuncRange *ProgramImage::function_at(uint64_t ea) const
{
    ...
    auto it =
        std::lower_bound(entries.begin(), entries.end(), ea,
                         [](const FuncRange &func, uint64_t value) { return func.start < value; });
...
void hash_chunk_bytes(uint64_t &hash, const ProgramImage &img, const FuncRange &func,
                      const FuncChunk &chunk)
{
...
HybridFunctionProfile hybrid_function_profile_from_name(const std::string &name)
{
    ...
    profile.name = name;
    if (name.size() < 5 || (name[0] != '-' && name[0] != '+') || name[1] != '[' ||
        name.back() != ']')
        return profile;
    ...
    if (separator == std::string::npos || separator + 1 >= name.size() - 1)
        return profile;
    profile.flavor =
        name[0] == '-' ? HybridFunctionFlavor::OBJC_INSTANCE : HybridFunctionFlavor::OBJC_CLASS;
    profile.objc_selector = name.substr(separator + 1, name.size() - separator - 2);
    profile.explicit_arguments =
        size_t(std::count(profile.objc_selector.begin(), profile.objc_selector.end(), ':'));
