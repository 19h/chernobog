...
struct BranchClaimCheck
{
    ...
    {
        return opposing_context_complete != 0 && (verdict == BranchClaimVerdict::COUNTEREXAMPLE ||
...
struct TargetEvidence
{
    ...
    BranchClaimCheck check_branch_claim(uint64_t instruction, bool expected_taken) const;
};
...
hybrid_consensus_native_read_strings(const TargetEvidence &evidence, size_t minimum_length = 4,
                                     size_t maximum_length = 4096);
