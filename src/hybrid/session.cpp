...
const char *job_status_name(EmulationJobStatus status)
{
    switch (status)
    {
    case EmulationJobStatus::COMPLETED:
        return "completed";
    case EmulationJobStatus::CANCELLED:
        return "cancelled";
    case EmulationJobStatus::UNAVAILABLE:
        return "unavailable";
    case EmulationJobStatus::FAILED:
        return "failed";
    default:
        return "unknown";
    }
...
const char *edge_kind_name(ExecEdge::Kind kind)
{
    switch (kind)
    {
    case ExecEdge::Kind::Call:
        return "call";
    case ExecEdge::Kind::Jump:
        return "jump";
    case ExecEdge::Kind::Return:
        return "return";
    default:
        return "unknown";
    }
...
const char *function_flavor_name(HybridFunctionFlavor flavor)
{
    switch (flavor)
    {
    case HybridFunctionFlavor::NATIVE:
        return "native";
    case HybridFunctionFlavor::OBJC_INSTANCE:
        return "objc-instance";
    case HybridFunctionFlavor::OBJC_CLASS:
        return "objc-class";
    default:
        return "unknown";
    }
...
struct Session::Impl
{
    ...
    void report(bool verbose)
    {
        ...
        {
            const size_t coverage_hundredths =
                summary.static_instructions == 0
                    ? 0
                    : (summary.executed_instruction_addresses * size_t(10000)) /
            ...
            msg("[chernobog][rax] function=%a focus=%a generation=%llu "
                ...
                job_status_name(accumulated.status), summary.completed_runs, evidence->runs.size(),
            ...
            msg("[chernobog][rax] entry: flavor=%s name=%s selector=%s "
                ...
                profile.explicit_arguments, profile.explicit_arguments_known ? "known" : "unknown",
            ...
            msg("[chernobog][rax] outcomes: returned=%zu terminal=%zu "
                ...
                summary.instruction_budget_runs, summary.timeout_runs, summary.escaped_image_runs,
                summary.function_boundary_runs, summary.unmodeled_external_runs,
                summary.environment_model_failure_runs, summary.permission_violating_runs,
                summary.external_model_runs, summary.synthetic_entry_context_runs,
            ...
            msg("[chernobog][rax] coverage: physical=%zu/%zu (%zu.%02zu%%) "
                ...
                summary.executed_addresses_without_static_record, summary.conditional_sites,
                summary.conditional_observations, summary.predicate_state_inputs,
                summary.indirect_sites, summary.unique_indirect_targets, summary.indirect_targets);
            msg("[chernobog][rax] static: IDA_heads=%zu physical=%zu "
                ...
                summary.ida_instruction_heads, summary.static_instructions, summary.ida_macro_heads,
                summary.ida_macro_components, summary.smir_effects, summary.decoder_comparisons,
                summary.decoder_disagreements, summary.decoder_disagreement_flags,
                summary.decoder_size_disagreements, summary.decoder_flow_disagreements,
                summary.decoder_target_disagreements, summary.decoder_fallthrough_disagreements);
            msg("[chernobog][rax] memory: observation_available=%zu/%zu "
                ...
                summary.memory_observation_requested_runs, summary.image_reads,
                summary.image_writes, summary.pointer_value_reads, summary.executable_pointer_reads,
                ...
                summary.completed_runs -
                    std::min(summary.completed_runs, summary.context_incomplete_runs),
        ...
        for (const RunObservation &run : evidence->runs)
        {
            msg("[chernobog][rax] run=%u seed=0x%llX ran=%s outcome=%s "
                ...
                "sp_delta=%lld%s\n",
                run.provenance.run_id, static_cast<unsigned long long>(run.provenance.seed),
                run.ran ? "yes" : "no", hybrid_emu_outcome_name(run.outcome),
                hybrid_rax_stop_reason_name(run.outcome.stop_reason), run.outcome.stop_reason,
                ea_t(run.outcome.stop_pc), run.outcome.stop_valid ? "available" : "unavailable",
                ...
                run.outcome.summarized_calls, run.outcome.external_model_used ? "used" : "none",
                run.outcome.synthetic_entry_context ? "synthetic" : "observed/native",
                run.outcome.memory_observation_available   ? "available"
                : run.outcome.memory_observation_requested ? "unavailable"
                                                           : "disabled",
            ...
            if (run.outcome.function_boundary)
            {
                msg("[chernobog][rax] run=%u function-boundary kind=%s source=%a target=%a "
                    ...
                    run.provenance.run_id, edge_kind_name(run.outcome.function_boundary_kind),
            ...
            if (run.outcome.stop_reason == RAX_STOP_ERROR)
            {
                msg("[chernobog][rax] run=%u engine-status=%s(%d)%s%s\n", run.provenance.run_id,
                    hybrid_rax_status_name(run.outcome.stop_status), run.outcome.stop_status,
                    run.outcome.engine_error.empty() ? "" : " detail=",
                    run.outcome.engine_error.c_str());
            }
            const auto input =
                std::find_if(evidence->inputs.begin(), evidence->inputs.end(),
            ...
            if (input != evidence->inputs.end())
            {
                msg("[chernobog][rax] run=%u input=%s positional_args=%zu "
                    ...
                    input->input.arg_overrides.size(), input->input.register_overrides.size(),
                ...
                {
                    msg("[chernobog][rax] run=%u physical-abi-overrides", run.provenance.run_id);
                    const size_t limit = std::min<size_t>(input->input.arg_overrides.size(), 8);
                    for (size_t index = 0; index < limit; ++index)
                    {
                        const EmuInput::ArgOverride &argument = input->input.arg_overrides[index];
            ...
            if (run.outcome.unmodeled_external || run.outcome.environment_model_failure)
            {
                msg("[chernobog][rax] run=%u external target=%a name=%s model=%s\n",
                    run.provenance.run_id, ea_t(run.outcome.external_target),
                    run.outcome.external_name.empty() ? "<unknown>"
                                                      : run.outcome.external_name.c_str(),
        ...
        shown = 0;
        for (const StaticInstructionEvidence &instruction : evidence->static_analysis.instructions)
        {
            const DecoderComparison &comparison = instruction.comparison;
            if (!comparison.comparable ||
                (!comparison.size_disagreement && !comparison.flow_disagreement &&
                 !comparison.target_disagreement && !comparison.fallthrough_disagreement))
                continue;
            msg("[chernobog][rax] decoder-diff %a: IDA(size=%u flow=%s) "
                "rax(size=%u flow=%s)\n",
                ea_t(instruction.address), instruction.ida.size,
                hybrid_decoder_flow_name(instruction.ida.flow), instruction.rax.instruction.size,
        ...
        for (const BranchObservation &branch : evidence->branches)
            ++branches[{branch.instruction, branch.observed_successor, branch.disposition}];
        shown = 0;
        for (const auto &entry : branches)
        {
            ...
            const char *name = disposition == BranchDisposition::TAKEN         ? "taken"
                               : disposition == BranchDisposition::FALLTHROUGH ? "fallthrough"
                                                                               : "other";
            msg("[chernobog][rax] branch %a -> %a: %s (%zu observations)\n", ea_t(instruction),
                ea_t(successor), name, entry.second);
            if (++shown == 20)
                break;
        }
        ...
        {
            msg("[chernobog][rax] indirect %a -> %a (%zu observations)\n", ea_t(entry.first.first),
                ea_t(entry.first.second), entry.second);
            if (++shown == 20)
                break;
        }
    ...
    bool submit_model_replays()
    {
        if (pending || !worker || !image)
            return false;
        std::vector<Z3ModelReplayRequest> requests =
        ...
        for (const Z3ModelReplayRequest &model : requests)
        {
            ...
            run.input.positional_argument_offset = uint32_t(function.profile.implicit_arguments());
        ...
        {
            inputs.insert(inputs.end(), std::make_move_iterator(added.begin()),
    ...
    bool wait_for_pending(const char *purpose)
    {
        ...
        while (pending)
        {
            ...
            const auto quantum = std::min(remaining, std::chrono::milliseconds(quantum_ms));
            EmulationJobResult result;
        ...
        return finish_wait(evidence && hybrid_current_evidence_is_fresh(function.start));
    }
};

Session::Session(int64_t database_id) : impl_(new Impl(this)), database_id_(database_id)
{
...
bool Session::enabled() const { return impl_->config.enabled; }

bool Session::explore_batch_target()
{
    const ea_t address = action_target_address(nullptr);
    return batch && address != BADADDR && explore(nullptr, uint64_t(address));
}
...
bool Session::explore(vdui_t *view, uint64_t fallback_address)
{
    ...
    const ProgramSnapshotStats snapshot =
        hybrid_snapshot_function(*mutable_image, impl_->config, function_start);
    ...
    impl_->static_analysis =
        hybrid_analyze_current_function(api, *mutable_image, impl_->function, impl_->config);
    if (user_cancelled())
    {
    ...
    const EntryInputPlan entry = hybrid_build_entry_inputs(mutable_image->arch, function_start,
                                                           impl_->config.max_callsite_inputs);
    ...
    for (size_t index = 0; index < deterministic_runs; ++index)
    {
        ...
        run.seed = index == 0 ? 0 : uint64_t(index) * UINT64_C(0x9E3779B97F4A7C15) + 1;
        run.record_pcs = true;
        job.runs.push_back(run);
        ConcreteInput input;
    ...
    options.windows_x64 = hybrid_detect_abi(mutable_image->arch) == HybridAbi::X86_64_WIN64;
    options.call_summaries = summaries;
    impl_->worker.reset(
        new EmulationWorkerPool(1, hybrid_make_rax_worker_factory(std::move(options)), 1));
    ...
    msg("[chernobog][rax] Exploring only %a (focus=%a, chunks=%zu, "
        ...
        static_cast<unsigned long long>(snapshot.image_bytes_copied), deterministic_runs,
        entry.inputs.size(), static_cast<unsigned long long>(impl_->pending_ticket));
...
EnsureExploredResult Session::ensure_explored(uint64_t function_start, uint64_t focus_address)
{
    ...
    const bool matching_pending = impl_->pending && impl_->function.start == function_start;
    if (!matching_pending)
    {
        const uint64_t focus = focus_address == UINT64_MAX ? function_start : focus_address;
    ...
    return user_cancelled() ? EnsureExploredResult::CANCELLED : EnsureExploredResult::FAILED;
}
...
void Session::show_last(vdui_t *view) const
{
    ...
    if (impl_->evidence && !hybrid_current_evidence_is_fresh(impl_->evidence->scope.function_start))
    {
...
std::shared_ptr<const TargetEvidence> Session::evidence() const { return impl_->evidence; }

uint64_t Session::explored_function() const { return impl_->function.start; }
...
EnsureExploredResult hybrid_ensure_current_function_explored(uint64_t function_start,
                                                             uint64_t focus_address)
{
    Session *session = hybrid_current_session();
    return session == nullptr ? EnsureExploredResult::UNAVAILABLE
...
struct hybrid_action_handler_t : public action_handler_t
{
    ...
    int idaapi activate(action_activation_ctx_t *context) override
    {
        if (context == nullptr)
            return 0;
        vdui_t *view = context->widget != nullptr ? get_widget_vdui(context->widget) : nullptr;
        chernobog::hybrid::Session *session = chernobog::hybrid::hybrid_current_session();
        if (session == nullptr)
            return 0;
        const ea_t context_address = chernobog::hybrid::action_target_address(context);
        switch (kind)
        {
        case Kind::Explore:
            return session->explore(view, context_address == BADADDR ? UINT64_MAX
                                                                     : uint64_t(context_address))
                       ? 1
                       : 0;
        case Kind::Show:
            if (is_idaq() && process_ui_action("chernobog:evidence_view"))
                return 1;
    ...
    action_state_t idaapi update(action_update_ctx_t *context) override
    {
        ...
        if (context->widget == nullptr)
            return AST_DISABLE_FOR_WIDGET;
        return get_widget_vdui(context->widget) != nullptr ? AST_ENABLE_FOR_WIDGET
...
const action_desc_t hybrid_actions[] = {
    ACTION_DESC_LITERAL("chernobog:rax_explore", "Explore current function with rax",
                        &explore_handler, "Ctrl+Shift+E", nullptr, -1),
    ACTION_DESC_LITERAL("chernobog:rax_show", "Show current-function rax evidence", &show_handler,
                        nullptr, nullptr, -1),
    ACTION_DESC_LITERAL("chernobog:rax_cancel", "Cancel current-function rax exploration",
                        &cancel_handler, nullptr, nullptr, -1),
};
...
bool hybrid_available() { return chernobog::hybrid::hybrid_load_config().enabled; }
...
void hybrid_done()
{
    const auto initialized = hybrid_initialized_databases.find(int64_t(get_dbctx_id()));
...
REGISTER_COMPONENT(hybrid_available, hybrid_active, hybrid_init, hybrid_done, hybrid_attach_popup,
                   "Current-function rax exploration", rax_hybrid, rax_hybrid)
