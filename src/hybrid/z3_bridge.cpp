...
struct Registry
{
    ...
    std::map<int64_t, std::shared_ptr<const DeobfuscationProjection>> deobfuscation_projections;
...
bool byte_identity_matches(const Identity &expected, const char *kind, size_t index,
                           std::string *reason)
{
    if (expected.bytes.empty())
    {
        set_reason(reason, std::string(kind) + " identity " + std::to_string(index) + " is empty");
        return false;
    }
    std::vector<uint8_t> bytes(expected.bytes.size(), 0);
    std::vector<uint8_t> mask((expected.bytes.size() + 7) / 8, 0);
    if (get_bytes(bytes.data(), ssize_t(bytes.size()), ea_t(expected.start), GMB_READALL,
                  mask.data()) < 0)
    {
    ...
    const IdentityComparison comparison =
        hybrid_compare_identity_bytes(expected.bytes, expected.loaded_mask, bytes, mask);
    if (comparison.matches())
        return true;

    std::ostringstream out;
    out << kind << " " << index << " at " << address_string(expected.start + comparison.offset)
        << ": ";
...
bool current_function_shape_matches(const TargetEvidence &evidence,
                                    ...
                                    std::string *reason = nullptr)
{
    ...
    if (current_entry_mode(evidence.architecture, function->start_ea) != evidence.entry_mode)
    {
    ...
    if (profile_policy == EntryProfilePolicy::REQUIRE_EXACT)
    {
        if (current_profile.flavor != expected_profile.flavor ||
            ...
            current_profile.explicit_arguments_known != expected_profile.explicit_arguments_known)
        {
    ...
    if (chunks.size() != expected_identity.size())
    {
        set_reason(reason, "function chunk count changed (expected=" +
                               std::to_string(expected_identity.size()) +
                               ", actual=" + std::to_string(chunks.size()) + ")");
        return false;
    }
    ...
    for (size_t index = 0; index < chunks.size(); ++index)
    {
        ...
        if (current.end_ea <= current.start_ea || uint64_t(current.start_ea) != expected.start ||
            uint64_t(current.end_ea - current.start_ea) != expected.bytes.size())
        {
            set_reason(reason, "function chunk topology changed at index " + std::to_string(index));
            return false;
        }
    }
...
bool current_function_identity_matches(const TargetEvidence &evidence,
                                       ...
                                       std::string *reason = nullptr)
{
    if (!current_function_shape_matches(evidence, expected_identity, expected_profile,
                                        profile_policy, nullptr, reason))
    {
        return false;
    }
    for (size_t index = 0; index < expected_identity.size(); ++index)
        if (!byte_identity_matches(expected_identity[index], "function chunk", index, reason))
            return false;
    return true;
}
...
bool current_function_identity_matches(const TargetEvidence &evidence,
                                       std::string *reason = nullptr)
{
    return current_function_identity_matches(evidence, evidence.function_identity,
                                             evidence.function_profile,
                                             EntryProfilePolicy::REQUIRE_EXACT, reason);
}

bool capture_current_function_identity(const TargetEvidence &source,
                                       ...
                                       std::string *reason = nullptr)
{
    ...
    if (!current_function_shape_matches(source, source.function_identity, source.function_profile,
    ...
    for (size_t index = 0; index < shape.chunks.size(); ++index)
    {
        ...
        if (get_bytes(identity.bytes.data(), ssize_t(size), chunk.start_ea, GMB_READALL,
                      identity.loaded_mask.data()) < 0)
        {
            set_reason(reason, "get_bytes failed while sealing function chunk " +
                                   std::to_string(index) + " at " + address_string(identity.start));
            return false;
        }
...
bool hybrid_publish_evidence(int64_t database_id, std::shared_ptr<const TargetEvidence> evidence)
{
    ...
    if (evidence && valid)
    {
        ...
        state.inspection_revisions[database_id] =
            state.next_inspection_revision == UINT64_MAX ? 0 : ++state.next_inspection_revision;
    }
    else
    {
...
uint64_t hybrid_evidence_snapshot_revision(const std::shared_ptr<const TargetEvidence> &snapshot)
{
    ...
    return snapshot && source != state.evidence.end() && source->second == snapshot &&
                   revision != state.inspection_revisions.end()
               ? revision->second
               : 0;
}
...
bool hybrid_begin_deobfuscation_projection(uint64_t function_start)
{
    ...
    const std::shared_ptr<const TargetEvidence> evidence = registry_evidence(database_id);
...
void hybrid_abandon_deobfuscation_projection(uint64_t function_start)
{
    ...
    if (found != state.deobfuscation_projections.end() && found->second && found->second->source &&
...
{
    if (size == 0 || size > 4096 || address > UINT64_MAX - uint64_t(size))
    {
        return false;
    }
    ...
    {
        if (address >= chunk.start && address - chunk.start <= chunk.bytes.size() &&
    ...
    if (get_bytes(bytes.data(), ssize_t(size), ea_t(address), GMB_READALL, mask.data()) < 0)
    {
        return false;
    }
    ...
    std::lock_guard<std::mutex> lock(state.mutex);
    const auto found = state.deobfuscation_projections.find(database_id);
    if (found == state.deobfuscation_projections.end() || found->second != projection)
    {
        return false;
    }
...
bool hybrid_seal_deobfuscation_projection(uint64_t function_start)
{
    ...
    std::string rejection_reason;
    if (!capture_current_function_identity(*projection->source, &sealed_identity, &sealed_profile,
                                           EntryProfilePolicy::IGNORE_FOR_DISPLAY_ONLY,
                                           &rejection_reason))
    {
        Registry &state = registry();
        std::lock_guard<std::mutex> lock(state.mutex);
    ...
    if (sealed_identity.size() == projection->function_identity.size())
    {
        for (size_t chunk = 0; chunk < sealed_identity.size(); ++chunk)
        {
            ...
            {
                const bool before_loaded =
                    offset / 8 < before.loaded_mask.size() &&
                    (before.loaded_mask[offset / 8] & uint8_t(1u << (offset & 7))) != 0;
                const bool after_loaded =
                    offset / 8 < after.loaded_mask.size() &&
                    (after.loaded_mask[offset / 8] & uint8_t(1u << (offset & 7))) != 0;
                const uint64_t byte_address = after.start + uint64_t(offset);
                if (before_loaded != after_loaded)
                {
                    unauthorized_reason = "loaded state changed at " + address_string(byte_address);
                    break;
                }
                if (before_loaded && before.bytes[offset] != after.bytes[offset])
                {
                    ...
                    {
                        unauthorized_reason =
                            "unregistered function-byte change at " + address_string(byte_address);
                        break;
                    }
                    ++changed_bytes;
                }
    ...
    if (changed_bytes != 0 && !projection->runtime_strings.empty())
    {
        msg("[chernobog][rax] Retained %zu runtime plaintexts across %zu trusted function-byte changes at %a\n",
            projection->runtime_strings.size(), changed_bytes, ea_t(function_start));
    }
    return true;
}
...
bool hybrid_finish_deobfuscation_projection(uint64_t function_start)
{
    ...
    if (found == state.deobfuscation_projections.end() || !found->second ||
        !found->second->source || found->second->source->scope.function_start != function_start)
    {
        return false;
    }
    auto finished = std::make_shared<DeobfuscationProjection>(*found->second);
...
hybrid_current_runtime_strings_for_decompilation(uint64_t function_start)
{
    ...
    std::shared_ptr<const DeobfuscationProjection> projection;
    {
        ...
        const auto projection_found = state.deobfuscation_projections.find(database_id);
    ...
    if (!projection || projection->source != evidence || !projection->source ||
        projection->source->scope.function_start != function_start ||
        !current_function_identity_matches(*projection->source, projection->function_identity,
...
hybrid_current_use_strings_for_decompilation(uint64_t function_start)
{
    ...
    std::shared_ptr<const DeobfuscationProjection> projection;
    {
        ...
        const int64_t database_id = int64_t(get_dbctx_id());
        const auto current = state.evidence.find(database_id);
        if (current != state.evidence.end())
            evidence = current->second;
        const auto display = state.deobfuscation_projections.find(database_id);
        if (display != state.deobfuscation_projections.end())
            projection = display->second;
    }
    if (!evidence || evidence->scope.function_start != function_start)
        return {};
    ...
    for (size_t index = 0; index < evidence->context_identity.size(); ++index)
        if (!byte_identity_matches(evidence->context_identity[index], "consumed use context", index,
                                   nullptr))
            return {};
    return hybrid_consensus_use_strings(*evidence);
}
                                                    ...
                                                    bool expected_taken)
{
    ...
    if (result.snapshot_current)
        result.claim = evidence->check_branch_claim(branch_instruction, expected_taken);
    return result;
}
                                                                       ...
                                                                       uint64_t branch_instruction)
{
    ...
    {
        if (state.kind != StatePoint::Kind::PredicateInput || state.source != branch_instruction)
            continue;
        ...
        {
            const std::string name = native_register_name(evidence->architecture, value.reg);
            if (name.empty())
                continue;
            ...
            if (micro_register == mr_none)
                continue;
            input.registers.push_back(
                Z3ConcreteRegisterInput{int(micro_register), value.width, value.value, name});
        }
        std::sort(input.registers.begin(), input.registers.end(),
                  [](const Z3ConcreteRegisterInput &left, const Z3ConcreteRegisterInput &right)
                  {
