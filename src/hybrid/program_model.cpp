...
bool hybrid_detect_arch(HybridArch &arch_out, bool &big_endian_out)
{
    ...
    if (sdk_cortex_m_id(id) || procname_is(procname, "Cortex-M") ||
        procname_is(procname, "CortexM"))
    {
        arch_out = HybridArch::CORTEX_M;
        return true;
    }
    if (sdk_hexagon_id(id) || procname_is(procname, "QDSP6") || procname_is(procname, "Hexagon"))
    {
                                              ...
                                              const ProgramSnapshotProgressCallback &progress)
{
    ...
    snapshot_progress.stats.segments_total = nsegs <= 0 ? 0 : static_cast<size_t>(nsegs);
    report();
    ...
    for (int i = 0; i < nsegs; ++i)
    {
        ...
        if (length >
            std::numeric_limits<uint64_t>::max() - snapshot_progress.stats.image_bytes_requested)
        {
    ...
    for (int i = 0; i < nsegs; ++i)
    {
        ...
        si.kind = s->type == SEG_XTRN ? HybridSegmentKind::EXTERNAL : HybridSegmentKind::NORMAL;
        ...
        ssize_t got =
            get_bytes(si.bytes.data(), (ssize_t)len, s->start_ea, GMB_READALL, si.mask.data());
        if (got < 0)
        {
...

    {
        ...
        if (old != prior.end() && old->second.hash == func.byte_hash && old->second.generation != 0)
        {
    ...
    if (!snapshot_progress.stats.complete && snapshot_progress.stats.diagnostic.empty())
