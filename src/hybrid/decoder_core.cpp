...
bool known_flow(int32_t flow) { return flow >= RAX_FLOW_FALLTHROUGH && flow <= RAX_FLOW_UNKNOWN; }

bool direct_flow(int32_t flow)
{
    return flow == RAX_FLOW_CALL || flow == RAX_FLOW_BRANCH || flow == RAX_FLOW_COND_BRANCH;
}

bool indirect_flow(int32_t flow)
{
    return flow == RAX_FLOW_INDIRECT_CALL || flow == RAX_FLOW_INDIRECT_JUMP;
}

} // namespace

DecoderArchitecture hybrid_decoder_architecture(HybridArch arch, bool big_endian)
{
    const uint32_t endian = big_endian ? RAX_MODE_BIG_ENDIAN : RAX_MODE_LITTLE_ENDIAN;
    switch (arch)
    {
    case HybridArch::X86_16:
    ...
    case HybridArch::CORTEX_M:
        return {true, RAX_ARCH_CORTEXM, uint32_t(endian | RAX_MODE_THUMB), false};
...
bool hybrid_decoder_mode(const DecoderArchitecture &architecture, DecoderArmState arm_state,
                         uint32_t &mode)
{
    mode = 0;
...
size_t hybrid_decoder_window_size(uint64_t pc, uint64_t chunk_end, size_t maximum_bytes)
{
    ...
    const size_t bounded_remaining =
        remaining > size_limit ? std::numeric_limits<size_t>::max() : size_t(remaining);
    return std::min(maximum_bytes, bounded_remaining);
}

DecoderDirectTarget hybrid_decoder_direct_target(const DecoderInstruction &instruction)
{
    ...
    else if (instruction.flow == RAX_FLOW_BRANCH || instruction.flow == RAX_FLOW_COND_BRANCH)
...
DecoderDecodeResult hybrid_accept_rax_decoded(const rax_decoded &decoded, size_t offered_bytes)
{
    ...
    if (decoded.valid != 1 || decoded.is_indirect > 1 || decoded.has_target > 1 ||
        decoded.size == 0 || size_t(decoded.size) > offered_bytes || !known_flow(decoded.flow))
    {
    ...
    if ((direct && (decoded.has_target == 0 || decoded.is_indirect != 0)) ||
        ...
        (!direct && !indirect && (decoded.has_target != 0 || decoded.is_indirect != 0)))
    {
        result.status = DecoderDecodeStatus::MalformedResult;
        return result;
    }
    ...
    result.instruction.has_fallthrough = decoded.flow == RAX_FLOW_COND_BRANCH;
    result.instruction.fallthrough = decoded.fallthrough;
    return result;
}

DecoderDecodeResult hybrid_decode_one(decltype(&rax_decode) decode, int arch, uint32_t mode,
                                      uint64_t pc, const uint8_t *bytes, size_t byte_count)
{
    DecoderDecodeResult result;
    if (decode == nullptr)
    {
    ...
    return hybrid_accept_rax_decoded(decoded, byte_count);
}

DecoderComparison hybrid_compare_decoders(const DecoderInstruction &left,
                                          const DecoderInstruction &right)
{
    DecoderComparison comparison;
    comparison.left_target = hybrid_decoder_direct_target(left);
    comparison.right_target = hybrid_decoder_direct_target(right);
    ...
    comparison.targets_comparable = comparison.left_target.valid || comparison.right_target.valid;
    ...
    comparison.fallthroughs_comparable = left.has_fallthrough || right.has_fallthrough;
    if (comparison.fallthroughs_comparable)
    {
        comparison.fallthrough_disagreement = left.has_fallthrough != right.has_fallthrough ||
                                              (left.has_fallthrough && right.has_fallthrough &&
                                               left.fallthrough != right.fallthrough);
    }
    return comparison;
}

const char *hybrid_decoder_flow_name(int32_t flow)
{
    switch (flow)
    {
    case RAX_FLOW_FALLTHROUGH:
        return "fallthrough";
    ...
    case RAX_FLOW_TRAP:
        return "trap";
    case RAX_FLOW_SYSCALL:
        return "syscall";
    case RAX_FLOW_UNKNOWN:
        return "unknown";
    default:
        return "invalid";
    }
