...
struct AllocationLifetime
{
  ...
  auto key() const
  {
    return std::tie(run_id, seed, id, generation, address, size, context,
                    site, callee, occurrence, allocated, released, live);
  }
};
...
struct UseSnapshot
{
  ...
  {
    return std::make_tuple(context, site, callee, occurrence, argument,
        producer, model_kind, scope, object_site, object_callee, object_occurrence,
        object_size, offset);
  }
  auto witness_key() const
  {
    return std::tie(run_id, seed, sequence, allocation_id, generation,
                    address, observed_size, status, bytes);
  }
};
...
public:
  ...
  uint64_t allocate(uint64_t size, uint64_t site, uint64_t callee,
                    uint64_t sequence)
  {
    ...
    for ( auto &block : blocks_ )
      ...
      { selected = &block; break; }
    if ( selected == nullptr )
    {
      const uint64_t next = blocks_.empty() ? heap_begin
          : blocks_.back().address + blocks_.back().capacity;
      if ( next > heap_end || capacity > heap_end - next ) return 0;
  ...
  bool release(uint64_t address, uint64_t sequence)
  {
    ...
    const auto block = std::lower_bound(blocks_.begin(), blocks_.end(), address,
        [](const Block &block, uint64_t value) { return block.address < value; });
    if ( block == blocks_.end() || block->address != address ) return false;
  ...
  UseCaptureStatus identify(uint64_t address, uint64_t size,
                            UseSnapshot *use = nullptr) const
  {
    auto next = std::upper_bound(blocks_.begin(), blocks_.end(), address,
        [](uint64_t value, const Block &block) { return value < block.address; });
    if ( next != blocks_.begin() )
    {
      const auto &block = *--next;
      if ( address - block.address >= block.capacity ) return UseCaptureStatus::UNKNOWN_OBJECT;
      ...
      if ( !allocation.live ) return UseCaptureStatus::OUTSIDE_LIFETIME;
  ...
  void capture(uint64_t site, uint64_t callee, int argument,
               UseProducer producer, DataScope scope, uint64_t address,
               const uint8_t *bytes, size_t available, uint64_t observed_size,
               uint64_t sequence, uint8_t model_kind = 0)
  {
    if ( !enabled ) return;
    if ( use_attempts_ >= use_limit ) { truncated = true; return; }
    ++use_attempts_;
    ...
    use.run_id = run_id;
    use.seed = seed;
    if ( invalid_span ) use.status = UseCaptureStatus::OBJECT_BOUNDARY;
    ...
    if ( bytes != nullptr && count != 0 ) use.bytes.assign(bytes, bytes + count);
    ...
    if ( use.bytes.size() < observed_size )
    {
      ...
      else use.status = UseCaptureStatus::INCOMPLETE_VALUE;
    }
    uses.push_back(std::move(use));
  }

private:
  struct Block { uint64_t address, capacity, generation; size_t current; };
