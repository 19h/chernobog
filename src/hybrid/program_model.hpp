...
struct HybridFunctionProfile
{
    ...
    size_t implicit_arguments() const { return flavor == HybridFunctionFlavor::NATIVE ? 0 : 2; }
    size_t total_arguments() const { return implicit_arguments() + explicit_arguments; }
};
...
using ProgramSnapshotProgressCallback = std::function<void(const ProgramSnapshotProgress &)>;
