...
DecoderInstruction project_ida(ea_t address, const insn_t &instruction)
{
    ...
    nonflow_targets.erase(std::unique(nonflow_targets.begin(), nonflow_targets.end()),
                          nonflow_targets.end());
    ...
    if (call)
        ...
        result.flow =
            direct ? (flow ? RAX_FLOW_COND_BRANCH : RAX_FLOW_BRANCH) : RAX_FLOW_INDIRECT_JUMP;
    else
        result.flow = RAX_FLOW_FALLTHROUGH;

    result.indirect =
        result.flow == RAX_FLOW_INDIRECT_CALL || result.flow == RAX_FLOW_INDIRECT_JUMP;
...
DecoderArmState arm_state_at(const DecoderArchitecture &architecture, int t_register, ea_t address)
{
...
StaticAnalysisResult hybrid_analyze_current_function(const RaxApi *api, const ProgramImage &image,
                                                     ...
                                                     const HybridConfig &config)
{
            ...
            if (is_code(flags) && is_head(flags))
            {
                ...
                if (decode_insn(&instruction, address) > 0)
                {
                    ...
                    while (component_offset < instruction.size)
                    {
                        ...
                        const DecoderArmState state =
                            arm_state_at(architecture, t_register, ea_t(component_address));
                        ...
                        if (bytes.size != 0 && hybrid_decoder_mode(architecture, state, item.mode))
                        {
                            ...
                            if (item.rax.status == DecoderDecodeStatus::Valid)
                            {
                                ...
                                {
                                    item.comparison =
                                        hybrid_compare_decoders(item.ida, item.rax.instruction);
    ...
    std::sort(result.instructions.begin(), result.instructions.end(),
              [](const StaticInstructionEvidence &left, const StaticInstructionEvidence &right)
