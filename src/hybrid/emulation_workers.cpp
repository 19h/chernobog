...
EmulationJobResult cancelled_result(uint64_t ticket, uint64_t function_start, const char *reason)
{
...
EmulationJobResult unavailable_result(uint64_t ticket, uint64_t function_start, const char *reason)
{
    EmulationJobResult result;
    result.ticket = ticket;
    result.function_start = function_start;
...
EmulationCancellation::EmulationCancellation(std::shared_ptr<const void> state, uint64_t generation)
...
bool EmulationCancellation::cancelled() const noexcept
{
    ...
    return state == nullptr || state->generation.load(std::memory_order_acquire) != generation_;
}
...
struct EmulationWorkerPool::Impl
{
    ...
    explicit Impl(size_t requested, EmulationExecutorFactory f, size_t max_queued)
        : requested_workers(requested), factory(std::move(f)), max_queued_jobs(max_queued),
          cancellation(std::make_shared<CancellationState>())
    {
    }
    ...
    void drain_queued_locked(EmulationJobStatus status, const char *reason)
    {
        while (!queue.empty())
        {
            ...
            queue.pop_front();
            EmulationJobResult result =
                status == EmulationJobStatus::UNAVAILABLE
...
EmulationWorkerPool::EmulationWorkerPool(size_t worker_count, EmulationExecutorFactory factory,
                                         size_t max_queued_jobs)
{
    ...
    catch (...)
    {
        ...
        const size_t missing =
            impl_->requested_workers > created ? impl_->requested_workers - created : 0;
...
EmulationWorkerPool::~EmulationWorkerPool() { shutdown(); }
...
bool EmulationWorkerPool::try_submit(EmulationJob job, uint64_t *ticket)
{
    ...
    if (impl_->initialized_workers == impl_->requested_workers && impl_->available_workers == 0)
    {
...
bool EmulationWorkerPool::wait_take_next(EmulationJobResult &out, std::chrono::milliseconds timeout)
{
    std::unique_lock<std::mutex> lock(impl_->mutex);
    const bool ready = impl_->cv.wait_for(
        lock, timeout,
...
void EmulationWorkerPool::cancel_pending()
{
    ...
    impl_->drain_queued_locked(EmulationJobStatus::CANCELLED, "emulation generation cancelled");
    impl_->cv.notify_all();
}
