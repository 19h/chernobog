...
std::vector<EmuCallSummary> hybrid_collect_call_summaries(const FuncRange &function,
                                                          size_t maximum_instruction_heads)
{
    ...
    for (const FuncChunk &chunk : function.chunks)
    {
        ...
        while (address != BADADDR && address < end && visited < maximum_instruction_heads)
        {
            ...
            if (is_code(flags) && is_head(flags))
            {
                ...
                {
                    xrefblk_t reference;
                    for (bool ok = reference.first_from(address, XREF_CODE); ok;
                         ok = reference.next_from())
                    {
                        ...
                        out.push_back(EmuCallSummary{uint64_t(reference.to),
                                                     kind.value_or(EmuSummaryKind::UNMODELED),
                                                     name.c_str()});
                    }
                }
            }
    ...
    out.erase(
        std::unique(out.begin(), out.end(), [](const EmuCallSummary &a, const EmuCallSummary &b)
