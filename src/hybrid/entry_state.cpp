...
FunctionTypePlan function_type_plan(ea_t function_start)
{
    ...
    if (!get_tinfo(&type, function_start) || !type.get_func_details(&details, GTD_CALC_ARGLOCS))
        return plan;
    plan.available = true;
...
int positional_register(const HybridAbiLayout &abi, int ida_register)
{
...
bool load_x86_push_value(ea_t instruction_ea, const op_t &operand, uint64_t *out)
{
    switch (operand.type)
    {
    case o_imm:
    ...
    case o_mem:
        if (is_mapped(operand.addr) && is_loaded(operand.addr) &&
            operand.addr <= std::numeric_limits<ea_t>::max() - 3 && is_loaded(operand.addr + 3))
        {
...
std::vector<uint64_t> x86_stack_arguments(ea_t callsite, size_t requested, uint64_t seed,
                                          size_t *known_values)
{
...
{
    return hybrid_abi_for_arch(arch, arch == HybridArch::X86_64 && inf_get_filetype() == f_PE);
}
...
{
    EntryInputPlan plan;
    plan.abi = hybrid_detect_abi(arch);
    plan.abi_name = hybrid_abi_name(plan.abi);
    if (max_inputs == 0)
        return plan;

    const HybridAbiLayout &regs = hybrid_abi_layout(plan.abi);
    ...
    for (bool ok = xb.first_to((ea_t)function_start, XREF_CODE); ok; ok = xb.next_to())
    {
        ...
        for (int ida_register : type_plan.register_arguments)
        {
            ...
            if (position >= 0)
            {
                const auto duplicate =
                    std::find_if(input.arg_overrides.begin(), input.arg_overrides.end(),
                                 [&](const EmuInput::ArgOverride &arg)
                                 { return arg.index == uint32_t(position); });
        ...
        if (plan.abi == HybridAbi::X86_32)
        {
            size_t known = 0;
            if (!type_plan.available || type_plan.stack_arguments != 0)
                input.stack_args =
                    x86_stack_arguments(xb.from, type_plan.stack_arguments, input.seed, &known);
            if (known != 0)
