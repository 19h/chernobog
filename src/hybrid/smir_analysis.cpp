...
SmirInstructionAnalysis hybrid_analyze_instruction_effects(const RaxApi *api,
                                                           const ProgramImage &image,
                                                           uint64_t instruction, uint32_t mode,
                                                           size_t maximum_bytes)
{
    ...
    const LoadedByteView bytes =
        image.loaded_view(instruction, std::min(kMaximumInstructionBytes, maximum_bytes));
    if (bytes.size == 0)
    {
    ...
    size_t required = 0;
    rax_status status =
        api->analyze(result.arch, result.mode, instruction, bytes.data, bytes.size, &result.summary,
                     result.effects.data(), result.effects.size(), &required);
    ...
    if (status == RAX_ERR_BOUNDS)
    {
        ...
        status = api->analyze(result.arch, result.mode, instruction, bytes.data, bytes.size,
                              &result.summary, result.effects.data(), result.effects.size(),
                              &retry_required);
        if (retry_required != required || !summary_abi_valid(result.summary, required))
        {
            result.effects.clear();
            result.status = SmirStatus::INVALID_ABI;
            return result;
