...
bool cancellation_probe(const void *user)
{
    const EmulationCancellation *token = static_cast<const EmulationCancellation *>(user);
...
{
  public:
    explicit RaxEmulationExecutor(const RaxWorkerOptions &options) : image_(options.image)
    {
        ...
        driver_.reset(new EmuDriver(options.api, *image_, options.strict_perms, options.windows_x64,
                                    options.call_summaries));
    ...
    bool available() const noexcept override { return driver_ != nullptr; }

    std::string unavailable_reason() const override { return unavailable_; }
                               ...
                               const EmulationCancellation &cancel) override
    {
        ...
        for (const EmulationRunRequest &request : job.runs)
        {
            ...
            const uint32_t effective_run =
                request.has_input ? request.input.run_id : request.run_id;
        ...
        for (const EmulationRunRequest &request : job.runs)
        {
            ...
            run.ran =
                driver_->emulate_from(job.function.start, job.function.end, job.config, run.events,
                                      &run.outcome, request.record_pcs, request.seed,
                                      request.run_id, input, cancellation_probe, &cancel);
            if (!run.ran)
            {
