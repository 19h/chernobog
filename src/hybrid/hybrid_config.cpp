...
size_t env_size(const char *name, size_t fallback)
{
    ...
    return parsed > uint64_t(std::numeric_limits<size_t>::max()) ? fallback : size_t(parsed);
}

int env_int(const char *name, int fallback)
{
    const uint64_t parsed = env_u64(name, uint64_t(fallback));
    return parsed > uint64_t(std::numeric_limits<int>::max()) ? fallback : int(parsed);
}
...
HybridConfig hybrid_load_config()
{
    ...
    if (level < int(HybridLogLevel::QUIET))
        level = int(HybridLogLevel::QUIET);
    if (level > int(HybridLogLevel::TRACE))
        level = int(HybridLogLevel::TRACE);
    ...
    config.max_callsite_inputs =
        env_size("CHERNOBOG_RAX_MAX_CALLSITE_INPUTS", config.max_callsite_inputs);
    config.max_image_bytes = env_u64("CHERNOBOG_RAX_MAX_IMAGE_BYTES", config.max_image_bytes);
    config.max_static_instructions =
        env_size("CHERNOBOG_RAX_MAX_STATIC_INSNS", config.max_static_instructions);
    ...
    config.want_runtime_strings =
        env_bool("CHERNOBOG_RAX_RUNTIME_STRINGS", config.want_runtime_strings);
    config.want_smc_evidence = env_bool("CHERNOBOG_RAX_SMC_EVIDENCE", config.want_smc_evidence);
    config.want_import_summaries =
        env_bool("CHERNOBOG_RAX_IMPORT_SUMMARIES", config.want_import_summaries);
    config.strict_perms = env_bool("CHERNOBOG_RAX_STRICT_PERMS", config.strict_perms);
    config.max_runtime_bytes = env_u64("CHERNOBOG_RAX_MAX_RUNTIME_BYTES", config.max_runtime_bytes);
    ...
    if (config.explore_runs < 1)
        config.explore_runs = 1;
    if (config.explore_runs > 32)
        config.explore_runs = 32;
    if (config.max_callsite_inputs > 32)
        config.max_callsite_inputs = 32;
    if (config.max_image_bytes < (1ull << 20))
        config.max_image_bytes = 1ull << 20;
    if (config.max_image_bytes > (4ull << 30))
        config.max_image_bytes = 4ull << 30;
    if (config.max_static_instructions == 0)
        config.max_static_instructions = 1;
    if (config.max_static_instructions > 1000000)
        config.max_static_instructions = 1000000;
    if (config.poll_ms < 10)
        config.poll_ms = 10;
    if (config.poll_ms > 1000)
        config.poll_ms = 1000;
