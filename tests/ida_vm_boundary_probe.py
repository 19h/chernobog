        ...
        result.append(
            (
                ...
                (
                    None
                    if f is None
                    else (int(f.start_ea), int(f.end_ea), int(f.flags), int(f.tailqty))
                ),
                (
                    None
                    if chunk is None
                    else (int(chunk.start_ea), int(chunk.end_ea), int(chunk.flags))
                ),
                None if seg is None else (int(seg.bitness), int(seg.perm)),
                refs(ea, True),
                refs(ea, False),
            )
        )
...
def inspect(name, root):
    ...
    check(
        name + " first inspection preserves independent native inventory",
        addresses <= scope and initial == snapshot(scope),
    )
    before = snapshot(addresses)
    second = api(root)
    check(
        name + " repeatable read-only inspection", first == second and before == snapshot(addresses)
    )
    check(name + " no execution admission", not second["execution_admitted"])
    check(
        name + " bounded output",
        len(second["nodes"]) <= 1024
        and len(second["edges"]) <= 2048
        and second["incoming_examined"] <= 8192,
    )
    accepted = [n for n in second["nodes"] if not n["rejection"]]
    check(
        name + " source bytes retained",
        all(
            n["bytes"] == ida_bytes.get_bytes(int(n["site"], 0), int(n["size"])).hex()
            for n in accepted
        ),
    )
    check(
        name + " decoded counts consistent",
        len(accepted) == second["decoded_heads"]
...
try:
    ...
    if os.environ.get("CHERNOBOG_BOUNDARY_CONTROLS") == "1":
        ...
        check(
            "ownerless native path reaches unresolved RET",
            a["decoded_heads"] == 3
            and a["ownerless_heads"] == 2
        ...
        check(
            "RET xref never followed",
            len(a["nodes"]) == 3 and all(int(n["site"], 0) != other for n in a["nodes"]),
        )
        ...
        check(
            "spurious internal xref reported",
            "unverified_internal_xref" in reasons(inspect("internal_xref", root)),
        )
        ...
        check(
            "shared tail reported", "shared_function_tail" in reasons(inspect("shared_tail", root))
        )
        ...
        check(
            "decoded branch does not require IDB xref",
            a["decoded_heads"] == 3 and any(int(e["target"], 0) == body for e in a["edges"]),
        )
        ...
        check(
            "head cap explicit",
            a["head_limit"] and len(a["nodes"]) == 1024 and "head_limit" in reasons(a),
        )
...
(Path(os.environ["IDAUSR"]).parent / "vm_boundaries.json").write_text(
