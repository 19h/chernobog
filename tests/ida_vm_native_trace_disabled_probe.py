...
try:
    ...
    checks.append(
        ...
        and result["scope"] == "native-region"
        and result["reason"] == "native decoder/emulator unavailable"
    )
    request = json.dumps(
        {"args": ["0x0"], "objects": [{"argument": 0, "offset": 0, "bytes": "00"}]}
    )
    assert not ida_expr.eval_idc_expr(
        value, ida_idaapi.BADADDR, f"chernobog_vm_trace_input({ea}, 0, {json.dumps(request)})"
    )
    result = json.loads(value.c_str())
    checks.append(
        result["available"] is False
    ...
    assert not ida_expr.eval_idc_expr(
        value, ida_idaapi.BADADDR, f"chernobog_vm_trace_walk({ea}, 0, {json.dumps(request)})"
    )
    result = json.loads(value.c_str())
    checks.append(
        result["available"] is False
    ...
    assert not ida_expr.eval_idc_expr(
        value, ida_idaapi.BADADDR, f"chernobog_vm_trace_check({ea}, 0, {json.dumps(request)})"
    )
    result = json.loads(value.c_str())
    checks.append(
        result["available"] is False
