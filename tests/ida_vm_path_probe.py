...
try:
    ...
    expected = {
        "corpus_transform": (11, 0, 8, 10, "backward"),
        "corpus_branch": (6, 8, 7, 10, "forward"),
    }
    ...
    for name, ea in entries.items():
        ...
        check(
            name + " all source bytes retained",
            candidate["bytes"]
            == "".join(ida_bytes.get_bytes(address, size).hex() for address, size in spans),
        )
        check(
            name + " physical jumps represented",
            any(a + size != b for (a, size), (b, _) in zip(spans, spans[1:])),
        )
        check(
            name + " ownerless code stays ownerless",
        ...
        check(
            name + " source-specific roles and direction",
            roles == expected[name][:4] and candidate["direction"] == expected[name][4],
        )
        check(
            name + " complete local effect summary",
            len(full["summaries"]) == 1 and full["summary_bindings"][0]["status"] == "reference",
        )
        summary = full["summaries"][0]
        check(
            name + " stack and memory effects preserved",
            summary["accesses"] == "5" and "ordered" in summary["memory"],
        )
        check(
            name + " no execution admission",
            "no VM execution" in candidate["ownership"] and "not admitted" in summary["transition"],
        )
        check(
            name + " no budget exhaustion",
            not full["path_scan_truncated"] and not full["reachability_truncated"],
        )
        check(
            name + " read-only repeatable recognition",
            plain["records"] == full["records"] == api("chernobog_vm_regions", ea)["records"],
        )
        ...
        if module and ida_kernwin.is_idaq() and name == "corpus_transform":
            ...
            check(
                "GUI recognition remains current after event processing",
                candidate["vm_candidate"] in form.vm_current,
            )
            check(
                "GUI shows complete local model and path provenance",
                "Modeled local effects: reference" in form.detail.toPlainText()
                and "instruction_spans" in form.detail.toPlainText(),
            )
            check(
                "GUI keeps candidate separate from execution", not form.current and not form.edges
            )
        ...
        if module:
            check(
                name + " UI rejects remote stale path",
                not module.current_vm_candidates(full, changed),
            )
        ida_bytes.patch_byte(remote, before)
        check(
            name + " exact restoration restores recognition",
            api("chernobog_vm_regions", ea)["records"] == plain["records"],
        )
        ...
        check(
            name + " alternate entry rejects path", not api("chernobog_vm_regions", ea)["records"]
        )
        ida_xref.del_cref(other, remote, False)
        check(
            name + " alternate-entry removal restores path",
            api("chernobog_vm_regions", ea)["records"] == plain["records"],
        )
        ...
        check(
            name + " foreign function is a traversal stop",
            not foreign["records"] and foreign["foreign_function_stops"] >= 1,
        )
        ida_funcs.del_func(remote)
        check(
            name + " removing foreign owner restores path",
            api("chernobog_vm_regions", ea)["records"] == plain["records"],
        )
    base = (
    ...
    check(
        "per-path exhaustion is explicit", not budget["records"] and budget["path_scan_truncated"]
    )
    check(
        "path and reachable-head budgets hold",
        budget["path_steps"] <= 8192 and budget["reachable_heads"] <= 1024,
    )
    if module and ida_kernwin.is_idaq():
        ...
        QtWidgets.QApplication.processEvents()
        check(
            "GUI reports path scan exhaustion", "scan truncated True" in budget_form.status.text()
        )
        budget_form.Close(ida_kernwin.PluginForm.WCLS_SAVE)
        QtWidgets.QApplication.processEvents()
        check(
            "GUI budget form closes its poller",
            budget_form.closed and not budget_form.timer.isActive(),
        )
...
(Path(os.environ["IDAUSR"]).parent / "vm_paths.json").write_text(
