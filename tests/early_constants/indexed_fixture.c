...
__attribute__((noinline)) uint32_t early_indexed_read(uint32_t index)
{
    return early_constant_table[index & 3];
}

__attribute__((noinline)) uint32_t early_fixed_read(void) { return early_constant_table[2]; }

__attribute__((noinline)) uint32_t early_known_index_read(void)
{
