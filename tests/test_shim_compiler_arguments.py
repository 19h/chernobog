...
class ShimCompilerArgumentsTests(unittest.TestCase):
    def configure_and_capture(self, generator):
        with tempfile.TemporaryDirectory(prefix="chernobog-shim-flags-") as directory:
            ...
            project.write_text(
                "\n".join(
                    [
                        ...
                        "add_test(NAME capture COMMAND [=[%s]=] [=[%s]=]"
            ...
            subprocess.run(
                [CMAKE, "-S", str(temporary), "-B", str(build), "-G", generator],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            for configuration in configurations:
                subprocess.run(
                    [CTEST, "--test-dir", str(build), "-C", configuration, "--output-on-failure"],
                    check=True,
                    capture_output=True,
                    text=True,
                )
                ...
                flags = [
                    value.removeprefix("--cxx-flag=")
                    for value in arguments
                    if value != "--cxx-flag="
                ]
                selected = (
                    ["-Og", "-DDEBUG_TEXT=debug;value>$<0:literal>"]
                ...
                self.assertEqual(
                    ...
                    [
                        "--driver-mode=g++",
                        "-DCOMMON=1",
                        "-DTEXT=space;value>$<0:literal>",
                        "-fsanitize=undefined",
                        *selected,
                        "--target=x86_64-apple-darwin",
                        "--gcc-toolchain=/toolchain with space",
                        "--sysroot=/generic sysroot",
                        "-arch",
                        "x86_64",
                        "-isysroot",
                        "/SDK with space",
