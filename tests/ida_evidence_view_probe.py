...
try:
    ...
    check("graph separates encodings and witnesses", {r["truth"] for r in snapshot["edges"]} == {"encoding", "witness"})
    check("allocation generations retained", {r["generation"] for r in snapshot["lifetimes"]} >= {"0x1", "0x2"})
    check("modeled uses retain bytes", any(r.get("producer") == "modeled-argument" and r.get("bytes_hex")
                                          for r in snapshot["events"]))
    check("run stops and model provenance", len(snapshot["runs"]) == 4
          and all(r["modeled"] == "true" and r["returned"] == "true" for r in snapshot["runs"]))
    tuples = [(int(r["run"], 0), int(r["seed"], 0), int(r["sequence"], 0)) for r in snapshot["events"]]
    ...
    check("stale capture remains inspectable", stale["available"] and not stale["fresh"]
          and stale["generation"] == snapshot["generation"] and stale["events"] == snapshot["events"])
    check("live state rejects consumed-key patch", not json.loads(evaluate(f"chernobog_evidence_state({target})"))["fresh"])
    ...
    check("foreign function rejected", not json.loads(evaluate(f"chernobog_evidence_view({other})"))["available"])
    ...
    if module_path:
        ...
        check("UI rejects superseded generation", not module.current_snapshot(snapshot, dict(state, generation="0x0")))
        check("UI rejects reused generation with new publication", not module.current_snapshot(snapshot, dict(state, revision="0x0")))
        ...
        check("linked lifetime filter", selected and {r["kind"] for r in selected} >= {"allocate", "use", "release"})
        if ida_kernwin.is_idaq():
            ...
            check("graph selection links events", form.tables["events"].topLevelItemCount()
                  == len(module.matching_events(snapshot, site=selected[0]["site"])))
            form.select_record(selected[0])
            check("selection includes scope detail", snapshot["generation"] in form.detail.toPlainText())
            ida_bytes.patch_byte(key, original_key ^ 1)
            form.poll()
            check("Qt stale state disables navigation", not form.current and not form.jump.isEnabled()
            ...
            check("allocation selection links lifetime events", form.tables["events"].topLevelItemCount() == len(selected))
            form.tables["runs"].setCurrentItem(form.tables["runs"].topLevelItem(0))
            first_run = snapshot["runs"][0]
            check("run selection links ordered events", form.tables["events"].topLevelItemCount()
            ...
            form.select_record(selected[0])
            QtWidgets.QApplication.processEvents()
            node_bounds = form.graph.mapFromScene(form.nodes[selected[0]["site"]].sceneBoundingRect()).boundingRect()
            check("selected node visible after layout", form.graph.viewport().rect().intersects(node_bounds))
            ...
            QtWidgets.QApplication.processEvents()
            check("registered action opens current function", dispatched and len(companion.forms) == 1
            ...
            companion.term()
            QtWidgets.QApplication.processEvents()
            check("companion termination closes workspace", all(item.closed for item in companion.forms.values()))
    evaluate("chernobog_rax_clear()")
    check("cleared capture unavailable", not json.loads(evaluate(f"chernobog_evidence_view({target})"))["available"])
    evaluate(f"chernobog_rax_explore({target})")
    renewed = json.loads(evaluate(f"chernobog_evidence_state({target})"))
    check("republication receives a distinct revision", renewed["fresh"] and renewed["revision"] != snapshot["revision"])
    evaluate("chernobog_rax_clear()")
...
line = "[chernobog][evidence-view] " + ("FAIL " + "; ".join(errors) if errors else "PASS checks=%d" % len(checks))
