...
CASES = {
    "np_plain_add": (3, False),
    "np_plain_nop": (1, False),
    "np_rep_add": (1, True),
    "np_repne_add": (1, True),
    "np_pause": (2, False),
    "np_rep_ret": (2, False),
    "np_rep_movsb": (2, False),
    "np_repne_cmpsb": (2, False),
    "np_mandatory_sse": (4, False),
    "np_rex_train": (5, False),
...
def instruction_record(instruction):
    return {
        ...
        "auxpref": int(instruction.auxpref),
        "segpref": int(instruction.segpref),
        "insnpref": int(instruction.insnpref),
        "flags": int(instruction.flags),
        "operands": [
            {
                key: int(getattr(operand, key))
                for key in (
                    "type",
                    "dtype",
                    "reg",
...
try:
    ...
    for name, (expected_size, redundant) in CASES.items():
        ...
        for _ in range(REPEATS):
            ...
            if size != expected_size:
                raise RuntimeError("%s: decoded size %d, expected %d" % (name, size, expected_size))
    ...
    finish(
        0,
        "PASS cases=%d decode_calls=%d output=%s"
