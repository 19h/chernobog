...
records, errors = [], []
checks = []
names = ("transform_bytes", "transform_words", "transform_byte_utf16",
         "transform_wrong_rotate", "transform_wrong_index", "transform_unknown_key",
         "transform_missing_terminator", "transform_narrow_rotate", "transform_word_byte_index",
         "transform_mutable", "transform_alias", "transform_extra_write")
op_names = {getattr(ida_hexrays, name): name for name in dir(ida_hexrays)
...
class Shapes(ida_hexrays.ctree_visitor_t):
    ...
    def visit_expr(self, expression):
        item = {"op": op_names.get(expression.op, str(expression.op)),
                "ea": int(expression.ea), "type": str(expression.type),
    ...
    def visit_insn(self, instruction):
        self.instructions.append({"op": op_names.get(instruction.op, str(instruction.op)),
...
try:
    ...
    for name in names:
        ...
        lines = display(cfunc)
        records.append({"name": name, "entry": int(ea), "text": str(cfunc),
                        "display": lines,
                        "expressions": shapes.expressions, "instructions": shapes.instructions})
        if os.environ.get("CHERNOBOG_EXPECT_STRING_TRANSFORMS") != "1":
            continue
        expected = {"transform_bytes": (8, 9, "UTF-8", "VMP byte"),
                    "transform_words": (16, 5, "UTF-16LE", "VMP\u03A9"),
                    "transform_byte_utf16": (8, 10, "UTF-16LE", "Wide")}.get(name)
        annotations = [line for line in lines if "rot32-xor[" in line]
        if expected:
            bits, units, encoding, text = expected
            check(name + " proven unit/key/bound/encoding", len(annotations) == 1
            ...
            check(name + " cipher patch invalidates", not any("rot32-xor[" in line for line in display(cfunc)))
            ida_bytes.patch_byte(source, original)
            cfunc.refresh_func_ctext()
            check(name + " exact cipher restoration", sum("rot32-xor[" in line for line in display(cfunc)) == 1)
            ...
            cfunc.refresh_func_ctext()
            check(name + " mutable source rejected", not any("rot32-xor[" in line for line in display(cfunc)))
            ...
            cfunc.refresh_func_ctext()
            check(name + " native patch invalidates", not any("rot32-xor[" in line for line in display(cfunc)))
            ida_bytes.patch_byte(ea, code_byte)
            cfunc.refresh_func_ctext()
            check(name + " exact native restoration", sum("rot32-xor[" in line for line in display(cfunc)) == 1)
        else:
            check(name + " abstains", not annotations)
        active = cfunc.user_cmts
        saved = ida_hexrays.restore_user_cmts(ea)
        check(name + " no saved/saveable comments", (active is None or active.size() == 0)
              and (saved is None or saved.size() == 0))
        check(name + " unchanged native bytes", ida_bytes.get_bytes(ea, len(native_bytes)) == native_bytes)
