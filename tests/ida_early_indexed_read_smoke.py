...
try:
    ...
    reads = [
        xref.to
        for xref in idautils.XrefsFrom(indexed_read, ida_xref.XREF_DATA)
    ...
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    if not any(
        instruction.opcode == ida_hexrays.m_ldx and instruction.ea == indexed_read
        for instruction in indexed_instructions
    ):
