...
try:
    ...
    cases = {
        "stack": {
            "vt_reg": "stack-transfer",
            "vt_mem": "stack-transfer",
            "vt_indexed": "stack-transfer",
        ...
        "flags": {
            "vf_e": "local-flag-branch",
            "vf_ne": "local-flag-branch",
            "vf_cmc": "local-flag-branch",
            "vf_set_false": "setcc-value",
            "vf_set_true": "setcc-value",
            "vf_cmov_false32": "cmov-condition",
        ...
        "getpc": {
            "gp_materialize": "stack-address-materialization",
            "gp_renamed": "stack-address-materialization",
            "gp_call": "get-pc-call",
            "gp_adjust": "get-pc-call",
        },
    ...
    for name, expected in cases.items():
        ...
        if expected is None:
            ...
            check(
                ...
                admitted and all(r["fresh"] == "true" for r in admitted),
            )
            check(
                name + " dependency detail",
                admitted
                and all(
                    int(r["dependency_count"]) > 0
                    and r["dependency_0"]
                    and r["assumption"]
                    and r["publication"] != "0x0"
                    for r in admitted
                ),
            )
            if expected == "candidate":
                check(
                    name + " unresolved has no edge",
                    all(r["edge"] == "false" and "target" not in r for r in admitted),
                )
            if expected == "stack-transfer":
                check(
                    name + " target and retained stack write",
                    all(
        ...
        check(
            name + " inspection is stable and read-only",
            before == database_observables(ea) and capture == again,
        )
    if mode == "caps":
        many, deep = captures["ve_many"], captures["ve_deep"]["records"][0]
        check(
            "native proof quota counts every omitted conclusion",
            len(many["records"]) == 128 and many["omitted"] == 12,
        )
        check(
            "native dependency quota is explicit",
            int(deep["dependency_count"]) > 64
            and int(deep["dependencies_omitted"]) == int(deep["dependency_count"]) - 64
            and len([k for k in deep if k.startswith("dependency_") and k != "dependency_count"])
            == 64,
        )
    if mode == "getpc":
        for name in ("gp_call", "gp_adjust"):
            check(
                name + " context-specific return is linked to caller",
                any(
                    r["kind"] == "call-context-return"
                    and r["fresh"] == "true"
                    and r["context_call"] == hex(address(name))
                    and "CALL" in r["effect_scope"]
                    for r in captures[name]["records"]
                ),
            )
    ...
    check(
        "consumed-byte patch revokes inspected publication",
        not any(r["publication"] == pub and r["fresh"] == "true" for r in stale["records"]),
    )
    ...
    check(
        "restoration requires a new live publication",
        any(
            r["source"] == observed["source"]
            and r["site"] == observed["site"]
            and r["kind"] == observed["kind"]
            and r["fresh"] == "true"
            and r["publication"] != pub
            for r in renewed["records"]
        ),
    )
    ...
    if mode == "flags":
        for name, value in (("vf_set_false", "0x0"), ("vf_set_true", "0x1")):
            check(
                name + " exact byte result",
                any(
                    r["kind"] == "setcc-value" and r["value"] == value and r["width_bits"] == "8"
                    for r in captures[name]["records"]
                ),
            )
    ...
    if module_path:
        ...
        check(
            "old native capture rejects replacement publication",
            pub not in module.current_native_publications(saved, renewed),
        )
        check(
            "native capture rejects foreign database",
            not module.current_native_publications(renewed, dict(renewed, database="different")),
        )
        check(
            "native capture rejects modified conclusion",
            not module.current_native_publications(
                renewed,
                dict(renewed, records=[dict(r, conclusion="changed") for r in renewed["records"]]),
            ),
        )
        if ida_kernwin.is_idaq():
            ...
            check(
                "native inspection works without rax capture",
                not snapshot["available"] and snapshot["native"]["records"],
            )
            ...
            check(
                "native proof has independent current navigation",
                not form.current and form.record_current(record) and form.jump.isEnabled(),
            )
            check(
                "native proof pane has supporting bytes",
                record["dependency_0"] in form.detail.toPlainText(),
            )
            edges = [item for item in form.scene.items() if isinstance(item, module.FlowEdge)]
            check(
                "native edge retains its proof category",
                all(e.row["truth"] == "native-proof" for e in edges)
                and (bool(edges) if record["edge"] == "true" else True),
            )
            ida_bytes.patch_byte(dependent, original ^ 1)
            form.poll()
            check(
                "native stale navigation disabled",
                not form.record_current(record) and not form.jump.isEnabled(),
            )
            check(
                "native table labels invalidated record",
                "historical" in form.tables["native"].topLevelItem(0).text(2),
            )
            ida_bytes.patch_byte(dependent, original)
            ida_auto.plan_range(function.start_ea, function.end_ea)
            ida_auto.auto_wait()
            form.poll()
            check(
                "old native view stays historical after reanalysis", not form.record_current(record)
            )
            form.reload()
...
line = "[chernobog][native-evidence] " + (
