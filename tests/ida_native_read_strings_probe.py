...
try:
    ...
    def explore():
        ...
        fields = ["fresh", "runs", "returned_runs", "allocation_lifetimes", "use_snapshots",
                  "temporal_observation_available_runs", "temporal_capture_complete_runs",
                  "temporal_capture_truncated_runs", "environment_model_failure_runs"]
        values = {field: int(evaluate(summary + "." + field)) for field in fields}
        check("completed publication", values["fresh"] == 1 and values["runs"] > 0
    ...
    check("stream timeline provenance", len(stream_rows) == 8
          and all(row["truth"] == "observation" and row["read_count"] == "8"
                  and row["first_sequence"] != row["last_sequence"]
                  and row["fragments_omitted"] == "0" for row in stream_rows), rows=stream_rows)
    ...
    if module_path:
        ...
        check("stream interval filter", selected and all(
            (item["run"], item["seed"]) == identity
            and interval[0] <= int(item["sequence"], 0) <= interval[1] for item in selected)
        ...
        if ida_kernwin.is_idaq():
            ...
            QtWidgets.QApplication.processEvents()
            check("Qt stream selection links interval", form.interval == interval and form.run == identity
            ...
            form.poll()
            check("Qt stale stream disables navigation", not form.current and not form.jump.isEnabled())
            ...
            QtWidgets.QApplication.processEvents()
            form.parent.grab().save(str(Path(os.environ["IDAUSR"]).parent / "native_read_streams.png"))
    ...
    for index in range(count()):
        ...
        candidate = {field: evaluate(expression + "." + field) for field in
                     ["ok", "value", "site", "callee", "occurrence", "argument", "producer", "model_kind", "read_count", "first_sequence", "last_sequence", "observed_bytes",
                      "object_site", "object_size", "observations", "eligible_runs"]}
        values.append(candidate["value"])
        check("native stream provenance", candidate["ok"] == 1
              and candidate["producer"] == "executed-read-stream" and candidate["argument"] in (-1, 2**64-1)
              and candidate["model_kind"] == 0 and candidate["read_count"] == 8
              and candidate["observed_bytes"] == 8 and candidate["last_sequence"] > candidate["first_sequence"]
              and candidate["object_size"] == 16
              and candidate["observations"] == candidate["eligible_runs"], candidate=candidate)
    ...
    check("transient use display", len(annotations) == 2
          and any('"secret!"' in line and "executed-read-stream,reads=8,use=1,runs=" in line for line in annotations)
          and any('"second!"' in line for line in annotations), lines=annotations)
    ...
    check("no saved or saveable use comments",
          (active_comments is None or active_comments.size() == 0)
          and (saved_comments is None or saved_comments.size() == 0))
    check("display preserves function bytes", ida_bytes.get_bytes(
        target, len(original_function)) == original_function)
    original_key = ida_bytes.get_byte(key)
    ida_bytes.patch_byte(key, original_key ^ 1)
    cfunc.refresh_func_ctext()
    check("sealed profile lease still checks consumed key", not any(
        "rax-use(" in line for line in rendered()))
    ida_bytes.patch_byte(key, original_key)
    cfunc.refresh_func_ctext()
    check("sealed profile lease restores only exact bytes", sum(
        "rax-use(" in line for line in rendered()) == 2)
    ...
    cfunc.refresh_func_ctext()
    check("later profile edit invalidates sealed display", not any(
        "rax-use(" in line for line in rendered()))
    ...
    cfunc.refresh_func_ctext()
    check("stale use text removed on reprint", not any("rax-use(" in line for line in rendered())
          and str(cfunc) == ast)
    ida_bytes.patch_byte(key, original_key)
    explore()
    check("restored key revalidates exact evidence", count() == 2)
    cfunc.refresh_func_ctext()
    check("restored exact evidence displays again", sum("rax-use(" in line for line in rendered()) == 2)
