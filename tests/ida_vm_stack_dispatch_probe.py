...
try:
    ...
    check("exact source roles and transfer", [row[k] for k in ("vip_register", "value_register", "key_register", "dispatch_base_register")]
          == ["5", "9", "8", "11"] and row["transfer_kind"] == "push/near-return")
    check("actual 25 instruction path retained", len(spans) == 25 and spans[-1][0] == int(row["dispatch"], 0)
          and ida_bytes.get_bytes(*spans[-1]) == bytes.fromhex("c3"))
    check("exact ordered source bytes", row["bytes"] == "".join(ida_bytes.get_bytes(*span).hex() for span in spans))
    check("ownerless path stays ownerless", all(ida_funcs.get_func(address) is None for address, _ in spans))
    check("protected path is complete within budgets", not full["path_scan_truncated"] and not full["reachability_truncated"])
    summary = full["summaries"][0]
    check("seven accesses include target push and return read", summary["accesses"] == "7"
          and summary["access_5_kind"] == "write" and summary["access_6_kind"] == "read"
    ...
    check("local model does not admit execution", "not admitted" in summary["transition"] and "no VM execution" in row["ownership"])
    check("unmodeled other seed-one prefix remains unresolved", not api("chernobog_vm_regions", entries["corpus_transform"])["records"])
    ...
    if ida_kernwin.is_idaq():
        ...
        check("GUI exposes transfer and stack contract", "push/near-return" in form.detail.toPlainText()
    ...
    check("far-return patch revokes protected candidate", not api("chernobog_vm_regions", ea)["records"])
    check("UI rejects changed return bytes", not module.current_vm_candidates(full, api("chernobog_vm_regions", ea)))
    ida_bytes.patch_byte(dispatch, 0xC3)
    check("restoring return restores candidate", api("chernobog_vm_regions", ea)["records"] == full["records"])
    assert ida_xref.add_cref(entries["corpus_transform"], dispatch, ida_xref.fl_JN | ida_xref.XREF_USER)
    check("alternate return entry rejects protected path", not api("chernobog_vm_regions", ea)["records"])
    ida_xref.del_cref(entries["corpus_transform"], dispatch, False)
    check("removing alternate entry restores protected path", api("chernobog_vm_regions", ea)["records"] == full["records"])
    for mode,base in ((32, 0x50000000), (64, 0x51000000)):
        ...
        cases = {"table": table + table_push + "c3", "relative": relative + "57c3",
                 "ret16": relative + "5766c3", "ret_adjust_zero": relative + "57c20000",
                 "ret_adjust_eight": relative + "57c20800", "far": relative + "57cb",
                 "far_adjust": relative + "57ca0800", "address_override": relative + "5767c3",
                 "repeat_prefix": relative + "57f3c3", "push16": relative + "6657c3",
        ...
        for index,(name,hexcode) in enumerate(cases.items()):
            ...
            check(str(mode) + " " + name + " inspection preserves bytes", ida_bytes.get_bytes(start, len(code)) == code)
            if positive:
                candidate = result["records"][0]
                effects = result["summaries"][0]
                check(str(mode) + " " + name + " widths/stack effects", candidate["transfer_kind"] == "push/near-return"
                      and candidate["address_bits"] == str(mode) and effects["accesses"] == ("4" if name == "table" else "3"))
                check(str(mode) + " " + name + " selected-function candidate", candidate["site"] == hex(start)
...
(Path(os.environ["IDAUSR"]).parent / "vm_stack_dispatch.json").write_text(json.dumps({"checks": checks, "errors": errors, "captures": captures}, indent=2) + "\n")
