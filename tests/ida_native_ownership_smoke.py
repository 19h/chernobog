...
def value_sites():
    ...
    return [(setter, setter + instruction(setter).size, "SETcc byte result 0"),
...
try:
    ...
    if stage == "write":
        ...
        ida_bytes.set_cmt(owned.ea,
                          (ida_bytes.get_cmt(owned.ea, True) or "") + "\n" + user_line, True)
        ...
        check("receipt recorded with bounded schema", data is not None
              and data[:4] == b"NPR\x01" and len(data) <= 615)
        check("database checkpoint saved", ida_loader.save_database(str(run_dir / "ownership.i64"), 0))
        ...
        check("user note survived reopening", user_line in (ida_bytes.get_cmt(owned.ea, True) or ""))
        if stage in ("rebase", "rebase_nodes"):
            ...
            check("rebased checkpoint saved", ida_loader.save_database(str(run_dir / "ownership_rebased.i64"), 0))
        if stage == "undo":
            check("undo point available", ida_undo.create_undo_point("chernobog-proof-test", "proof input patch"))
        ida_bytes.patch_byte(address("vf_e"), 0x85)  # XOR -> TEST; input unknown.
        check("recovered ownership revokes old edge immediately", owned.Op1.addr not in outgoing(owned.ea))
        check("recovered comment ownership removes only proof", ida_bytes.get_cmt(owned.ea, True) == user_line)
        settle(address("vf_e"), owned.ea)
        check("unknown branch has normal successors", outgoing(owned.ea) == {owned.Op1.addr, owned.ea + owned.size})
        ...
        check("no stale user-branch proof comment", "locally proven" not in (ida_bytes.get_cmt(user.ea, True) or ""))
        for defining, site, marker in value_sites():
            ...
            check(marker + " leaves instruction intact", ida_bytes.get_bytes(site, instruction(site).size) == original)
        if stage == "undo":
            check("undo succeeds", ida_undo.perform_undo())
            settle(address("vf_e"), owned.ea)
            check("undo restores defining bytes", ida_bytes.get_byte(address("vf_e")) == 0x31)
            check("undo restores exact proof", outgoing(owned.ea) == {owned.Op1.addr}
            ...
            check("redo has no stale proof", outgoing(owned.ea) == {owned.Op1.addr, owned.ea + owned.size}
