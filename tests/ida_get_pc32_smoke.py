...
def check(label, condition, **evidence):
    if baseline and not (label.endswith(" rejected") or label in (
        "32-bit database", "target function retained")):
        return
...
try:
    ...
    check("PUSH-next materialization", all(s in text for s in (
        "stack address materialization", "width=32 bits", "net SP delta=-4 bytes",
        "flags preserved", "stack write retained")), comment=text)
    check("arbitrary literal has no get-PC claim",
    ...
    for name, index in (("gp32_materialize", 4), ("gp32_reg", 2), ("gp32_mem", 1)):
        site = nth(name, index)
        check(name + " exact target", jumps(site) == {target} and "width=32 bits" in comment(site),
              targets=sorted(jumps(site)), comment=comment(site))
    for name in ("gp32_unknown", "gp32_writable"):
        site = nth(name, 1)
        check(name + " unresolved", not jumps(site) and "unresolved target" in comment(site),
              targets=sorted(jumps(site)), comment=comment(site))
    for name, index in (("gp32_width", 1), ("gp32_far", 1), ("gp32_extra", 1), ("gp32_alternate", 3)):
        site = nth(name, index)
        check(name + " rejected", "stack-mediated transfer" not in comment(site)
              and "exact push/return" not in comment(site),
              targets=sorted(jumps(site)), comment=comment(site))
    for name in ("gp32_call", "gp32_next"):
        root = address(name)
        decoded = instruction(root)
        check(name + " exact entry", jumps(root) == {int(decoded.Op1.addr)}
              and "width=32 bits" in comment(root), targets=sorted(jumps(root)), comment=comment(root))
    call = instruction(address("gp32_call"))
    site = call.Op1.addr
    for _ in range(2):
        site += instruction(site).size
    check("32-bit call-context return", jumps(site) == {int(call.ea + call.size)},
          targets=sorted(jumps(site)), comment=comment(site))
    ...
    for index in range(5):
        ...
        while ok:
            incoming.append({"source": int(xref.frm), "type": int(xref.type), "code": bool(xref.iscode)})
            ok = xref.next_to()
        diagnostics.append({"ea": int(ea), "itype": int(decoded.itype), "auxpref": int(decoded.auxpref),
                            "call": bool(ida_idp.is_call_insn(decoded)),
                            "block_end": bool(ida_idp.is_basic_block_end(decoded, False)),
                            "owner": int(ida_funcs.get_func_start(ea)), "incoming": incoming,
                            "value": int(decoded.Op1.value), "bytes": ida_bytes.get_bytes(ea, decoded.size).hex()})
except BaseException as error:
    errors.append(type(error).__name__ + ": " + str(error))

(Path(os.environ["IDAUSR"]).parent / "get_pc32.json").write_text(
    json.dumps({"records": records, "errors": errors, "diagnostics": diagnostics,
