...
#define ROT32(k, n) (((uint32_t)(k) << ((n) & 31u)) | ((uint32_t)(k) >> ((32u - ((n) & 31u)) & 31u)))
...
const uint8_t transform_bytes_source[] = {
    ENC8('V', 0), ENC8('M', 1), ENC8('P', 2), ENC8(' ', 3),
    ENC8('b', 4), ENC8('y', 5), ENC8('t', 6), ENC8('e', 7), ENC8(0, 8)
};
const uint16_t transform_words_source[] = {
...
const uint8_t transform_byte_utf16_source[] = {
    ENC8('W', 0), ENC8(0, 1), ENC8('i', 2), ENC8(0, 3), ENC8('d', 4),
    ENC8(0, 5), ENC8('e', 6), ENC8(0, 7), ENC8(0, 8), ENC8(0, 9)
};
...
uint8_t transform_mutable_source[] = {
    ENC8('V', 0), ENC8('M', 1), ENC8('P', 2), ENC8(' ', 3),
    ENC8('b', 4), ENC8('y', 5), ENC8('t', 6), ENC8('e', 7), ENC8(0, 8)
};
...
FIXTURE void transform_byte_utf16(void)
{
    for (size_t i = 0; i < sizeof(transform_byte_utf16_source); ++i)
        transform_byte_utf16_output[i] = transform_byte_utf16_source[i] ^ (uint8_t)(ROT32(KEY, i) + i);
}

FIXTURE void transform_wrong_rotate(void)
{
    for (size_t i = 0; i < sizeof(transform_bytes_source); ++i)
        transform_negative_output[i] = transform_bytes_source[i]
            ^ (uint8_t)(ROT32(KEY, 32u - (i & 31u)) + i);
}

FIXTURE void transform_wrong_index(void)
{
    for (size_t i = 0; i < sizeof(transform_bytes_source); ++i)
        transform_negative_output[i] = transform_bytes_source[i] ^ (uint8_t)(ROT32(KEY, i) + i + 1u);
}
...
{
    for (size_t i = 0; i < sizeof(transform_words_source) / sizeof(uint16_t); ++i)
        transform_negative_words[i] = transform_words_source[i] ^ (uint16_t)(ROT32(KEY, 2 * i) + 2 * i);
}
...
int main(void)
{
    ...
    for (size_t i = 0; i < sizeof(expected_bytes); ++i)
        if (transform_bytes_output[i] != expected_bytes[i]) return 1;
    for (size_t i = 0; i < sizeof(expected_words) / 2; ++i)
        if (transform_words_output[i] != expected_words[i]) return 2;
    for (size_t i = 0; i < sizeof(expected_byte_utf16); ++i)
        if (transform_byte_utf16_output[i] != expected_byte_utf16[i]) return 3;
    return 0;
}
