...
#define CONDITIONS(X)                                                                              \
    ...
    X(vc_cm_mem32_true)                                                                            \
    X(vc_cm_mem32_false)                                                                           \
    X(vc_cm_order_true) X(vc_cm_order_false) X(vc_unknown) X(vc_depth) X(vc_alternate)
...
int main(void)
{
    ...
    {
        uint64_t destination = UINT64_C(0xFEDCBA9876543210) ^ (UINT64_C(0x9E3779B97F4A7C15) * seed);
        uint64_t source =
            seed == 0 ? 0 : UINT64_C(0x1020304050607080) ^ (UINT64_C(0xD1342543DE82EF95) * seed);
        ...
        for (unsigned i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i)
        {
            ...
            printf("{\"name\":\"%s\",\"seed\":%u,\"destination\":%" PRIu64 ",\"source\":%" PRIu64
                   ",\"memory_before\":%" PRIu64 ",\"result\":%" PRIu64 ",\"memory_after\":%" PRIu64
                   ",\"flags\":%" PRIu64 "}\n",
