...
static void run(unsigned scenario,uint32_t encoded,uint64_t key)
{
  ...
  struct capture out={0};out.encoded=encoded;out.key=key;
  out.delta=(decoded&UINT32_C(0x80000000))?(uint64_t)decoded-UINT64_C(0x100000000):decoded;
  out.table=(uint64_t)(uintptr_t)table;
  vm_semantic_run(&out,0,scenario);
  printf("%x %x %llx",scenario,encoded,(unsigned long long)key);
  uint64_t words[11];memcpy(words,&out.vip_before,sizeof(words));
  for(unsigned i=0;i<11;++i)printf(" %llx",(unsigned long long)words[i]);
  putchar('\n');
}
int main(void)
{
  for(unsigned i=0;i<256;++i)table[i]=(uint64_t)(uintptr_t)vm_semantic_capture;
  ...
  {
    for(unsigned i=0;i<8;++i)for(unsigned j=0;j<5;++j)run(scenario,values[i],keys[j]);
    for(unsigned i=0;i<64;++i){uint32_t value=(uint32_t)random_word();run(scenario,value,random_word());}
  }
  return 0;
}
