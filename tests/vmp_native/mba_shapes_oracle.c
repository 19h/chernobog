...
int main(void)
{
    ...
    for (unsigned seed = 0; seed < 2; ++seed) {
        ...
        for (unsigned trial = 0; trial < 256; ++trial) {
            ...
            if (mba_demorgan32(a, b) != (a & b)) return 1;
            if (mba_carry32(a, b) != a + b) return 2;
            if (mba_carry64(x, y) != x + y) return 3;
            if (mba_stack32(a, b) != a + b) return 4;
            if (mba_truncate8(a, b) != (uint8_t)(a + b)) return 5;
            if (mba_extend_not(a) != (uint8_t)~a) return 6;
            if (mba_not_extend(a) != ~(uint32_t)(uint8_t)a) return 7;
            uint32_t cell = a;
            if (mba_alias_write(&cell, b) != (a ^ b) || cell != b) return 8;
            cell = a;
            const uint32_t written = (a & UINT32_C(0xffff0000)) | (b & 65535);
            if (mba_alias_partial(&cell, b) != (a ^ written) || cell != written) return 9;
            if (mba_order32(a, b, (uint32_t)next(&state)) != a + b) return 10;
            comparisons += 10;
        }
    }
    for (unsigned x = 0; x < 256; ++x)
        for (unsigned y = 0; y < 256; ++y) {
            if (mba_truncate8(x, y) != (uint8_t)(x + y)) return 11;
            ++comparisons;
        }
