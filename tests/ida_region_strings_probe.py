...
def api(name, *args):
    ...
    assert not ida_expr.eval_idc_expr(value, ida_idaapi.BADADDR, name + "(" + ",".join(encoded) + ")")
...
try:
    ...
    check("separate native inspection", snapshot.get("available") and snapshot["scope"] == "native-region-strings"
          and not snapshot["function_evidence_published"] and not snapshot["vm_identity_proved"])
    check("four retained capture identities", len(snapshot["runs"]) == 4
          and len({row["capture"] for row in snapshot["runs"]}) == 4)
    expected = os.environ["CHERNOBOG_EXPECT_RETURN"] == "1"
    check("all-run admission", snapshot["consensus_available"] == expected)
    if expected:
        check("both protected values", sorted(row["value"] for row in snapshot["observations"]) == ["second!", "secret!"])
        check("complete witness table", len(snapshot["witnesses"]) == 8 and len(snapshot["fragments"]) == 64
              and snapshot["observations_omitted"] == 0)
        for observation in snapshot["observations"]:
            witnesses = [row for row in snapshot["witnesses"] if row["observation"] == observation["index"]]
            check("consensus preserves all scheduled runs", len(witnesses) == 4 and observation["eligible_runs"] == "4"
                  and {row["capture"] for row in witnesses} == {row["capture"] for row in snapshot["runs"]})
            for witness in witnesses:
                fragments = [row for row in snapshot["fragments"] if row["observation"] == observation["index"]
                             and row["capture"] == witness["capture"]]
                raw = b"".join(bytes.fromhex(row["bytes"]) for row in fragments)
                check("exact original reads", raw == observation["value"].encode() + b"\0"
                      and len(fragments) == int(witness["read_count"]) == 8
                      and witness["fragments_omitted"] == "0"
                      and all(int(row["data_sequence"]) == int(row["sequence"]) + 1 for row in fragments))
    else:
        check("partial executions cannot publish strings", not snapshot["observations"] and not snapshot["witnesses"]
              and not snapshot["fragments"] and all(row["complete"] == "false" for row in snapshot["runs"]))
    ...
    if os.environ.get("CHERNOBOG_VIEW_MODULE") and expected:
        spec = importlib.util.spec_from_file_location("region_string_view", os.environ["CHERNOBOG_VIEW_MODULE"])
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        witness = snapshot["witnesses"][0]
        selected, fragments = module.select_rows(snapshot, witness["observation"], witness["capture"])
        ...
        if ida_kernwin.is_idaq():
            ...
            check("Qt linked read witnesses", form.tables["observations"].topLevelItemCount() == 2
                  and form.tables["witnesses"].topLevelItemCount() == 4
                  and form.tables["fragments"].topLevelItemCount() == 8 and form.jump.isEnabled())
            ida_bytes.patch_byte(key, original ^ 1)
            form.poll()
            check("Qt changed input disables navigation", not form.current and not form.jump.isEnabled())
            ida_bytes.patch_byte(key, original)
            form.poll()
    ...
    check("replacement supersedes only old lease", not fresh() and replacement["ticket"] != ticket
          and api("chernobog_vm_temporal_string_state", replacement["ticket"])["fresh"])
    if form:
        form.poll()
        check("Qt superseded capture disables navigation", not form.current and not form.jump.isEnabled())
...
(Path(os.environ["IDAUSR"]).parent / "region_strings.json").write_text(json.dumps(
    {"records": records, "errors": errors, "snapshot": snapshot, "freshness_elapsed_ns": freshness_elapsed_ns}, indent=2) + "\n")
