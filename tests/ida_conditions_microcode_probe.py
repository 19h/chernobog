...
def operand(op):
    ...
    if op.t == hx.mop_r:
        ...
        result["call"] = {"arguments": [operand(arg) for arg in op.f.args],
                          "flags": int(op.f.flags), "spoiled": op.f.spoiled.dstr(),
...
def stats():
    return {key: int(idc.eval_idc("chernobog_early_stats()." + key + ";"))
...
def capture(name, entry, phase):
    ...
    for ea in idautils.FuncItems(entry):
        ...
        native.append({"ea": int(ea), "size": int(insn.size), "mnemonic": mnemonic,
    ...
    mba = hx.gen_microcode(ranges, failure, None,
    ...
    if phase == "initial" and os.environ.get("CHERNOBOG_CONDITION_DECOMPILE") == "1":
        ...
        ctree = str(cfunc)
    return {"name": name, "phase": phase, "entry": int(entry), "native": native,
            "sites": sites, "snippet": snippet, "blocks": blocks,
            "ctree": ctree,
...
try:
    ...
    functions = sorted((ida_name.get_name(ea).lstrip("_"), ea) for ea in idautils.Functions()
    ...
    prefix = next(insn for record in records if record["name"] == "vc_e"
                  for insn in record["native"] if insn["bytes"] == "31c9")
    site = prefix["ea"]
...
report = {"records": records, "errors": errors, "registers": registers,
          "word_bytes": 8 if ida_ida.inf_is_64bit() else 4,
          "maturity": os.environ.get("CHERNOBOG_CONDITION_MATURITY", "MMAT_GENERATED"),
          "opcodes": {name: int(getattr(hx, name)) for name in
                      ("m_mov", "m_xdu", "m_stx", "m_nop", "m_call")},
...
line = "[chernobog][conditions-microcode] " + ("FAIL " + "; ".join(errors) if errors else
                                               "PASS captures=%d" % len(records))
