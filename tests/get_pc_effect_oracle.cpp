...
static Effects execute(int (*function)())
{
    ...
    asm volatile(
        ...
        "movq -16(%%rsp), %[slot]\n\t"
        "pushfq\n\tpopq %[after_flags]"
        : [result] "=&a" (e.result), [before] "=&r" (e.before_sp),
          [after] "=&r" (e.after_sp), [slot] "=&r" (e.slot),
          [before_flags] "=&r" (e.before_flags), [after_flags] "=&r" (e.after_flags)
        : [function] "r" (function)
        : "r11", "cc", "memory");
    return e;
}
...
static uint64_t add_three_flags(uint64_t value)
{
    ...
    unsigned ones = 0;
    for (unsigned bit = 0; bit < 8; ++bit) ones += unsigned((sum >> bit) & 1);
    return uint64_t(sum < value)
        | (uint64_t((ones & 1) == 0) << 2)
        | (uint64_t((value & 15) + 3 > 15) << 4)
        | (uint64_t(sum == 0) << 6)
        | ((sum >> 63) << 7)
...
int main()
{
    ...
    const bool ok = call.result == 7 && adjust.result == 7
        && call.before_sp == call.after_sp && adjust.before_sp == adjust.after_sp
        && call.slot == call_pc && adjust.slot == adjust_pc + 3
