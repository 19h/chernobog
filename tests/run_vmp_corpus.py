                ...
                found.append(
                    {
                        ...
                        "offset": offset,
                        "flags": flags,
                        "file_backed": offset > 0
                        and (flags & 255) == 0
                        and offset + size_bytes <= len(data),
                    }
                )
        cursor += size
...
def elf32_text_section(data):
    ...
    kind, machine, version, _, phoff, shoff, _, ehsize, phsize, phcount, shsize, shcount, names = (
    ...
    if (
        phsize != 32
        or not 0 < phcount <= 4096
        or phoff < 52
        or phoff + phcount * phsize > len(data)
        or shsize != 40
        or not 0 < shcount <= 4096
        or shoff < 52
        or shoff + shcount * shsize > len(data)
    ...
    for section in sections:
        ...
        initialized = (
            ...
            and address + size <= 1 << 32
        )
        mapped = False
        for i in range(phcount):
            pkind, poff, va, _, files, memory, perms, _ = struct.unpack_from(
...
def execute(args, cwd=None, env=None, timeout=30):
    ...
    with tempfile.TemporaryFile() as stdout, tempfile.TemporaryFile() as stderr:
        ...
        output_exceeded |= (
            os.fstat(stdout.fileno()).st_size + os.fstat(stderr.fileno()).st_size > 2 * 1024 * 1024
        )
        stdout.seek(0)
        stderr.seek(0)
    ...
    for index, line in enumerate(lines):
        ...
        if observed != wanted:
            if len(mismatches) < 16:
                mismatches.append(
                    {"case": case, "function": function, "observed": observed, "expected": wanted}
                )
...
def project(path, addresses, mode):
    ...
    for address in addresses.values():
        ET.SubElement(
            ...
            "Procedure",
            Address=hex(address),
            CompilationType=str(MODES[mode]),
            Options="0",
        )
...
def main():
    ...
    parser.add_argument(
        "--linux32-image", help="Prebuilt Linux32 image; resolved to immutable image ID"
    )
    parser.add_argument("--docker-context", default="default")
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="One mutation seed and one input seed for configuration checks",
    )
    ...
    report = {
        ...
        "format": "ELF" if args.architecture == "i386" else "Mach-O",
        "host_architecture": platform.machine(),
        "source_build_attestation": "unknown",
        "protector_sha256": digest(protector),
        ...
        "vm_options": 0,
        "procedure_options": 0,
        "seed_policy": "test-only C srand interposition; every requested seed replaced with the recorded seed",
        "smoke": args.smoke,
        ...
        "source_sha256": {
            ...
            for name in (
                ...
                "tests/vmp_corpus/pair.c",
                "tests/vmp_corpus/pair.S",
                "tests/vmp_corpus/pair32.S",
                "tests/vmp_corpus/linux32.py",
                "tests/vmp_corpus/linux32_exec.py",
                "tests/vmp_corpus/linux32.Dockerfile",
                "tests/vmp_corpus/seed.lua",
                "tests/vmp_corpus/seed_interpose.c",
                "tests/vmp_corpus/seed_probe.c",
            )
        },
        "reference_source_sha256": {
            "vmp/" + name: digest(source_tree / name)
            for name in (
                "core/core.cc",
                "core/core.h",
                "core/files.cc",
    ...
    try:
        ...
        command = [
            ...
            "-Wl,-no_data_const",
            "-Wl,-headerpad,0x4000",
            root / "tests/vmp_corpus/pair.c",
            root / "tests/vmp_corpus/pair.S",
            "-o",
            original,
        ]
        if linux32:
            command = [
                ...
                "-no-pie",
                "-fcf-protection=none",
                "/source/vmp_corpus/pair.c",
                "/source/vmp_corpus/pair32.S",
                "-o",
                "/output/original",
            ]
        ...
        if linux32:
            ...
            report["build_accounting_scope"] = (
                "Docker client; guest execution is measured separately with Linux wait4"
            )
        if build["exit_code"]:
        ...
        for source, target, extra in [
            ("seed_interpose.c", instrument, ["-dynamiclib"]),
            ("seed_probe.c", probe, []),
        ]:
            result, _, _ = execute(
                [
        ...
        injected = dict(
            base_environment(),
            DYLD_INSERT_LIBRARIES=str(instrument),
            CHERNOBOG_CORPUS_PROTECTOR_SEED="17",
        )
        ...
        seed_verified = (
            normal["exit_code"] == reference["exit_code"] == overridden["exit_code"] == 0
            and normal_out != reference_out
            and overridden_out == reference_out
            and b"CHERNOBOG_CORPUS_SRAND=17 requested=1" in overridden_err
        )
        report["seed_instrument"] = {
            "sha256": digest(instrument),
        ...
        nm, symbols, _ = (
            linux32.execute(["i686-linux-gnu-nm", "-n", "-g", "/output/original"])
            if linux32
            else execute(["xcrun", "nm", "-n", "-g", original])
        )
        if nm["exit_code"]:
        ...
        addresses = {
            name: int(
                re.search(
                    rb"^([0-9a-fA-F]+) T " + symbol_prefix + name.encode() + rb"$", symbols, re.M
                )[1],
                16,
            )
        ...
        ends = {
            name: int(
                re.search(
                    rb"^([0-9a-fA-F]+) T " + symbol_prefix + name.encode() + rb"_end$",
                    symbols,
                    re.M,
                )[1],
                16,
        ...
        original_bytes = {
            name: function_bytes(original, original_text, address, ends[name] - address)
            for name, address in addresses.items()
        }
        report["selected_function_bytes"] = {
            name: data.hex() for name, data in original_bytes.items()
        }
        ...
        for mode in (["mutation"] if args.smoke else MODES):
            ...
            for seed in report["protector_seeds"]:
                ...
                env = dict(
                    base_environment(),
                    CHERNOBOG_CORPUS_PROTECTOR_SEED=str(seed),
                    DYLD_INSERT_LIBRARIES=str(instrument),
                )
                measurement, out, err = execute(
                    [
                        protector,
                        original,
                        target,
                        ...
                        config,
                        "-sf",
                        root / "tests/vmp_corpus/seed.lua",
                        "-we",
                    ],
                    cwd=protector.parent,
                    env=env,
                    timeout=60,
                )
                ...
                item = {
                    ...
                    "mode": mode,
                    "protector_seed": seed,
                    "project_sha256": digest(config),
                    "measurement": measurement,
                    "seed_markers": [int(s) for s in markers],
                    "seed_applications": [
                        {"applied": int(a), "requested": int(r)} for a, r in applied
                    ],
                }
                report["protection"].append(item)
                if (
                    measurement["exit_code"]
                    or [int(s) for s in markers] != [seed]
                    or not target.exists()
                    or not applied
                    or any(int(a) != seed for a, _ in applied)
                ):
                    ...
                    diagnostics = [
                        ...
                        and not any(
                            word in line.lower()
                            for word in ("license", "serial", "registered", "@")
                        )
                    ]
                ...
                item["selected_entry_changed"] = {
                    name: function_bytes(target, section, address, len(original_bytes[name]))
                    != original_bytes[name]
                    for name, address in addresses.items()
                }
                if not section["file_backed"] or not all(item["selected_entry_changed"].values()):
                    raise RuntimeError(
                        "selected code was not visibly transformed in initialized text"
                    )
                repeat_target = repeats / label
                repeated, repeat_out, repeat_err = execute(
                    [
                        protector,
                        original,
                        repeat_target,
                ...
                repeat_applied = re.findall(
                    rb"CHERNOBOG_CORPUS_SRAND=(\d+) requested=(\d+)", repeat_out + repeat_err
                )
                repeat_markers = re.findall(
                    rb"CHERNOBOG_CORPUS_SEED=(\d+)", repeat_out + repeat_err
                )
                item["repeat_measurement"] = repeated
                item["repeat_seed_attested"] = (
                    bool(repeat_applied)
                    and all(int(a) == seed for a, _ in repeat_applied)
        ...
        for label, binary in binaries:
            for seed in report["input_seeds"]:
                measurement, stdout, _ = (
                    linux32.run(binary, seed)
                    if linux32
                    else execute([binary, str(seed)], cwd=output, timeout=10)
                )
                verdict = (
                    verify(stdout, seed)
                    if measurement["exit_code"] == 0 and not measurement["output_exceeded"]
                    else {"passed": False, "reason": "execution did not complete within limits"}
                )
                if label == "original":
                ...
                report["runs"].append(
                    {
                        ...
                        "measurement": measurement,
                        "oracle": verdict,
                        "observations": observed_file,
                        "matches_original_stdout": measurement["stdout_sha256"]
                        == original_stdout[seed],
                    }
                )
        ...
        report["inputs_unchanged"] = (
            digest(original) == report["original_sha256"]
        ...
        report["sources_unchanged"] = all(
            digest(root / name) == h for name, h in report["source_sha256"].items()
        ) and all(
            digest(source_tree / name.removeprefix("vmp/")) == h
            for name, h in report["reference_source_sha256"].items()
        )
        report["protected_artifacts_unchanged"] = all(
            digest(output / p["label"]) == p["sha256"] for p in report["protection"]
        )
        if (
            not report["inputs_unchanged"]
            or not report["sources_unchanged"]
            or not report["protected_artifacts_unchanged"]
        ...
        report["unique_inputs_per_binary"] = len(
            {(f, *values) for s in report["input_seeds"] for values in inputs(s) for f in (0, 1)}
        )
    except Exception as error:
        report["failure"] = (
            type(error).__name__ + ": " + re.sub(r"/Users/[^\s'\"]+", "<local path>", str(error))
        )
    finally:
        ...
        json.dumps(
            {
                ...
                "protected_files": len(report["protection"]),
                "behavior_runs": len(report["runs"]),
