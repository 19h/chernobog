"""Execute multiply oracles and inspect the same binaries in fresh IDA databases."""
...
def main():
    ...
    sources = [
        ...
        "tests/vmp_native/multiply_asm.c",
        "tests/vmp_native/multiply.cpp",
        "tests/ida_multiply_probe.py",
        "tests/run_multiply.py",
    ...
    try:
        for architecture in (["x86_64", "i386"] if args.linux32_image else ["x86_64"]):
            ...
            if architecture == "x86_64":
                build, _, _ = execute(
                    [
                        ...
                        "-O2",
                        "-g0",
                        root / "tests/vmp_native/multiply_asm.c",
                        root / "tests/vmp_native/multiply.cpp",
                        "-o",
                        binary,
                    ]
                )
                ...
                build, _, _ = backend.execute(
                    [
                        ...
                        "-fno-pie",
                        "-no-pie",
                        "/source/vmp_native/multiply_asm.c",
                        "/source/vmp_native/multiply.cpp",
                        "-o",
                        "/output/multiply",
                    ]
                )
            ...
            assert row["native_result"] == {
                "passed": True,
                "byte_cases": 524288,
                "corner_cases": 244800 if architecture == "x86_64" else 172800,
                "static_inputs": 512,
            }
            ...
            measurement, _, _ = execute(
                [
                    ...
                    binary,
                    root / "tests/ida_multiply_probe.py",
                    "--ida",
                    args.ida.resolve(),
                    "--plugin",
                    args.plugin.resolve(),
                    ...
                    *(["--set", "CHERNOBOG_MULTIPLY_BASELINE=1"] if args.baseline else []),
                ],
                timeout=120,
            )
            row["inspection"] = measurement
            assert measurement["exit_code"] == 0 and not measurement["timed_out"]
            capture = json.loads((directory / "inspection/multiply.json").read_text())
            ...
            row["artifact_sha256"] = {
                ...
                for path in ("inspection/run.json", "inspection/multiply.json")
            }
            print(
                json.dumps(
                    {
    ...
    (output / "multiply_analysis.json").write_text(json.dumps(report, indent=2) + "\n")
