...
def api(ea, supplied=None, semantic=False):
    ...
    request = json.dumps(json.dumps(supplied if supplied is not None else {"args": [], "objects": []}))
...
try:
    ...
    for extra, code in ((0, b"\xc3"), (8, b"\xc2\x08\x00")):
        ...
        captures[name] = trace
        check(name, trace["available"] and trace["ran"] and trace["reached_sentinel"]
              and trace["sp_valid"] and trace["sp_delta"] == mode // 8 + extra
              and len(trace["execution"]) == 1 and not trace["native_admissions"])
    far = {"far_return": b"\xcb", "far_return_adjusted": b"\xca\x08\x00", "interrupt_return": b"\xcf",
           "indirect_far_call": b"\xff\x18", "indirect_far_jump": b"\xff\x28",
    ...
    for name, code in far.items():
        ...
        trace = api(ea)
        captures[name] = trace
        check(name + " rejected before entry", not trace["available"]
              and trace["reason"] == "entry has no admissible native instruction")
    prefix = (b"\x48\xb8" + struct.pack("<Q", ea + 32) if mode == 64
              else b"\xb8" + struct.pack("<I", ea + 32)) + b"\xff\xe0"
    patch(prefix + b"\xcc" * (32 - len(prefix)) + b"\xcb")
    trace = api(ea)
    captures["observed_far_return_destination"] = trace
    check("far destination remains rejected after observed transfer", trace["available"] and trace["ran"]
          and trace["region_boundary"] and not trace["reached_sentinel"] and len(trace["execution"]) == 2
          and len(trace["native_admissions"]) == 1 and trace["native_admissions"][0]["admitted"] == "false"
    ...
    for split in (False, True):
        ...
        captures[name] = trace
        check(name + " actual execution", trace["available"] and trace["ran"] and trace["reached_sentinel"]
              and trace["native_state_capture_complete"])
        view = trace["native_observations"]
        check(name + " exact local transition", view["available"] and len(view["records"]) == 1
              and view["transition_attempts"] == 1 and view["queries"] == 2
              and view["records"][0]["semantic_validation"] == "corroborated for captured transition"
              and int(view["records"][0]["site"], 0) == ea + len(setup))
        check(name + " explicit incomplete VM identity", view["records"][0]["logical_state_complete"] == "false"
              and view["records"][0]["virtual_stack"] == "unknown" and not trace["function_evidence_published"])
    ...
    if original is not None:
        ...
        check("original function topology retained", (int(ida_funcs.get_func(ea).flags), list(idautils.Chunks(ea))) == topology)

report = {"passed": not errors, "checks": checks, "errors": errors, "captures": captures}
(Path(os.environ["IDAUSR"]).parent / "vm_native_walk_controls.json").write_text(json.dumps(report, indent=2) + "\n")
