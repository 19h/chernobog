...
inline void layout_path(std::vector<Instruction> &code)
{
  ...
  for(auto &i:code)
  {
    i.address=at;i.size=4;at+=4;
    if(i.op==Op::direct_jump){at-=0x1000;i.dst.value=at;}
  }
}
...
inline Candidate path_candidate(unsigned mode,bool backward,int vip=6,int value=0,int key=3,int base=7)
{
  const auto original=candidate(mode,backward,true,true,vip,value,key,base);
  std::vector<Instruction> code;
  auto add=[&](Op op,Operand dst={},Operand src={}){code.push_back({0,4,op,dst,src,false});};
  for(const auto &i:original.support)
  {
    code.push_back(i);
    if(backward && i.address==original.start)
    {add(Op::rotate_right,reg(value,32),imm(8));add(Op::scan_forward,reg(value,16),reg(2,16));}
    if(i.op==Op::load)
    {auto high=reg(value,8);high.bit_offset=8;add(Op::compare,high,imm(0xaa));}
    if(i.op==Op::rotate_left)
    {auto target=imm(0);target.bits=mode;add(Op::direct_jump,target);add(Op::carry_set);add(Op::carry_toggle);}
    if(i.op==Op::push)
    {add(Op::bit_xor,reg(key,16),reg(9,16));add(Op::load,reg(key,16),reg(value,8));}
    if(i.dst.kind==Kind::memory)
      add(Op::negate,reg(key,mode));
    if(i.op==Op::pop)add(Op::test,reg(2,16),imm(0x8fe7));
  }
  layout_path(code);const auto result=recognize(code,mode);
  if(!result)throw std::runtime_error("path candidate rejected");
  return *result;
}
inline Candidate stack_candidate(const Candidate &original,bool split=false)
{
  auto code=original.support;
  if(code.back().op!=Op::jump)throw std::runtime_error("expected jump source");
  code.back().op=Op::push;
  if(split)
  {
    auto target=imm(0);target.bits=original.address_bits;
  ...
  if(!result)throw std::runtime_error("push/return candidate rejected");
  return *result;
}
}
