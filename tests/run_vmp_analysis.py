...
def main():
    ...
    sources = {
        ...
        for name in (
            "tests/run_vmp_analysis.py",
            "tests/ida_vmp_corpus_probe.py",
    ...
    report = {
        ...
        "runs": [],
        "scope": "selected-entry code and xref inventories; complete oracle edge rates unknown",
    }
    binaries = {
        ...
        for label, expected_hash in binaries.items():
            ...
            for enabled in (False, True):
                ...
                command = [
                    ...
                    binary,
                    root / "tests/ida_vmp_corpus_probe.py",
                    "--ida",
                    args.ida.resolve(),
                    "--plugin",
                    args.plugin.resolve(),
                    ...
                    run_dir,
                    "--set",
                    "CHERNOBOG_IDA_DIRECT_JUMP_DECODE=" + str(int(enabled)),
                    "--set",
                ...
                item["entries"] = {
                    name: {
                        "reachable_heads": len(entry["instructions"]),
                        "undefined_heads": sum(not i["is_code"] for i in entry["instructions"]),
                        "truncated": entry["traversal_truncated"],
                        "owners_omitted": entry["owners_omitted"],
                        "native_records": sum(
                            len(i["native"].get("records", [])) for i in entry["inspections"]
                        ),
                        "vm_candidates": sum(
                            len(i["vm"].get("records", [])) for i in entry["inspections"]
                        ),
                        "solver_records": sum(
                            len(i["solver"].get("records", [])) for i in entry["inspections"]
                        ),
                        "inspection_preserved_code_and_xrefs": entry[
                            "inspection_preserved_code_and_xrefs"
                        ],
                    }
                    for name, entry in inspection["entries"].items()
                }
                item["artifact_sha256"] = {
                    name: digest(run_dir / name) for name in ("run.json", "corpus_inspection.json")
                }
                print(
                    json.dumps({"run": run_name, "passed": True, "entries": item["entries"]}),
                    flush=True,
                )
