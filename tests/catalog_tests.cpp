...
extern "C" void *chernobog_catalog_hexdsp(int code, ...)
{
    if ( code == hx_minsn_t_init )
    {
        ...
        instruction->l.zero(); instruction->r.zero(); instruction->d.zero();
        return nullptr;
    }
...
struct ValueInsn : minsn_t
{
    ...
    { opcode = operation; d.size = bytes; }
    ~ValueInsn() { l.zero(); r.zero(); d.zero(); }
};
void reg(mop_t &operand, int id, int bytes)
{ operand.t = mop_r; operand.r = id; operand.size = bytes; }
void nested(mop_t &operand, minsn_t &instruction)
{ operand.t = mop_d; operand.d = &instruction; operand.size = instruction.d.size; }
void constant(mop_t &operand, mnumber_t &number, int bytes)
{ operand.t = mop_n; operand.nnn = &number; operand.size = bytes; }

bool test_typed_instances()
{
    using namespace chernobog::rules;
    RuleVerifier verifier;
    const auto expect = [&](minsn_t &before, minsn_t &after, RuleVerificationStatus status, const char *label) {
    ...
    for ( int bytes : {1, 2, 4, 8} )
    {
        ...
        reg(both.l, 100, bytes); reg(both.r, 200, bytes);
        reg(either.l, 100, bytes); reg(either.r, 200, bytes);
        nested(sum.l, both); nested(sum.r, either);
        reg(direct.l, 100, bytes); reg(direct.r, 200, bytes);
    ...
    ValueInsn narrow_not(m_bnot, 1), wide_input(m_xdu, 4), extend_not(m_xdu, 4), wide_not(m_bnot, 4);
    reg(narrow_not.l, 100, 1); reg(wide_input.l, 100, 1);
    nested(extend_not.l, narrow_not); nested(wide_not.l, wide_input);
    okay &= expect(extend_not, wide_not, RuleVerificationStatus::DISPROVED, "NOT cannot cross zero extension");
    ValueInsn signed_input(m_xds, 4);
    reg(signed_input.l, 100, 1);
    okay &= expect(wide_input, signed_input, RuleVerificationStatus::DISPROVED, "signed versus unsigned extension");
    ValueInsn narrow_add(m_add, 1), extended_sum(m_xdu, 4), other_input(m_xdu, 4), wide_add(m_add, 4), masked(m_and, 4);
    reg(narrow_add.l, 100, 1); reg(narrow_add.r, 200, 1);
    nested(extended_sum.l, narrow_add); reg(other_input.l, 200, 1);
    nested(wide_add.l, wide_input); nested(wide_add.r, other_input);
    mnumber_t mask(255), zero(0);
    nested(masked.l, wide_add); constant(masked.r, mask, 4);
    okay &= expect(extended_sum, masked, RuleVerificationStatus::VERIFIED, "explicit truncation preserves narrow carry");
    ...
    nested(low.l, wide_input); nested(high.l, wide_input); reg(direct.l, 100, 1);
    ...
    reg(samples.l, 100, 4); reg(samples.r, 200, 4); constant(zero_value.l, zero, 4);
    okay &= expect(samples, zero_value, RuleVerificationStatus::DISPROVED, "distinct reaching values cannot cancel");
    samples.r.r = 100;
    okay &= expect(samples, zero_value, RuleVerificationStatus::VERIFIED, "same value can cancel");
    samples.r.size = 1;
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "implicit width conversion");
    samples.r.size = 4; samples.l.oprops |= OPROP_UDEFVAL;
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "undefined input");
    samples.l.oprops = 0; samples.iprops |= IPROP_MBARRIER;
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "memory barrier");
    samples.iprops = 0;
    samples.d.oprops |= OPROP_FLOAT;
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "floating destination");
    samples.d.oprops = 0;
    ValueInsn effect(m_stx, 4);
    nested(samples.l, effect); nested(samples.r, effect);
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "intervening write cannot be an opaque equal leaf");
    nested(samples.l, samples);
    okay &= expect(samples, zero_value, RuleVerificationStatus::UNSUPPORTED, "cyclic expression budget");
    ...
    okay &= expect(samples, zero_value, RuleVerificationStatus::DISPROVED, "different frame owners cannot cancel");
    if ( okay ) std::cout << "MBA typed instances: 25 positive/negative controls passed\n";
    return okay;
}
