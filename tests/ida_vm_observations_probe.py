...
try:
    ...
    check(
        "fresh joined capture",
        captured["states_available"]
        and evidence["fresh"]
        and captured["revision"] == evidence["revision"]
        and len(rows) == 12,
    )
    check(
        "one native candidate",
        len(captured["records"]) == 1 and len({r["vm_candidate"] for r in rows}) == 1,
    )
    ...
    for run in sorted({r["run"] for r in rows}):
        ...
        check(
            "VIP progression " + run,
            [int(r["entry_vip"], 0) for r in group] == [base, base + 1, base + 2],
        )
        check("key progression " + run, [r["entry_key"] for r in group] == ["0x5a", "0x58", "0x58"])
        check(
            "output decode " + run,
            [r["output_decoded_register"] for r in group] == ["0x2", "0x0", "0x1"],
        )
    check(
        "observed local path",
        bool(rows) and all(r["path"] == "sampled local address/size path" for r in rows),
    )
    check(
        "memory access order",
        bool(rows)
        and all(
            r["accesses_captured"] == "2"
            and r["access_0_kind"] == r["access_1_kind"] == "read"
            and r["access_0_size_bytes"] == "1"
            and r["access_1_size_bytes"] == "8"
            for r in rows
        ),
    )
    check(
        "unknown VM identity remains explicit",
        bool(rows)
        and all(
            r["virtual_stack"] == r["vm_context"] == r["memory_epoch"] == "unknown"
            and r["logical_state_complete"] == "false"
            and r["merge"] == "not admitted"
            and r["semantic_validation"] == "not performed"
            for r in rows
        ),
    )
    before = api("chernobog_solver_evidence", target)
    check(
        "repeatable pure projection",
        api("chernobog_vm_states", target) == captured
    ...
    check(
        "consumed data patch prevents fresh role join",
        not stale["states_available"] and not stale["states"],
    )
    ...
    check(
        "UI publication identity",
        module.current_snapshot(captured, state)
    ...
    if ida_kernwin.is_idaq():
        ...
        check(
            "GUI displays captured states",
            form.tables["vm_states"].topLevelItemCount() == 12
            and form.jump.isEnabled()
            and "Virtual stack, VM context and memory epoch: unknown" in form.detail.toPlainText(),
        )
        check(
            "state selection links its run",
            form.run == (rows[0]["run"], rows[0]["seed"])
            and form.tables["events"].topLevelItemCount()
            == len(module.matching_events(evidence, run=form.run)),
        )
        proof_before = api("chernobog_solver_evidence", target)
        form.poll()
        check(
            "polling does not run solver", proof_before == api("chernobog_solver_evidence", target)
        )
        ida_bytes.patch_byte(base, original ^ 1)
        form.poll()
        check(
            "stale join disables navigation",
            not form.jump.isEnabled() and form.tables["vm_states"].topLevelItemCount() == 12,
        )
        ida_bytes.patch_byte(base, original)
        form.poll()
    ...
    check(
        "foreign function cannot borrow capture",
        not api("chernobog_vm_states", boundary)["states_available"],
    )
    ...
    check(
        "boundary output sampled",
        b["states_available"]
        and len(b["states"]) == 4
        and all(
            r["exit"].startswith("function-boundary") and r["target"] == hex(address("_outside"))
            for r in b["states"]
        ),
    )
    ...
    check(
        "boundary remains run stop",
        len(raw["runs"]) == 4 and all(r["kind"] == "function-boundary" for r in raw["runs"]),
    )
    evaluate("chernobog_rax_clear()")
    check(
        "cleared states unavailable", not api("chernobog_vm_states", boundary)["states_available"]
    )
...
line = "[chernobog][vm-observations] " + (
