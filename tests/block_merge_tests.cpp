...
void make_chain(Graph &graph, size_t candidate_count, bool cycle)
{
    for (size_t index = 0; index < graph.blocks.size(); ++index)
    {
        ...
        {
            const int successor = index + 1 < candidate_count ? int(index + 1)
                                  : cycle                     ? 0
                                                              : int(candidate_count);
...
void benchmark_case(const char *name, Graph &graph, size_t repetitions)
{
    for (size_t sample = 0; sample < 7; ++sample)
    {
        ...
        std::cout << name << ',' << graph.mba.qty << ',' << sample << ',' << repetitions << ','
                  << before.elapsed_ns << ',' << after.elapsed_ns << ',' << before.block_lookups
                  << ',' << after.block_lookups << ',' << before.link_reads << ','
                  << after.link_reads << '\n';
    }
}
...
{
    std::cout
        << "case,blocks,sample,repetitions,reference_ns,bounded_ns,reference_block_lookups,bounded_block_lookups,reference_link_reads,bounded_link_reads\n";
    Graph dense(4096);
    ...
    for (size_t index = 0; index < dense.blocks.size(); ++index)
    {
        ...
        dense.blocks[index].successors = {
            int(index + 1 == dense.blocks.size() ? index : index / 3 * 3 + (index + 1) % 3)};
    }
