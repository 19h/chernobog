...
rax_status decode_rax(int, uint32_t, uint64_t, const void *, size_t, rax_decoded *out)
{
    ++fixture.rax_calls;
    *out = {};
    out->valid = 1;
...
rax_status analyze_rax(int, uint32_t, uint64_t, const void *, size_t, rax_analysis *out,
                       rax_analysis_effect *, size_t, size_t *required)
{
    ++fixture.smir_calls;
    *out = {};
...
StaticAnalysisResult analyze(const std::vector<FuncChunk> &chunks, size_t cap,
                             HybridArch architecture = HybridArch::X86_64, bool want_static = true,
                             bool want_smir = true)
{
    ProgramImage image;
    ...
    if (!chunks.empty())
    {
        const auto first = std::min_element(chunks.begin(), chunks.end(),
                                            [](const FuncChunk &a, const FuncChunk &b)
...
void test_large_tail_budget()
{
    ...
    check(fixture.projection_calls == 2 && fixture.rax_calls == 2 && fixture.smir_calls == 2 &&
              fixture.flags_calls == 3,
          "the cap must bound xrefs, rax, SMIR, and dense-head visits");
    check(fixture.next_calls == 2, "truncation must not call next_head again");

    fixture = {};
    fixture.heads = {{0x1000, {}}, {0x2000, {}}, {0x3000, {}}};
    result = analyze({{0x1000, 0x1001}, {0x2000, 0x2001}, {0x3000, 0x3001}}, 1);
    check(result.stats.truncated && fixture.decode_calls == 2 && fixture.projection_calls == 1 &&
              fixture.flags_calls == 2,
...
{
    fixture = {};
    fixture.heads = {{0x2000, {}},
                     {0x1000, {}},
                     {0x1001, {1, false, false}},
                     {0x1002, {1, true, false}},
                     {0x3000, {1, false, false}}};
    const auto result =
        analyze({{0x2000, 0x2001}, {0x1000, 0x1003}, {0x2800, 0x2800}, {0x3000, 0x3001}}, 2);
    ...
    check(!analyze({{0x1000, 0x1002}}, 0).stats.truncated && fixture.projection_calls == 0 &&
              fixture.rax_calls == 0,
          "zero budget with no decodable instructions is complete");
    fixture = {};
    fixture.heads = {{0x1000, {}}, {0x1001, {}}};
    check(analyze({{0x1000, 0x1002}}, 0).stats.truncated && fixture.decode_calls == 1 &&
              fixture.projection_calls == 0,
...
void test_macro_components()
{
    ...
    check(fixture.decode_calls == 1 && fixture.projection_calls == 1 && fixture.rax_calls == 3 &&
              fixture.smir_calls == 3 && fixture.next_calls == 0,
          "a partial macro must stop head/chunk traversal immediately");
    check(result.instructions.size() == 3 && result.instructions[0].ida_projection_present &&
    ...
    check(!result.stats.truncated && result.stats.canonical_instructions == 4 &&
              fixture.decode_calls == 1 && fixture.rax_calls == 4 && fixture.smir_calls == 4 &&
              result.stats.decoder_comparisons == 0,
...
void test_cancellation()
{
    ...
    check(result.stats.truncated && result.instructions.empty() && fixture.decode_calls == 0 &&
              fixture.flags_calls == 0,
    ...
    check(result.stats.truncated && result.instructions.size() > 0 &&
              ...
              fixture.decode_calls == 1 && fixture.projection_calls == 1 && fixture.next_calls == 0,
