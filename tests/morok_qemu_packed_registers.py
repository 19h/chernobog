"""Record bounded x86-64 register states at Morok packed instruction entries."""
...
def main():
    output = Path(os.environ["CHERNOBOG_PACKED_REGISTER_OUTPUT"])
    binary = Path(os.environ["CHERNOBOG_PACKED_BINARY"])
    stdin = Path(os.environ["CHERNOBOG_PACKED_STDIN"])
    limit = int(os.environ["CHERNOBOG_PACKED_REGISTER_LIMIT"])
    if not 1 <= limit <= 4096:
    ...
    memory = gdb.selected_inferior().read_memory(ROOT, 65536).tobytes()
    report = {
        "schema": 1,
        "binary_sha256": hashlib.sha256(binary.read_bytes()).hexdigest(),
        "input_sha256": hashlib.sha256(stdin.read_bytes()).hexdigest(),
        "entry_packed_65536_sha256": hashlib.sha256(memory).hexdigest(),
    ...
    for _ in range(limit):
        state = registers()
        pc = int(state["rip"], 16)
        encoded = gdb.selected_inferior().read_memory(pc, 16).tobytes()
        report["entries"].append({"pc": hex(pc), "bytes_16_hex": encoded.hex(), "registers": state})
        try:
    ...
    print(f"[chernobog][packed-registers] {len(report['entries'])} entries; {report['stop']}")
