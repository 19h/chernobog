...
uint8_t arithmetic_oracle(unsigned a, unsigned b, unsigned carry, bool sub)
{
    ...
    const int nibble = sub ? int(a % 16) - int(b % 16) - int(carry)
                          : int(a % 16 + b % 16 + carry);
    unsigned ones = 0;
    for ( unsigned v = byte; v; v /= 2 ) ones += v % 2;
    return uint8_t((exact < 0 || exact > 255 ? CF : 0)
        | (ones % 2 == 0 ? PF : 0) | (nibble < 0 || nibble > 15 ? AF : 0)
        | (byte == 0 ? ZF : 0) | (byte >= 128 ? SF : 0)
        | (signed_exact < -128 || signed_exact > 127 ? OF : 0));
}

#if defined(__x86_64__) && (defined(__GNUC__) || defined(__clang__))
uint8_t architectural_flags(uint64_t flags)
{
    return uint8_t(((flags & 1) ? CF : 0) | ((flags & 4) ? PF : 0)
        | ((flags & 16) ? AF : 0) | ((flags & 64) ? ZF : 0)
        | ((flags & 128) ? SF : 0) | ((flags & 2048) ? OF : 0));
}

std::pair<uint8_t, uint8_t> native_arithmetic(
    uint8_t a, uint8_t b, uint64_t carry, Operation op)
{
    uint64_t flags = 0;
#define NATIVE(binary) asm volatile("btq $0, %3; " binary " %b2, %b0; pushfq; popq %1" \
    : "+q"(a), "=r"(flags) : "q"(b), "r"(carry) : "cc", "memory")
    switch ( op )
    {
        case Operation::add: NATIVE("addb"); break;
        case Operation::adc: NATIVE("adcb"); break;
        case Operation::sub: NATIVE("subb"); break;
        case Operation::sbb: NATIVE("sbbb"); break;
        default: std::abort();
    }
#undef NATIVE
...
std::pair<uint8_t, uint8_t> native_shift(
    uint8_t a, uint8_t count, uint64_t initial, Operation op)
{
    uint64_t flags = 0;
#define SHIFT(binary) asm volatile("pushq %3; popfq; " binary " %%cl, %b0; pushfq; popq %1" \
    : "+q"(a), "=r"(flags) : "c"(count), "r"(initial) : "cc", "memory")
    switch ( op )
    {
        case Operation::shift_left: SHIFT("shlb"); break;
        case Operation::shift_right: SHIFT("shrb"); break;
        case Operation::arithmetic_right: SHIFT("sarb"); break;
        default: std::abort();
    }
#undef SHIFT
    return {a, architectural_flags(flags)};
...
{
    size_t cases = 0;
    for ( Operation op : {Operation::shift_left, Operation::shift_right, Operation::arithmetic_right} )
...
{
    constexpr std::array<Operation, 4> ops = {
...
void exhaustive_conditions()
{
    for ( unsigned profile = 0; profile < 729; ++profile )
    {
        ...
        for ( unsigned bit = 0; bit < 6; ++bit, encoded /= 3 )
            if ( encoded % 3 != 0 ) flags.set(uint8_t(1u << bit), encoded % 3 == 2);
        for ( unsigned code = 0; code < 16; ++code )
        {
            ...
            for ( unsigned f = 0; f < 64; ++f )
                ...
                {
                    if ( condition_oracle(code, f) ) yes = true;
                    else no = true;
                }
            const auto result = evaluate(Condition(code), flags);
            check(result.has_value() == (yes != no), "condition defined exactly when every completion agrees");
            if ( result ) check(*result == yes, "condition truth value");
        }
    }
...
void boundaries()
{
    for ( unsigned bits : {8u, 16u, 32u, 64u} )
    {
        ...
        Flags f;
        check(transfer(Operation::add, bits, m, 1, false, f) == 0
              && f.get(CF) == true && f.get(ZF) == true && f.get(OF) == false,
              "all widths wrap unsigned addition");
        check(transfer(Operation::add, bits, s - 1, 1, false, f) == s
              && f.get(OF) == true && f.get(SF) == true && f.get(CF) == false,
              "all widths signed addition overflow");
        check(transfer(Operation::sub, bits, s, 1, false, f) == s - 1
              && f.get(OF) == true && f.get(SF) == false,
              "all widths signed subtraction overflow");
        f.set(CF, true);
        check(transfer(Operation::increment, bits, m, {}, false, f) == 0
              && f.get(CF) == true, "INC preserves known carry");
        f.forget(CF);
        transfer(Operation::decrement, bits, 0, {}, false, f);
        check(!f.get(CF) && f.get(SF) == true, "DEC preserves unknown carry");
        check(transfer(Operation::sub, bits, {}, {}, true, f) == 0
              && f.get(CF) == false && f.get(ZF) == true, "SUB self known zero");
        ...
        check(!f.get(OF) && !f.get(AF) && f.get(SF) == true, "SAR multi-count defined and undefined flags");
        transfer(Operation::shift_left, bits, 1, {}, false, f);
        check(f.known == 0, "unknown shift count invalidates flags");
    }
    Flags f;
    f.set(CF, true); f.set(ZF, true);
...
void mapping_counterexamples()
{
    for ( unsigned x = 0; x < 256; ++x )
        for ( unsigned c = 0; c < 256; ++c )
        {
            ...
            check(((~(~a)) & 255) == a && ((~(~o)) & 255) == o,
                  "correct double-NOT identities");
        }
    const uint32_t a = 0x1000, b = 0x10ff, k = 1;
    check(((a & ~k) + (b & k)) == 0x1001
          && ((a & ~k) + (b & k)) != (k ? b : a),
