...
static Program program(uint32_t key, uint8_t unit_bytes)
{
    Program result;
    result.nodes = {
        {Operation::CONSTANT, 32, false, 0, 0, key, 0},
...
static std::vector<uint8_t> reference_encrypt(const std::vector<uint8_t> &plain,
                                             uint32_t key, uint8_t unit_bytes, bool big_endian)
{
...
int main()
{
    ...
    for ( unsigned trial = 0; trial < 64; ++trial )
    {
        ...
        for ( uint8_t width : {uint8_t(1), uint8_t(2)} )
        {
            ...
            for ( size_t index = 0; index < contract.units; ++index )
            {
                ...
                const auto expected = (uint64_t(reference_rotate(key, index)) + index) & mask(uint8_t(width * 8));
                check(value && value->constant == expected, "schedule disagrees with independent rotate oracle");
                ++checked_indices;
            }
        }
    }
    ...
    for ( const auto &plain : {utf8, utf16le, utf16be} )
    {
        for ( uint8_t width : {uint8_t(1), uint8_t(2)} )
        {
            if ( plain.size() % width ) continue;
            for ( bool big_endian : {false, true} )
            {
                const auto encrypted = reference_encrypt(plain, key, width, big_endian);
                const auto decoded = decode_bytes(program(key, width),
                    Contract{key, plain.size() / width, width, big_endian}, encrypted);
                check(decoded && *decoded == plain, "typed byte/word endian transform must recover exact bytes");
            }
        }
    }
    const auto byte_utf16_cipher = reference_encrypt(utf16le, key, 1, false);
    const auto byte_decoded = decode_bytes(program(key, 1), Contract{key, 10, 1, false}, byte_utf16_cipher);
    ...
    const auto wrong_unit = decode_bytes(program(key, 2), Contract{key, 5, 2, false}, byte_utf16_cipher);
    check(wrong_unit && *wrong_unit != utf16le, "UTF-16 content must not select the wide-unit schedule");
    check(decode_text(utf8, text_encoding_t::utf8)->utf8 == u8"VMP \u03A9",
          "UTF-8 validation must preserve non-ASCII scalars");
    check(decode_text(utf16be, text_encoding_t::utf16_be)->utf8 == "Wide", "explicit UTF-16BE decode");
    auto invalid = utf16le;
    invalid[0] = 0; invalid[1] = 0xD8;
    check(!decode_text(invalid, text_encoding_t::utf16_le), "unpaired UTF-16 surrogate must reject text");
    invalid = utf8; invalid.pop_back();
    check(!decode_text(invalid, text_encoding_t::utf8), "unterminated output must not become a string");
    ...
    for ( unsigned negative = 0; negative < 10; ++negative )
    {
        ...
        switch ( negative )
        {
            case 0: expression.nodes[2].operation = Operation::ROR32; break;
            case 1: expression.nodes[1].step = 2; break;
            case 2: expression.nodes[0].value ^= 1; break;
            case 3: expression.nodes[2].bits = 64; break;
            case 4: expression.nodes[5].operation = Operation::ADD; break;
            case 5: expression.nodes[4].bits = 16; break;
            case 6: expression.nodes[5].left = 5; break;
            case 7: contract.units = maximum_bytes + 1; break;
            case 8: expression.nodes[1].value = 1; break;
            case 9: expression.nodes[2].bits = 8; break;
        }
        check(!prove(expression, contract), "wrong direction/index/key/width/input/cycle/bounds must abstain");
    }
    ...
    for ( uint64_t index = 0; index < 65; ++index )
    {
        ...
        for ( uint32_t input = 0; input < 256; ++input )
        {
            ...
            for ( unsigned bit = 0; bit < symbolic->bits; ++bit )
            {
                ...
                unsigned parity = 0;
                while ( terms ) { parity ^= terms & 1; terms >>= 1; }
                actual ^= uint64_t(parity) << bit;
            }
            const uint8_t expected = uint8_t(input ^ (uint64_t(reference_rotate(key, index)) + index));
            check(uint8_t(actual) == expected, "symbolic input coefficients disagree with scalar truth table");
        }
    }
    auto narrow_counter = program(key, 1);
    narrow_counter.nodes[1].bits = 8;
    check(!prove(narrow_counter, Contract{key, 129, 1, false}), "signed induction overflow must reject proof");
