...
try:
    ...
    def capture(models, seed=0, args=None):
        ...
        return json.loads(
            evaluate(
                f"chernobog_vm_trace_temporal({target},{seed},{json.dumps(request)},{json.dumps(models)})"
            )
        )
    ...
    for seed in (0, 1, 17, 0xC0FFEE):
        ...
        traces.append(trace)
        check(
            "available explicit region",
            trace.get("available") and trace["native_temporal_requested"] and trace["ran"],
        )
        check(
            "separate publication contract",
            not trace["function_evidence_published"]
            and not trace["vm_identity_proved"]
            and not trace["temporal_capture_complete"],
        )
        check(
            "bound model identities",
            trace["environment_bindings"]
        ...
        if trace["native_temporal_complete"]:
            ...
            check(
                "independent native byte oracle",
                registers.get(0x100) == int.from_bytes(b"secret!\0", "little")
                and registers.get(0x102) == int.from_bytes(b"second!\0", "little"),
            )
            check(
                "completed allocation lifetimes",
                len(trace["allocations"]) == 2
                and all(
                    r["live"] == "false" and int(r["released"]) > int(r["allocated"])
                    for r in trace["allocations"]
                ),
            )
            check(
                "completed modeled stack",
                trace["reached_sentinel"]
                and trace["sp_valid"]
                and trace["sp_delta"] == 8
                and trace["summarized_calls"] == 6
                and not trace["data_trace_complete"],
            )
        else:
            ...
            trace["rejected_instruction"] = {
                "site": hex(site),
                "bytes": (ida_bytes.get_bytes(site, size) or b"").hex(),
    ...
    check(
        "no ordinary evidence publication", evaluate(f"chernobog_evidence_view({target})") == before
    )
    invalid = [
        "{",
        "[" * 3 + "]" * 3,
        ...
        [dict(bindings[0], extra=1)],
        [dict(bindings[0], name="unknown_model")],
        [dict(bindings[0], address="0xffffffffffffffff")],
    ...
    check(
        "overlapping caller object rejected",
        capture(
            bindings,
            args={"args": ["0x0"], "objects": [{"argument": 0, "offset": 0, "bytes": "00"}]},
        ).get("available")
        is False,
    )
    old = bindings[0]["name"]
...
(Path(os.environ["IDAUSR"]).parent / "region_temporal.json").write_text(
