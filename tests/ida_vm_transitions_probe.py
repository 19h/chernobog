...
try:
    ...
    for suffix, count in (("", 12), ("_boundary", 4), ("_relative", 4)):
        ...
        check(suffix + " pure state projection has no SMT", api("chernobog_solver_evidence", target) == before)
        check(suffix + " complete direct trace", len(observed["states"]) == count
        ...
        captures[suffix or "repeated"] = {"observed": observed, "checked": checked, "queries": proof}
        check(suffix + " every local transition corroborated", len(checked["states"]) == count
              and all(r["semantic_validation"] == "corroborated for captured transition" for r in checked["states"]))
        check(suffix + " bounded real query counts", checked["transition_attempts"] == count
              and checked["transition_queries"] == 2*count)
        rows = [r for r in proof["records"] if r["phase"] == "vm-observed-transition"]
        check(suffix + " SAT inputs before UNSAT mismatch", len(rows) == 2*count
              and [r["result"] for r in rows] == ["sat", "unsat"]*count
              and all(r["role"] in ("VM observed input consistency", "VM observed output mismatch") for r in rows))
        check(suffix + " queries identify exact check and visit", all(
            len([q for q in rows if q.get("transition_check") == r["transition_check"]
                 and q.get("capture_revision") == r["revision"]
                 and all(q.get(k) == r[k] for k in ("run", "seed", "sequence", "site"))]) == 2
            for r in checked["states"]))
        check(suffix + " scope remains local and partial", all(r["logical_state_complete"] == "false"
              and r["merge"] == "not admitted" and "target execution not admitted" in r["transition_contract"]
              for r in checked["states"]))
        ...
        if suffix == "_relative":
            ...
            check("stale capture yields no checks", not stale["states_available"] and not stale["states"]
                  and stale["transition_queries"] == 0 and stale["transition_attempts"] == 0)
            ida_bytes.patch_byte(source, byte)
            check("restored capture permits explicit checks", api("chernobog_vm_transitions", target)["transition_queries"] == 8)
    if ida_kernwin.is_idaq():
        from PySide6 import QtWidgets
        target = address("_relative")
        spec = importlib.util.spec_from_file_location("vm_transition_view", os.environ["CHERNOBOG_VIEW_MODULE"])
        ...
        check("Qt transition result and contract visible", "corroborated for captured transition" in form.detail.toPlainText()
              and "one captured normal-completion transition" in form.detail.toPlainText() and form.jump.isEnabled())
        check("Qt result links its two SMT queries", '"transition_query_references"' in form.detail.toPlainText()
        ...
        check("Qt SMT tab includes real transition queries", any(r["phase"] == "vm-observed-transition" for r in form.queries["records"]))
        source = address("_relative_dispatch")
        byte = ida_bytes.get_byte(source)
        ida_bytes.patch_byte(source, 0x90)
        form.poll()
        check("historical result loses navigation", not form.jump.isEnabled()
...
line = "[chernobog][vm-transitions] " + ("FAIL " + "; ".join(errors) if errors else "PASS checks=%d" % len(checks))
