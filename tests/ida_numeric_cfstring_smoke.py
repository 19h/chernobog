...
SYMBOL_SIZES = {
    "cf_projection_good_one": 32,
    "cf_projection_good_two": 32,
    "cf_projection_bad_flags": 32,
    "cf_projection_bad_length": 32,
    "cf_projection_bad_class": 32,
    "cf_projection_no_fact": 32,
    "cf_projection_store_offsets": 40,
    "cf_projection_key": 1,
    "cf_projection_delta": 8,
    "cf_projection_selector": 4,
    "cf_projection_output_one": 15,
    "cf_projection_output_two": 15,
    "cf_projection_stores": 80,
    "cf_projection_not_class": 8,
    "cf_projection_stores_ptr": 8,
    "cf_projection_encoded_one": 15,
    "cf_projection_encoded_two": 15,
    "cf_projection_no_fact_bytes": 15,
}
OP_NAMES = {
    getattr(ida_hexrays, name): name
    for name in dir(ida_hexrays)
    if name.startswith("cot_") and isinstance(getattr(ida_hexrays, name), int)
}
PREFIX = "IDB CFString "
...
def metadata_snapshot():
    ...
    return {
        "ranges_sha256": digest(ranges),
        "saved_ctree_comments": saved_count,
        "range_components": {
            name: {
                "bytes_sha256": hashlib.sha256(bytes.fromhex(data)).hexdigest(),
...
def shape(expression):
    result = {
        ...
        "ea": expression.ea,
        "type": scalar_type(expression),
    }
    if expression.op == ida_hexrays.cot_num:
        nf = expression.n.nf
        result.update(
            value=expression.numval(),
            number_format={
                "flags": nf.flags,
                "props": char_number(nf.props),
...
class Stores(ida_hexrays.ctree_visitor_t):
    ...
    def visit_expr(self, expression):
        if expression.op == ida_hexrays.cot_asg and expression.ea in store_names:
...
def read_stage(name, expected_one):
    ...
    check(set(stores.assignments) == set(CASES), name + ": expected every native store in ctree")
    for case in CASES:
        assignment = stores.assignments.get(case)
        if assignment is None:
            continue
        coords = cfunc.find_item_coords(assignment)
        line = lines[coords[1]] if coords is not None and 0 <= coords[1] < len(lines) else None
        rhs = assignment.y
        row = {
            "assignment_ea": assignment.ea,
            "lhs": shape(assignment.x),
            "rhs": shape(rhs),
            "address_identity": address_identity(rhs),
            "coordinates": coords,
            "line": line,
        }
        stage["stores"][case] = row
        check(line is not None, name + ": missing coordinates for " + case)
        check(rhs.op != ida_hexrays.cot_str, name + ": object address became cot_str in " + case)
        if case in ("good_one", "good_two"):
            expected = (
                expected_one
                if case == "good_one"
                else (symbols["cf_projection_output_two"], "projection-two")
            )
            object_address = symbols["cf_projection_" + case]
            if expected is not None:
                check(
                    row["address_identity"] == object_address,
                    name + ": address AST missing/changed for " + case,
                )
                check(
                    rhs.type.get_size() == 8
                    and rhs.type.is_signed()
                    and assignment.x.type.get_size() == 8,
                    name + ": signed 64-bit store type changed in " + case,
                )
                annotation = 'IDB CFString 0x%X -> bytes 0x%X; rax-final: "%s"' % (
                    object_address,
                    expected[0],
                    expected[1],
                )
                check(
                    line is not None and annotation in line,
                    name + ": exact use-site relation absent in " + case,
                )
            else:
                check(
                    line is not None and PREFIX not in line,
                    name + ": invalid descriptor retained overlay",
                )
                check(
                    rhs.op == ida_hexrays.cot_num and rhs.numval() == object_address,
            ...
            if case in ("bad_flags", "bad_length", "bad_class", "no_fact", "mov_only"):
                ...
                check(rhs.op == ida_hexrays.cot_add, name + ": arithmetic expression changed")
                if rhs.op == ida_hexrays.cot_add:
                    check(
                        any(
                            e.op == ida_hexrays.cot_num
                ...
                check(
                    assignment.x.type.get_size() == 4 and rhs.type.get_size() == 4,
                    name + ": narrow store widened",
                )
    check(
        len(stage["overlay_lines"]) == 1 + (expected_one is not None),
        name + ": unexpected overlay line count",
    )
    check(str(cfunc) == text, name + ": repeated direct AST printing changed")
    second_lines = [ida_lines.tag_remove(line.line) for line in cfunc.get_pseudocode()]
    check(
        second_lines == lines, name + ": repeated displayed pseudocode changed/duplicated overlays"
    )
    active = cfunc.user_cmts
    stage["active_ctree_comments"] = 0 if active is None else active.size()
    check(stage["active_ctree_comments"] == 0, name + ": overlay entered saveable ctree comments")
    stage["metadata_after"] = metadata_snapshot()
    check(
        stage["metadata_after"] == before,
    ...
    output.joinpath("numeric_cfstring_" + name + "_ast.txt").write_text(text, encoding="utf-8")
    save_report()
    return cfunc
...
def mutate_header(offset, value, name, expected):
    ...
    report["probe_mutations"].append(
        {"name": name, "address": address, "original": original.hex(), "replacement": changed.hex()}
    )
    try:
    ...
    check(
        ida_bytes.get_bytes(address, 8) == original,
...
try:
    ...
    monitored_ranges += [(name, symbols[name], size) for name, size in SYMBOL_SIZES.items()]
    store_sites = {
        case: function + ida_bytes.get_dword(symbols["cf_projection_store_offsets"] + 4 * index)
    ...
    for case, address in store_sites.items():
        ...
        row = {
            "address": address,
            "instruction": ida_lines.tag_remove(ida_lines.generate_disasm_line(address, 0) or ""),
            "window_bytes": ida_bytes.get_bytes(address - 8, 12).hex(),
            "code_xrefs": [
                {"from": xref.frm, "type": xref.type}
        ...
        check(native.get_canon_mnem().upper() == "STR", "fixture site is not native STR: " + case)
        check(
            row["store_bytes"] == (4 if case == "narrow" else 8),
            "fixture native store width differs: " + case,
        )
    check(
        any(
            (entry["type"] & ida_xref.XREF_MASK) != ida_xref.fl_F
    ...
    for case in CASES[:6]:
        ...
        check(
            slots[0]
            == (symbols["cf_projection_not_class"] if case == "bad_class" else descriptor_class),
            "fixture descriptor class mismatch: " + case,
        )
        check(
            slots[1] == (0x7D0 if case == "bad_flags" else 0x7C8),
            "fixture descriptor flags mismatch: " + case,
        )
        check(
            slots[3] == (13 if case == "bad_length" else 14),
    ...
    for name in SYMBOL_SIZES:
        check(
            report["before_exploration"]["range_components"][name]["bytes_sha256"]
            == initial_snapshot["range_components"][name]["bytes_sha256"],
            "runtime exploration changed fixture data bytes: " + name,
        )
    check(initial_snapshot["saved_ctree_comments"] == 0, "fresh fixture has saved ctree comments")
    ...
    mutate_header(
        16,
        symbols["cf_projection_output_two"],
        "retargeted",
    ...
    check(
        metadata_snapshot() == initial_snapshot,
    ...
    finish(
        0,
        "PASS preserved-addresses=2 rejected-use-sites=8 header-mutations=3 transient-overlay=verified",
    )
