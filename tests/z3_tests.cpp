...
struct QueryCollector : chernobog::solver_evidence::Collector
{
    ...
    void publish(const chernobog::solver_evidence::Origin &,
                 chernobog::solver_evidence::Row row) override
    {
...
void test_unique_model_values()
{
    ...
    check(seven.has_value() && *seven == 7, "an equality-constrained value is unique");
    check(solver.check() == z3::sat, "uniqueness probing preserves caller solver assertions");
    ...
    check(zero.has_value() && *zero == 0, "an algebraically constant expression is unique");
...
void test_query_evidence()
{
          ...
          "unscoped queries must not inherit an invented source location");
    {
        ...
        check(captured.rows.back().at("result") == "unsat" &&
                  !captured.rows.back().count("model_0_symbol"),
              "UNSAT must not borrow an earlier SAT model");
        solver.pop();
        ...
        bounded.set(parameters);
        const auto a = context.bv_const("budget_a", 32), b = context.bv_const("budget_b", 32);
        bounded.add(a * b == 1234567);
        check(chernobog::solver_evidence::check(bounded, "resource control", "rlimit=1") ==
                  z3::unknown,
              "resource exhaustion remains UNKNOWN");
        check(captured.rows.back().at("result") == "unknown" &&
                  !captured.rows.back().at("reason").empty() &&
                  !captured.rows.back().count("model_0_symbol"),
              "UNKNOWN retains its reason without a model");
        ...
        check(limited.at("formula_complete") == "false" &&
                  limited.at("formula").size() == formula_limit &&
                  limited.at("model_constants_omitted") == "32" &&
                  limited.at("model_complete") == "false",
        ...
        check(chernobog::solver_evidence::check(deep, "AST bound") == z3::sat &&
                  ...
                  !captured.rows.back().count("formula"),
              "oversized AST serialization is omitted, not executed again");
    }
...
void test_recurrent_selector_normalization()
{
    ...
    const z3::expr r14 = (bv(0xC2CC86A3U) * ((sp + bv(8)) ^ bv(0x4AE62630U))).rotate_left(7);
    const z3::expr mixed = bv(0x279D2A17U) * (state ^ bv(0x50256DAEU)) - bv(0x2E6FD155U);
    ...
    {
        return bv(0x83DAC1F1U) * ((next_aux ^ (decoder_r14 ^ ((r12 ^ next_state) ^
                                                              z3::lshr(next_state, bv(13))))) ^
                                  bv(0x2ED4B00EU)) -
               bv(0x0E5A9399U);
    };
