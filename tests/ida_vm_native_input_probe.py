...
def inventory():
    ...
    for i in range(ida_segment.get_segm_qty()):
        ...
        h.update(str((int(segment.start_ea), int(segment.end_ea), int(segment.bitness), int(segment.perm))).encode())
...
def request(ea, value, seed=0, walk=None):
    ...
    function = ("chernobog_vm_trace_check" if os.environ.get("CHERNOBOG_NATIVE_CHECK") == "1"
                else "chernobog_vm_trace_walk") if walk else "chernobog_vm_trace_input"
...
try:
    ...
    cases = [(0, 0, 0), (0, 1, 0xffffffff), (1, 0, 0x80000000),
             (0x7fffffff, 1, 0), (0x80000000, 0xffffffff, 1),
             (0xffffffff, 0x7fffffff, 0xfffffffe), (15, 1, 0), (1, 1, 1)]
    state = 0xc0ffee
    for _ in range(8):
        values = []
        for _ in range(3):
    ...
    for name, ea in entries.items():
        for index, (x, y, initial) in enumerate(cases):
            ...
            check(prefix + " isolated evidence", trace["explicit_input"] and trace["scope"] == "native-region"
                  and not trace["function_evidence_published"] and not trace["vm_identity_proved"])
            objects = trace["input_objects"]
            check(prefix + " object snapshot", len(objects) == 1 and objects[0]["initial"] == data
                  and objects[0]["readable"] == "true" and len(objects[0]["final"]) == len(data))
            ...
            if os.environ.get("CHERNOBOG_NATIVE_WALK") == "1":
                ...
                check(prefix + " bounded native walk", trace["native_walk"]
                      and len(trace["native_admissions"]) <= 64 and trace["planned_heads"] <= 4096
                      and len(trace["execution"]) <= 4096 and len(trace["data"]) <= 4096)
                check(prefix + " matched snapshot and input", baseline["available"] and baseline["ran"]
                ...
                captures[-1]["separate_run_prefix_equal"] = {
                    field: ([s for s in trace[field] if s["kind"] != "native instruction entry"]
                            if field == "states" else trace[field])[:len(baseline[field])] == baseline[field]
                ...
                for step in trace["native_admissions"]:
                    edge = [row for row in trace["edges"] if row["source"] == step["source"]
                            and row["target"] == step["target"] and row["sequence"] == step["sequence"]]
                    check(prefix + " observed admission provenance", len(edge) == 1 and step["before_identity"] == identity)
                    identity = step["after_identity"]
                    if step["admitted"] == "true":
                        check(prefix + " admitted target in plan", int(step["added_heads"]) > 0
                              and any(head["site"] == step["target"] for head in trace["heads"]))
                    else:
                        check(prefix + " rejected admission preserves plan", step["after_identity"] == step["before_identity"]
            ...
            if os.environ.get("CHERNOBOG_NATIVE_CHECK") == "1":
                ...
                check(prefix + " native projection available", view["available"] and trace["native_state_capture_requested"]
                      and trace["native_state_capture_complete"])
                samples = [state for state in trace["states"] if state["kind"] == "native instruction entry"]
                check(prefix + " one state per entered instruction", [(s["site"], s["sequence"]) for s in samples]
                      == [(e["site"], e["sequence"]) for e in trace["execution"]])
                check(prefix + " projection quotas", len(trace["states"]) <= 12289 and view["path_steps"] <= 8192
                      and len(view["records"]) <= 128 and view["transition_attempts"] <= 16 and view["queries"] <= 32)
                for row in view["records"]:
                    check(prefix + " native candidate provenance", row["scope"] == "native-region"
                          and int(row["capture"], 0) == trace["capture"] and row["region_identity"] == trace["region_identity"]
                          and row["image_hash"] == trace["image_hash"] and row["logical_state_complete"] == "false"
                          and row["virtual_stack"] == row["vm_context"] == row["memory_epoch"] == "unknown"
                          and row["merge"] == "not admitted")
                    if row["semantic_validation"] == "corroborated for captured transition":
                        check(prefix + " nonvacuous captured transition", row["transition_queries"] == "2"
                              and row["path"] == "complete captured native path" and row["data_capture_complete"] == "true")
            ...
            if trace["reached_sentinel"]:
                ...
                check(prefix + " defined flags", registers.get(0x12 if mode == 64 else 0x13, -1) & 0x8d5 == flags)
                check(prefix + " selected memory and guards", len(objects) == 1 and objects[0]["final"] == memory)
    ...
    invalid = ["{", '{"args":[],"objects":[],"extra":0}',
               '{"args":[],"args":[],"objects":[]}', '[' * 9 + ']' * 9,
               {"args": [1], "objects": []}, {"args": ["0x10000000000000000"], "objects": []},
               {"args": ["0x0"] * 33, "objects": []},
               {"args": ["0x0"], "objects": [{"argument": 0, "offset": 0, "bytes": "00"}] * 17}]
    for field, bad in (("offset", 1), ("offset", -1), ("argument", 1), ("argument", -1),
                       ("bytes", ""), ("bytes", "0"), ("bytes", "gg"), ("bytes", "00" * 4097)):
    ...
    for index, bad in enumerate(invalid):
        ...
        check("invalid input " + str(index), not answer["available"] and answer["reason"] == "invalid bounded native input")
    if captures[0]["trace"]["address_bits"] == 32:
        answer = request(ea, {"args": ["0x100000000"], "objects": []})
        check("architecture width rejection", not answer["available"] and answer["reason"] == "argument exceeds architecture width")
    after = inventory()
    check("database unchanged", before == after)
    check("ordinary publications unchanged", publication == {
...
report = {"schema": 1, "passed": not errors, "checks": checks, "errors": errors,
          "inventory_before": before, "inventory_after": after, "captures": captures}
(Path(os.environ["IDAUSR"]).parent / "vm_native_inputs.json").write_text(json.dumps(report, indent=2) + "\n")
