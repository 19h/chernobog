...
def main():
    ...
    parser.add_argument("--strings", action="store_true", help="Measure scoped string consensus and freshness")
    ...
    files = ("tests/run_vmp_region_temporal.py", "tests/" + probe_name, "tests/run_ida_smoke.py",
             ...
             "src/hybrid/emu_driver.cpp", "src/hybrid/temporal_memory.hpp", "src/hybrid/call_summary_policy.cpp",
             "src/plugin/idc_api.cpp", "src/plugin/deobf_plugin.cpp", "tests/hybrid_tests.cpp",
             "src/hybrid/evidence.hpp", "src/hybrid/evidence.cpp", "src/hybrid/native_read_strings.cpp",
    ...
    report = {"schema": 1, "passed": False, "source_sha256": sources, "plugin_sha256": plugin_hash,
              "ida_sha256": ida_hash, "corpus_report_sha256": digest(corpus_path), "runs": [], "strings": args.strings,
    ...
    try:
        for label, expected in binaries.items():
            ...
            measurement, _, _ = execute([sys.executable, "-B", root / "tests/run_ida_smoke.py", binary,
                root / "tests" / probe_name, "--ida", args.ida.resolve(), "--plugin", args.plugin.resolve(),
                "--output-dir", run_dir, "--enable-rax", "--set", "CHERNOBOG_STRING_ENTRY=" + corpus["selected_entry"],
                "--set", "CHERNOBOG_EXPECT_RETURN=" + str(int(expected_return))], timeout=120)
            ...
            item["artifact_sha256"] = {name: digest(run_dir / name) for name in ("run.json", artifact)}
            print(json.dumps({k: item[k] for k in ("label", "checks", "completed", "instructions")}), flush=True)
        ...
        (output / ("region_strings_analysis.json" if args.strings else "region_temporal_analysis.json")).write_text(json.dumps(report, indent=2) + "\n")
