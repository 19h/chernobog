...
def main():
    ...
    files = ("tests/run_vmp_region_temporal.py", "tests/ida_region_temporal_probe.py", "tests/run_ida_smoke.py",
             ...
             "src/plugin/idc_api.cpp", "tests/hybrid_tests.cpp")
    ...
    report = {"schema": 1, "passed": False, "source_sha256": sources, "plugin_sha256": plugin_hash,
              "ida_sha256": ida_hash, "corpus_report_sha256": digest(corpus_path), "runs": [],
    ...
    try:
        for label, expected in binaries.items():
            ...
            measurement, _, _ = execute([sys.executable, "-B", root / "tests/run_ida_smoke.py", binary,
                root / "tests/ida_region_temporal_probe.py", "--ida", args.ida.resolve(), "--plugin", args.plugin.resolve(),
            ...
            capture = json.loads((run_dir / "region_temporal.json").read_text())
            ...
            assert run["script_sha256"] == sources["tests/ida_region_temporal_probe.py"]
            assert not capture["errors"] and all(r["passed"] for r in capture["records"]) and len(capture["traces"]) == 4
            ...
            item["artifact_sha256"] = {name: digest(run_dir / name) for name in ("run.json", "region_temporal.json")}
        ...
        (output / "region_temporal_analysis.json").write_text(json.dumps(report, indent=2) + "\n")
