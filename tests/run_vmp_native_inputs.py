...
def main():
    ...
    parser.add_argument(
        "--walk", action="store_true", help="opt in to bounded observed native transfers"
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="sample native instruction entries and check local VM transitions",
    )
    ...
    files = [
        "tests/run_vmp_native_inputs.py",
        "tests/ida_vm_native_input_probe.py",
        ...
        "src/hybrid/emu_input.hpp",
        "src/hybrid/abi_policy.cpp",
    ...
    if args.check:
        files += [
            "src/vm/native_observations.hpp",
        ...
        files += [
            "vendor/rax/src/isa/x86_64/decode/dispatch/legacy.rs",
            "vendor/rax/capi/src/reg.rs",
        ]
    ...
    try:
        for label, sha in binaries.items():
            ...
            command = [
                ...
                binary,
                root / "tests/ida_vm_native_input_probe.py",
                "--ida",
                args.ida.resolve(),
                "--plugin",
                args.plugin.resolve(),
                ...
                "--set",
                "CHERNOBOG_NATIVE_WALK=" + str(int(args.walk)),
                "--set",
                "CHERNOBOG_NATIVE_CHECK=" + str(int(args.check)),
                "--set",
                "CHERNOBOG_EXPECT_RETURN="
                + str(int(label == "original" or label.startswith("mutation-"))),
            ]
            ...
            assert (
                ...
                and all(c["passed"] for c in capture["checks"])
            )
            ...
            for record in capture["captures"]:
                ...
                initial = int.from_bytes(
                    bytes.fromhex(supplied["objects"][0]["bytes"])[4:8], "little"
                )
                result, flags, delta, left, changed, right = expected(
                    int(record["name"] == "corpus_branch"), x, y, initial
                )
            ...
            item["artifact_sha256"] = {
                name: digest(run_dir / name) for name in ("run.json", "vm_native_inputs.json")
            }
            print(
                json.dumps({key: item[key] for key in ("label", "checks", "returned", "stops")}),
                flush=True,
            )
        assert all(digest(root / name) == value for name, value in sources.items())
        assert (
            digest(args.plugin) == plugin_hash
            and digest(args.ida) == ida_hash
            and digest(corpus_path) == corpus_hash
        )
