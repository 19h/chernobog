...
{
    std::cerr << "unsupported executor fixture operation: " << description << '\n';
    std::abort();
}
...
{
    const bool memory_only_control =
        argc == 2 && std::strcmp(argv[1], "--memory-only-control") == 0;
    ...
    check(executor.check_feasibility_with(entry_value != 7) == feasibility_t::infeasible,
          "entry assumptions survive havoc");
    check(executor.check_feasibility_with(current_value == 8) == feasibility_t::feasible,
          "fresh recurrence register can differ from its entry value");
    check(executor.check_feasibility_with(current_value == 9) == feasibility_t::feasible,
          "a temporary feasible query does not constrain the next query");
    check(executor.check_feasibility_with(current_memory == 24) == feasibility_t::feasible,
          "fresh recurrence memory can differ from its prior binding");
    check(executor.check_feasibility_with(current_memory != 24) == feasibility_t::feasible,
          "memory query conditions are not retained");
    check(executor.check_feasibility_with(z3::ugt(current_value, 7)) == feasibility_t::feasible,
    ...
    check(executor.check_feasibility_with(executor.evaluate_operand(invariant_register) != 7) ==
    ...
    check(executor.check_feasibility_with(foreign_context.bool_val(true)) == feasibility_t::unknown,
          "a foreign-context query fails closed");
    check(executor.check_feasibility_with(z3::expr(context.ctx())) == feasibility_t::unknown,
    ...
    check(executor.check_feasibility_with(z3::ugt(executor.evaluate_operand(recurrent_register),
                                                  7)) == feasibility_t::infeasible,
    ...
    check(queries.check_feasibility_with(query_value == 31) == feasibility_t::feasible,
    ...
    check(queries.assume(query_value == 41), "a new constraint replaces the earlier witness");
    ...
    check(queries.check_feasibility_with(query_value != 41) == feasibility_t::infeasible,
