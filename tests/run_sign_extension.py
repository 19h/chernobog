"""Execute sign-extension oracles and inspect the same binaries in fresh IDA databases."""
...
def main():
    ...
    sources = [
        ...
        "tests/vmp_native/sign_extension_asm.c",
        "tests/vmp_native/sign_extension.c",
        "tests/ida_sign_extension_probe.py",
        "tests/run_sign_extension.py",
    ...
    try:
        for architecture in (["x86_64", "i386"] if args.linux32_image else ["x86_64"]):
            ...
            directory.mkdir()
            binary = directory / "sign_extension"
            ...
            if architecture == "x86_64":
                build, _, _ = execute(
                    [
                        ...
                        "-O2",
                        "-g0",
                        root / "tests/vmp_native/sign_extension_asm.c",
                        root / "tests/vmp_native/sign_extension.c",
                        "-o",
                        binary,
                    ]
                )
                ...
                build, _, _ = backend.execute(
                    [
                        ...
                        "-fno-pie",
                        "-no-pie",
                        "/source/vmp_native/sign_extension_asm.c",
                        "/source/vmp_native/sign_extension.c",
                        "-o",
                        "/output/sign_extension",
                    ]
                )
                assert build["exit_code"] == 0 and not build["timed_out"]
            ...
            assert row["native_result"] == {
                "passed": True,
                "checks": 6272 if architecture == "x86_64" else 4352,
                "oracle_cases": 5760 if architecture == "x86_64" else 3840,
                "static_inputs": 512,
            }
            ...
            measurement, _, _ = execute(
                [
                    ...
                    binary,
                    root / "tests/ida_sign_extension_probe.py",
                    "--ida",
                    args.ida.resolve(),
                    "--plugin",
                    args.plugin.resolve(),
                    ...
                    *(["--set", "CHERNOBOG_EXTENSION_BASELINE=1"] if args.baseline else []),
                ],
                timeout=120,
            )
            row["inspection"] = measurement
            assert measurement["exit_code"] == 0 and not measurement["timed_out"]
            capture = json.loads((directory / "inspection/sign_extension.json").read_text())
            ...
            row["artifact_sha256"] = {
                ...
                for path in ("inspection/run.json", "inspection/sign_extension.json")
            }
            print(
                json.dumps(
                    {
                        "architecture": architecture,
                        "native_checks": row["native_result"]["checks"],
    ...
    (output / "sign_extension_analysis.json").write_text(json.dumps(report, indent=2) + "\n")
