...
try:
    ...
    names = ["table_forward", "table_clone", "table_backward", "relative_backward", "relative_forward",
             "bad_stride", "bad_alias", "bad_extension", "quota", "scan_limit", "semantic_groups"]
    targets = {}
    for name in names:
        ...
        data = snapshot(start)
        captures[name] = data
        check(name + " inspection read-only and repeatable", before == ida_bytes.get_bytes(start, end-start)
              and data == snapshot(start))
        expected = (0 if name.startswith("bad_") or name == "scan_limit" else 64 if name == "quota"
                    else 4 if name == "semantic_groups" else 1)
        check(name + " candidate count", len(data["records"]) == expected)
        check(name + " no semantic or execution admission", all(r["truth"] == "candidate"
              and r["summary_reuse"].startswith("unproved") and "no VM execution" in r["ownership"]
              and "target" not in r for r in data["records"]))
        full = api("chernobog_vm_summaries", start)
        summary_captures[name] = full
        check(name + " summary inspection retains candidate bytes", data["records"] == full["records"]
              and before == ida_bytes.get_bytes(start, end-start))
        check(name + " summaries retain normal-completion contract", all("normal completion" in r["contract"]
              and "ordered data" in r["memory"] and "not admitted" in r["transition"] for r in full["summaries"]))
    a, b = captures["table_forward"]["records"][0], captures["table_clone"]["records"][0]
    check("renamed clone preserves normalized syntax", a["normalized_shape"] == b["normalized_shape"]
          and a["vip_register"] != b["vip_register"] and a["bytes"] != b["bytes"])
    check("backward predecrement recognized", captures["table_backward"]["records"][0]["direction"] == "backward")
    r = captures["relative_backward"]["records"][0]
    check("relative decode retains width-specific key update", r["read_bits"] == "32"
          and r["stack_key_update"] == ("true" if r["address_bits"] == "64" else "false")
          and r["dispatch_kind"] == "relative register")
    check("candidate quota reports exact omissions", captures["quota"]["omitted"] == 6 and not captures["quota"]["truncated"])
    check("scan exhaustion explicit", captures["scan_limit"]["truncated"] and captures["scan_limit"]["instructions"] == 1024)
    groups = summary_captures["semantic_groups"]
    bindings = groups["summary_bindings"]
    check("production modeled-effect references deduplicate", len(groups["summaries"]) == 2 and len(bindings) == 4)
    check("register-renamed clone reuses after UNSAT", bindings[1]["status"] == "reused after UNSAT"
          and bindings[1]["summary_id"] == bindings[0]["summary_id"])
    check("distinct syntax reuses after UNSAT", bindings[2]["status"] == "reused after UNSAT"
          and groups["records"][2]["normalized_shape"] != groups["records"][0]["normalized_shape"])
    check("changed decode has separate reference", bindings[3]["status"] == "reference"
          and bindings[3]["summary_id"] != bindings[0]["summary_id"] and "SAT:" in bindings[3]["reason"])
    query_rows = api("chernobog_solver_evidence", targets["semantic_groups"])["records"]
    check("summary proofs are real scoped SMT queries", len(query_rows) == 3
          and all(r["phase"] == "vm-local-summary" and r["role"] == "VM local summary effect mismatch" for r in query_rows)
          and [r["result"] for r in query_rows] == ["unsat", "unsat", "sat"])
    quota = summary_captures["quota"]
    check("summary bounds report scanned omissions", len(quota["summary_bindings"]) == 16
          and quota["summary_omitted"] == 54 and quota["comparison_attempts"] == 15)
    ...
    check("removing alternate entry restores candidate", snapshot(target) == captures["table_forward"])
    ...
    check("UI accepts recomputed candidate", a["vm_candidate"] in module.current_vm_candidates(saved, snapshot(target)))
    check("UI rejects foreign database", not module.current_vm_candidates(saved, dict(saved, database="foreign")))
    byte = ida_bytes.get_byte(target)
    ida_bytes.patch_byte(target, 0x90)
    check("changed decode invalidates candidate", not module.current_vm_candidates(saved, snapshot(target)))
    ida_bytes.patch_byte(target, byte)
    check("restoration permits candidate navigation", a["vm_candidate"] in module.current_vm_candidates(saved, snapshot(target)))
    if ida_kernwin.is_idaq():
        ...
        check("candidate-only view opens", not form.current and form.jump.isEnabled()
              and form.tables["vm"].topLevelItemCount() == 1)
        check("VM detail states unresolved contracts", "Unresolved:" in form.detail.toPlainText()
        ...
        check("GUI exposes proof-gated local reuse", "Modeled local effects: reused after UNSAT" in form.detail.toPlainText()
              and '"sources_current": true' in form.detail.toPlainText())
        proof_before = api("chernobog_solver_evidence", group_ea)
        form.poll()
        check("GUI polling never reruns solver", proof_before == api("chernobog_solver_evidence", group_ea))
        reference = int(groups["records"][0]["site"], 0)
        original = ida_bytes.get_byte(reference)
        ida_bytes.patch_byte(reference, 0x90)
        form.poll()
        check("reference invalidation does not borrow candidate freshness", form.jump.isEnabled()
              and '"sources_current": false' in form.detail.toPlainText())
        ida_bytes.patch_byte(reference, original)
        form.poll()
        check("restored reference revalidates local summary sources", '"sources_current": true' in form.detail.toPlainText())
        form.parent.window().resize(1600, 1000)
        QtWidgets.QApplication.processEvents()
        form.fit_graph()
        QtWidgets.QApplication.processEvents()
...
(Path(os.environ["IDAUSR"]).parent / "vm_regions.json").write_text(
    json.dumps({"checks": checks, "errors": errors, "captures": captures, "summary_captures": summary_captures}, indent=2) + "\n")
line = "[chernobog][vm-regions] " + ("FAIL " + "; ".join(errors) if errors else "PASS checks=%d" % len(checks))
