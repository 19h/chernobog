...
def load(directory):
    ...
    require(
        report["runner_return_code"] == 0 and report["expected_log_found"],
    ...
    require(
        len(records) == 11 and set(by_name) == POSITIVE | NEGATIVE,
        "%s: missing or duplicate fixture function" % directory,
    )
    require(
        sum(len(record["native_reference_checks"]) for record in records) == 16,
...
def main():
    ...
    for name in sorted(before):
        ...
        require(
            old["native_byte_integrity"]["unchanged"]
        ...
        native_byte_count += sum(
            part["end"] - part["start"] for part in new["native_byte_integrity"]["before"]
        )
        for record in (old, new):
            require(
                all(
                    mapping["present_at_LOCOPT"]
        ...
        old_semantics = [
            {key: scenario[key] for key in ("argument", "result", "expected", "global_counter")}
        ...
        if name in POSITIVE:
            ...
            require(
                old["microcode"] == new["microcode"], name + ": rejected fixture microcode changed"
            )
            require(
                old["native_reference_checks"] == new["native_reference_checks"],
                name + ": rejected fixture execution trace changed",
            )
        rows.append(
            {
                "name": name,
                "before_blocks": old_final["qty"],
