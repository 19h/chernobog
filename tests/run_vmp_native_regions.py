...
def main():
    ...
    files = [
        "tests/run_vmp_native_regions.py",
        "tests/ida_vm_native_trace_probe.py",
    ...
    try:
        for label, expected in binaries.items():
            ...
            command = [
                ...
                binary,
                root / "tests/ida_vm_native_trace_probe.py",
                "--ida",
                args.ida.resolve(),
                "--plugin",
                args.plugin.resolve(),
                ...
                "--set",
                "CHERNOBOG_EXPECT_VM_ENTRY=" + str(int(expected_prefix)),
            ]
            ...
            for name, trace in traces.items():
                item["traces"][name] = {
                    ...
                    for k in (
                        ...
                        "ownerless_executed_heads",
                        "foreign_executed_heads",
                        "reached_sentinel",
                        "region_code_changed",
                        "prefix_oracle_applicable",
                    )
                }
                item["traces"][name]["execution_records"] = len(trace["execution"])
                item["traces"][name]["accesses"] = len(trace["data"])
            item["artifact_sha256"] = {
                name: digest(run_dir / name) for name in ("run.json", "vm_native_traces.json")
            }
            print(
                json.dumps(
                    {
                        ...
                        "instruction_count": sum(t["instruction_count"] for t in traces.values()),
                    }
                ),
                flush=True,
            )
