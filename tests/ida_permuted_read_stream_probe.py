"""Check a permuted exact-read string against one native function in IDA."""
...
try:
    ...
    target = ida_name.get_name_ea(ida_idaapi.BADADDR, "_native_permuted_strings")
    key = ida_name.get_name_ea(ida_idaapi.BADADDR, "_native_permuted_key")
    ...
    check(
        ...
        and counts["allocation_lifetimes"] == 4
        and counts["use_snapshots"] >= 32,
    )
    ...
    check(
        ...
        and all(
            ...
            and row["observations"] == row["eligible_runs"] == 4
    ...
    check(
        "read-order evidence",
        len(streams) == 4 * expected
        and all(
            row["bytes_hex"] == "7365637265742100"
            and row["read_count"] == "8"
            and row["address"] != row["fragments"].split(":")[2]
            for row in streams
        ),
    )
    ...
    check(
        ...
        sum("rax-use(" in line for line in rendered()) == expected,
    )
except BaseException as error:
    report["errors"].append(type(error).__name__)

report["passed"] = not report["errors"]
(Path(os.environ["IDAUSR"]).parent / "permuted_read_stream.json").write_text(
    json.dumps(report, indent=2) + "\n"
)
line = "[chernobog][permuted-read-stream] " + (
