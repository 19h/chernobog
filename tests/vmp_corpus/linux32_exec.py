...
def main():
    ...
    result, out, _ = execute(
        ["/usr/bin/qemu-i386", "-L", "/usr/i686-linux-gnu", binary, seed], timeout=10
    )
    result["binary_sha256"] = before
    result["binary_unchanged"] = digest(binary) == before
    result["accounting_scope"] = (
        "Linux wait4 of qemu-i386 process, including translated guest execution"
    )
