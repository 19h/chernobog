...
struct Observation { uint64_t result, flags; int64_t stack_delta; };
...
static void sample(unsigned id, uint32_t x, uint32_t y, uint32_t initial)
{
    ...
    for(unsigned function=0;function<2;++function)
    {
        ...
        printf("%u %u %08" PRIx32 " %08" PRIx32 " %08" PRIx32
               " %016" PRIx64 " %04" PRIx64 " %" PRId64
...
{
    if(argc!=2)return 2;
    char *end=0;unsigned long parsed=strtoul(argv[1],&end,0);
    if(!end || *end || !parsed || parsed>UINT32_MAX)return 2;
    uint32_t seed=(uint32_t)parsed;
    const uint32_t corners[]={0,1,UINT32_C(0x7FFFFFFF),UINT32_C(0x80000000),UINT32_C(0xFFFFFFFE),UINT32_MAX};
    unsigned id=0;
