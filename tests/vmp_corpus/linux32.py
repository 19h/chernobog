...
class Linux32:
    def __init__(self, root, output, image, context):
        ...
        status, out, _ = execute(
            self.docker + ["image", "inspect", image, "--format", "{{.Id}} {{.Architecture}}"]
        )
        ...
        self.metadata = {
            "image_id": self.image,
            "container_architecture": fields[1],
            "execution": "qemu-i386 user-mode translation in Linux container",
        }
        self.mounts = [
            "--mount",
            "type=bind,src=" + str(root / "tests") + ",dst=/source,readonly",
            "--mount",
    ...
    def run(self, binary, seed):
        ...
        launcher, _, _ = self.execute(
            [
                "python3",
                "-B",
                "/source/vmp_corpus/linux32_exec.py",
                "/output/" + binary.name,
                str(seed),
                "/output/" + report,
                "/output/" + stdout,
        ...
        if (
            not measurement["binary_unchanged"]
            or measurement["binary_sha256"] != digest(binary)
            ...
            or any(
                self.metadata["sha256"][name] != value
                for name, value in measurement["runtime_sha256"].items()
            )
        ):
