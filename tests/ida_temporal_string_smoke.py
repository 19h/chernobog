...
try:
    ...
    for index in range(count()):
        ...
        candidate = {field: evaluate(expression + "." + field) for field in
                     ["ok", "value", "site", "callee", "occurrence", "argument", "producer", "model_kind",
                      "object_site", "object_size", "observations", "eligible_runs"]}
        values.append(candidate["value"])
        check("modeled use provenance", candidate["ok"] == 1
              and candidate["producer"] == "modeled-argument" and candidate["argument"] == 0
    ...
    check("transient use display", len(annotations) == 2
          and any('"secret!"' in line and "modeled strlen,arg=0,use=1,runs=" in line for line in annotations)
