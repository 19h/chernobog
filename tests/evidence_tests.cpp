...
template <class Identity> void verify_identity(const ProgramImage &image, const Identity &identity)
{
...
void verify(const ProgramImage &image, const FuncRange &function)
{
    ...
    const auto result =
        hybrid_build_target_evidence(image, function, function.start, {}, {}, emulation);
    ...
    check(result.context_identity.size() == image.segs.size(),
          "overlapping context reads must merge within each segment");
...
void regressions()
{
    ...
    segment.bytes.assign(40, 0x81);
    segment.mask.assign(5, 255);
    image.segs.push_back(segment);
    ...
    for (size_t segments : {1u, 256u})
        for (const std::string pattern : {"loaded", "unloaded", "alternating"})
        {
            ...
            for (size_t i = 0; i < segments; ++i)
            {
                ...
                segment.mask.assign(total / segments / 8, pattern == "loaded"     ? 255
                                                          : pattern == "unloaded" ? 0
                                                                                  : 0x55);
...
IdentityComparison reference_comparison(const std::vector<uint8_t> &expected,
                                        ...
                                        const std::vector<uint8_t> &actual_mask)
{
    ...
    for (size_t i = 0; i < expected.size(); ++i)
    {
        ...
        if (left != right)
        {
            ...
            result.offset = i;
            result.expected_byte = left;
            result.actual_byte = right;
            return result;
        }
        if (left && expected[i] != actual[i])
        {
            ...
            result.offset = i;
            result.expected_byte = expected[i];
            result.actual_byte = actual[i];
            return result;
        }
    }
    return result;
...
void verify_comparison(const std::vector<uint8_t> &expected,
                       const std::vector<uint8_t> &expected_mask,
                       const std::vector<uint8_t> &actual, const std::vector<uint8_t> &actual_mask)
{
    ...
    check(result.mismatch == reference.mismatch && result.offset == reference.offset &&
              result.expected_size == reference.expected_size &&
              result.actual_size == reference.actual_size &&
              result.expected_byte == reference.expected_byte &&
              result.actual_byte == reference.actual_byte,
...
void comparison_regressions()
{
    ...
    for (unsigned trial = 0; trial < 10000; ++trial)
    {
        ...
        if (size != 0)
        {
            ...
            if (trial % 4 != 0)
                right[index / 8] ^= static_cast<uint8_t>(1u << (index % 8));
        }
...
void comparison_benchmark()
{
    ...
    for (size_t size : {32u, 256u, 8u * 1024u * 1024u})
        for (const std::string pattern :
             {"loaded-equal", "unloaded-equal", "mixed-equal", "mixed-unloaded-diff", "first-byte",
              "last-byte", "last-loaded-state"})
        {
            if (size < 1024 && pattern != "loaded-equal")
                continue;
            std::vector<uint8_t> expected(size, 0x81);
            auto actual = expected;
            std::vector<uint8_t> left((size + 7) / 8,
                                      pattern == "unloaded-equal" ? 0
                                      : pattern == "mixed-equal" || pattern == "mixed-unloaded-diff"
                                          ? 0x55
                                          : 255);
            auto right = left;
            if (pattern == "mixed-unloaded-diff")
                for (size_t index = 1; index < size; index += 2)
                    actual[index] ^= 1;
            if (pattern == "first-byte")
                actual.front() ^= 1;
            if (pattern == "last-byte")
                actual.back() ^= 1;
            if (pattern == "last-loaded-state")
                right.back() ^= 0x80;
            verify_comparison(expected, left, actual, right);
...
void runtime_string_regressions()
{
    ...
    for (const std::string text : {u8"Gr\u00fc\u00dfe \u4e16\u754c", u8"\u03b1\u03b2\u03b3\u03b4",
                                   u8"\U0001f680\U0001f680\U0001f680\U0001f680", "ASCII"})
    {
    ...
    check(!recover_runtime_utf8_prefix(greek, 0, 8) && !recover_runtime_utf8_prefix(greek, 4, 3),
          "invalid runtime limits must reject");

    const std::vector<std::vector<uint8_t>> invalid_suffixes = {{0xc0, 0xaf},
                                                                {0xe0, 0x80, 0xaf},
                                                                {0xf0, 0x80, 0x80, 0x80},
                                                                {0xed, 0xa0, 0x80},
                                                                {0xed, 0xbf, 0xbf},
    ...
    for (const std::vector<uint8_t> &invalid :
         {std::vector<uint8_t>{}, std::vector<uint8_t>{'a', 'b', 'c', 'd'},
...
void temporal_memory_regressions()
{
    ...
    for (unsigned negative = 0; negative < 13; ++negative)
    {
        ...
        {
        case 0:
            changed.events.uses.erase(changed.events.uses.begin());
            break;
        case 1:
            changed.events.uses[0].bytes[0] = 'X';
            break;
        case 2:
            changed.events.uses[0].bytes.back() = 'X';
            break;
        case 3:
            changed.events.uses[0].status = UseCaptureStatus::BYTE_LIMIT;
            break;
        case 4:
        ...
        case 6:
            ...
            break; // allocation boundary
        case 7:
            changed.events.uses[0].offset = 15;
            break;
        case 8:
            changed.events.uses[0].bytes[1] = 0xFF;
            break;
        case 9:
        ...
        case 10:
            ...
            break;
        case 11:
            changed.events.uses[0].object_site += 1;
            break;
        case 12:
            changed.events.uses[0].producer = UseProducer::EXECUTED_READ;
            break;
        }
    ...
    for (unsigned negative = 0; negative < 4; ++negative)
    {
        ...
        {
        case 0:
            changed.runs[0].ran = false;
            break;
        case 1:
            changed.runs[0].outcome.temporal_capture_complete = false;
            break;
        case 2:
            changed.runs[0].outcome.temporal_observation_available = false;
            break;
        case 3:
            changed.runs[0].outcome.temporal_capture_truncated = true;
            break;
        }
    ...
    bounded.capture(0x10, 0, -1, UseProducer::EXECUTED_READ, DataScope::IMAGE, 0x20, value,
    ...
    for (size_t i = 1; i <= TemporalMemory::use_limit; ++i)
        bounded.capture(i, 0, -1, UseProducer::EXECUTED_READ, DataScope::IMAGE, 0x20, value, 1, 1,
                        i);
    ...
    check(occurrences.allocate(0, 0x10, 0x20, 0) == 0,
          "zero-size allocation follows explicit failure model");
    ...
    occurrences.capture(0x30, 0x40, 1, UseProducer::MODELED_ARGUMENT, DataScope::HEAP, object,
                        nullptr, 0, 0, 2);
    occurrences.capture(0x30, 0x40, 1, UseProducer::MODELED_ARGUMENT, DataScope::HEAP, object,
                        value, sizeof(value), sizeof(value), 3);
    ...
    long_bytes.back() = 0;
    wide.capture(1, 2, 0, UseProducer::MODELED_ARGUMENT, DataScope::IMAGE, 0x1000,
                 long_bytes.data(), long_bytes.size(), long_bytes.size(), 0);
    ...
    wide.capture(2, 0, -1, UseProducer::EXECUTED_READ, DataScope::IMAGE, 0x1000, nullptr, 0, 16, 1);
...
void evidence_view_regressions()
{
    ...
    allocation.id = 1;
    allocation.generation = 2;
    allocation.address = 0x8000;
    allocation.size = 16;
    allocation.site = 0x1010;
    allocation.allocated = 2;
    allocation.released = 8;
    ...
    branch.instruction = 0x1030;
    branch.sequence = 6;
    branch.provenance.run_id = 1;
    branch.provenance.seed = 8;
    branch.disposition = BranchDisposition::FALLTHROUGH;
    ...
    for (size_t i = 0; i < 129; ++i)
    {
        ...
        item.address = 0x1000 + i;
        item.ida.valid = true;
        item.ida.has_target = true;
        item.ida.target = 0x2000 + i;
    ...
    check(view.edges.size() == EvidenceView::edge_limit && view.omitted.at("encoded_edges") == 1 &&
              view.omitted.at("observed_edges") == 1,
          "encoded and observed edges have separate retention allowances");
    check(view.edges.front().at("truth") == "encoding" &&
              view.edges.back().at("truth") == "witness",
    ...
    view = project_evidence_view(source);
    check(view.runs.size() == 128 && view.omitted.at("runs") == 2 && view.lifetimes.size() == 128 &&
              view.omitted.at("lifetimes") == 1 && view.claims.size() == 128 &&
              view.omitted.at("claims") == 130,
...
void native_read_stream_regressions()
{
    ...
    for (uint32_t id = 1; id <= 2; ++id)
    {
        ...
        memory.enabled = true;
        memory.context = 0x1000;
        memory.heap_begin = 0x8000 + id * 0x1000;
        memory.heap_end = memory.heap_begin + 0x1000;
        memory.run_id = id;
        memory.seed = id + 10;
        for (unsigned lifetime = 0; lifetime < 2; ++lifetime)
        {
            ...
            check(memory.release(address, epoch + 45),
                  "native stream fixture releases exact allocation");
        }
        RunObservation run;
        run.ran = true;
        ...
        source.events.allocations.insert(source.events.allocations.end(),
                                         memory.allocations.begin(), memory.allocations.end());
    }
    const auto result = hybrid_consensus_use_strings(source);
    {
        ...
        for (const auto &run : source.runs)
        {
            ...
            copy.capture = run.provenance.run_id + 100;
            copy.context = 0x1000;
            copy.image_hash = 123;
            copy.generation = 1;
            copy.run_id = run.provenance.run_id;
            copy.seed = run.provenance.seed;
            copy.ran = true;
            copy.outcome = run.outcome;
            copy.outcome.temporal_capture_complete = false;
            copy.outcome.native_region = copy.outcome.native_temporal_requested =
                copy.outcome.native_temporal_complete = true;
            copy.outcome.returned = copy.outcome.stop_valid = true;
            copy.outcome.stop_reason = RAX_STOP_UNTIL;
            copy.outcome.region_identity = copy.capture;
            copy.bindings = {{0x2000, EmuSummaryKind::ALLOCATE, "malloc"},
                             {0x2010, EmuSummaryKind::DEALLOCATE, "free"}};
            for (const auto &u : source.events.uses)
                if (u.run_id == copy.run_id)
                    copy.events.uses.push_back(u);
            for (const auto &d : source.events.data)
                if (d.run_id == copy.run_id)
                    copy.events.data.push_back(d);
            for (const auto &a : source.events.allocations)
                if (a.run_id == copy.run_id)
                    copy.events.allocations.push_back(a);
            native.push_back(std::move(copy));
        }
        const auto projected = hybrid_native_temporal_strings(native);
        check(
            projected.available && projected.observations.size() == 2 &&
        ...
        check(
            hybrid_consensus_use_strings(forged_function).empty(),
            "native scope cannot enter ordinary use publication even with forged function completeness");
        auto oversized = native;
        oversized.resize(17);
        check(!hybrid_native_temporal_strings({}).available &&
                  !hybrid_native_temporal_strings(oversized).available &&
        ...
        check(hybrid_native_temporal_strings(native).observations.size() == 2,
              "binding order does not change model contract");
        for (unsigned failure = 0; failure < 21; ++failure)
        {
            ...
            {
            case 0:
                run.outcome.native_temporal_complete = false;
                break;
            case 1:
                run.outcome.temporal_capture_complete = true;
                break;
            case 2:
                run.capture = bad[0].capture;
                break;
            case 3:
                run.seed = bad[0].seed;
                run.run_id = bad[0].run_id;
                break;
            case 4:
                run.image_hash++;
                break;
            ...
            case 6:
                ...
                break;
            case 7:
                run.bindings[0].kind = EmuSummaryKind::MEMSET;
                break;
            case 8:
                run.bindings[0].name = "renamed";
                break;
            case 9:
                run.bindings.push_back(run.bindings[0]);
                break;
            case 10:
                run.events.uses[0].seed++;
                break;
            case 11:
                run.outcome.region_code_changed = true;
                break;
            case 12:
                run.outcome.native_region = false;
                break;
            case 13:
                run.outcome.stop_valid = false;
                break;
            case 14:
                run.outcome.data_trace_filtered = true;
                break;
            case 15:
                run.outcome.environment_model_failure = true;
                break;
            case 16:
                run.outcome.consumed_context_complete = true;
                break;
            case 17:
                run.events.data.push_back(run.events.data.front());
                break;
            case 18:
                run.events.uses.push_back(run.events.uses.front());
                break;
            case 19:
                run.outcome.temporal_capture_truncated = true;
                break;
            case 20:
                run.ran = false;
                break;
            ...
            check(
                !rejected.available && rejected.observations.empty() && rejected.captures.empty(),
                "native corpus cannot shrink around an incompatible, duplicate or incomplete capture");
        }
        auto divergent = native;
        divergent[1].events.uses[0].bytes[0] = 'x';
        divergent[1].events.data[0].value = 'x';
    ...
    for (const auto &candidate : result)
        check(candidate.value == "secret!" &&
                  candidate.use.producer == UseProducer::EXECUTED_READ_STREAM &&
                  candidate.read_fragments.size() == 2 &&
                  candidate.read_fragments.front().size() == 8 &&
                  candidate.read_fragments[0][0].address !=
                      candidate.read_fragments[1][0].address &&
    ...
    for (const auto &event : view.events)
        if (event.at("kind") == "read-stream")
        {
            ++displayed;
            check(event.at("truth") == "observation" && event.at("read_count") == "8" &&
                      event.at("fragments_omitted") == "0" &&
                      event.at("bytes_hex") == "7365637265742100" &&
    ...
    {
        auto unicode = source;
        unicode.events.uses.clear();
        unicode.events.data.clear();
        ...
        for (size_t start = 0; start < source.events.uses.size(); start += 8)
            for (size_t i = 0; i < 8; i += width)
            {
                ...
                use.bytes.assign(encoded + i, encoded + i + width);
                use.observed_size = width;
                unicode.events.uses.push_back(use);
                uint64_t scalar = 0;
                for (size_t byte = 0; byte < width; ++byte)
                    scalar |= uint64_t(encoded[i + byte]) << (8 * byte);
                unicode.events.data.push_back({use.site, use.address, scalar, uint32_t(width),
                                               RAX_MEM_READ, use.scope, use.sequence + 1,
                                               use.run_id, use.seed});
            }
        const auto streams = hybrid_consensus_native_read_strings(unicode);
        check(streams.size() == 2 &&
                  streams[0].value == std::string("s\xc3\xa9"
                                                  "cret") &&
    ...
    for (auto scope : {DataScope::IMAGE, DataScope::STACK})
    {
        ...
        relocated.events.allocations.clear();
        for (size_t i = 0; i < relocated.events.uses.size(); ++i)
        {
            auto &use = relocated.events.uses[i];
            auto &data = relocated.events.data[i];
            use.scope = scope;
            use.address = 0x4000 + i % 8 + (scope == DataScope::STACK ? 0x1000 * use.run_id : 0);
            use.object_site = scope == DataScope::STACK ? use.context : use.address;
            use.offset = scope == DataScope::STACK ? int64_t(i % 8) : 0;
            use.object_callee = use.object_occurrence = use.object_size = use.allocation_id =
                use.generation = 0;
            data.addr = use.address;
            data.scope = scope;
        }
    ...
    for (unsigned failure = 0; failure < 21; ++failure)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            changed.events.data[0].value = 'X';
            break;
        case 2:
            changed.events.data[0].sequence += 1;
            break;
        case 3:
            changed.events.uses[0].status = UseCaptureStatus::INCOMPLETE_VALUE;
            break;
        case 4:
            changed.events.uses[0].generation += 1;
            break;
        case 5:
            changed.events.allocations[0].released = 12;
            break;
        case 6:
            changed.events.data.erase(changed.events.data.begin());
            break;
        case 7:
            changed.events.uses[0].producer = UseProducer::EXECUTED_READ_STREAM;
            break;
        case 8:
            changed.events.uses[0].offset += 1;
            break;
        case 9:
            changed.events.uses[0].object_callee += 1;
            break;
        case 10:
            changed.events.uses[0].sequence = UINT64_MAX;
            break;
        case 11:
            changed.events.uses[0].observed_size = 9;
            break;
        case 12:
            changed.events.data[0].scope = DataScope::OTHER;
            break;
        case 13:
            changed.events.uses[7].bytes[0] = 'x';
            changed.events.data[7].value = 'x';
            break;
        case 14:
        ...
        case 16:
            ...
            break;
        case 17:
            changed.events.edges.push_back({0x1300, 0x2000, 1, 11, ExecEdge::Kind::Unknown, 12});
            break;
        case 18:
            changed.events.uses[1].site += 1;
            changed.events.data[1].from += 1;
            break;
        case 19:
            changed.events.uses[0].occurrence += 1;
            break;
        case 20:
            changed.events.uses[0].context += 1;
            break;
        }
        const auto rejected = hybrid_consensus_native_read_strings(changed);
        check(
            rejected.size() == 1 && rejected[0].use.object_occurrence == 2,
    ...
    for (unsigned failure = 0; failure < 7; ++failure)
    {
        ...
        {
        case 0:
            changed.runs[0].outcome.memory_observation_available = false;
            break;
        case 1:
            changed.runs[0].outcome.temporal_capture_truncated = true;
            break;
        case 2:
            ...
            break;
        case 3:
            changed.events.data.push_back(changed.events.data[0]);
            break;
        case 4:
            changed.events.uses.push_back(changed.events.uses[0]);
            break;
        case 5:
            ...
            break;
        case 6:
            changed.runs[0].outcome.data_trace_filtered = true;
            break;
        }
    ...
    {
        TargetEvidence bounded;
        bounded.runs.push_back(source.runs[0]);
        ...
        for (size_t i = 0; i < length; ++i)
        {
            ...
            use.site = 0x1200;
            use.occurrence = i + 1;
            use.scope = DataScope::IMAGE;
            use.address = use.object_site = 0x4000 + i;
            use.sequence = 10 + 4 * i;
            use.observed_size = 1;
            use.run_id = 1;
            use.seed = 11;
        ...
        check(
            length == 1040   ? streams.size() == 130 && streams[0].value.size() == 7
            : length <= 4096 ? streams.size() == 1 && streams[0].value.size() == length - 1
                             : streams.empty(),
        ...
        if (length == 20)
        {
            ...
            const auto item =
                std::find_if(timeline.events.begin(), timeline.events.end(),
                             [](const auto &row) { return row.at("kind") == "read-stream"; });
        ...
        if (length == 4096)
        {
            const auto timeline = project_evidence_view(bounded);
            check(
                timeline.omitted.at("events") > 0 && timeline.read_streams.size() == 1 &&
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2 && std::string(argv[1]) == "--benchmark")
        benchmark();
    if (argc == 2 && std::string(argv[1]) == "--compare-benchmark")
