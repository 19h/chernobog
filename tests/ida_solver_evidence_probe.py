...
try:
    ...
    for name in names:
        ...
        captures[name] = data
        check(
            name + " native bytes retained", raw == ida_bytes.get_bytes(function.start_ea, len(raw))
        )
        check(
            name + " query scope is exact",
            all(
                r["function"] == hex(ea)
                and r["phase"]
                and r["applicability"].startswith("recorded formula only")
        ...
        check(
            name + " compact polling omits formula copies",
            len(state["records"]) == len(data["records"])
    ...
    check(
        "production SAT assignment retained",
        any(
            r["result"] == "sat" and r.get("model_constants") != "0" and "model_0_symbol" in r
            for r in all_rows
        ),
    )
    check("production universal exclusion retained", any(r["result"] == "unsat" for r in all_rows))
    check(
        "nonlinear affine fit has an actual counterexample",
        any(
            r["result"] == "sat"
            and r["role"] == "bitvector-equivalence mismatch"
            and "model_0_symbol" in r
            for r in all_rows
        ),
    )
    check(
        "formula text survives JSON encoding",
        any(
            r.get("formula_complete") == "true"
            and "\n" in r["formula"]
            and "assert" in r["formula"]
            for r in all_rows
        ),
    )
    check(
        "source navigation does not claim IR applicability",
        all("navigation only" in r["source_contract"] for r in all_rows),
    )
    eligible = [
        (name, r)
    ...
    selected_name, selected = next(
        ...
        if pair[1]["role"] == "bitvector-equivalence mismatch" and pair[1]["result"] == "sat"
    )
    ...
    check(
        "source byte patch invalidates navigation",
        not any(
            r["query_id"] == selected["query_id"] and r["source_bytes_current"] == "true"
            for r in changed["records"]
        ),
    )
    ida_bytes.patch_byte(site, byte)
    check(
        "source byte restoration retains historical query",
        snapshot(target) == captures[selected_name],
    )
    ...
    if module_path:
        ...
        check(
            "UI accepts exact source snapshot",
            selected["query_id"] in module.current_solver_sources(saved, saved),
        )
        check(
            "UI rejects foreign database",
            not module.current_solver_sources(saved, dict(saved, database="different")),
        )
        check(
            "UI rejects newer query generation",
            not module.current_solver_sources(saved, dict(saved, generation="0x0")),
        )
        if ida_kernwin.is_idaq():
            ...
            check(
                "SMT pane exposes actual query",
                selected["role"] in form.detail.toPlainText()
                and "query_applicability" in form.detail.toPlainText(),
            )
            check(
                "SMT formula is displayed with original newlines",
                selected["formula"] in form.detail.toPlainText(),
            )
            ...
            bar.setValue(0)
            check(
                "SMT navigation guard independent of execution",
                not form.current and form.jump.isEnabled(),
            )
            ida_bytes.patch_byte(site, byte ^ 1)
            form.poll()
            ...
            form.poll()
            check(
                "SMT source restoration permits navigation only",
                form.jump.isEnabled()
    ...
    check(
        "new decompilation replaces query generation", before["generation"] != after["generation"]
    )
    if module_path:
...
line = "[chernobog][solver-evidence] " + (
