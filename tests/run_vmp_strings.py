...
def seed_attested(output, seed):
    ...
    return (
        [int(value) for value in callbacks] == [seed]
        and bool(applied)
...
def summarize(probe):
    ...
    assert not candidates or (
        view.get("available") is True and view.get("fresh") is True
    ), "stale publication"
    runs = view.get("runs", [])
    complete = bool(runs) and all(
        run["ran"] == "true"
        and run["returned"] == "true"
        and run["temporal_complete"] == "true"
        and run["temporal_truncated"] == "false"
        and run["memory_observation_available"] == "true"
        and run["data_trace_truncated"] == "false"
        and run["data_trace_filtered"] == "false"
        for run in runs
    )
    ...
    for candidate in candidates:
        assert (
            candidate["ok"] == 1
            and candidate["eligible_runs"] == len(runs)
            and candidate["observations"] == len(runs)
    ...
    return {
        ...
        "returned_runs": sum(run["returned"] == "true" for run in runs),
        "temporal_corpus_complete": complete,
        "candidate_instances": len(values),
        "correct_value_instances": correct,
        "unexpected_value_instances": len(values) - correct,
        "distinct_expected_values": distinct,
        ...
        "stop_reasons": dict(
            collections.Counter(run["kind"] + ":" + run["stop_reason_name"] for run in runs)
        ),
...
def main():
    ...
    protector, tree, ida, plugin = (
        p.resolve() for p in (args.protector, args.source_tree, args.ida, args.plugin)
    )
    sources = (
        SOURCE,
        PROBE,
    ...
    report = {
        ...
        "protector_seeds": list(PROTECTOR_SEEDS),
        "reserved_protector_seed": PROTECTOR_SEEDS[-1],
        ...
        "oracle": "unprotected main compares all bytes consumed into RAX and RDX",
        "source_sha256": {name: digest(root / name) for name in sources},
        "reference_source_sha256": {
            "vmp/" + name: digest(tree / name)
            for name in (
                "core/intel.cc",
                "core/macfile.cc",
                "core/core.cc",
        ...
        "runtime_sha256": {
            "protector": digest(protector),
            "protector_sdk": digest(protector.parent / "libVMProtectSDK.dylib"),
            "ida": digest(ida),
            "plugin": digest(plugin),
        },
    ...
    try:
        ...
        report["build"], _, _ = execute(
            ["xcrun", "clang", *BUILD_FLAGS, root / SOURCE, "-o", original], timeout=60
        )
        ...
        addresses = {
            name: int(
                re.search(rb"^([0-9a-fA-F]+) T _" + name.encode() + rb"$", symbols, re.M)[1], 16
            )
        ...
        main_span = function_bytes(
            original, section, main_entry, section["address"] + section["size_bytes"] - main_entry
        )
        ...
        for index, value in enumerate((0x5A7B2E3F28393F29, 0x5A7B3E3435393F29)):
            ...
            assert (
                at >= main_offset and data.find(pattern, at + 1, text_end) == -1
            ), "ambiguous oracle immediate"
            data[at] ^= 1
            ...
            report["negative_oracles"].append(
                {
                    "index": index,
        ...
        assert (
            all(successful(item) for item in (normal, reference, injected))
            and a != b
            and b == c
            and b"CHERNOBOG_CORPUS_SRAND=17 requested=1" in err
        ), "seed control failed"
        report["seed_instrument"] = {
            "sha256": digest(instrument),
            "probe_sha256": digest(seed_probe),
            "verified": True,
        }
            ...
            for seed in PROTECTOR_SEEDS:
                ...
                for binary in (target, repeat):
                    ...
                    item["generations"].append(
                        {"measurement": measurement, "seed_attested": attested}
                    )
                    assert (
                        successful(measurement) and attested and binary.is_file()
                    ), "protection failed"
                item.update(
                    sha256=digest(target),
                    repeat_sha256=digest(repeat),
                    size_bytes=target.stat().st_size,
                )
                assert item["sha256"] == item["repeat_sha256"], "nondeterministic protection"
                item["entry_changed"] = (
                    function_bytes(target, text_section(target), entry, len(span)) != span
                )
                assert item["entry_changed"], "selected bytes unchanged"
                item["oracle_unchanged"] = (
                    function_bytes(target, text_section(target), main_entry, len(main_span))
                    == main_span
                )
        ...
        for label, binary, before in binaries:
            assert all(
                value.encode() not in binary.read_bytes() for value in EXPECTED
            ), "plaintext stored in image"
            for repetition in range(3):
                measurement, _, _ = execute([binary], timeout=10)
                report["native_runs"].append(
                    {"label": label, "repetition": repetition, "measurement": measurement}
                )
            ...
            measurement, _, _ = execute(
                [
                    ...
                    "--enable-rax",
                    "--set",
                    "CHERNOBOG_STRING_ENTRY=" + hex(entry),
                ],
                timeout=120,
            )
            ...
            report["analysis"].append(
                {
                    ...
                    "measurement": measurement,
                    "summary": summary,
                    "probe_sha256": digest(run_dir / "protected_strings.json"),
                    "runner_sha256": digest(run_dir / "run.json"),
                }
            )
            assert not summary["unexpected_value_instances"], "unexpected literal value"
            if label == "original":
                assert (
                    summary["distinct_expected_values"] == 2 and summary["display_annotations"] == 2
                ), "original positive control failed"
        ...
        assert all(
            digest(tree / name.removeprefix("vmp/")) == value
            for name, value in report["reference_source_sha256"].items()
        ), "reference changed"
        for name, path in (
            ("protector", protector),
            ("protector_sdk", protector.parent / "libVMProtectSDK.dylib"),
            ("ida", ida),
            ("plugin", plugin),
        ):
        ...
        json.dumps(
            {
                ...
                "native_runs": len(report["native_runs"]),
                "analysis_runs": len(report["analysis"]),
