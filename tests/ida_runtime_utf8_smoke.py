...
try:
    ...
    if (
        ida_bytes.get_bytes(output, len(payload)) != original_output
    ...
    if (
        ida_bytes.is_strlit(ida_bytes.get_full_flags(named))
    ...
    if not ida_bytes.is_strlit(ida_bytes.get_full_flags(existing)):
        for offset in range(len(payload)):
            ...
            print(
                "utf8 range +%d: flags=%X unknown=%s loaded=%s name=%r"
                % (
                    offset,
                    ida_bytes.get_full_flags(current),
                    ida_bytes.is_unknown(ida_bytes.get_full_flags(current)),
                    ida_bytes.is_loaded(current),
