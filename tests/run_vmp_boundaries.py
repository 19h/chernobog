...
def main():
    ...
    files = [
        "tests/run_vmp_boundaries.py",
        "tests/ida_vm_boundary_probe.py",
        "tests/run_ida_smoke.py",
        "tests/run_vmp_corpus.py",
        "src/vm/boundary.hpp",
        "src/vm/boundary.cpp",
        "src/vm/ida_regions.cpp",
        "tests/vm_boundary_tests.cpp",
        "CMakeLists.txt",
    ]
    ...
    try:
        for label, expected in binaries.items():
            ...
            command = [
                ...
                binary,
                root / "tests/ida_vm_boundary_probe.py",
                "--ida",
                args.ida.resolve(),
                "--plugin",
                args.plugin.resolve(),
                ...
                "--set",
                "CHERNOBOG_BOUNDARY_CONTROLS=" + str(int(label == "original")),
            ]
            ...
            for name in corpus["selected_functions"]:
                ...
                item["entries"][name]["frontiers"] = dict(
                    Counter(f["reason"] for f in audit["frontiers"])
                )
                item["entries"][name]["edges"] = len(audit["edges"])
            item["artifact_sha256"] = {
                name: digest(run_dir / name) for name in ("run.json", "vm_boundaries.json")
            }
            print(
                json.dumps({"label": label, "checks": item["checks"], "entries": item["entries"]}),
                flush=True,
            )
