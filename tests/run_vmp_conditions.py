...
SOURCES = (
    "tests/run_vmp_conditions.py",
    "tests/ida_vmp_conditions_probe.py",
...
def summarize(inspection):
    reachable = {
        row["ea"]: row
        for entry in inspection["entries"].values()
        for row in entry["instructions"]
        if row["kind"]
    }
    owners = inspection["owners"]
    deltas = {
        key: sum(
            owner.get("generated", {}).get("codegen_delta", {}).get(key, 0) for owner in owners
        )
        for key in ("codegen_setcc", "codegen_cmov", "codegen_cmov_memory")
    }
    return {
        "reachable_condition_sites": len(reachable),
        ...
        "owners_inspected": len(owners),
        "owners_omitted": inspection["owners_omitted"],
        "native_budget_skips": sum(owner["status"] == "native_budget" for owner in owners),
        "generated_successes": sum(
            owner.get("generated", {}).get("status") == "captured" for owner in owners
        ),
        "decompiled_successes": sum(
            owner.get("decompiled", {}).get("status") == "success" for owner in owners
        ),
        "generated_failures": [
            {
                "entry": owner["entry"],
                "failure_code": owner["generated"].get("failure_code"),
                "exception_type": owner["generated"].get("exception_type"),
            }
            for owner in owners
            if "generated" in owner and owner["generated"]["status"] != "captured"
        ],
        "decompiled_failures": [
            {
                "entry": owner["entry"],
                "failure_code": owner["decompiled"].get("failure_code"),
                "exception_type": owner["decompiled"].get("exception_type"),
            }
            for owner in owners
            if "decompiled" in owner and owner["decompiled"]["status"] != "success"
        ],
        "codegen_events": deltas,
        "traversal_truncated": any(entry["truncated"] for entry in inspection["entries"].values()),
        "microcode_truncated": any(
            owner.get("generated", {}).get("capture_truncated", False) for owner in owners
        ),
        "reachability_preserved": inspection["reachability_preserved"],
        "native_inventories_preserved": all(
            owner.get("native_preserved", True) for owner in owners
        ),
    }
...
def compare(off, on):
    ...
    for entry, before in left.items():
        ...
        assert not any(
            a.get("codegen_delta", {}).values()
        ), "disabled condition consumer emitted code"
        comparable = a.get("status") == b.get("status") == "captured" and not (
            a["capture_truncated"] or b["capture_truncated"]
        )
        changed = []
        if comparable:
            assert a["conditions"].keys() == b["conditions"].keys()
            changed = [
                int(ea) for ea in a["conditions"] if a["conditions"][ea] != b["conditions"][ea]
            ]
        result["owners"].append(
            {
                "entry": entry,
                "generated_comparable": comparable,
                "generated_status_off": a.get("status"),
                "generated_status_on": b.get("status"),
                "generated_changed": (
                    a["microcode_sha256"] != b["microcode_sha256"] if comparable else None
                ),
                ...
                "decompiled_text_changed": (
                    (before["decompiled"]["sha256"] != after["decompiled"]["sha256"])
                    if before.get("decompiled", {}).get("status")
                    == after.get("decompiled", {}).get("status")
                    == "success"
                    else None
                ),
            }
...
def main():
    ...
    report = {
        "schema": 1,
        "passed": False,
        "partial_labels": bool(args.label),
        ...
        for label, expected in binaries.items():
            ...
            for enabled in (False, True):
                ...
                command = [
                    ...
                    binary,
                    root / "tests/ida_vmp_conditions_probe.py",
                    "--ida",
                    args.ida.resolve(),
                    "--plugin",
                    args.plugin.resolve(),
                    ...
                    directory,
                    "--set",
                    "CHERNOBOG_IDA_CONDITION_CODEGEN=" + str(int(enabled)),
                    "--set",
                    "CHERNOBOG_CORPUS_ENTRIES=" + json.dumps(corpus["selected_functions"]),
                ]
                ...
                assert (
                    run["script_sha256"]
                    == report["source_sha256"]["tests/ida_vmp_conditions_probe.py"]
                )
                assert (
                    inspection["passed"]
                    and not inspection["errors"]
                    and inspection["condition_codegen"] == enabled
                )
                captures[enabled] = inspection
                item["summary"] = summarize(inspection)
                item["artifact_sha256"] = {
                    p: digest(directory / p) for p in ("run.json", "conditions_corpus.json")
                }
