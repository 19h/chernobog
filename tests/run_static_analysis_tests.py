...
def main():
    ...
    parser.add_argument(
        "--cxx", type=Path, help="Exact compiler executable; otherwise use CXX or c++"
    )
    parser.add_argument(
        "--cxx-flag",
        action="append",
    ...
    with tempfile.TemporaryDirectory(prefix="chernobog-static-tests-") as directory:
        ...
        for name in (
            ...
            (temporary / name).write_text('#include "static_analysis_ida_stub.hpp"\n')
        executable = temporary / "static_analysis_tests"
        compiler = (
            [str(args.cxx)] if args.cxx is not None else shlex.split(os.environ.get("CXX", "c++"))
        )
        command = (
            compiler
            + [
            ...
            + [
                ...
                str(root / "src/hybrid"),
                "-I",
                str(root / "vendor/rax/capi/include"),
