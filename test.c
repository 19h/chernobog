...
static unsigned char byte_100014110[] = {0x18, 0x52, 0x43, 0x54, 0x18, 0x47,
                                         0x56, 0x44, 0x44, 0x40, 0x53, 0x00};
static unsigned char byte_100014170[32] = {0};

// Encrypted: "Configuration loaded successfully" ^ 0x4C
static unsigned char byte_100014200[] = {0x0F, 0x23, 0x22, 0x2A, 0x25, 0x2B, 0x39, 0x3E, 0x2D,
                                         0x38, 0x25, 0x23, 0x22, 0x6C, 0x20, 0x23, 0x2D, 0x28,
                                         0x29, 0x28, 0x6C, 0x3F, 0x39, 0x2F, 0x2F, 0x29, 0x3F,
                                         0x3F, 0x2A, 0x39, 0x20, 0x20, 0x35, 0x00};
static unsigned char byte_100014280[64] = {0};

// Encrypted: "sudo rm -rf /tmp/cache/*" ^ 0x5A
static unsigned char byte_100014300[] = {0x29, 0x2F, 0x3E, 0x35, 0x7A, 0x28, 0x37, 0x7A, 0x77,
                                         0x28, 0x3C, 0x7A, 0x75, 0x2E, 0x37, 0x2A, 0x75, 0x39,
                                         0x3B, 0x39, 0x32, 0x3F, 0x75, 0x70, 0x00};
static unsigned char byte_100014380[64] = {0};

// Encrypted: "https://api.example.com/check" ^ 0x1F
static unsigned char byte_100014400[] = {
    0x77, 0x6B, 0x6B, 0x6F, 0x6C, 0x25, 0x30, 0x30, 0x7E, 0x6F, 0x76, 0x31, 0x7A, 0x67, 0x7E,
    0x72, 0x6F, 0x73, 0x7A, 0x31, 0x7C, 0x70, 0x72, 0x30, 0x7C, 0x77, 0x7A, 0x7C, 0x74, 0x00};
static unsigned char byte_100014480[64] = {0};

// Encrypted: "AUTH_TOKEN=%s&USER=%s" ^ 0x23
static unsigned char byte_100014500[] = {0x62, 0x76, 0x77, 0x6B, 0x7C, 0x77, 0x6C, 0x68,
                                         0x66, 0x6D, 0x1E, 0x06, 0x50, 0x05, 0x76, 0x70,
                                         0x66, 0x71, 0x1E, 0x06, 0x50, 0x00};
...
// Obfuscated: a + b
static inline int mba_add(int a, int b) { return (a ^ b) + 2 * (a & b); }

// Obfuscated: a - b
static inline int mba_sub(int a, int b) { return (a ^ b) - 2 * (~a & b); }

// Obfuscated: a * 2
static inline int mba_mul2(int a) { return (a << 1) ^ (a & 0) | (a + a); }
...
static inline int mba_zero(int x) { return (x ^ x) & ((x | ~x) ^ (x | ~x)); }
...
void _build_stack_string_2(char *buf)
{
    ...
    unsigned char enc[] = {0x03, 0x21, 0x21, 0x27, 0x31, 0x31, 0x62,
                           0x26, 0x27, 0x2C, 0x2B, 0x27, 0x26, 0x00};
...
#define LODWORD(x) ((uint32_t)(x))
    ...
    { // Hikari signature
        ...
        case STATE_ERROR_HANDLER:
            ((wrapper_func_str_t)_HikariFunctionWrapper_GetPtr(1))(
