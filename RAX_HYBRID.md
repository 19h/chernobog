...
Runtime consensus accepts NUL-terminated UTF-8, including ASCII. The default
minimum is four Unicode scalar values and the maximum is 4,096 payload bytes;
ASCII/C1 controls, malformed encodings, missing observable runs, and invalid or
conflicting duplicate records reject the candidate. Agreement remains byte-exact
without Unicode normalization. Matching undefined IDB bytes receive an explicit
UTF-8 string encoding; dummy labels generated by data references are admissible,
while real names and comments anywhere in the range remain protected.
The display projection is sealed before the final ctree lookup so late prototype
...
`strncpy`, `strlen`, `strcmp`, `memchr`, `strnlen`, allocation/callocation/deallocation,
...
- `coverage: physical=A/B` counts distinct executed physical instruction
  ...
  `static_truncated=1` marks an incomplete static denominator caused by the
  instruction cap or cancellation; `100%` then describes only retained records.
...
| `chernobog_vm_temporal_strings(ea, input_json, bindings_json)` | Four-run native read-stream observations with capture/lifetime/byte witnesses; separate exact freshness lease and Qt companion. See `docs/VMP_REGION_STRINGS.md` |
| `chernobog_vm_temporal_prefix_strings(ea, input_json, bindings_json)` | Opt-in four-run observations from intact event prefixes ending at return or before an unexecuted region frontier. Retains exclusive sequence cutoffs and incomplete execution; Qt `show(..., prefix=True)`. See `docs/VMP_PREFIX_STRINGS.md` |
| `chernobog_vm_temporal_string_state(ticket, lease)` | Validate the opaque capture lease, then compare exact input bytes, metadata, profile and model names for source navigation; no execution. Pass both fields returned by either temporal-string API; numeric tickets alone are rejected. |
...
| `CHERNOBOG_RAX_MAX_STATIC_INSNS` | `65536` | Retained physical static instructions, range 1–1000000; one decoded IDA head may be inspected after exhaustion to detect truncation |
