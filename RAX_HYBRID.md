...
| `chernobog_vm_temporal_string_state(ticket, lease)` | Validate the opaque capture lease, then compare exact input bytes, metadata, profile and model names for source navigation; no execution. Pass both fields returned by `chernobog_vm_temporal_strings`; numeric tickets alone are rejected. |
