...
| 6a. Separate bounded VM-region and logical-state model | Separate `src/vm` descriptors and logical-state comparison; local recognition supports role permutations, both directions, clones, table/relative dispatch and stateful decoding. Fresh production captures now expose per-visit VIP/key/register observations and bounded native paths. Virtual stack, VM context, complete memory identity and cross-function VM ownership remain unresolved; see `VMP_VM_REGIONS.md` and `VMP_VM_OBSERVATIONS.md` | In progress |
